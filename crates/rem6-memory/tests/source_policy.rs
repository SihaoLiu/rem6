...
#[test]
fn memory_lib_rs_remains_a_facade() {
    let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/lib.rs");
    let lines = line_count(&path);

    assert!(
        lines <= MAX_FACADE_LINES,
        "src/lib.rs should remain a facade over focused memory modules, but it has {lines} lines"
    );
}

#[test]
fn memory_request_response_lives_in_focused_module() {
    let crate_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let lib_rs = fs::read_to_string(crate_dir.join("src/lib.rs")).unwrap();
    let request_rs = crate_dir.join("src/request.rs");

    assert!(
        request_rs.exists(),
        "memory request and response code belongs in src/request.rs"
    );
    assert!(
        !lib_rs.contains("pub struct MemoryRequest {"),
        "src/lib.rs should re-export memory request types from a focused module"
    );
    assert!(
        !lib_rs.contains("pub struct MemoryResponse {"),
        "src/lib.rs should re-export memory response types from a focused module"
    );
}
