...
#[test]
fn traffic_lib_rs_remains_a_facade() {
    let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/lib.rs");
    let lines = line_count(&path);

    assert!(
        lines <= MAX_FACADE_LINES,
        "src/lib.rs should remain a facade over focused traffic modules, but it has {lines} lines"
    );
}

#[test]
fn traffic_source_files_stay_within_size_limit() {
