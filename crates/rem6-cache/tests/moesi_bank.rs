...
#[test]
fn moesi_cache_bank_can_select_skewed_replacement_indexing() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_indexed_replacement_directory(
        cache_agent,
        layout(),
    ...
    assert_eq!(bank.state(Address::new(0x80)), Some(MoesiState::Shared));
...
#[test]
fn moesi_cache_bank_reports_invalid_indexed_replacement_shapes() {
    assert_eq!(
        MoesiCacheBank::new_with_indexed_replacement_directory(
            agent(30),
            layout(),
            CacheReplacementPolicyKind::Lru,
            CacheIndexingPolicyKind::SkewedAssociative,
        ...
        .unwrap_err(),
        MoesiCacheBankError::Replacement(CacheReplacementPolicyError::IndexingPolicyConfig {
...
#[test]
fn moesi_cache_bank_mshr_coalesces_same_line_read_misses() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_mshr(
        cache_agent,
        layout(),
        MshrQueueConfig::new(2, 2, 0).unwrap(),
    );

    let first = read(cache_agent, 100, 0x4004);
    ...
    assert_eq!(bank.mshr_target_count(Address::new(0x4000)), Some(1));

    let second = read(cache_agent, 101, 0x4008);
    ...
    assert_eq!(bank.pending_fill_count(), 1);
    assert_eq!(bank.mshr_allocated_count(), 1);
    assert_eq!(bank.mshr_target_count(Address::new(0x4000)), Some(2));

    let fill_result = bank
        .accept_fill(fill(&first_downstream, 0x66), MoesiEvent::DataShared)
        .unwrap();
    ...
    assert_eq!(response_data(&fill_result.target_outcomes()[0]), &[0x66; 8]);
...
#[test]
fn moesi_cache_bank_write_queue_orders_issues_and_restores_snapshot() {
    let cache_agent = agent(30);
    ...
    assert_eq!(
        ...
        Err(MoesiCacheBankError::WriteQueue(
...
#[test]
fn moesi_cache_bank_direct_write_clean_enters_write_queue_without_local_response() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
...
#[test]
fn moesi_cache_bank_write_queue_tracks_conflicts_functional_reads_and_uncached_matches() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
...
#[test]
fn moesi_cache_bank_write_queue_rejects_foreign_line_layouts() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
    ...
    let expected_error = MoesiCacheBankError::Controller(MoesiCacheControllerError::Memory(
...
#[test]
fn moesi_cache_bank_uncacheable_atomic_blocks_same_line_until_response() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new(cache_agent, layout());

    let atomic = uncacheable_atomic(cache_agent, 124, 0x4a08, vec![0x33, 0x44]);
    let atomic_miss = bank.accept_cpu_request(atomic.clone()).unwrap();
    let downstream = atomic_miss.downstream_request().unwrap().clone();

    assert_eq!(atomic_miss.kind(), MoesiCacheControllerResultKind::Miss);
    ...
    let write_error = bank
        .accept_cpu_request(write(cache_agent, 125, 0x4a04, vec![0x55]))
        .expect_err("pending uncacheable atomic must block same-line writes");
    assert_eq!(
        write_error.to_string(),
        "MOESI cache bank has pending uncacheable request for line 0x4a00"
    );
    ...
    let later_write = bank
        .accept_cpu_request(write(cache_agent, 127, 0x4a04, vec![0x88]))
        .unwrap();
    assert_eq!(later_write.kind(), MoesiCacheControllerResultKind::Miss);
}

#[test]
fn moesi_cache_bank_dirty_uncacheable_atomic_blocks_same_line_reads_until_writeback() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
    ...
    let store = write(cache_agent, 128, 0x4b04, vec![0xde, 0xad]);
    ...
    let atomic = uncacheable_atomic(cache_agent, 129, 0x4b08, vec![0x33, 0x44]);
    let atomic_miss = bank.accept_cpu_request(atomic).unwrap();
    assert!(atomic_miss.downstream_request().is_none());
    assert_eq!(bank.write_queue_allocated_count(), 1);

    let read_error = bank
        .accept_cpu_request(read(cache_agent, 130, 0x4b04))
        .expect_err("dirty pending atomic must block same-line reads");
    assert_eq!(
        read_error.to_string(),
        "MOESI cache bank has pending uncacheable request for line 0x4b00"
    );
}
...
#[test]
fn moesi_cache_bank_locked_rmw_read_waits_for_write_queue_conflict() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
    ...
    let forwarded = bank
        .accept_cpu_request(read(cache_agent, 135, 0x4604))
        .unwrap();
    assert_eq!(forwarded.kind(), MoesiCacheControllerResultKind::Hit);
    assert!(matches!(
        bank.accept_cpu_request(read_unique(cache_agent, 136, 0x4604)),
        Err(MoesiCacheBankError::WriteQueueConflict { line })
            if line == Address::new(0x4600)
    ));
    assert!(matches!(
        bank.accept_cpu_request(locked_rmw_read(cache_agent, 137, 0x4604)),
        Err(MoesiCacheBankError::WriteQueueConflict { line })
            if line == Address::new(0x4600)
    ));
}
...
#[test]
fn moesi_cache_bank_snapshot_restores_pending_uncacheable_read_after_dirty_writeback() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
    ...
    let store = write(cache_agent, 135, 0x4704, vec![0xde, 0xad]);
    let miss = bank.accept_cpu_request(store).unwrap();
    bank.accept_fill(
        fill(miss.downstream_request().unwrap(), 0x00),
        MoesiEvent::DataModified,
    ...
    let uncached = uncacheable_read(cache_agent, 136, 0x4708);
    let result = bank.accept_cpu_request(uncached.clone()).unwrap();
    assert!(result.downstream_request().is_none());
    let snapshot = bank.snapshot();

    let mut restored = MoesiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
        CacheWriteQueueConfig::new(1, 0).unwrap(),
    );
    ...
    assert_eq!(restored.state(Address::new(0x4700)), None);
}

#[test]
fn moesi_cache_bank_snapshot_keeps_clean_pending_read_from_dirty_writeback_read() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
    ...
    let cached = read(cache_agent, 137, 0x4804);
    ...
    let clean_uncached = uncacheable_read(cache_agent, 138, 0x4808);
    ...
    let store = write(cache_agent, 139, 0x4804, vec![0xde, 0xad]);
    let store_miss = bank.accept_cpu_request(store).unwrap();
    bank.accept_fill(
    ...
    let dirty_uncached = uncacheable_read(cache_agent, 140, 0x480c);
...
#[test]
fn moesi_cache_bank_restore_rejects_uncacheable_read_with_missing_blocking_writeback() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
    ...
    let store = write(cache_agent, 141, 0x4904, vec![0xde, 0xad]);
    let miss = bank.accept_cpu_request(store).unwrap();
    bank.accept_fill(
        fill(miss.downstream_request().unwrap(), 0x00),
        MoesiEvent::DataModified,
    ...
    let uncached = uncacheable_read(cache_agent, 142, 0x4908);
    ...
    let snapshot = bank.snapshot().with_pending_uncacheable_reads(vec![
        MoesiPendingUncacheableReadSnapshot::new(uncached.clone(), Some(missing)),
    ]);
    ...
    assert!(matches!(
        ...
        Err(MoesiCacheBankError::SnapshotPendingUncacheableReadWritebackMismatch {
            response,
            handle,
...
fn moesi_cache_bank_restore_rejects_malformed_pending_uncacheable_request() {
    ...
    let foreign_agent_read = uncacheable_read(agent(31), 146, 0x4940);
    let snapshot = MoesiCacheBank::new(cache_agent, layout())
        .snapshot()
        .with_pending_uncacheable_reads(vec![MoesiPendingUncacheableReadSnapshot::new(
            foreign_agent_read,
    ...
    assert_eq!(
        restored.restore(&snapshot),
        Err(MoesiCacheBankError::WrongAgent {
    ...
    let wrong_layout_read = MemoryRequest::read_shared(
        MemoryRequestId::new(cache_agent, 147),
        Address::new(0x4950),
...
fn moesi_cache_bank_uncacheable_write_enters_write_queue_without_mshr() {
    ...
    assert_eq!(bank.state(Address::new(0x4820)), None);
...
#[test]
fn moesi_cache_bank_uncacheable_write_queues_dirty_writeback_first() {
    ...
    let store = write(cache_agent, 124, 0x4444, vec![0xde, 0xad]);
    let miss = bank.accept_cpu_request(store).unwrap();
    let downstream = miss.downstream_request().unwrap().clone();
    bank.accept_fill(fill(&downstream, 0x00), MoesiEvent::DataModified)
        .unwrap();
    assert_eq!(bank.state(Address::new(0x4440)), Some(MoesiState::Modified));

    let uncached = uncacheable_write(cache_agent, 125, 0x4448, vec![0xca, 0xfe], vec![true, true]);
    let result = bank.accept_cpu_request(uncached.clone()).unwrap();

    assert_eq!(result.kind(), MoesiCacheControllerResultKind::Miss);
    assert!(result.downstream_request().is_none());
    assert_eq!(bank.state(Address::new(0x4440)), None);
    assert_eq!(bank.write_queue_allocated_count(), 2);

    let forwarded = bank
        .accept_cpu_request(read(cache_agent, 126, 0x4444))
        .unwrap();
    assert_eq!(forwarded.kind(), MoesiCacheControllerResultKind::Hit);
    assert!(forwarded.downstream_request().is_none());
    assert_eq!(
    ...
    let writeback = bank.issue_write_queue(0).unwrap().unwrap();
    assert_eq!(writeback.kind(), CacheWriteQueueEntryKind::WritebackDirty);
    assert_eq!(writeback.request().line_address(), Address::new(0x4440));
    assert_eq!(&writeback.request().data().unwrap()[4..6], &[0xde, 0xad]);
...
#[test]
fn moesi_cache_bank_uncacheable_write_response_uses_inflight_record_after_restore() {
    let cache_agent = agent(30);
    let mut bank = MoesiCacheBank::new_with_mshr_and_write_queue(
        cache_agent,
        layout(),
    ...
    let mut restored = MoesiCacheBank::new_with_mshr_and_write_queue(
        cache_agent,
        layout(),
        MshrQueueConfig::new(2, 2, 0).unwrap(),
        CacheWriteQueueConfig::new(2, 0).unwrap(),
    ...
    assert!(matches!(
        ...
        Err(MoesiCacheBankError::UnknownUncacheableWriteResponse { response })
            if response == request.id()
    ));
}

#[test]
fn moesi_cache_bank_restore_rejects_malformed_inflight_uncacheable_write() {
    let cache_agent = agent(30);
    let mut restored = MoesiCacheBank::new(cache_agent, layout());

    let cacheable_write = write(cache_agent, 146, 0x4940, vec![0xde, 0xad]);
    ...
    assert!(matches!(
        restored.restore(&snapshot),
        Err(MoesiCacheBankError::SnapshotInflightUncacheableWriteMismatch {
            response,
            operation: MemoryOperation::Write,
            uncacheable: false,
    ...
    let uncached_read = uncacheable_read(cache_agent, 147, 0x4950);
    ...
    let uncached_atomic = uncacheable_atomic(cache_agent, 148, 0x4960, vec![0x11, 0x22]);
    ...
    let uncached_no_response =
        clean_evict(cache_agent, 149, 0x4970).with_uncacheable_strict_order();
    let uncached_no_response_id = uncached_no_response.id();
    let snapshot = MoesiCacheBank::new(cache_agent, layout())
        .snapshot()
    ...
    let foreign_agent_write =
        uncacheable_write(agent(31), 150, 0x4980, vec![0xde, 0xad], vec![true, true]);
    let snapshot = MoesiCacheBank::new(cache_agent, layout())
        .snapshot()
