...
use rem6_memory::{
    AccessSize, Address, AgentId, ByteMask, CacheLineLayout, MemoryAccessOrdering, MemoryError,
    MemoryOperation, MemoryRequest, MemoryRequestId, MemoryRequestSnapshot, MemoryResponse,
};
...
#[test]
fn msi_cache_bank_uncacheable_atomic_blocks_same_line_until_response() {
    let cache_agent = agent(7);
    let mut bank = MsiCacheBank::new(cache_agent, layout());

    let atomic = uncacheable_atomic(cache_agent, 132, 0x2108, vec![0x33, 0x44]);
    let atomic_miss = bank.accept_cpu_request(atomic.clone()).unwrap();
    let downstream = atomic_miss.downstream_request().unwrap().clone();

    assert_eq!(atomic_miss.kind(), CacheControllerResultKind::Miss);
    ...
    let write_error = bank
        .accept_cpu_request(write(cache_agent, 133, 0x2104, vec![0x55]))
        .expect_err("pending uncacheable atomic must block same-line writes");
    assert_eq!(
        write_error.to_string(),
        "MSI cache bank has pending uncacheable request for line 0x2100"
    );
    ...
    let atomic_fill = bank
        .accept_fill(MemoryResponse::completed(&downstream, Some(vec![0xaa, 0xbb])).unwrap())
        .unwrap();
    assert_eq!(
    ...
    let later_write = bank
        .accept_cpu_request(write(cache_agent, 135, 0x2104, vec![0x88]))
        .unwrap();
    assert_eq!(later_write.kind(), CacheControllerResultKind::Miss);
}

#[test]
fn msi_cache_bank_dirty_uncacheable_atomic_blocks_same_line_reads_until_writeback() {
    let cache_agent = agent(7);
    ...
    let store = write(cache_agent, 136, 0x2204, vec![0xde, 0xad]);
    let store_miss = bank.accept_cpu_request(store).unwrap();
    bank.accept_fill(fill(store_miss.downstream_request().unwrap(), 0x11))
        .unwrap();

    let atomic = uncacheable_atomic(cache_agent, 137, 0x2208, vec![0x33, 0x44]);
    ...
    let read_error = bank
        .accept_cpu_request(read(cache_agent, 138, 0x2204))
        .expect_err("dirty pending atomic must block same-line reads");
    assert_eq!(
        read_error.to_string(),
        "MSI cache bank has pending uncacheable request for line 0x2200"
    );
}
...
#[test]
fn msi_cache_bank_restore_rejects_malformed_inflight_uncacheable_write() {
    let cache_agent = agent(7);
    let mut restored = MsiCacheBank::new(cache_agent, layout());

    let cacheable_write = write(cache_agent, 150, 0x2140, vec![0xde, 0xad]);
    ...
    assert!(matches!(
        restored.restore(&snapshot),
        Err(MsiCacheBankError::SnapshotInflightUncacheableWriteMismatch {
            response,
            operation: MemoryOperation::Write,
            uncacheable: false,
    ...
    let uncached_read = uncacheable_read(cache_agent, 151, 0x2150);
    ...
    let uncached_atomic = uncacheable_atomic(cache_agent, 152, 0x2160, vec![0x11, 0x22]);
    ...
    let uncached_no_response =
        clean_evict(cache_agent, 153, 0x2170).with_uncacheable_strict_order();
    let uncached_no_response_id = uncached_no_response.id();
    let snapshot = MsiCacheBank::new(cache_agent, layout())
        .snapshot()
    ...
    let foreign_agent_write =
        uncacheable_write(agent(8), 154, 0x2180, vec![0xde, 0xad], vec![true, true]);
    let snapshot = MsiCacheBank::new(cache_agent, layout())
        .snapshot()
    ...
    let wrong_layout_write = MemoryRequest::write(
        MemoryRequestId::new(cache_agent, 155),
        Address::new(0x2190),
...
#[test]
fn msi_cache_bank_rejects_post_fill_writeback_targets_without_write_queue() {
    let cache_agent = agent(7);
    let config = MshrQueueConfig::new(2, 3, 0).unwrap();
    let mut bank = MsiCacheBank::new_with_mshr(cache_agent, layout(), config.clone());

    let first = read(cache_agent, 122, 0x1004);
    ...
    let current_mshr = snapshot.mshr().unwrap();
    let current_entry = &current_mshr.entries()[0];
    let clean = clean_writeback(cache_agent, 123, 0x1000, 0xdd);
    let mut targets = current_entry.targets().to_vec();
    targets.push(MshrTarget::from_parts(
        clean,
        1,
    ...
    assert_eq!(
        ...
        Err(MsiCacheBankError::WriteQueueDisabled)
    );
...
#[test]
fn msi_cache_bank_rejects_post_fill_writeback_targets_when_write_queue_full() {
    let cache_agent = agent(7);
    let config = MshrQueueConfig::new(2, 3, 0).unwrap();
    let mut bank = MsiCacheBank::new_with_mshr_and_write_queue(
        cache_agent,
    ...
    let current_mshr = snapshot.mshr().unwrap();
    let current_entry = &current_mshr.entries()[0];
    let clean = clean_writeback(cache_agent, 125, 0x1000, 0xee);
    let mut targets = current_entry.targets().to_vec();
    targets.push(MshrTarget::from_parts(
        clean,
        1,
    ...
    restored.restore(&restored_snapshot).unwrap();
    restored
        .enqueue_writeback(clean_evict(cache_agent, 126, 0x2000), false, 0)
        .unwrap();
