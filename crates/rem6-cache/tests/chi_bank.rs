...
#[test]
fn chi_cache_bank_locked_rmw_read_waits_for_write_queue_conflict() {
    let cache_agent = agent(40);
    let mut bank = ChiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
    ...
    let store = write(cache_agent, 544, 0x3604, vec![0xde, 0xad]);
    let miss = bank.accept_cpu_request(store).unwrap();
    bank.accept_fill(
        fill(miss.downstream_request().unwrap(), 0x00),
        ChiEvent::CompDataUniqueDirty,
    ...
    let forwarded = bank
        .accept_cpu_request(read(cache_agent, 546, 0x3604))
        .unwrap();
    assert_eq!(forwarded.kind(), ChiCacheControllerResultKind::Hit);
    assert!(matches!(
        bank.accept_cpu_request(read_unique(cache_agent, 547, 0x3604)),
        Err(ChiCacheBankError::WriteQueueConflict { line })
            if line == Address::new(0x3600)
    ));
    assert!(matches!(
        bank.accept_cpu_request(locked_rmw_read(cache_agent, 548, 0x3604)),
        Err(ChiCacheBankError::WriteQueueConflict { line })
            if line == Address::new(0x3600)
    ));
}
...
#[test]
fn chi_cache_bank_direct_write_clean_enters_write_queue_without_local_response() {
    let cache_agent = agent(40);
    let mut bank = ChiCacheBank::new_with_write_queue(
        cache_agent,
        layout(),
