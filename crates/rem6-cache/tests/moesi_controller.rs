...
#[test]
fn moesi_controller_store_conditional_respects_cache_resident_reservations() {
    let mut controller = controller();
    controller.install_modified((0..64).collect()).unwrap();

    let failed_store = store_conditional(8, 0x4010, vec![0xaa, 0xbb, 0xcc, 0xdd]);
    let failed = controller.accept_cpu_request(failed_store.clone()).unwrap();

    assert_eq!(failed.kind(), MoesiCacheControllerResultKind::Hit);
    assert_eq!(
    ...
    let load = load_locked(9, 0x4018, 8);
    ...
    let completed_store = store_conditional(10, 0x4010, vec![0xaa, 0xbb, 0xcc, 0xdd]);
    let completed = controller
...
#[test]
fn moesi_controller_store_conditional_miss_fails_without_reservation_after_fill() {
    let mut controller = controller();
    let store = store_conditional(11, 0x4010, vec![0xaa, 0xbb, 0xcc, 0xdd]);
...
#[test]
fn moesi_controller_public_line_install_clears_load_locked_reservations() {
    let mut controller = controller();
    controller.install_modified((0..64).collect()).unwrap();

    let load = load_locked(12, 0x4038, 8);
    controller.accept_cpu_request(load).unwrap();
    controller.install_modified(vec![0xee; 64]).unwrap();
    let store = store_conditional(13, 0x4038, vec![0xaa, 0xbb, 0xcc, 0xdd]);
