    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SnapshotPendingUncacheableRequestMismatch {
                ...
                formatter,
                "MESI cache bank snapshot pending uncacheable request {} from agent {} has operation {:?} and uncacheable flag {}",
                response.sequence(),
                response.agent().get(),
                operation,
                uncacheable
            ...
            Self::SnapshotPendingUncacheableReadWritebackMismatch { response, handle } => write!(
                formatter,
                "MESI cache bank snapshot pending uncacheable read {} from agent {} references invalid blocking writeback {:?}",
                response.sequence(),
                response.agent().get(),
                handle
            ),
            ...
            Self::SnapshotInflightUncacheableWriteMismatch {
                ...
                formatter,
                "MESI cache bank snapshot in-flight uncacheable write {} from agent {} has operation {:?} and uncacheable flag {}",
                response.sequence(),
                response.agent().get(),
                operation,
                uncacheable
...
impl From<CacheSectorTagsError> for MesiCacheBankError {
...
impl MesiCacheBank {
    ...
    pub fn new_with_sector_tags(
        ...
        if sector_tags_config.line_layout() != layout {
            return Err(MesiCacheBankError::SectorTagsLayoutMismatch {
                expected: layout,
        ...
        let pending_uncacheable_reads = self
            ...
            .filter_map(|pending| match pending {
                ...
                PendingBankFill::Uncacheable {
                    original,
                    blocked_by,
                } => Some(MesiPendingUncacheableReadSnapshot::new(
                    original.clone(),
                    *blocked_by,
                )),
            })
    ...
    pub fn restore(&mut self, snapshot: &MesiCacheBankSnapshot) -> Result<(), MesiCacheBankError> {
        ...
        let restored_write_queue = match (&self.write_queue, snapshot.write_queue()) {
            ...
            (None, None) => None,
            (None, Some(_)) => {
                return Err(MesiCacheBankError::SnapshotWriteQueueModeMismatch {
        ...
        for request in snapshot.inflight_uncacheable_writes() {
            ...
            {
                return Err(
                    MesiCacheBankError::SnapshotInflightUncacheableWriteMismatch {
                        response: request.id(),
    ...
    fn validate_pending_uncacheable_read_snapshot(
        ...
        {
            return Err(
                MesiCacheBankError::SnapshotPendingUncacheableRequestMismatch {
                    response: request.id(),
                    operation: request.operation(),
                    uncacheable: request.is_uncacheable(),
                },
        ...
        if valid {
            ...
            Err(
                MesiCacheBankError::SnapshotPendingUncacheableReadWritebackMismatch {
                    response: request.id(),
                    handle,
                },
            )
    ...
    fn accept_cpu_request_inner(
        ...
        if request.operation() == MemoryOperation::NoAccess {
            return crate::no_access::mesi(&request, self.lines.get(&line));
        ...
        if crate::clean_maintenance::is_clean_maintenance(&request) {
            return Ok(crate::clean_maintenance::mesi_result(self, request, line));
        }
    ...
    fn accept_fill_inner(
        ...
        post_fill::preflight_downstream_requests(
            ...
            MesiCacheBankError::WriteQueueDisabled,
    ...
    pub fn enqueue_writeback(
        ...
        secure: bool,
        ready_tick: u64,
    ) -> Result<CacheWriteQueueUpdate, MesiCacheBankError> {
        self.validate_write_queue_request(&request)?;
        self.write_queue_mut()?
    ...
    pub fn write_queue_satisfy_read(
        ...
        size: AccessSize,
        secure: bool,
    ) -> Result<Option<Vec<u8>>, MesiCacheBankError> {
    ...
    fn validate_write_queue_request(
        ...
        if actual != self.layout {
            return Err(
                MesiCacheControllerError::Memory(MemoryError::LineLayoutMismatch {
                    request: request.id(),
    ...
    ) -> Result<Option<CacheWriteQueueHandle>, MesiCacheBankError> {
    ...
    fn write_queue_mut(&mut self) -> Result<&mut CacheWriteQueue, MesiCacheBankError> {
        self.write_queue
            .as_mut()
            .ok_or(MesiCacheBankError::WriteQueueDisabled)
    }
