...
mod msi_compressed_tags;
mod msi_sector_tags;
mod snapshot;

pub use snapshot::{MsiCacheBankSnapshot, MsiPendingUncacheableReadSnapshot};
...
use crate::{
    CacheCompressedTags, CacheCompressedTagsConfig, CacheCompressedTagsError, CacheControllerError,
    CacheControllerResult, CacheControllerResultKind, CacheIndexingPolicyKind,
    CacheReplacementDirectory, CacheReplacementDirectoryConfig, CacheReplacementPolicyError,
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ReplacementDirectoryLayoutMismatch { expected, actual } => write!(
                formatter,
                "MSI cache bank replacement directory line size {} cannot attach to bank line size {}",
                actual.bytes(),
                expected.bytes()
            ),
            Self::SectorTagsLayoutMismatch { expected, actual } => write!(
                formatter,
                "MSI cache bank sector tags line size {} cannot attach to bank line size {}",
                actual.bytes(),
                expected.bytes()
            ),
            Self::CompressedTagsLayoutMismatch { expected, actual } => write!(
                formatter,
                "MSI cache bank compressed tags line size {} cannot attach to bank line size {}",
                actual.bytes(),
                expected.bytes()
            ),
            ...
            Self::DirtyReplacementRequiresWriteQueue { line } => write!(
                formatter,
                "MSI cache bank cannot evict modified line {:#x} without replacement writeback support",
                line.get()
            ),
            Self::TransientReplacementRequiresStableLine { line } => write!(
                formatter,
                "MSI cache bank cannot evict transient line {:#x} through replacement capacity",
                line.get()
            ),
            ...
            Self::SnapshotReplacementDirectoryModeMismatch {
                ...
                formatter,
                "MSI cache bank snapshot replacement directory mode {snapshot_has_replacement_directory} cannot restore bank replacement directory mode {bank_has_replacement_directory}"
            ),
            Self::SnapshotSectorTagsModeMismatch {
                ...
                formatter,
                "MSI cache bank snapshot sector tags mode {snapshot_has_sector_tags} cannot restore bank sector tags mode {bank_has_sector_tags}"
            ),
            Self::SnapshotCompressedTagsModeMismatch {
                ...
                formatter,
                "MSI cache bank snapshot compressed tags mode {snapshot_has_compressed_tags} cannot restore bank compressed tags mode {bank_has_compressed_tags}"
            ),
...
impl From<CacheReplacementPolicyError> for MsiCacheBankError {
    fn from(error: CacheReplacementPolicyError) -> Self {
        Self::Replacement(error)
    }
}

impl From<CacheSectorTagsError> for MsiCacheBankError {
    fn from(error: CacheSectorTagsError) -> Self {
        Self::SectorTags(error)
    }
}

impl From<CacheCompressedTagsError> for MsiCacheBankError {
...
impl MsiCacheBank {
    ...
    pub fn new_with_replacement_directory(
        ...
        if replacement_config.line_layout() != layout {
            return Err(MsiCacheBankError::ReplacementDirectoryLayoutMismatch {
                expected: layout,
    ...
    pub fn new_with_sector_tags(
        ...
        if sector_tags_config.line_layout() != layout {
            return Err(MsiCacheBankError::SectorTagsLayoutMismatch {
                expected: layout,
    ...
    pub fn new_with_compressed_tags(
        ...
        if compressed_tags_config.line_layout() != layout {
            return Err(MsiCacheBankError::CompressedTagsLayoutMismatch {
                expected: layout,
    ...
    pub fn restore(&mut self, snapshot: &MsiCacheBankSnapshot) -> Result<(), MsiCacheBankError> {
        ...
        if snapshot.replacement_directory().is_some() && snapshot.sector_tags().is_some() {
            return Err(MsiCacheBankError::SnapshotSectorTagsModeMismatch {
        ...
        {
            return Err(MsiCacheBankError::SnapshotCompressedTagsModeMismatch {
    ...
    fn accept_cpu_request_inner(
        ...
        } else if result.kind() == CacheControllerResultKind::Hit {
    ...
    pub fn accept_fill(
        ...
        self.accept_fill_inner(response, None)
    }
    ...
    pub fn accept_fill_with_compressed_size_bits(
        ...
        self.accept_fill_inner(response, Some(compressed_size_bits))
    }
    ...
    fn remove_resident_line(&mut self, line: Address) -> Result<(), MsiCacheBankError> {
    ...
    fn touch_replacement_line(&mut self, line: Address) -> Result<(), MsiCacheBankError> {
        if let Some(directory) = &mut self.replacement_directory {
