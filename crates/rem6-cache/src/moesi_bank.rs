...
pub use snapshot::{MoesiCacheBankSnapshot, MoesiPendingUncacheableReadSnapshot};
...
use crate::{
    ...
    MoesiCacheControllerResult, MoesiCacheControllerResultKind, MshrCompletion, MshrHandle,
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::CompressedTagsLayoutMismatch { expected, actual } => write!(
                formatter,
                "MOESI cache bank compressed tags line size {} cannot attach to bank line size {}",
                actual.bytes(),
                expected.bytes()
            ),
            ...
            Self::WriteQueueConflict { line } => write!(
                formatter,
                "MOESI cache bank has pending write-queue work for line {:#x}",
                line.get()
            ),
            Self::PendingUncacheableConflict { line } => write!(
                formatter,
                "MOESI cache bank has pending uncacheable request for line {:#x}",
                line.get()
            ),
            ...
            Self::UnknownUncacheableWriteResponse { response } => write!(
                formatter,
                "MOESI cache bank has no in-flight uncacheable write for response {} from agent {}",
                response.sequence(),
                response.agent().get()
            ),
            ...
            Self::SnapshotCompressedTagsModeMismatch {
                ...
                formatter,
                "MOESI cache bank snapshot compressed tags mode {snapshot_has_compressed_tags} cannot restore bank compressed tags mode {bank_has_compressed_tags}"
            ),
...
impl From<CacheCompressedTagsError> for MoesiCacheBankError {
...
impl MoesiCacheBank {
    ...
    pub fn new_with_compressed_tags(
        ...
        if compressed_tags_config.line_layout() != layout {
            return Err(MoesiCacheBankError::CompressedTagsLayoutMismatch {
                expected: layout,
    ...
    pub fn restore(
        ...
        {
            return Err(MoesiCacheBankError::SnapshotCompressedTagsModeMismatch {
        ...
        for request in snapshot.inflight_uncacheable_writes() {
            ...
            if inflight_uncacheable_writes
                ...
                .is_some()
            {
                return Err(MoesiCacheBankError::DuplicateSnapshotUncacheableWrite {
                    response: request.id(),
                });
            }
        }
    ...
    pub fn accept_uncacheable_write_response(
        ...
        let original = self.inflight_uncacheable_writes.get(&response_id).ok_or(
            MoesiCacheBankError::UnknownUncacheableWriteResponse {
                response: response_id,
            },
        )?;
        let outcome = crate::downstream::uncacheable_write_response_outcome(original, response)
            .map_err(MoesiCacheControllerError::Memory)
            .map_err(MoesiCacheBankError::from)?;
    ...
    fn accept_write_queue_conflict(
        &self,
        request: &MemoryRequest,
    ) -> Result<Option<MoesiCacheControllerResult>, MoesiCacheBankError> {
        let line = request.line_address();
        ...
        if request.returns_data() && !request.carries_data() {
            if let Some(data) = self.write_queue_ref()?.satisfy_read(
                ...
                let response = MemoryResponse::completed(request, Some(data))
                    .map_err(MoesiCacheControllerError::Memory)?;
                return Ok(Some(MoesiCacheControllerResult::new(
    ...
    fn dirty_resident_writeback_request(
        &self,
        line: Address,
    ) -> Result<Option<MemoryRequest>, MoesiCacheBankError> {
        let Some(controller) = self.lines.get(&line) else {
            return Ok(None);
        };
        if !controller.state().is_dirty_owner() {
            return Ok(None);
        }
        let data = controller
            .cached_data()
    ...
    fn require_write_queue_entries(&self, entries: usize) -> Result<(), MoesiCacheBankError> {
