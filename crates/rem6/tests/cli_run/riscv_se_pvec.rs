...
#[test]
fn rem6_run_riscv_se_runs_static_raw_process_vm_iovecs_against_qemu() {
    let Some(gcc) = find_riscv_tool("riscv64-unknown-elf-gcc") else {
        eprintln!(
            "skipping static RISC-V SE process_vm iovec smoke: riscv64-unknown-elf-gcc not found"
        );
        return;
    };
    let Some(qemu) = find_riscv_tool("qemu-riscv64") else {
        eprintln!("skipping static RISC-V SE process_vm iovec smoke: qemu-riscv64 not found");
        return;
    };
    let workspace = temp_workspace("riscv-se-raw-process-vm");
    let source = workspace.join("process_vm.c");
    let binary = workspace.join("process_vm");
    fs::write(
        &source,
        r#"#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    char local_a[4] = {0};
    char local_b[4] = {0};
    struct iovec local_read[2] = {{local_a, 3}, {local_b, 3}};
    struct iovec remote_read[2] = {{remote_a, 2}, {remote_b, 4}};
    char local_write_a[] = "XY";
    char local_write_b[] = "Z123";
    char remote_write_a[5] = {0};
    char remote_write_b[3] = {0};
    struct iovec local_write[2] = {{local_write_a, 2}, {local_write_b, 4}};
    struct iovec remote_write[2] = {{remote_write_a, 4}, {remote_write_b, 2}};

    long read_status = linux_syscall6(270, pid, (long)local_read, 2, (long)remote_read, 2, 0);
    long write_status = linux_syscall6(271, pid, (long)local_write, 2, (long)remote_write, 2, 0);
    long bad_flags = linux_syscall6(270, pid, (long)local_read, 2, (long)remote_read, 2, 1);
    long bad_pid = linux_syscall6(271, 99999999, (long)local_write, 2, (long)remote_write, 2, 0);
    int ok = read_status == 6 &&
             write_status == 6 &&
             bad_flags == -22 &&
             bad_pid == -3 &&
             strcmp(local_a, "abc") == 0 &&
             strcmp(local_b, "def") == 0 &&
             strcmp(remote_write_a, "XYZ1") == 0 &&
             strcmp(remote_write_b, "23") == 0;

    printf("process-vm:%ld:%ld:%ld:%ld:%s:%s:%s:%s:%d\n",
           read_status, write_status, bad_flags, bad_pid,
           local_a, local_b, remote_write_a, remote_write_b, ok);
    return ok ? 76 : 77;
}
"#,
    )
    ...
    assert_eq!(
        qemu_output.stdout,
        b"process-vm:6:6:-22:-3:abc:def:XYZ1:23:1\n"
    );
    ...
    assert!(stdout.contains("\"text\":\"process-vm:6:6:-22:-3:abc:def:XYZ1:23:1\\n\""));
    assert!(stdout.contains("\"riscv_unknown_syscalls\":[]"));
    assert_stat(&stdout, "sim.riscv.se", "Count", 1, "constant");
