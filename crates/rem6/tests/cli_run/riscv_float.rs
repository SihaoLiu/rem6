...
fn fmadd_d_round_up(rs3: u8, rs2: u8, rs1: u8, rd: u8) -> u32 {
    r4_type(rs3, 0x1, rs2, rs1, 0x3, rd, 0x43)
}
...
#[test]
fn rem6_run_executes_rv64d_fsub_directed_rounding_from_elf() {
    let mut program = riscv64_program(&[
        u_type(0, 10, 0x17),           // auipc a0, 0
        i_type(40, 10, 0x3, 11, 0x03), // ld a1, 40(a0)
        i_type(48, 10, 0x3, 12, 0x03), // ld a2, 48(a0)
    ...
    program.extend_from_slice(&1.0f64.to_bits().to_le_bytes());
    program.extend_from_slice(&0x3c90_0000_0000_0000_u64.to_le_bytes());
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);
    let path = temp_binary("rv64d-fsub-round-down", &elf);
    ...
    assert!(stdout.contains("\"status\":\"executed_until_trap\""));
    assert!(stdout.contains("\"trap\":\"breakpoint\""));
    assert!(stdout.contains("\"x5\":\"0x3fefffffffffffff\""));
    assert!(stdout.contains("\"x6\":\"0x1\""));
}

#[test]
fn rem6_run_executes_rv64d_fsub_directed_overflow_from_elf() {
    let mut program = riscv64_program(&[
        u_type(0, 10, 0x17),           // auipc a0, 0
        i_type(40, 10, 0x3, 11, 0x03), // ld a1, 40(a0)
        i_type(48, 10, 0x3, 12, 0x03), // ld a2, 48(a0)
    ...
    program.extend_from_slice(&0x7fef_ffff_ffff_ffff_u64.to_le_bytes());
    program.extend_from_slice(&0xffef_ffff_ffff_ffff_u64.to_le_bytes());
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);
    let path = temp_binary("rv64d-fsub-overflow-rdn", &elf);
    ...
    assert!(stdout.contains("\"status\":\"executed_until_trap\""));
    assert!(stdout.contains("\"trap\":\"breakpoint\""));
    assert!(stdout.contains("\"x5\":\"0x7fefffffffffffff\""));
...
#[test]
fn rem6_run_executes_rv64d_fmadd_product_overflow_from_elf() {
    let mut program = riscv64_program(&[
        u_type(0, 10, 0x17),           // auipc a0, 0
        i_type(48, 10, 0x3, 11, 0x03), // ld a1, 48(a0)
        i_type(56, 10, 0x3, 12, 0x03), // ld a2, 56(a0)
        i_type(64, 10, 0x3, 13, 0x03), // ld a3, 64(a0)
        fmv_d_x(11, 1),
        fmv_d_x(12, 2),
        fmv_d_x(13, 3),
        fmadd_d_round_down(3, 2, 1, 4),
        fmv_x_d(4, 5),
        csr_read(0x001, 6),
        0x0010_0073, // ebreak
        0x0000_0013, // addi x0, x0, 0
    ...
    program.extend_from_slice(&0x7fef_ffff_ffff_ffff_u64.to_le_bytes());
    program.extend_from_slice(&0x7fef_ffff_ffff_ffff_u64.to_le_bytes());
    program.extend_from_slice(&0x0000_0000_0000_0001_u64.to_le_bytes());
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);
    let path = temp_binary("rv64d-fmadd-product-overflow-rdn", &elf);
...
#[test]
fn rem6_run_executes_rv64d_fmadd_max_boundary_overflow_from_elf() {
    let mut program = riscv64_program(&[
        u_type(0, 10, 0x17),           // auipc a0, 0
        i_type(48, 10, 0x3, 11, 0x03), // ld a1, 48(a0)
        i_type(56, 10, 0x3, 12, 0x03), // ld a2, 56(a0)
    ...
    program.extend_from_slice(&0x0000_0000_0000_0001_u64.to_le_bytes());
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);
    let path = temp_binary("rv64d-fmadd-boundary-overflow-rup", &elf);
