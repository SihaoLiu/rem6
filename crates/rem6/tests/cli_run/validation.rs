...
#[test]
fn rem6_run_accepts_riscv_o3_issue_width_cli_min_and_max() {
    let program = riscv64_program(&[0x0000_0013; 64]);
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);

    for value in ["1", "4"] {
        let path = temp_binary(&format!("riscv-o3-issue-width-cli-{value}"), &elf);
        let output = Command::new(env!("CARGO_BIN_EXE_rem6"))
            .args([
                "run",
                "--isa",
...
fn rem6_run_accepts_riscv_o3_writeback_width_cli_min_and_max() {
    ...
    for value in ["1", "4"] {
        let path = temp_binary(&format!("riscv-o3-writeback-width-cli-{value}"), &elf);
        let output = Command::new(env!("CARGO_BIN_EXE_rem6"))
            .args([
                "run",
                "--isa",
...
#[test]
fn rem6_run_accepts_riscv_o3_issue_width_from_config() {
    let program = riscv64_program(&[0x0000_0013; 64]);
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);
    let path = temp_binary("riscv-o3-issue-width-config", &elf);
    let config = temp_output("riscv-o3-issue-width-config.toml");
...
#[test]
fn rem6_run_accepts_riscv_o3_writeback_width_from_config() {
    let program = riscv64_program(&[0x0000_0013; 64]);
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);

    for value in [1, 4] {
        let path = temp_binary(&format!("riscv-o3-writeback-width-config-{value}"), &elf);
        let config = temp_output(&format!("riscv-o3-writeback-width-config-{value}.toml"));
        std::fs::write(
            &config,
            format!("[run]\nriscv_o3_writeback_width = {value}\n"),
        )
        .unwrap();
...
#[test]
fn rem6_run_validates_toml_riscv_o3_issue_width_requirements() {
    ...
    for (name, isa, elf, execute, expected) in [
        (
            "riscv-o3-issue-width-config-without-execute",
            "riscv",
            riscv.as_slice(),
            false,
        ...
        (
            "riscv-o3-issue-width-config-without-riscv",
            "x86",
            x86.as_slice(),
            true,
...
#[test]
fn rem6_run_validates_toml_riscv_o3_writeback_width_requirements() {
    let riscv = riscv64_elf(0x8000_0000, 0x8000_0000, &[0x13, 0, 0, 0]);
    let x86 = x86_64_elf(0x1000_0000, 0x1000_0000, &[0x90]);

    for (name, isa, elf, execute, expected) in [
        (
            "riscv-o3-writeback-width-config-without-execute",
            "riscv",
            riscv.as_slice(),
            false,
        ...
        (
            "riscv-o3-writeback-width-config-without-riscv",
            "x86",
            x86.as_slice(),
            true,
...
#[test]
fn rem6_run_rejects_riscv_o3_issue_width_without_execution() {
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &[0x13, 0, 0, 0]);
    let path = temp_binary("riscv-o3-issue-width-without-execute", &elf);
    ...
    assert!(output.stdout.is_empty());
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("--riscv-o3-issue-width requires --execute"));
}

#[test]
fn rem6_run_rejects_riscv_o3_writeback_width_without_execution() {
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &[0x13, 0, 0, 0]);
    let path = temp_binary("riscv-o3-writeback-width-without-execute", &elf);
    ...
    assert!(output.stdout.is_empty());
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("--riscv-o3-writeback-width requires --execute"));
}
...
#[test]
fn rem6_run_rejects_m5_switch_cpu_mode_without_execution() {
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &[0x13, 0, 0, 0]);
    let path = temp_binary("m5-switch-cpu-mode-without-execute", &elf);
    ...
    assert!(output.stdout.is_empty());
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("--m5-switch-cpu-mode requires --execute"));
}
...
#[test]
fn rem6_run_rejects_riscv_o3_issue_width_without_riscv_isa() {
    let elf = x86_64_elf(0x1000_0000, 0x1000_0000, &[0x90]);
    let path = temp_binary("riscv-o3-issue-width-without-riscv", &elf);
    ...
    assert!(output.stdout.is_empty());
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("--riscv-o3-issue-width requires --isa riscv"));
}

#[test]
fn rem6_run_rejects_riscv_o3_writeback_width_without_riscv_isa() {
    let elf = x86_64_elf(0x1000_0000, 0x1000_0000, &[0x90]);
    let path = temp_binary("riscv-o3-writeback-width-without-riscv", &elf);
    ...
    assert!(output.stdout.is_empty());
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("--riscv-o3-writeback-width requires --isa riscv"));
}
...
#[test]
fn rem6_run_rejects_m5_switch_cpu_mode_without_riscv_isa() {
    let elf = x86_64_elf(0x1000_0000, 0x1000_0000, &[0x90]);
    let path = temp_binary("m5-switch-cpu-mode-without-riscv", &elf);
    ...
    assert!(output.stdout.is_empty());
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("--m5-switch-cpu-mode requires --isa riscv"));
}
...
#[test]
fn rem6_run_rejects_invalid_riscv_o3_issue_width_values() {
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &[0x13, 0, 0, 0]);
    ...
    for value in ["0", "5", "wide"] {
        ...
        let stderr = String::from_utf8(output.stderr).unwrap();
        assert!(
            stderr.contains(&format!("invalid RISC-V O3 issue width {value}")),
            "{value}: {stderr}"
        );
    }
}
...
fn rem6_run_rejects_invalid_riscv_o3_writeback_width_values() {
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &[0x13, 0, 0, 0]);
    let path = temp_binary("riscv-o3-writeback-width-invalid", &elf);
    ...
    for value in ["0", "5", "wide"] {
        ...
        let stderr = String::from_utf8(output.stderr).unwrap();
        assert!(
            stderr.contains(&format!("invalid RISC-V O3 writeback width {value}")),
            "{value}: {stderr}"
        );
    }
}
...
#[test]
fn rem6_run_rejects_invalid_riscv_o3_issue_width_from_config() {
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &[0x13, 0, 0, 0]);

    for value in [0, 5] {
        let path = temp_binary(
            &format!("riscv-o3-issue-width-invalid-config-{value}"),
            &elf,
        );
        let config = temp_output(&format!("riscv-o3-issue-width-invalid-config-{value}.toml"));
...
#[test]
fn rem6_run_rejects_invalid_riscv_o3_writeback_width_from_config() {
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &[0x13, 0, 0, 0]);

    for value in [0, 5] {
        let path = temp_binary(
            &format!("riscv-o3-writeback-width-invalid-config-{value}"),
            &elf,
        );
        let config = temp_output(&format!(
            "riscv-o3-writeback-width-invalid-config-{value}.toml"
        ));
        std::fs::write(
            &config,
            format!("[run]\nriscv_o3_writeback_width = {value}\n"),
...
#[test]
fn rem6_run_rejects_invalid_m5_switch_cpu_mode_values() {
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &[0x13, 0, 0, 0]);
    let path = temp_binary("m5-switch-cpu-mode-invalid", &elf);
...
#[test]
fn rem6_run_config_scan_treats_riscv_o3_issue_width_as_value_taking() {
    let bogus_config = temp_output("riscv-o3-issue-width-prescan-bogus-config");
    ...
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(
        stderr.contains("invalid RISC-V O3 issue width --config"),
        "stderr: {stderr}"
    );
    assert!(!stderr.contains(&format!("failed to read config {}", bogus_config.display())));
}
...
#[test]
fn rem6_run_config_scan_treats_riscv_o3_writeback_width_as_value_taking() {
    let bogus_config = temp_output("riscv-o3-writeback-width-prescan-bogus-config");
    ...
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(
        stderr.contains("invalid RISC-V O3 writeback width --config"),
        "stderr: {stderr}"
    );
    assert!(!stderr.contains(&format!("failed to read config {}", bogus_config.display())));
}
...
#[test]
fn rem6_run_config_scan_treats_m5_switch_cpu_mode_as_value_taking() {
    let bogus_config = temp_output("m5-switch-cpu-mode-prescan-bogus-config");
