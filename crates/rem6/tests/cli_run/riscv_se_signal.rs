...
#[test]
fn rem6_run_riscv_se_runs_static_raw_rt_tgsigqueueinfo() {
    let mut program = riscv64_program(&[
        i_type(172, 0, 0x0, 17, 0x13),   // addi a7, x0, getpid
        0x0000_0073,                     // ecall
        ...
        i_type(0, 8, 0x0, 10, 0x13),     // addi a0, s0, 0
        i_type(0, 9, 0x0, 11, 0x13),     // addi a1, s1, 0
        i_type(0, 0, 0x0, 12, 0x13),     // addi a2, x0, 0
        u_type(0, 13, 0x17),             // auipc a3, 0
        i_type(0xdc, 13, 0x0, 13, 0x13), // addi a3, a3, siginfo offset
        i_type(240, 0, 0x0, 17, 0x13),   // addi a7, x0, rt_tgsigqueueinfo
        0x0000_0073,                     // ecall
        b_type(52, 0, 10, 0x1),          // bne a0, x0, fail
        i_type(0, 8, 0x0, 10, 0x13),     // addi a0, s0, 0
        i_type(0, 9, 0x0, 11, 0x13),     // addi a1, s1, 0
        i_type(10, 0, 0x0, 12, 0x13),    // addi a2, x0, SIGUSR1
        u_type(0, 13, 0x17),             // auipc a3, 0
        i_type(0xbc, 13, 0x0, 13, 0x13), // addi a3, a3, siginfo offset
        i_type(240, 0, 0x0, 17, 0x13),   // addi a7, x0, rt_tgsigqueueinfo
        0x0000_0073,                     // ecall
        i_type(-38, 0, 0x0, 5, 0x13),    // addi t0, x0, -ENOSYS
        b_type(16, 5, 10, 0x1),          // bne a0, t0, fail
    ...
    program.extend_from_slice(&siginfo);
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);
    let path = temp_binary("riscv-se-rt-tgsigqueueinfo", &elf);
    ...
    assert!(stdout.contains("\"stop_code\":82"), "stdout: {stdout}");
    assert!(
        stdout.contains("\"riscv_unknown_syscalls\":[{\"pc\":\"0x80000050\""),
        "stdout: {stdout}"
    );
    assert!(stdout.contains("\"number\":240"), "stdout: {stdout}");
    assert_stat(&stdout, "sim.riscv.se", "Count", 1, "constant");
    assert_stat(&stdout, "sim.stop_code", "Count", 82, "constant");
    assert_stat(
        &stdout,
