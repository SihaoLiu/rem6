...
fn rem6_run_o3_timing_store_load_preserves_architecture_without_o3_window() {
    let path = store_load_binary("o3-store-load-timing", 64);
...
#[test]
fn rem6_run_o3_detailed_aliasing_store_load_remain_serialized() {
    ...
    assert!(
        ...
        "aliasing load must not issue before the older store response: older={older}, younger={younger}"
    );
    assert_json_stat(
        &json,
        "sim.cpu0.o3.max_lsq_occupancy",
