...
fn rem6_run_o3_detailed_contained_store_load_forwarding_direct_matrix() {
    for case in CONTAINED_LOAD_CASES {
        ...
        let json = contained_store_load_json(&path, "direct", 1100, None);
...
fn rem6_run_o3_detailed_contained_store_load_forwarding_cache_fabric_dram_matrix() {
    for case in CONTAINED_LOAD_CASES {
        ...
        let json = contained_store_load_json(&path, "cache-fabric-dram", 1800, None);
...
fn rem6_run_o3_timing_contained_store_load_uses_transport_without_o3_window() {
    ...
    let json = contained_store_load_json(&path, "direct", 1100, Some("timing"));
...
#[test]
fn rem6_run_o3_detailed_partially_covered_load_remains_serialized() {
    let case = ContainedLoadCase {
        name: "ld-partial-tail",
        offset: 0,
        funct3: 0b011,
        size: 8,
        expected: 0x0000_0063_007f_8000,
    };
    let path =
        contained_store_load_binary("o3-partially-covered-store-load", case.offset, case.funct3);
    let json = contained_store_load_json(&path, "direct", 1100, None);
    ...
    assert!(
        ...
        "partially covered load must wait for the store response: older={older}, younger={younger}"
    );
    assert_eq!(
        younger
    ...
    for (path, value) in [
        ("sim.cpu0.o3.lsq_store_to_load_forwarding_candidates", 0),
        ("sim.cpu0.o3.lsq_store_to_load_forwarding_matches", 0),
        ("sim.cpu0.o3.lsq_store_to_load_forwarding_suppressed", 1),
        (
