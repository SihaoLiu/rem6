...
const MIDDLE_STORE_PC: &str = "0x8000001c";
const YOUNGEST_STORE_PC: &str = "0x80000020";
const LOAD_PC: &str = "0x80000024";
const DEPENDENT_ALU_PC: &str = "0x80000028";
const DATA_ADDRESS: &str = "0x80000100";
const RESULTS: &str = "aa00dd0655667788aa00dd0655667788";

#[test]
fn rem6_run_o3_detailed_multi_store_byte_composition_direct() {
    let path = multi_store_byte_composition_binary("o3-multi-store-byte-compose-direct");
    let json = multi_store_byte_composition_json(&path, "direct", 1200, None);

    assert_completed_multi_store_byte_composition(&json);
...
#[test]
fn rem6_run_o3_detailed_multi_store_byte_composition_cache_fabric_dram() {
    let path = multi_store_byte_composition_binary("o3-multi-store-byte-compose-hierarchy");
    let json = multi_store_byte_composition_json(&path, "cache-fabric-dram", 2400, None);
    ...
    for pointer in [
        ...
        assert!(
            json.pointer(pointer)
                .and_then(Value::as_u64)
                .is_some_and(|value| value > 0),
            "hierarchy-backed multi-store composition should expose {pointer}: {json}"
        );
    }
    for path in [
        "sim.memory.resources.cache.data.activity",
...
#[test]
fn rem6_run_o3_multi_store_byte_composition_rows_are_resident_before_responses() {
    let path = multi_store_byte_composition_binary("o3-multi-store-byte-compose-resident");
    let completed = multi_store_byte_composition_json(&path, "direct", 1200, None);
    let events = [
        event_at_pc(&completed, OLDER_STORE_PC),
        event_at_pc(&completed, MIDDLE_STORE_PC),
        event_at_pc(&completed, YOUNGEST_STORE_PC),
    ...
    let json = multi_store_byte_composition_json(&path, "direct", stop_tick, None);
    ...
    let rob = json
        ...
        .expect("resident multi-store composition ROB rows");
    assert_eq!(
        rob.iter()
            .map(|entry| {
                (
        ...
        vec![
            ...
            (Some(MIDDLE_STORE_PC), Some(false), Some(true)),
            (Some(YOUNGEST_STORE_PC), Some(false), Some(true)),
    ...
    let lsq = json
        ...
        .and_then(Value::as_array)
        .expect("resident multi-store composition LSQ rows");
    assert_eq!(
        lsq.iter()
            .map(|entry| {
                (
        ...
        vec![
            ...
            (Some("store"), Some("0x80000102"), Some(1), Some(false)),
...
#[test]
fn rem6_run_o3_timing_multi_store_byte_composition_uses_transport_without_o3_merge() {
    let path = multi_store_byte_composition_binary("o3-multi-store-byte-compose-timing");
    let json = multi_store_byte_composition_json(&path, "direct", 1200, Some("timing"));
...
fn assert_completed_multi_store_byte_composition(json: &Value) {
    assert_final_architecture(json);
    let older = event_at_pc(json, OLDER_STORE_PC);
    let middle = event_at_pc(json, MIDDLE_STORE_PC);
    let youngest = event_at_pc(json, YOUNGEST_STORE_PC);
    ...
    for store in [middle, youngest] {
        assert!(
            event_u64(store, "issue_tick") < older_response,
            "younger stores must occupy O3 before the leading response: older={older}, store={store}"
        );
    }
    assert!(
        event_u64(load, "issue_tick") < older_response,
        "the partial load must issue while all three stores are resident: older={older}, load={load}"
    );
    let memory_trace = json
        .pointer("/debug/memory_trace")
        .and_then(Value::as_array)
        .expect("multi-store composition run should expose Memory trace");
    ...
    let expected_requests = BTreeSet::from([all_data_requests[0], all_data_requests[3]]);
    assert_eq!(
        requests_before_older_response, expected_requests,
    ...
    assert!(event_u64(middle, "commit_tick") <= event_u64(youngest, "commit_tick"));
    assert!(event_u64(youngest, "commit_tick") <= event_u64(load, "commit_tick"));
    ...
    let data = json
        ...
        .and_then(Value::as_array)
        .expect("multi-store composition run should expose Data trace");
...
fn assert_final_architecture(json: &Value) {
    ...
    assert_eq!(
        ...
        Some("0x8877665506dd00aa")
    );
    assert_eq!(
        json.pointer("/cores/0/registers/x15")
            .and_then(Value::as_str),
        Some("0x8877665506dd00ab")
    );
}

fn multi_store_byte_composition_json(
    path: &Path,
    memory_system: &str,
    max_tick: u64,
...
fn multi_store_byte_composition_binary(name: &str) -> std::path::PathBuf {
