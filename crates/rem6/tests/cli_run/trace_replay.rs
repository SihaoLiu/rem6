...
#[test]
fn rem6_trace_replay_loads_qualified_suite_trace_resource() {
    let workspace = temp_workspace("trace-replay-suite-resource-config-qualified");
    let trace_dir = workspace.join("artifacts");
    std::fs::create_dir(&trace_dir).unwrap();
    std::fs::write(
        trace_dir.join("trace-a.pb"),
        packet_trace_bytes(
            1_000,
            &[
                PacketFields {
    ...
    std::fs::write(
        &resource_config,
        r#"[resource_acquire]
suite_id = "qualified-trace-suite-cli"
stats_format = "json"
...
"#,
    ...
    std::fs::write(
        &config,
        "[trace_replay]\nresource_config = \"resource-acquire-suite.toml\"\ntrace_resource = \"suite-resource:trace-workload-a/trace\"\nroute = \"cpu0.qualified-suite-resource\"\nmemory_start = 4096\nmemory_size = 4096\nmax_tick = 64\ntick_frequency = 1000\nline_bytes = 64\nagent = 7\ncontrol_partition = 2\nstats_format = \"json\"\n",
    )
    .unwrap();
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("\"schema\":\"rem6.cli.trace_replay.v1\""));
    assert!(stdout.contains("\"trace_resource\":\"suite-resource:trace-workload-a/trace\""));
    assert!(stdout.contains("\"route\":\"cpu0.qualified-suite-resource\""));
    assert!(stdout.contains("\"scheduled_count\":1"));
    assert!(stdout.contains("\"trace_read_response_count\":1"));
    assert!(stdout.contains("\"trace_response_data_byte_count\":8"));
    assert_stat(
...
#[test]
fn rem6_trace_replay_cli_trace_resource_overrides_toml_selector() {
    let workspace = temp_workspace("trace-replay-suite-resource-config-cli-selector");
    let trace_dir = workspace.join("artifacts");
    std::fs::create_dir(&trace_dir).unwrap();
    std::fs::write(
        trace_dir.join("trace-a.pb"),
    ...
    std::fs::write(
        &resource_config,
        r#"[resource_acquire]
suite_id = "cli-qualified-trace-suite"
stats_format = "json"
...
id = "trace"
kind = "input"
digest = "sha256:cli-trace-resource-a"
locator = "resources/trace-a.pb"
required = true
acquisition_kind = "local-file"
acquisition_locator = "catalog://trace-a"
artifact = "artifacts/trace-a.pb"
artifact_digest = "sha256:cli-trace-resource-a"
...
id = "trace"
kind = "input"
digest = "sha256:cli-trace-resource-b"
locator = "resources/trace-b.pb"
required = true
acquisition_kind = "local-file"
acquisition_locator = "catalog://trace-b"
artifact = "artifacts/trace-b.pb"
artifact_digest = "sha256:cli-trace-resource-b"
"#,
    )
    .unwrap();
    let config = workspace.join("trace-replay.toml");
    std::fs::write(
        &config,
        "[trace_replay]\nresource_config = \"resource-acquire-suite.toml\"\ntrace_resource = \"suite-resource:trace-workload-b/trace\"\nroute = \"cpu0.config-selector\"\nmemory_start = 4096\nmemory_size = 4096\nmax_tick = 64\ntick_frequency = 1000\nline_bytes = 64\nagent = 7\ncontrol_partition = 2\nstats_format = \"json\"\n",
    )
    .unwrap();
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_rem6"))
        ...
        .args([
            ...
            config.to_str().unwrap(),
            "--trace-resource",
            "suite-resource:trace-workload-a/trace",
            "--route",
            "cpu0.cli-selector",
        ])
        .output()
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("\"trace_resource\":\"suite-resource:trace-workload-a/trace\""));
    assert!(stdout.contains("\"route\":\"cpu0.cli-selector\""));
    assert!(stdout.contains("\"scheduled_count\":1"));
    assert!(stdout.contains("\"trace_read_response_count\":1"));
    assert!(stdout.contains("\"trace_response_data_byte_count\":8"));
}
...
#[test]
fn rem6_trace_replay_power_output_exports_activity_records() {
    let workspace = temp_workspace("trace-replay-power-output");
    ...
    .unwrap();
    let power_path = workspace.join("trace-power.xml");
