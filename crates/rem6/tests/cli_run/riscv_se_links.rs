...
#[test]
fn rem6_run_riscv_se_runs_static_raw_symlinkat_against_qemu() {
    let Some(gcc) = find_riscv_tool("riscv64-unknown-elf-gcc") else {
        eprintln!(
            "skipping static RISC-V SE raw symlinkat smoke: riscv64-unknown-elf-gcc not found"
        );
        return;
    };
    let Some(qemu) = find_riscv_tool("qemu-riscv64") else {
        eprintln!("skipping static RISC-V SE raw symlinkat smoke: qemu-riscv64 not found");
        return;
    };
    let workspace = temp_workspace("riscv-se-raw-symlinkat");
    let source = workspace.join("raw-symlinkat.c");
    let binary = workspace.join("raw-symlinkat");
    let input = workspace.join("guest.txt");
    fs::write(&input, b"file-backed input\n").unwrap();
    fs::write(
        &source,
...
int main(void) {
    char target[32] = {0};
    long linked = linux_syscall3(36, (long)"guest.txt", AT_FDCWD, (long)"alias.txt");
    long read = linux_syscall4(78, AT_FDCWD, (long)"alias.txt", (long)target, sizeof(target));
    long access = linux_syscall4(48, AT_FDCWD, (long)"alias.txt", 0, 0);

    printf("raw-symlinkat:%ld:%ld:%s:%ld\n", linked, read, target, access);
    return linked == 0 && read == 9 && access == 0 ? 43 : 75;
}
"#,
    )
    .unwrap();
    ...
    assert_eq!(qemu_output.stdout, b"raw-symlinkat:0:9:guest.txt:0\n");
    fs::write(&input, b"file-backed input\n").unwrap();
    ...
    assert!(stdout.contains("\"riscv_guest_writes\":[{\"fd\":1"));
    assert!(stdout.contains("\"text\":\"raw-symlinkat:0:9:guest.txt:0\\n\""));
    assert!(stdout.contains("\"riscv_unknown_syscalls\":[]"));
}
