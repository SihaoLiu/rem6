...
fn rem6_run_accepts_scheduler_min_remote_delay_runtime_option() {
    ...
    assert!(stdout.contains("\"executed_ticks\":28"));
    assert!(stdout.contains("\"final_tick\":28"));
    assert_stat(
        &stdout,
    ...
    assert_stat(&stdout, "sim.final_tick", "Tick", 28, "monotonic");
...
fn rem6_run_accepts_memory_route_delay_runtime_option() {
    ...
    assert!(stdout.contains("\"executed_ticks\":68"));
    assert!(stdout.contains("\"final_tick\":68"));
    assert!(stdout.contains("\"x5\":\"0x7\""));
    assert_stat(&stdout, "sim.memory.route_delay", "Tick", 5, "constant");
    assert_stat(&stdout, "sim.final_tick", "Tick", 68, "monotonic");
...
fn rem6_run_lpddr_fetches_record_dram_low_power_residency() {
    ...
    for (json_suffix, stat_suffix, unit) in [
        ...
        for (dram_prefix, resource_prefix, stat_prefix, resource_stat_prefix) in [
            (
                "/dram/targets/0",
            ...
            let hierarchy_value =
                json_u64(&json, &format!("{dram_prefix}/low_power/{json_suffix}"));
            assert_eq!(
                json_u64(&json, &format!("{resource_prefix}/low_power/{json_suffix}")),
                hierarchy_value
            );
            assert_stat(
                &stdout,
            ...
            assert_stat(
                &stdout,
                &format!("{resource_stat_prefix}.low_power.{stat_suffix}"),
                unit,
                hierarchy_value,
                "monotonic",
            );
...
#[test]
fn rem6_run_accepts_bank_group_dram_refresh_policy() {
    let program = riscv64_program(&[
        b_type(0, 0, 0, 0x0), // beq x0, x0, self
    ]);
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);
    let path = temp_binary("dram-memory-bank-group-refresh-policy", &elf);
    ...
    assert_eq!(json_u64(&json, "/dram/profile/scheduler_bank_groups"), 8);
    assert_eq!(
        json.pointer("/dram/profile/timing/refresh_policy")
            .and_then(Value::as_str),
        Some("bank-group")
    ...
    assert_eq!(
        json_u64(&json, "/dram/refresh_ticks"),
        json_u64(&json, "/dram/refreshes") * 4
    );

    assert_stat(
        &stdout,
        "sim.memory.dram.profile.timing.refresh_policy.bank_group",
        "Count",
        1,
        "constant",
    );
...
fn rem6_run_accepts_host_event_delay_runtime_option() {
    ...
    assert!(stdout.contains("\"memory_route_delay\":5"));
    assert!(stdout.contains("\"min_remote_delay\":2"));
    assert!(stdout.contains("\"executed_ticks\":29"));
    assert!(stdout.contains("\"final_tick\":29"));
    assert!(stdout.contains("\"x5\":\"0x7\""));
    assert_stat(&stdout, "sim.host.event_delay", "Tick", 7, "constant");
    assert_stat(&stdout, "sim.final_tick", "Tick", 29, "monotonic");
...
fn rem6_run_accepts_start_address_runtime_option() {
    ...
    assert_stat(&stdout, "sim.final_tick", "Tick", 7, "monotonic");
