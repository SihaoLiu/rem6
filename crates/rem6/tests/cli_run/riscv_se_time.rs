...
#[test]
fn rem6_run_riscv_se_runs_raw_restart_syscall_no_restart_block() {
    let program = riscv64_program(&[
        i_type(-4, 0, 0x0, 5, 0x13),   // addi t0, x0, -EINTR
        i_type(128, 0, 0x0, 17, 0x13), // addi a7, x0, restart_syscall
        0x0000_0073,                   // ecall
        b_type(16, 5, 10, 0x1),        // bne a0, t0, fail
        i_type(90, 0, 0x0, 10, 0x13),  // addi a0, x0, 90
        i_type(93, 0, 0x0, 17, 0x13),  // addi a7, x0, exit
        0x0000_0073,                   // ecall
        i_type(91, 0, 0x0, 10, 0x13),  // addi a0, x0, 91
        i_type(93, 0, 0x0, 17, 0x13),  // addi a7, x0, exit
        0x0000_0073,                   // ecall
    ]);
    let elf = riscv64_elf(0x8000_0000, 0x8000_0000, &program);
    let path = temp_binary("riscv-se-restart-syscall", &elf);
    ...
    assert!(stdout.contains("\"stop_code\":90"), "stdout: {stdout}");
