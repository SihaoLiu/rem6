...
pub(crate) fn riscv64_elf_with_symbol_bindings(
    entry: u64,
    physical: u64,
    payload: &[u8],
) -> Vec<u8> {
...
fn riscv64_elf_with_symbol_bindings_and_visibility(
    ...
    write_u32(&mut bytes, symtab_base + 4, 2);
    write_u64(&mut bytes, symtab_base + 24, symbol_table_offset as u64);
    write_u64(&mut bytes, symtab_base + 32, 4 * 24);
    write_u32(&mut bytes, symtab_base + 40, 2);
    write_u64(&mut bytes, symtab_base + 56, 24);
...
pub(crate) fn riscv64_elf_with_relocation_sections(
    ...
    let relr_offset = bytes.len();
    bytes.resize(relr_offset + 2 * 8, 0);

    let names = b"\0.rela.text\0.rel.meta\0.relr.dyn\0.shstrtab\0";
    let shstr_offset = bytes.len();
    bytes.extend_from_slice(names);
    while bytes.len() % 8 != 0 {
        bytes.push(0);
    ...
    write_u32(&mut bytes, section_table_offset + 196, SHT_RELR);
    write_u64(&mut bytes, section_table_offset + 216, relr_offset as u64);
    write_u64(&mut bytes, section_table_offset + 224, 16);
    write_u64(&mut bytes, section_table_offset + 248, 8);

    write_u32(&mut bytes, section_table_offset + 256, 32);
...
pub(crate) fn riscv64_elf_with_section_hashes(
    entry: u64,
    physical: u64,
    payload: &[u8],
) -> Vec<u8> {
    let mut bytes = riscv64_elf(entry, physical, payload);
    let sysv_hash_offset = bytes.len();
    bytes.resize(sysv_hash_offset + 16, 0);
    let gnu_hash_offset = bytes.len();
    bytes.resize(gnu_hash_offset + 20, 0);

    let names = b"\0.hash\0.gnu.hash\0.shstrtab\0";
    let shstr_offset = bytes.len();
    bytes.extend_from_slice(names);
    while bytes.len() % 8 != 0 {
        bytes.push(0);
    ...
    write_u32(&mut bytes, section_table_offset + 64, 1);
    write_u32(&mut bytes, section_table_offset + 68, SHT_HASH);
    write_u64(
        &mut bytes,
        section_table_offset + 88,
        sysv_hash_offset as u64,
    );
    write_u64(&mut bytes, section_table_offset + 96, 16);

    write_u32(&mut bytes, section_table_offset + 128, 7);
    write_u32(&mut bytes, section_table_offset + 132, SHT_GNU_HASH);
    write_u64(
        &mut bytes,
        section_table_offset + 152,
        gnu_hash_offset as u64,
    );
    write_u64(&mut bytes, section_table_offset + 160, 20);

    write_u32(&mut bytes, section_table_offset + 192, 17);
    write_u32(&mut bytes, section_table_offset + 196, 3);
    write_u64(&mut bytes, section_table_offset + 216, shstr_offset as u64);
    write_u64(&mut bytes, section_table_offset + 224, names.len() as u64);
    bytes
...
pub(crate) fn riscv64_elf_with_section_index_table(
    entry: u64,
    physical: u64,
    payload: &[u8],
) -> Vec<u8> {
    let mut bytes = riscv64_elf(entry, physical, payload);
    let index_table_offset = bytes.len();
    bytes.resize(index_table_offset + 12, 0);

    let names = b"\0.symtab_shndx\0.shstrtab\0";
    let shstr_offset = bytes.len();
    bytes.extend_from_slice(names);
    while bytes.len() % 8 != 0 {
        bytes.push(0);
    ...
    write_u32(&mut bytes, section_table_offset + 64, 1);
    write_u32(&mut bytes, section_table_offset + 68, SHT_SYMTAB_SHNDX);
    write_u64(
        &mut bytes,
        section_table_offset + 88,
        index_table_offset as u64,
    );
