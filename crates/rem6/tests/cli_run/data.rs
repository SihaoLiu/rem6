...
fn rem6_run_executes_riscv_elf_load_store_and_emits_data_stats() {
    ...
    assert!(stdout.contains("\"in_order_pipeline\":{\"cycles\":23,\"retired\":6,"));
...
fn rem6_run_data_cache_prefetcher_accounts_dram_line_fills() {
    ...
    let stdout = run_tagged_next_line_prefetch(&path, 240, true);

    assert!(stdout.contains("\"data_cache_runs\":4"));
    assert!(stdout.contains("\"data_cache_cpu_responses\":2"));
    assert!(stdout.contains("\"data_cache_dram_accesses\":3"));
    assert!(stdout.contains("\"data_cache_prefetch_issued\":2"));
    assert_stat(
        &stdout,
        "sim.data_cache.dram_accesses",
