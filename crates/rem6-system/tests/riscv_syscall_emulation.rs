...
#[test]
fn user_ecall_mlockall_empty_flags_returns_einval_before_exit() {
    let host = PartitionId::new(3);
    ...
    let stop = StopRequest::new(run.final_tick().unwrap(), GuestEventId::new(345), source, 0);
    assert_eq!(run.host_stop(), Some(stop));
    assert!(run.scheduled_traps().is_empty());
    assert_eq!(core.read_register(reg(5)), 0u64.wrapping_sub(22));
    assert_eq!(core.read_register(reg(10)), 0);
    assert_eq!(
        controller.lock().unwrap().run().action_outcomes(),
        &[SystemActionOutcome::Stop(stop)]
...
#[test]
fn user_ecall_fcntl_getlk_writes_no_conflict_lock_before_exit() {
    let host = PartitionId::new(3);
    ...
    core.set_privilege_mode(RiscvPrivilegeMode::User);
    let cluster = RiscvCluster::new([core.clone()]).unwrap();
    let mut flock = vec![0; 32];
    flock[0..2].copy_from_slice(&1u16.to_le_bytes());
    let store = loaded_program_store_with_data(
        &[
            (0x8000, addi(10, 0, 1)),
            (0x8004, addi(11, 0, 5)),
    ...
    let stop = StopRequest::new(run.final_tick().unwrap(), GuestEventId::new(430), source, 2);
    assert_eq!(run.host_stop(), Some(stop));
    assert!(run.scheduled_traps().is_empty());
    assert_eq!(core.read_register(reg(5)), 0);
    assert_eq!(core.read_register(reg(6)), 2);
    assert_eq!(core.read_register(reg(10)), 2);
    assert_eq!(
        controller.lock().unwrap().run().action_outcomes(),
        &[SystemActionOutcome::Stop(stop)]
    );
