...
fn riscv_gdb_remote_session_reports_rv32_hart_csr_snapshot_and_writes() {
    ...
    assert_eq!(registers.len(), rv32_register_hex_offset(168));
...
fn riscv_gdb_remote_register_write_applies_all_rv32_registers() {
    ...
    assert_eq!(hart.machine_counter_inhibit(), 0x05);
    assert_eq!(hart.status().bits(), 0x1234_5678_3000_0007);
    assert_eq!(hart.counter_snapshot().cycle() >> 32, 0x78);
    assert_eq!(hart.counter_snapshot().time() >> 32, 0x34);
    assert_eq!(hart.counter_snapshot().instret() >> 32, 0x9a);
}
...
fn rv32_register_hex_offset(number: u64) -> usize {
    let byte_offset = match number {
        ...
        122..=168 => (33 * 4) + (32 * 8) + (4 * 4) + (20 * 4) + (32 * 16) + ((number - 122) * 4),
...
fn rv32_register_set_write_bytes() -> Vec<u8> {
    ...
    bytes.extend_from_slice(&0x12_u32.to_le_bytes());
    bytes.extend_from_slice(&0x34_u32.to_le_bytes());
    bytes.extend_from_slice(&0x56_u32.to_le_bytes());
    bytes.extend_from_slice(&0x78_u32.to_le_bytes());
    bytes.extend_from_slice(&0x9a_u32.to_le_bytes());
    bytes.extend_from_slice(&0_u32.to_le_bytes());
    bytes.extend_from_slice(&0x05_u32.to_le_bytes());
    bytes
}
