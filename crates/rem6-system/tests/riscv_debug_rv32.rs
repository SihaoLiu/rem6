...
use rem6_isa_riscv::{
    ...
    RiscvStatusWord, RiscvVectorFixedRoundingMode, VectorRegister,
};
use rem6_system::{
...
fn riscv_gdb_remote_session_reports_rv32_hart_csr_snapshot_and_writes() {
    ...
    assert_eq!(registers.len(), rv32_register_hex_offset(124));
...
fn rv32_register_hex_offset(number: u64) -> usize {
    let byte_offset = match number {
        ...
        90..=122 => (33 * 4) + (32 * 8) + (4 * 4) + (20 * 4) + ((number - 90) * 16),
        123..=124 => (33 * 4) + (32 * 8) + (4 * 4) + (20 * 4) + (32 * 16) + ((number - 122) * 4),
