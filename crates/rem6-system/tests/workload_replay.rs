...
use rem6_net::{
    SinicDataDescriptor, SinicInterrupts, SinicRegisterBlock, SinicRegisterOffset,
    SinicRegisterParams,
};
...
fn boot_image_with_sinic_mmio_load() -> BootImage {
    BootImage::new(Address::new(0x8000))
...
fn boot_image_with_sinic_mmio_interrupt_restore_probe() -> BootImage {
    let nop = i_type(0, 0, 0x0, 0, 0x13);
    let mut program = vec![
        u_type(0x20000, 2, 0x37),
        i_type(2, 0, 0x4, 3, 0x13),
        s_type(SinicRegisterOffset::COMMAND.addr() as i32, 3, 2, 0x2, 0x23),
    ];
    program.extend(std::iter::repeat_n(nop, 24));
    program.extend([i_type(
        SinicRegisterOffset::INTR_STATUS.addr() as i32,
        2,
        0x2,
        4,
        0x03,
    ...
    program.extend([
        ...
        i_type(SinicInterrupts::SOFT.bits() as i32, 5, 0x7, 5, 0x13),
        i_type(8, 5, 0x5, 5, 0x13),
        0x0000_0073,
    ]);
...
fn replay_topology_with_sinic_pci_mmio_data() -> WorkloadTopology {
    WorkloadTopology::new(4, 2, 2, WorkloadHostPlacement::new(3, 2, 51).unwrap())
        .unwrap()
        .add_memory_target(
            WorkloadMemoryTarget::new(
        ...
        .add_riscv_core(
            ...
            .unwrap()
            .with_data("cpu0.dmem", route_id("sinic0.mmio"))
            .unwrap(),
        )
        .unwrap()
        .add_sinic_pci_device(
...
fn replay_topology_with_shared_sinic_pci_mmio_endpoint() -> WorkloadTopology {
    WorkloadTopology::new(4, 2, 2, WorkloadHostPlacement::new(3, 2, 51).unwrap())
        .unwrap()
        .add_memory_target(
            WorkloadMemoryTarget::new(
...
fn replay_topology_with_nvm_msi_data_cache() -> WorkloadTopology {
    add_data_cache_backing_route(replay_topology_with_nvm_data_route())
        .with_riscv_data_cache(
            WorkloadRiscvDataCache::new(
                WorkloadDataCacheProtocol::Msi,
                0,
...
fn replay_manifest_with_sinic_pci_mmio_load() -> WorkloadManifest {
    WorkloadManifest::builder(
        workload_id("riscv-replay-sinic-pci-mmio-load"),
        boot_image_with_sinic_mmio_load(),
    )
...
fn replay_manifest_with_sinic_pci_interrupt_checkpoint_restore() -> WorkloadManifest {
    WorkloadManifest::builder(
        workload_id("riscv-replay-sinic-pci-interrupt-checkpoint-restore"),
        boot_image_with_sinic_mmio_interrupt_restore_probe(),
    )
    .with_topology(replay_topology_with_sinic_pci_mmio_data())
    .add_resource(kernel_resource())
    .unwrap()
...
fn replay_manifest_with_shared_sinic_pci_mmio_endpoint_load() -> WorkloadManifest {
    ...
    .with_topology(replay_topology_with_shared_sinic_pci_mmio_endpoint())
    .add_resource(kernel_resource())
    .unwrap()
    .add_required_resource(resource_id("kernel"))
    .add_host_event(WorkloadHostEvent::new(
...
#[test]
fn workload_replay_routes_sinic_pci_bar_loads_to_mmio_registers() {
    let manifest = replay_manifest_with_sinic_pci_mmio_load();
    let plan = WorkloadReplayPlan::from_manifest(&manifest).unwrap();
...
#[test]
fn workload_replay_routes_shared_endpoint_sinic_pci_bar_loads_to_matching_device() {
    let manifest = replay_manifest_with_shared_sinic_pci_mmio_endpoint_load();
    let plan = WorkloadReplayPlan::from_manifest(&manifest).unwrap();
...
#[test]
fn workload_replay_checkpoint_restore_replays_sinic_pci_interrupt_status() {
    let manifest = replay_manifest_with_sinic_pci_interrupt_checkpoint_restore();
    let plan = WorkloadReplayPlan::from_manifest(&manifest).unwrap();
    ...
    let mut registers = SinicRegisterBlock::new(SinicRegisterParams::default()).unwrap();
    registers.restore(snapshot.registers()).unwrap();
    assert_eq!(registers.interrupt_status().bits(), 0);
    assert_eq!(
        outcome.result().checkpoint_labels(),
        &["sinic-interrupt".to_string()]
    );
    assert_eq!(
        outcome.result().restored_checkpoint_labels(),
        &["sinic-interrupt".to_string()]
