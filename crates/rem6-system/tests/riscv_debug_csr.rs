...
fn riscv_gdb_remote_packet_handler_reads_and_writes_advertised_rv64_csr_registers() {
    ...
    hart.set_machine_environment_config(0x0a0b_0c0d_0e0f_1011);
    hart.set_machine_counter_inhibit(0x01);
    ...
    assert_eq!(
        packet_payload(
            handle_riscv_gdb_remote_packet(
                ...
                &mut session,
                &mut hart,
                &GdbRemotePacket::new(b"p9e".to_vec()).unwrap(),
            )
            .unwrap(),
        ),
        b"11100f0e0d0c0b0a",
    ...
    assert_eq!(
        packet_payload(
            handle_riscv_gdb_remote_packet(
                ...
                &mut session,
                &mut hart,
                &GdbRemotePacket::new(b"P9e=554433221100ffee".to_vec()).unwrap(),
            )
            .unwrap(),
        ),
        b"OK",
    );
    assert_eq!(hart.machine_environment_config(), 0xeeff_0011_2233_4455);
    assert_eq!(
        packet_payload(
            handle_riscv_gdb_remote_packet(
                RiscvGdbXlen::Rv64,
                &mut session,
                &mut hart,
                &GdbRemotePacket::new(b"Pa0=0500000000000000".to_vec()).unwrap(),
            )
            .unwrap(),
        ),
        b"OK",
    ...
    assert_eq!(
        &registers[rv64_register_hex_range(160)],
        b"0500000000000000"
    );
    assert_eq!(
