...
use rem6_isa_riscv::{
    FloatRegister, Register, RiscvCounterEnableCsr, RiscvEnvironmentConfigCsr, RiscvFloatCsr,
    RiscvFloatStatus, RiscvGdbXlen, RiscvHartState, RiscvMachineTrapCsr, RiscvStatusWord,
...
fn riscv_gdb_remote_packet_handler_reads_and_writes_rv64d_float_registers() {
    ...
    assert_eq!(registers.len(), rv64_register_hex_offset(160));
...
fn riscv_gdb_remote_register_write_reports_invalid_requests() {
    ...
    assert_eq!(
        ...
        Err(RiscvGdbRegisterWriteError::UnsupportedRegister { number: 160 }),
    );
    ...
    assert_eq!(
        ...
        Err(RiscvGdbRegisterWriteError::InvalidRegisterSetBytes {
            expected: 33 * 4 + 32 * 8 + 4 * 4 + 20 * 4 + 32 * 16 + 45 * 4,
            actual: 8,
        }),
    );
...
fn riscv_gdb_remote_packet_handler_reports_writeback_errors() {
    ...
    assert_eq!(
        handle_riscv_gdb_remote_packet(
            ...
            &mut session,
            &mut hart,
            &GdbRemotePacket::new(b"Pa0=0000000000000000".to_vec()).unwrap(),
        ),
        Err(RiscvGdbRemotePacketError::RegisterWrite(
            RiscvGdbRegisterWriteError::UnsupportedRegister { number: 160 },
        )),
    );
    ...
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"pa0".to_vec()).unwrap())
                .unwrap(),
        ),
        b"E01",
    );
...
fn rv64_register_hex_offset(number: u64) -> usize {
    let byte_offset = match number {
        ...
        122..=160 => (33 * 8) + (32 * 8) + (4 * 4) + (20 * 8) + (32 * 16) + ((number - 122) * 8),
