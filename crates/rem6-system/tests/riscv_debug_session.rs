...
fn riscv_gdb_remote_register_write_reports_invalid_requests() {
    ...
    assert_eq!(
        ...
        Err(RiscvGdbRegisterWriteError::UnsupportedRegister { number: 142 }),
    );
    ...
    assert_eq!(
        ...
        Err(RiscvGdbRegisterWriteError::InvalidRegisterSetBytes {
            expected: 33 * 4 + 32 * 8 + 4 * 4 + 20 * 4 + 32 * 16 + 20 * 4,
            actual: 8,
        }),
    );
...
fn riscv_gdb_remote_packet_handler_reports_writeback_errors() {
    ...
    assert_eq!(
        handle_riscv_gdb_remote_packet(
            ...
            &mut session,
            &mut hart,
            &GdbRemotePacket::new(b"P8e=0000000000000000".to_vec()).unwrap(),
        ),
        Err(RiscvGdbRemotePacketError::RegisterWrite(
            RiscvGdbRegisterWriteError::UnsupportedRegister { number: 142 },
        )),
    );
    ...
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"p8e".to_vec()).unwrap())
                .unwrap(),
        ),
        b"E01",
    );
