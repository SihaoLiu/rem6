...
use rem6_isa_riscv::{
    FloatRegister, Register, RiscvCounterEnableCsr, RiscvCounterInhibitCsr,
    RiscvEnvironmentConfigCsr, RiscvFloatCsr, RiscvFloatStatus, RiscvGdbXlen, RiscvHartState,
...
fn riscv_gdb_remote_packet_handler_reads_and_writes_rv64d_float_registers() {
    ...
    assert_eq!(registers.len(), rv64_register_hex_offset(161));
...
fn riscv_gdb_remote_register_write_reports_invalid_requests() {
    ...
    assert_eq!(
        ...
        Err(RiscvGdbRegisterWriteError::UnsupportedRegister { number: 161 }),
    );
    ...
    assert_eq!(
        ...
        Err(RiscvGdbRegisterWriteError::InvalidRegisterSetBytes {
            expected: 33 * 4 + 32 * 8 + 4 * 4 + 20 * 4 + 32 * 16 + 46 * 4,
            actual: 8,
        }),
    );
...
fn riscv_gdb_remote_packet_handler_reports_writeback_errors() {
    ...
    assert_eq!(
        handle_riscv_gdb_remote_packet(
            ...
            &mut session,
            &mut hart,
            &GdbRemotePacket::new(b"Pa1=0000000000000000".to_vec()).unwrap(),
        ),
        Err(RiscvGdbRemotePacketError::RegisterWrite(
            RiscvGdbRegisterWriteError::UnsupportedRegister { number: 161 },
        )),
    );
    ...
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"pa1".to_vec()).unwrap())
                .unwrap(),
        ),
        b"E01",
    );
...
fn riscv_gdb_remote_core_packet_handler_applies_all_register_vector_fixed_point_csrs() {
    ...
    assert_eq!(
        core.counter_enable_csr(RiscvCounterEnableCsr::Scounteren),
        0x67
    );
    assert_eq!(
        core.counter_enable_csr(RiscvCounterEnableCsr::Mcounteren),
        0x05
    );
    assert_eq!(
    ...
    assert_eq!(
        core.counter_inhibit_csr(RiscvCounterInhibitCsr::Mcountinhibit),
        0x05
    );
    assert_eq!(
        packet_payload(
    ...
    assert_eq!(core.vector_config(), RiscvVectorConfig::new(9, 0xc8));
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"p84".to_vec()).unwrap())
                .unwrap(),
        ),
        b"0900000000000000",
    );
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"p85".to_vec()).unwrap())
                .unwrap(),
        ),
        b"c800000000000000",
    );
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"p86".to_vec()).unwrap())
                .unwrap(),
        ),
        b"1000000000000000",
    );
...
#[test]
fn riscv_gdb_remote_core_packet_handler_rejects_locked_pmp_write_without_ok_response() {
    let core = riscv_core(0x8000);
    core.write_pmp_config_bits(0, 0x80).unwrap();
    let mut session = riscv_gdb_remote_session_from_core(RiscvGdbXlen::Rv64, &core);
    ...
    assert_eq!(
        ...
        Err(RiscvGdbRemotePacketError::RegisterWrite(
            RiscvGdbRegisterWriteError::RejectedRegisterWrite { number: 136 },
        )),
    );
    assert_eq!(core.pmp_snapshot().entries()[0].config().bits(), 0x80);
    assert_eq!(
        packet_payload(
            session
...
#[test]
fn riscv_gdb_remote_core_packet_handler_rejects_locked_pmp_register_set_without_partial_mutation() {
    let core = riscv_core(0x8000);
    core.write_pmp_config_bits(0, 0x80).unwrap();
    let mut session = riscv_gdb_remote_session_from_core(RiscvGdbXlen::Rv64, &core);
    let mut bytes = rv64_register_set_write_bytes();
    bytes[rv64_register_byte_range(1)].copy_from_slice(&0xfeed_face_cafe_beef_u64.to_le_bytes());
    bytes[rv64_register_byte_range(136)].copy_from_slice(&0x0f_u64.to_le_bytes());
...
#[test]
fn riscv_gdb_remote_core_packet_handler_rejects_pmp_register_set_that_locks_before_addr_write_without_partial_mutation(
) {
    let core = riscv_core(0x8000);
    let mut session = riscv_gdb_remote_session_from_core(RiscvGdbXlen::Rv64, &core);
    let mut bytes = rv64_register_set_write_bytes();
    bytes[rv64_register_byte_range(1)].copy_from_slice(&0xfeed_face_cafe_beef_u64.to_le_bytes());
    bytes[rv64_register_byte_range(136)].copy_from_slice(&0x80_u64.to_le_bytes());
    bytes[rv64_register_byte_range(137)].copy_from_slice(&0x200_u64.to_le_bytes());
    ...
    assert_eq!(
        ...
        Err(RiscvGdbRemotePacketError::RegisterWrite(
            RiscvGdbRegisterWriteError::RejectedRegisterWrite { number: 137 },
        )),
    );
    assert_eq!(core.read_register(Register::new(1).unwrap()), 0);
    assert_eq!(core.pmp_snapshot().entries()[0].config().bits(), 0);
    assert_eq!(core.pmp_snapshot().entries()[0].raw_addr(), 0);
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"p1".to_vec()).unwrap())
    ...
    assert_eq!(&registers[rv64_register_hex_range(1)], b"0000000000000000");
    assert_eq!(
        &registers[rv64_register_hex_range(136)],
        b"0000000000000000"
    );
...
fn rv64_register_byte_range(number: u64) -> std::ops::Range<usize> {
    let range = rv64_register_hex_range(number);
...
fn rv64_register_hex_offset(number: u64) -> usize {
    let byte_offset = match number {
        ...
        122..=161 => (33 * 8) + (32 * 8) + (4 * 4) + (20 * 8) + (32 * 16) + ((number - 122) * 8),
