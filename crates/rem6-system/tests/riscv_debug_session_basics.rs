...
fn riscv_gdb_remote_session_serves_rv64_target_documents() {
    ...
    assert!(csr.contains("<reg name=\"mhartid\" bitsize=\"64\" regnum=\"127\"/>"));
...
fn riscv_gdb_remote_session_reports_rv64_hart_register_snapshot() {
    ...
    hart.set_supervisor_environment_config(0x7766_5544_3322_1100);
    ...
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"p7e".to_vec()).unwrap())
                .unwrap(),
        ),
        b"0000000000000000",
    );
    ...
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"p8b".to_vec()).unwrap())
                .unwrap(),
        ),
        b"0000000000000000",
    );
    ...
    assert_eq!(
        packet_payload(
            session
                .handle_packet(&GdbRemotePacket::new(b"p92".to_vec()).unwrap())
                .unwrap(),
        ),
        b"0000000000000000",
    );
