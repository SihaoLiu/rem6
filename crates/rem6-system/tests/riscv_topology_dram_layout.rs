...
use rem6_system::{
    GuestEventId, GuestSourceId, RiscvTopologyDramConfig, RiscvTopologyHostConfig,
    RiscvTopologySystem,
};
use rem6_topology::{
...
fn two_target_dram(second_region: Address) -> RiscvTopologyDramConfig {
    ...
    .add_target(
        ...
        layout(32),
        DramGeometry::new(4, 128, 32).unwrap(),
...
fn system_with_dram(image: &BootImage, dram: RiscvTopologyDramConfig) -> RiscvTopologySystem {
    ...
    .with_host_controller(
        RiscvTopologyHostConfig::new(PartitionId::new(3), 2, GuestSourceId::new(51)),
        StatsRegistry::new(),
    )
    .unwrap()
}
...
#[test]
fn topology_dram_fetch_uses_addressed_target_layout_after_pc_redirect() {
    let image = BootImage::new(Address::new(0x8000))
        ...
        .unwrap();
    let system = system_with_dram(&image, two_target_dram(Address::new(0x9000)));
...
#[test]
fn topology_dram_data_access_uses_addressed_target_layout() {
    let image = BootImage::new(Address::new(0x8000))
        ...
        .unwrap();
    let system = system_with_dram(&image, two_target_dram(Address::new(0xa000)));
    system
        .cluster()
        .core(CpuId::new(0))
        .unwrap()
