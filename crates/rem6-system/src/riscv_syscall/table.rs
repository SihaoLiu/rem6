...
impl RiscvSyscallTable {
    ...
    pub fn handle_with_guest_memory_io_at_tick(
        ...
        match request.number() {
            ...
            RISCV_LINUX_PROCESS_VM_READV => guest_memory_reader.and_then(|reader| {
                guest_memory_writer.map(|writer| RiscvSyscallOutcome::Return {
                    value: syscall_process_vm_readv(request, state, reader, writer),
                })
            }),
            ...
            RISCV_LINUX_PROCESS_VM_WRITEV => guest_memory_reader.and_then(|reader| {
                guest_memory_writer.map(|writer| RiscvSyscallOutcome::Return {
                    value: syscall_process_vm_writev(request, state, reader, writer),
                })
            }),
            ...
            RISCV_LINUX_RECVMMSG => guest_memory_reader.and_then(|reader| {
                guest_memory_writer.map(|writer| {
                    match syscall_recvmmsg(request, state, reader, writer) {
                        Some(value) => RiscvSyscallOutcome::Return { value },
                        None => RiscvSyscallOutcome::Blocked,
                    }
                })
            ...
            RISCV_LINUX_SETTIMEOFDAY => syscall_settimeofday(request, guest_memory_reader)
                .map(|value| RiscvSyscallOutcome::Return { value }),
            RISCV_LINUX_ADJTIMEX => {
                syscall_adjtimex(request, tick, guest_memory_reader, guest_memory_writer)
                    .map(|value| RiscvSyscallOutcome::Return { value })
            }
            RISCV_LINUX_CLOCK_ADJTIME => {
