...
pub type LivelockDiagnosticSubjectSummary = (WaitForNode, usize, u64, Tick, Tick);
...
impl RiscvSystemRun {
    ...
    pub fn parallel_scheduler_livelock_diagnostic_subjects(
        &self,
        threshold: u64,
    ) -> Result<Vec<WaitForNode>, ProgressMonitorError> {
        let diagnostics = self.parallel_scheduler_livelock_diagnostics(threshold)?;
        Ok(collect_livelock_diagnostic_subjects(&diagnostics))
    }

    pub fn parallel_scheduler_livelock_diagnostic_subject_summaries(
        &self,
        threshold: u64,
    ) -> Result<Vec<LivelockDiagnosticSubjectSummary>, ProgressMonitorError> {
        let diagnostics = self.parallel_scheduler_livelock_diagnostics(threshold)?;
        Ok(collect_livelock_diagnostic_subject_summaries(&diagnostics))
    }
    ...
    pub fn parallel_scheduler_livelock_diagnostics_by_subject(
        ...
        let diagnostics = self.parallel_scheduler_livelock_diagnostics(threshold)?;
        Ok(collect_livelock_diagnostics_by_subject(
            &diagnostics,
            subject,
        ))
    }
    ...
    pub fn parallel_scheduler_livelock_diagnostic_tick_window_by_subject(
        ...
        threshold: u64,
        subject: &WaitForNode,
    ) -> Result<Option<(Tick, Tick)>, ProgressMonitorError> {
        let diagnostics = self.parallel_scheduler_livelock_diagnostics(threshold)?;
        Ok(livelock_diagnostic_tick_window(
            &diagnostics,
    ...
    pub fn parallel_scheduler_livelock_diagnostic_subjects_by_transition_kind(
        ...
        let diagnostics = self.parallel_scheduler_livelock_diagnostics(threshold)?;
        Ok(collect_livelock_diagnostic_subjects_by_transition_kind(
            &diagnostics,
            kind,
        ))
    }
    ...
    pub fn parallel_scheduler_livelock_diagnostics_by_transition_kind(
        ...
        let diagnostics = self.parallel_scheduler_livelock_diagnostics(threshold)?;
        Ok(collect_livelock_diagnostics_by_transition_kind(
            &diagnostics,
            kind,
        ))
    }
    ...
    pub fn parallel_scheduler_livelock_diagnostic_tick_window_by_transition_kind(
        ...
        let diagnostics = self.parallel_scheduler_livelock_diagnostics(threshold)?;
        Ok(livelock_diagnostic_transition_kind_tick_window(
            &diagnostics,
            kind,
        ))
    }
    ...
    pub fn parallel_scheduler_livelock_diagnostic_transition_count_by_kind(
        ...
        threshold: u64,
        kind: LivelockTransitionKind,
    ) -> Result<u64, ProgressMonitorError> {
        let diagnostics = self.parallel_scheduler_livelock_diagnostics(threshold)?;
        Ok(livelock_diagnostic_transition_count_by_kind(
            &diagnostics,
            kind,
        ))
    }
    ...
    pub fn parallel_scheduler_livelock_diagnostic_transition_kind_summaries(
        &self,
        threshold: u64,
    ) -> Result<Vec<(LivelockTransitionKind, u64)>, ProgressMonitorError> {
        let diagnostics = self.parallel_scheduler_livelock_diagnostics(threshold)?;
        Ok(collect_livelock_diagnostic_transition_kind_summaries(
            &diagnostics,
        ))
    }
    ...
    pub fn data_cache_parallel_scheduler_livelock_diagnostic_subjects(
        &self,
        threshold: u64,
    ) -> Result<Vec<WaitForNode>, ProgressMonitorError> {
    ...
    pub fn full_system_livelock_diagnostic_subjects(
        &self,
        threshold: u64,
    ) -> Result<Vec<WaitForNode>, ProgressMonitorError> {
...
fn collect_livelock_diagnostic_subject_summaries<'a>(
    diagnostics: impl IntoIterator<Item = &'a LivelockDiagnostic>,
) -> Vec<LivelockDiagnosticSubjectSummary> {
