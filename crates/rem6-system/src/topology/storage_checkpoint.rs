...
use crate::{
    IdeControllerCheckpointBank, IdeControllerCheckpointPort, StorageImageCheckpointBank,
    StorageImageCheckpointPort, SystemError,
};
...
impl RiscvTopologySystem {
    ...
    pub fn with_ide_controller_checkpoint_port(
        mut self,
        port: IdeControllerCheckpointPort,
    ) -> Result<Self, RiscvTopologySystemError> {
        let component = port.component().clone();
        if self.ide_checkpoint_ports.contains_key(&component) {
            return Err(storage_duplicate_component(component));
        }
        if let Some(host) = self.host.as_ref() {
            host.controller
                ...
                .expect("topology host controller lock")
                .executor_mut()
                .attach_ide_controller_checkpoint_port(port.clone())
                .map_err(SystemError::Checkpoint)
                .map_err(RiscvTopologySystemError::System)?;
        }
        self.ide_checkpoint_ports.insert(component, port);
        Ok(self)
    }

    pub fn ide_controller_checkpoint_components(&self) -> Vec<CheckpointComponentId> {
        self.ide_checkpoint_ports.keys().cloned().collect()
    }
    ...
    pub(super) fn attach_storage_checkpoint_to_host(
        ...
        if !self.storage_checkpoint_ports.is_empty() {
            let bank =
                StorageImageCheckpointBank::new(self.storage_checkpoint_ports.values().cloned())
                    .map_err(SystemError::Checkpoint)
                    .map_err(RiscvTopologySystemError::System)?;
            host.controller
                .lock()
        ...
        if !self.ide_checkpoint_ports.is_empty() {
            let bank =
                IdeControllerCheckpointBank::new(self.ide_checkpoint_ports.values().cloned())
                    .map_err(SystemError::Checkpoint)
                    .map_err(RiscvTopologySystemError::System)?;
            host.controller
                .lock()
