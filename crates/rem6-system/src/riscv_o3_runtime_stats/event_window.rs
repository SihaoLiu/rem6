...
impl O3EventWindowRow {
    fn from_event(event: &O3RuntimeTraceRecord) -> Self {
        Self {
            ...
            issue_to_writeback_ticks: event.writeback_tick().saturating_sub(event.issue_tick()),
            writeback_to_commit_ticks: event.commit_tick().saturating_sub(event.writeback_tick()),
            issue_to_commit_ticks: event.commit_tick().saturating_sub(event.issue_tick()),
            pc: event.pc().get(),
