...
use rem6_traffic::{
    TrafficControllerEvent, TrafficStateGeneratorSnapshot, TrafficTraceReplaySource,
};
use rem6_transport::{
...
use crate::{
    DramMemoryCheckpointBank, DramMemoryCheckpointPort, ExecutionMode, GuestEventDelivery,
    GuestEventId, GuestSourceId, HostEventPolicy, MemoryStoreCheckpointBank,
...
fn planned_host_data_cache_checkpoint_action(event: &WorkloadHostEvent) -> Option<&'static str> {
    match event.intent() {
...
fn planned_data_cache_trace_overlap_in_batch(
    batch: &rem6_traffic::TrafficControllerEventBatch,
    planned_ticks: &BTreeMap<Tick, &'static str>,
    layout: CacheLineLayout,
    lines: &BTreeSet<Address>,
...
fn traffic_controller_event_batch_min_tick(
    batch: &rem6_traffic::TrafficControllerEventBatch,
) -> Option<Tick> {
    batch
        .events()
...
fn take_planned_host_data_cache_sync_error(
    errors: &Arc<Mutex<Vec<RiscvWorkloadReplayError>>>,
