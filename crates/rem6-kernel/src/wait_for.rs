...
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct WaitForEdgeKindWindow {
...
impl WaitForEdgeKindWindow {
    ...
    pub fn from_edges(edges: impl IntoIterator<Item = WaitForEdge>) -> Vec<Self> {
        ...
        for edge in edges {
            ...
            by_kind
                .entry(edge.kind())
                .and_modify(|stored: &mut Self| stored.merge(window))
                .or_insert(window);
        }
...
#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct WaitForTargetNodeWindow {
    node: WaitForNode,
    edge_count: usize,
    first_tick: Tick,
    last_tick: Tick,
...
impl WaitForGraphSnapshot {
    ...
    pub fn target_node_windows(&self) -> Vec<WaitForTargetNodeWindow> {
        WaitForTargetNodeWindow::from_edges(self.edges.iter().cloned())
    }
    ...
    pub fn edge_kind_windows(&self) -> Vec<WaitForEdgeKindWindow> {
        WaitForEdgeKindWindow::from_edges(self.edges.iter().cloned())
    }
...
impl WaitForGraph {
    ...
    pub fn target_node_windows(&self) -> Vec<WaitForTargetNodeWindow> {
        WaitForTargetNodeWindow::from_edges(self.edges())
    }
