...
fn invalidate_writable(agent_id: u32, sequence: u64, address: u64) -> MemoryRequest {
    MemoryRequest::invalidate_writable(
        request_id(agent_id, sequence),
        Address::new(address),
        layout(),
    )
...
fn locked_rmw_read(agent_id: u32, sequence: u64, address: u64) -> MemoryRequest {
...
fn locked_rmw_write(agent_id: u32, sequence: u64, address: u64, data: Vec<u8>) -> MemoryRequest {
    let size = size(data.len() as u64);
    MemoryRequest::locked_rmw_write(
        request_id(agent_id, sequence),
        Address::new(address),
        size,
        data,
...
fn load_locked(agent_id: u32, sequence: u64, address: u64) -> MemoryRequest {
...
fn store_conditional(agent_id: u32, sequence: u64, address: u64, data: Vec<u8>) -> MemoryRequest {
    let size = size(data.len() as u64);
...
#[test]
fn msi_bank_harness_byte_snapshot_round_trips_pending_writable_invalidate_target() {
    let config = MshrQueueConfig::new(2, 3, 0).unwrap();
    let mut bank = MsiCacheBank::new_with_mshr(agent(1), layout(), config.clone());
    bank.accept_cpu_request(read(1, 50, 0x2004)).unwrap();

    let snapshot = bank.snapshot();
    let current_mshr = snapshot.mshr().unwrap();
    let current_entry = &current_mshr.entries()[0];
    let invalidation = invalidate_writable(1, 51, 0x2000);
    let mut targets = current_entry.targets().to_vec();
    targets.push(MshrTarget::from_parts(
        invalidation.clone(),
        1,
...
#[test]
fn msi_bank_harness_byte_snapshot_round_trips_pending_locked_rmw_targets() {
    let config = MshrQueueConfig::new(2, 4, 0).unwrap();
    let mut bank = MsiCacheBank::new_with_mshr(agent(1), layout(), config.clone());
    bank.accept_cpu_request(read(1, 60, 0x2004)).unwrap();

    let snapshot = bank.snapshot();
    let current_mshr = snapshot.mshr().unwrap();
    let current_entry = &current_mshr.entries()[0];
    let locked_read = locked_rmw_read(1, 61, 0x2008);
    let locked_write = locked_rmw_write(1, 62, 0x2008, vec![0x5a; 8]);
    let mut targets = current_entry.targets().to_vec();
    targets.push(MshrTarget::from_parts(
        locked_read.clone(),
        1,
    ...
    assert_eq!(rebuilt, snapshot);
    assert_eq!(targets[1].request(), &locked_read);
    assert_eq!(targets[2].request(), &locked_write);
}

#[test]
fn msi_bank_harness_byte_snapshot_round_trips_pending_llsc_targets() {
    let config = MshrQueueConfig::new(2, 4, 0).unwrap();
    let mut bank = MsiCacheBank::new_with_mshr(agent(1), layout(), config.clone());
    bank.accept_cpu_request(read(1, 63, 0x2104)).unwrap();

    let snapshot = bank.snapshot();
    let current_mshr = snapshot.mshr().unwrap();
    let current_entry = &current_mshr.entries()[0];
    let load = load_locked(1, 64, 0x2108);
    let store = store_conditional(1, 65, 0x2108, vec![0x6b; 8]);
    let mut targets = current_entry.targets().to_vec();
    targets.push(MshrTarget::from_parts(
        load.clone(),
        1,
    ...
    assert_eq!(rebuilt, snapshot);
    assert_eq!(targets[1].request(), &load);
    assert_eq!(targets[2].request(), &store);
}
