...
fn fp_vector_live_issue_locks_ledger_scope() {
    ...
    assert!(cpu.contains(
        "broader FP load shapes, conversions, comparisons, moves, classification, dynamic-CSR/status-sensitive chains, true vector-register/load/VCSR forwarding, arbitrary dependency graphs, positive system issue rows, a general load/store queue scheduler, dependent atomics or broader dependent-store shapes, checkpoint-restorable live IQ/transport state, and a general O3 engine remain incomplete"
    ));
