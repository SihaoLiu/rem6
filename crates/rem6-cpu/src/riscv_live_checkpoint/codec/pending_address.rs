...
use super::{
    ...
    read_register, read_request, u32v, u64v, write_address, write_fetch, write_register,
    write_request, Error, Reader, MAX_ENDPOINT_BYTES, MAX_FETCH_BYTES, MAX_ROWS,
};
use crate::riscv_live_checkpoint::RiscvO3LiveCheckpointPendingDataAddress;

pub(super) fn preflight(
    value: Option<&RiscvO3LiveCheckpointPendingDataAddress>,
) -> Result<(), Error> {
...
pub(super) fn write(
    ...
    u64v(out, value.published_producer_ready_tick);
    u64v(out, value.requested_wake_tick);
    Ok(())
}
...
pub(super) fn read(
    ...
    Ok(Some(RiscvO3LiveCheckpointPendingDataAddress {
        ...
        lsq_kind,
        expected_lsq_bytes,
        published_producer_ready_tick: reader.u64("pending published producer-ready tick")?,
        requested_wake_tick: reader.u64("pending requested wake tick")?,
    }))
}
