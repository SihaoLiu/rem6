...
fn completed_response_marks_only_matching_rows_ready() {
    ...
    assert_eq!(
        runtime.take_ready_live_scalar_memory_event(u64::MAX),
        Some(completed)
    );
    let snapshot = runtime.snapshot();
    let live_rob = snapshot
...
fn retry_response_removes_load_head_younger_rows_and_readies_one_abort_event() {
    ...
    assert!(runtime
        .complete_live_scalar_memory_response(&retry, data_request, 44, 7, None,)
        .unwrap());
...
fn failed_response_drains_rows_and_never_counts_o3_retirement() {
    ...
    assert!(runtime
        .complete_live_scalar_memory_response(&failed, data_request, 43, 12, None,)
        .unwrap());
