...
fn executed_direct_return_runtime(
    ...
    let (mut runtime, forwarded, _) = super::same_link_return::recorded_x1_same_link_runtime();
    if retire_data_head {
...
fn executed_scalar_return_runtime() -> (O3RuntimeState, u64) {
    let (mut runtime, _, descendant) =
        super::same_link_scalar_return::recorded_same_link_scalar_runtime();
    super::same_link_scalar_return::retire_data_head(&mut runtime);
    let instruction = jalr_return(1);
    let sequence = runtime
        ...
        .expect("same-link scalar return append");
    let candidate = runtime
        .live_speculative_issue_candidate(Address::new(0x9004), instruction)
        .expect("same-link scalar return candidate");
    assert!(runtime
        .record_live_speculative_execution(
            candidate,
            &[request(14)],
...
fn direct_return_requires_dependency_and_window_membership() {
    ...
    assert_eq!(
        runtime.producer_forwarded_same_link_return_descendant(),
        None
    );
...
fn direct_return_requires_bound_fetch_identity() {
    let (mut runtime, _, sequence) = executed_direct_return_runtime(false);
    assert!(runtime.replace_same_link_chain_fetch_identity_for_test(sequence, &[request(99)],));
    assert_eq!(
        runtime.producer_forwarded_same_link_return_descendant(),
        None
    );
...
fn head_retired_direct_return_reconstructs_exact_recorded_parent() {
    ...
    assert!(runtime.replace_same_link_chain_fetch_identity_for_test(
