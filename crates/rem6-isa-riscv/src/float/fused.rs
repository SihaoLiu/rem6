...
use super::{
    box_canonical_single, has_single_sign, is_infinity_single, is_nan_single,
    is_signaling_nan_single, is_zero_single, unbox_single, FLOAT_FLAG_INEXACT, FLOAT_FLAG_INVALID,
...
fn single_overflows(
    ...
    if f32::from_bits(lhs)
        .mul_add(f32::from_bits(rhs), f32::from_bits(addend))
        .is_infinite()
    {
        return true;
    }
    exact_single_value_from_bits(lhs, rhs, addend)
        .map(|exact| {
            result_bits_are_infinite(round_exact_single(lhs, rhs, addend, exact, rounding_mode))
        })
        .unwrap_or(false)
}
...
fn result_bits_are_infinite(bits: u32) -> bool {
    bits & !SINGLE_SIGN_BIT == SINGLE_EXP_MASK
}
