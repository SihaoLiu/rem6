...
#[test]
fn decoder_accepts_rv64f_arithmetic_static_and_dynamic_rounding_modes() {
    assert_eq!(
    ...
    assert_eq!(
        RiscvInstruction::decode(r_type(0x00, 3, 2, 0x1, 5, 0x53)).unwrap(),
        RiscvInstruction::FloatAddS {
            rd: freg(5),
            rs1: freg(2),
            rs2: freg(3),
    ...
    assert_eq!(
        RiscvInstruction::decode(r_type(0x2c, 0, 2, 0x2, 5, 0x53)).unwrap(),
        RiscvInstruction::FloatSqrtS {
            rd: freg(5),
            rs1: freg(2),
    ...
    assert_eq!(
        RiscvInstruction::decode(r4_type(4, 0x0, 3, 2, 0x4, 5, 0x43)).unwrap(),
        RiscvInstruction::FloatMultiplyAddS {
            rd: freg(5),
            rs1: freg(2),
            rs2: freg(3),
...
#[test]
fn hart_rv64f_comparisons_raise_invalid_for_nan_by_opcode() {
    let mut hart = RiscvHartState::new(0x8000);
    hart.write_float(freg(1), box_single(0x7fc0_0001));
    ...
    let equal_quiet = hart
        .execute(RiscvInstruction::FloatEqualS {
            rd: reg(6),
            rs1: freg(1),
            rs2: freg(2),
        })
