...
fn decoder_accepts_vsetvli() {
    let decoded = RiscvInstruction::decode(vsetvli_type(0x02, 10, 5)).unwrap();
...
fn hart_executes_vsetvli_and_updates_vl_vtype() {
    ...
    let record = hart
        .execute(RiscvInstruction::decode(vsetvli_type(0x02, 10, 5)).unwrap())
        .unwrap();

    assert_eq!(hart.pc(), 0x8004);
    assert_eq!(hart.vector_config(), RiscvVectorConfig::new(4, 0x02));
