...
mod manifest_dram_low_power_activity;
...
pub use parallel_expectation::{
    ...
    WorkloadExpectedDramLowPowerActivity, WorkloadExpectedFabricHopActivity,
...
pub use resource::{
    WorkloadDiskImageConstruction, WorkloadDiskImageConstructionStep, WorkloadResource,
...
pub struct WorkloadManifest {
    ...
    expected_dram_low_power_activity: Option<WorkloadExpectedDramLowPowerActivity>,
...
impl WorkloadManifestBuilder {
    fn new(id: WorkloadId, boot: WorkloadBootImage) -> Self {
        Self {
            ...
            expected_dram_low_power_activity: None,
    ...
    pub fn add_expected_gups_run_summary(
        mut self,
        expected: WorkloadExpectedGupsRunSummary,
    ) -> Result<Self, WorkloadError> {
        if self
            .expected_gups_run_summaries
            .iter()
            .any(|existing| existing.sort_key() == expected.sort_key())
        {
    ...
    pub fn build(mut self) -> Result<WorkloadManifest, WorkloadError> {
        ...
        let identity = manifest_identity(ManifestIdentityInput {
            ...
            expected_planned_parallel_batch_utilization: &self
                .expected_planned_parallel_batch_utilization,
            ...
            expected_dram_low_power_activity: self.expected_dram_low_power_activity,
