...
fn set_elf64_section_alignment(bytes: &mut [u8], index: usize, alignment: u64) {
    let section_table_offset = read_u64(bytes, 40) as usize;
    let section_header_size = usize::from(read_u16(bytes, 58));
    let section_base = section_table_offset + index * section_header_size;
    write_u64(bytes, section_base + 48, alignment);
}
...
fn elf64_image_with_section_index_table(machine: u16) -> Vec<u8> {
    let mut elf = elf64_image_with_named_sections(machine, &[".symtab_shndx"]);
...
fn elf64_image_with_extra_symbol_section_index(machine: u16, extra_section_index: u16) -> Vec<u8> {
    elf64_image_with_symbol_section_options(
        machine,
        ".symtab",
        2,
...
fn elf64_image_with_symbol_visibility(machine: u16) -> Vec<u8> {
    elf64_image_with_symbol_section_and_visibility(machine, ".symtab", 2, ".strtab", 1, 2, 3)
}
...
fn elf64_image_with_symbol_section_and_object_type(
    machine: u16,
    symbol_section_name: &str,
    symbol_section_kind: u32,
    string_section_name: &str,
...
fn elf64_image_with_symbol_section_options(
    machine: u16,
    symbol_section_name: &str,
    symbol_section_kind: u32,
    string_section_name: &str,
    ...
    let extra_name_offset = extra_symbol.map(|_| {
        let offset = symbol_names.len() as u32;
        symbol_names.extend_from_slice(b"extra_sym\0");
        offset
    });
    ...
    let symbol_entry_count = if extra_symbol.is_some() { 4 } else { 3 };
    bytes.resize(bytes.len() + symbol_entry_count * 24, 0);
...
#[test]
fn workload_boot_image_preserves_elf_tls_symbol_summary_round_trip() {
    let image = BootImage::from_elf64_le(&elf64_image_with_tls_symbol(243)).unwrap();
    ...
    assert_eq!(round_trip_metadata.symbol_summary().tls_count(), 1);
}
...
fn workload_boot_image_preserves_elf_symbol_section_index_summary_round_trip() {
    ...
    assert_eq!(symbols.absolute_section_count(), 0);
    assert_eq!(symbols.undefined_section_count(), 0);
}
...
fn workload_boot_image_preserves_elf_section_flags_round_trip() {
    ...
    set_elf64_section_address(&mut elf, 2, 0x8000_2002);
    set_elf64_section_address(&mut elf, 3, 0x8000_3000);
    set_elf64_section_alignment(&mut elf, 1, 0x1000);
    set_elf64_section_alignment(&mut elf, 2, 4);
    set_elf64_section_alignment(&mut elf, 3, 16);
    set_elf64_section_size(&mut elf, 1, 4);
    set_elf64_section_size(&mut elf, 2, 8);
    ...
    let storage = round_trip_metadata.section_storage();
    assert_eq!(
        storage.file_backed_bytes(),
        4 + 8 + round_trip_metadata.section_name_table().byte_size()
    );
    assert_eq!(storage.allocated_bytes(), 4 + 8 + 16);
    ...
    let alignment = round_trip_metadata.section_alignment();
...
#[test]
fn workload_boot_image_preserves_elf_section_index_table_round_trip() {
    let image = BootImage::from_elf64_le(&elf64_image_with_section_index_table(243)).unwrap();
    ...
    let round_trip_metadata = workload_image
        ...
        .elf_metadata()
        .unwrap();
    let index_tables = round_trip_metadata.section_index_tables();
...
fn workload_manifest_identity_includes_elf_symbol_summary() {
    ...
    let defined_symbols =
        BootImage::from_elf64_le(&elf64_image_with_extra_symbol_section_index(243, 1)).unwrap();
    let common_symbols = BootImage::from_elf64_le(&elf64_image_with_extra_symbol_section_index(
        243, SHN_COMMON,
    ))
    .unwrap();
    ...
    let symbol_manifest = WorkloadManifest::builder(id("same"), symbols)
        .build()
        .unwrap();
    let extra_symbol_manifest = WorkloadManifest::builder(id("same"), extra_symbols)
        .build()
        .unwrap();
    ...
    let ifunc_symbol_manifest = WorkloadManifest::builder(id("same"), ifunc_symbols)
        .build()
        .unwrap();
    let defined_symbol_manifest = WorkloadManifest::builder(id("same"), defined_symbols)
        .build()
        .unwrap();
    let common_symbol_manifest = WorkloadManifest::builder(id("same"), common_symbols)
        .build()
        .unwrap();
    ...
    assert_ne!(
        ...
        common_symbol_manifest.identity()
    );
    assert_ne!(
        defined_symbol_manifest.identity(),
...
#[test]
fn workload_manifest_identity_includes_elf_section_storage() {
    let mut small_elf = elf64_image_with_named_sections(243, &[".data"]);
    set_elf64_section_kind_flags(&mut small_elf, 1, 1, SHF_ALLOC | SHF_WRITE);
    set_elf64_section_size(&mut small_elf, 1, 4);
    let mut large_elf = elf64_image_with_named_sections(243, &[".data"]);
    set_elf64_section_kind_flags(&mut large_elf, 1, 1, SHF_ALLOC | SHF_WRITE);
    set_elf64_section_size(&mut large_elf, 1, 8);
    let small_source = BootImage::from_elf64_le(&small_elf).unwrap();
    let large_source = BootImage::from_elf64_le(&large_elf).unwrap();

    assert_eq!(small_source.entry(), large_source.entry());
    assert_eq!(small_source.segments(), large_source.segments());
    assert_eq!(
        small_source.elf_metadata().unwrap().section_header_table(),
        large_source.elf_metadata().unwrap().section_header_table(),
    );
    assert_eq!(
        small_source.elf_metadata().unwrap().section_flags(),
        large_source.elf_metadata().unwrap().section_flags(),
    );
    assert_ne!(
        small_source
            .elf_metadata()
    ...
    let small = WorkloadManifest::builder(id("same"), small_source)
        .build()
        .unwrap();
    let large = WorkloadManifest::builder(id("same"), large_source)
        .build()
        .unwrap();
...
#[test]
fn workload_manifest_identity_includes_elf_section_index_tables() {
    let mut baseline_elf = elf64_image_with_named_sections(243, &[".symtab_shndx"]);
    set_elf64_section_size(&mut baseline_elf, 1, 12);
    set_elf64_section_entry_size(&mut baseline_elf, 1, 4);
    let index_elf = elf64_image_with_section_index_table(243);
    let baseline_source = BootImage::from_elf64_le(&baseline_elf).unwrap();
    let index_source = BootImage::from_elf64_le(&index_elf).unwrap();

    assert_eq!(baseline_source.entry(), index_source.entry());
    assert_eq!(baseline_source.segments(), index_source.segments());
    assert_eq!(
        baseline_source
            .elf_metadata()
            .unwrap()
            .section_header_table(),
        index_source.elf_metadata().unwrap().section_header_table(),
    );
    assert_eq!(
        baseline_source.elf_metadata().unwrap().section_storage(),
        index_source.elf_metadata().unwrap().section_storage(),
    );
    assert_ne!(
        baseline_source
            .elf_metadata()
    ...
    let baseline = WorkloadManifest::builder(id("same"), baseline_source)
        .build()
        .unwrap();
    let index = WorkloadManifest::builder(id("same"), index_source)
        .build()
        .unwrap();
...
#[test]
fn workload_manifest_identity_includes_elf_section_alignment() {
    let mut loose_elf = elf64_image_with_named_sections(243, &[".data"]);
    set_elf64_section_kind_flags(&mut loose_elf, 1, 1, SHF_ALLOC | SHF_WRITE);
    set_elf64_section_address(&mut loose_elf, 1, 0x8000_1000);
    set_elf64_section_alignment(&mut loose_elf, 1, 8);
    set_elf64_section_size(&mut loose_elf, 1, 4);
    let mut strict_elf = elf64_image_with_named_sections(243, &[".data"]);
    set_elf64_section_kind_flags(&mut strict_elf, 1, 1, SHF_ALLOC | SHF_WRITE);
    set_elf64_section_address(&mut strict_elf, 1, 0x8000_1000);
    set_elf64_section_alignment(&mut strict_elf, 1, 16);
    set_elf64_section_size(&mut strict_elf, 1, 4);
    let loose_source = BootImage::from_elf64_le(&loose_elf).unwrap();
    let strict_source = BootImage::from_elf64_le(&strict_elf).unwrap();

    assert_eq!(loose_source.entry(), strict_source.entry());
    assert_eq!(loose_source.segments(), strict_source.segments());
    assert_eq!(
        loose_source.elf_metadata().unwrap().section_address_range(),
        strict_source
            .elf_metadata()
            .unwrap()
            .section_address_range(),
    ...
    assert_ne!(
        loose_source.elf_metadata().unwrap().section_alignment(),
        strict_source.elf_metadata().unwrap().section_alignment(),
    );

    let loose = WorkloadManifest::builder(id("same"), loose_source)
        .build()
        .unwrap();
    let strict = WorkloadManifest::builder(id("same"), strict_source)
        .build()
        .unwrap();
