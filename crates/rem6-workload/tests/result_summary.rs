...
fn workload_result_records_parallel_execution_summary() {
    ...
    assert_eq!(summary.fabric_credit_delay_ticks(), 11);
    assert_eq!(summary.fabric_max_credit_delay_ticks(), 7);
...
#[test]
fn workload_result_uses_explicit_full_system_batch_worker_tick_streak_summaries() {
    let cpu = PartitionId::new(1);
    let cache = PartitionId::new(2);
    let scoped_scheduler = WorkloadParallelBatchTimelineRecord::new(
        WorkloadParallelBatchScope::Scheduler,
...
#[test]
fn workload_result_batch_threshold_uses_merged_worker_bucket_evidence() {
    let cpu = PartitionId::new(1);
    let cache = PartitionId::new(2);
    let dma = PartitionId::new(3);
    let summary = WorkloadParallelExecutionSummary::default()
...
#[test]
fn workload_result_batch_count_uses_merged_worker_bucket_evidence() {
    let cpu = PartitionId::new(1);
    let cache = PartitionId::new(2);
    let dma = PartitionId::new(3);
    let summary = WorkloadParallelExecutionSummary::default()
...
#[test]
fn workload_result_dispatch_count_uses_merged_worker_bucket_evidence() {
    let cpu = PartitionId::new(1);
    let cache = PartitionId::new(2);
    let dma = PartitionId::new(3);
    let summary = WorkloadParallelExecutionSummary::default()
    ...
    assert_eq!(summary.full_system_parallel_scheduler_dispatch_count(), 19);
}
