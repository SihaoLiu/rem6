...
#[test]
fn workload_result_merges_trace_data_cache_maintenance_response_policy_counts() {
    ...
    let manifest =
        rem6_workload::WorkloadManifest::builder(id("manifest-trace-cache-merge"), boot_image())
            .add_resource(kernel_resource())
            .unwrap()
            .add_required_resource(resource_id("kernel"))
    ...
    let clean = WorkloadTrafficTraceReplaySummary::new(route_id("trace.cache"), 1)
    ...
    let invalidate = WorkloadTrafficTraceReplaySummary::new(route_id("trace.cache"), 2)
        .with_trace_data_cache_response_count(2)
        .with_trace_data_cache_maintenance_response_count(1)
        .with_trace_data_cache_invalidate_maintenance_response_count(1);
    let result = WorkloadResult::new(plan.manifest_identity(), 32)
        .with_traffic_trace_replay_summary(clean)
        .with_traffic_trace_replay_summary(invalidate);

    let summaries = result.traffic_trace_replay_summaries();
    assert_eq!(summaries.len(), 1);
    let summary = &summaries[0];
    assert_eq!(summary.trace_data_cache_response_count(), 3);
    assert_eq!(summary.trace_data_cache_maintenance_response_count(), 2);
    assert_eq!(
...
fn workload_manifest_records_typed_traffic_trace_sideband_expectations() {
    let expected = expected_trace_summary("trace.sideband", 4, 0, 0, 0, 0, 0, 4)
        ...
        .with_minimum_trace_tlb_sync_flushed_entry_count(1)
    ...
    let actual = actual_trace_summary("trace.sideband", 4, 0, 0, 0, 0, 0, 4)
        ...
        .with_trace_tlb_sync_flushed_entry_count(1)
    ...
    assert_eq!(actual.trace_tlb_sync_flushed_entry_count(), 1);
    let result = WorkloadResult::new(plan.manifest_identity(), 32)
        .with_traffic_trace_replay_summary(actual.clone());
...
fn workload_manifest_records_trace_write_completion_expectations() {
    let expected = WorkloadExpectedTrafficTraceReplaySummary::new(route_id("trace.write"))
        ...
        .with_minimum_memory_write_completion_byte_count(8);
    let manifest = rem6_workload::WorkloadManifest::builder(
    ...
    let actual = WorkloadTrafficTraceReplaySummary::new(route_id("trace.write"), 3)
        ...
        .with_memory_write_completion_byte_count(8);
    let result = WorkloadResult::new(plan.manifest_identity(), 32)
        .with_traffic_trace_replay_summary(actual.clone());
    assert_eq!(result.traffic_trace_replay_summaries(), &[actual]);
    plan.verify_result(&result).unwrap();
...
fn workload_manifest_records_trace_data_cache_response_expectations() {
    let expected = WorkloadExpectedTrafficTraceReplaySummary::new(route_id("trace.cache"))
        ...
        .with_minimum_trace_data_cache_clean_maintenance_response_count(1)
        .with_minimum_trace_data_cache_invalidate_maintenance_response_count(0);
    let manifest = rem6_workload::WorkloadManifest::builder(
    ...
    let actual = WorkloadTrafficTraceReplaySummary::new(route_id("trace.cache"), 2)
        ...
        .with_trace_data_cache_maintenance_response_count(1)
        .with_trace_data_cache_clean_maintenance_response_count(1)
        .with_trace_data_cache_invalidate_maintenance_response_count(0);
    assert_eq!(actual.trace_data_cache_maintenance_response_count(), 1);
    assert_eq!(
        actual.trace_data_cache_clean_maintenance_response_count(),
        1
    );
    assert_eq!(
        actual.trace_data_cache_invalidate_maintenance_response_count(),
        0
    );
    let result = WorkloadResult::new(plan.manifest_identity(), 32)
        .with_traffic_trace_replay_summary(actual.clone());
...
fn workload_manifest_identity_changes_with_typed_trace_sideband_expectations() {
    ...
    let trace_tlb_sync =
        rem6_workload::WorkloadManifest::builder(id("identity-typed-sideband"), boot_image())
            ...
            .build()
            .unwrap();
    let trace_tlb_sync_flushed_entries =
        rem6_workload::WorkloadManifest::builder(id("identity-typed-sideband"), boot_image())
            .add_resource(kernel_resource())
            .unwrap()
            .add_required_resource(resource_id("kernel"))
    ...
    assert_ne!(
        trace_tlb_sync.identity(),
        trace_tlb_sync_flushed_entries.identity()
    );
    assert_ne!(
...
fn workload_manifest_identity_changes_with_trace_data_cache_response_expectations() {
    ...
    .build()
    .unwrap();
    let clean_maintenance_response = rem6_workload::WorkloadManifest::builder(
        id("identity-trace-data-cache-response"),
        boot_image(),
    )
    .add_resource(kernel_resource())
    ...
    .build()
    .unwrap();
    let invalidate_maintenance_response = rem6_workload::WorkloadManifest::builder(
        id("identity-trace-data-cache-response"),
        boot_image(),
    )
    .add_resource(kernel_resource())
    ...
    assert_ne!(generic.identity(), clean_maintenance_response.identity());
    assert_ne!(
        generic.identity(),
        invalidate_maintenance_response.identity()
    );
    assert_ne!(cache_response.identity(), maintenance_response.identity());
    assert_ne!(
        clean_maintenance_response.identity(),
        invalidate_maintenance_response.identity()
    );
}
...
fn workload_replay_plan_rejects_underreported_typed_sideband_summary() {
    ...
    assert_eq!(
        error.to_string(),
        "traffic trace replay summary for route trace.sideband has scheduled 4/4, responses 0/0, trace completed responses 0/0, trace retry responses 0/0, trace store-conditional failed responses 0/0, trace read responses 0/0, trace write responses 0/0, trace prefetch responses 0/0, trace invalidate responses 0/0, trace clean responses 0/0, trace upgrade responses 0/0, trace llsc responses 0/0, trace locked-rmw responses 0/0, trace writable-intent responses 0/0, trace response data bytes 0/0, trace response fill data bytes 0/0, memory trace events 0/0, memory write completions 0/0, memory write completion bytes 0/0, trace data-cache responses 0/0, trace data-cache maintenance responses 0/0, trace data-cache clean maintenance responses 0/0, trace data-cache invalidate maintenance responses 0/0, trace data-cache errors 0/0, trace data-cache invalid-destination errors 0/0, trace data-cache bad-address errors 0/0, trace data-cache read errors 0/0, trace data-cache write errors 0/0, trace data-cache functional-read errors 0/0, trace data-cache functional-write errors 0/0, memory failures 0/0, memory failure invalid destinations 0/0, memory failure bad addresses 0/0, memory failure reads 0/0, memory failure writes 0/0, memory failure functional reads 0/0, memory failure functional writes 0/0, trace errors 0/0, trace invalid-destination errors 0/0, trace bad-address errors 0/0, trace read errors 0/0, trace write errors 0/0, trace functional-read errors 0/0, trace functional-write errors 0/0, trace htm accesses 0/0, trace htm begins 0/0, control acks 0/0, sync control acks 0/0, htm control acks 0/0, control failures 0/0, control failure invalid destinations 0/0, control failure bad addresses 0/0, control failure reads 0/0, control failure writes 0/0, control failure functional reads 0/0, control failure functional writes 0/0, sync control failures 0/0, tlb control failures 0/0, cache control failures 0/0, htm control failures 0/0, diagnostic control failures 0/0, sideband events 4/4, trace sideband failures 0/0, tlb sync events 1/1, trace tlb syncs 0/1, trace tlb sync flushed entries 0/1, cache flush events 0/1, trace cache flushes 0/1, trace cache flush data bytes 0/0, trace l1 invalidations 0/1, diagnostic print events 0/0, trace diagnostics 0/1, htm abort events 0/0, trace htm aborts 0/1",
    );
}
...
#[test]
fn workload_replay_plan_rejects_underreported_trace_write_completion_bytes() {
    let expected = expected_trace_summary("trace.write.bytes", 3, 1, 3, 0, 0, 0, 0)
        .with_minimum_memory_write_completion_count(1)
        .with_minimum_memory_write_completion_byte_count(16);
    let manifest = rem6_workload::WorkloadManifest::builder(
        id("trace-write-completion-bytes-mismatch"),
        boot_image(),
    )
    .add_resource(kernel_resource())
    .unwrap()
    ...
    let actual = actual_trace_summary("trace.write.bytes", 3, 1, 3, 0, 0, 0, 0)
        .with_memory_write_completion_count(1)
        .with_memory_write_completion_byte_count(8);
    let underreported = WorkloadResult::new(plan.manifest_identity(), 32)
        .with_traffic_trace_replay_summary(actual.clone());
...
fn workload_replay_plan_rejects_underreported_trace_data_cache_response_summary() {
    ...
    assert_eq!(
        error.to_string(),
        "traffic trace replay summary for route trace.cache has scheduled 2/0, responses 2/2, trace completed responses 0/0, trace retry responses 0/0, trace store-conditional failed responses 0/0, trace read responses 0/0, trace write responses 0/0, trace prefetch responses 0/0, trace invalidate responses 0/0, trace clean responses 0/0, trace upgrade responses 0/0, trace llsc responses 0/0, trace locked-rmw responses 0/0, trace writable-intent responses 0/0, trace response data bytes 0/0, trace response fill data bytes 0/0, memory trace events 0/0, memory write completions 0/0, memory write completion bytes 0/0, trace data-cache responses 1/2, trace data-cache maintenance responses 0/0, trace data-cache clean maintenance responses 0/0, trace data-cache invalidate maintenance responses 0/0, trace data-cache errors 0/0, trace data-cache invalid-destination errors 0/0, trace data-cache bad-address errors 0/0, trace data-cache read errors 0/0, trace data-cache write errors 0/0, trace data-cache functional-read errors 0/0, trace data-cache functional-write errors 0/0, memory failures 0/0, memory failure invalid destinations 0/0, memory failure bad addresses 0/0, memory failure reads 0/0, memory failure writes 0/0, memory failure functional reads 0/0, memory failure functional writes 0/0, trace errors 0/0, trace invalid-destination errors 0/0, trace bad-address errors 0/0, trace read errors 0/0, trace write errors 0/0, trace functional-read errors 0/0, trace functional-write errors 0/0, trace htm accesses 0/0, trace htm begins 0/0, control acks 0/0, sync control acks 0/0, htm control acks 0/0, control failures 0/0, control failure invalid destinations 0/0, control failure bad addresses 0/0, control failure reads 0/0, control failure writes 0/0, control failure functional reads 0/0, control failure functional writes 0/0, sync control failures 0/0, tlb control failures 0/0, cache control failures 0/0, htm control failures 0/0, diagnostic control failures 0/0, sideband events 0/0, trace sideband failures 0/0, tlb sync events 0/0, trace tlb syncs 0/0, trace tlb sync flushed entries 0/0, cache flush events 0/0, trace cache flushes 0/0, trace cache flush data bytes 0/0, trace l1 invalidations 0/0, diagnostic print events 0/0, trace diagnostics 0/0, htm abort events 0/0, trace htm aborts 0/0",
    );
}
...
#[test]
fn workload_replay_plan_rejects_underreported_trace_data_cache_clean_response_summary() {
    let expected = WorkloadExpectedTrafficTraceReplaySummary::new(route_id("trace.cache.clean"))
        .with_minimum_trace_data_cache_response_count(1)
        .with_minimum_trace_data_cache_maintenance_response_count(1)
        .with_minimum_trace_data_cache_clean_maintenance_response_count(1);
    let manifest = rem6_workload::WorkloadManifest::builder(
        id("trace-data-cache-clean-maintenance-response-mismatch"),
        boot_image(),
    )
    .add_resource(kernel_resource())
    .unwrap()
    ...
    let actual = WorkloadTrafficTraceReplaySummary::new(route_id("trace.cache.clean"), 1)
        .with_trace_data_cache_response_count(1)
        .with_trace_data_cache_maintenance_response_count(1)
        .with_trace_data_cache_clean_maintenance_response_count(0);
    let underreported = WorkloadResult::new(plan.manifest_identity(), 32)
        .with_traffic_trace_replay_summary(actual.clone());
    let error = plan.verify_result(&underreported).unwrap_err();
    assert_eq!(
    ...
    assert!(
        error
            .to_string()
            .contains("trace data-cache clean maintenance responses 0/1"),
        "{error}"
    );
}

#[test]
fn workload_replay_plan_rejects_underreported_trace_data_cache_invalidate_response_summary() {
    let expected =
        WorkloadExpectedTrafficTraceReplaySummary::new(route_id("trace.cache.invalidate"))
            .with_minimum_trace_data_cache_response_count(1)
            .with_minimum_trace_data_cache_maintenance_response_count(1)
            .with_minimum_trace_data_cache_invalidate_maintenance_response_count(1);
    let manifest = rem6_workload::WorkloadManifest::builder(
        id("trace-data-cache-invalidate-maintenance-response-mismatch"),
        boot_image(),
    )
    .add_resource(kernel_resource())
    .unwrap()
    ...
    let actual = WorkloadTrafficTraceReplaySummary::new(route_id("trace.cache.invalidate"), 1)
        .with_trace_data_cache_response_count(1)
        .with_trace_data_cache_maintenance_response_count(1)
        .with_trace_data_cache_invalidate_maintenance_response_count(0);
    let underreported = WorkloadResult::new(plan.manifest_identity(), 32)
    ...
    assert!(
        error
            .to_string()
            .contains("trace data-cache invalidate maintenance responses 0/1"),
        "{error}"
    );
}
...
fn workload_replay_plan_rejects_underreported_trace_error_summary() {
    ...
    assert_eq!(
        error.to_string(),
        "traffic trace replay summary for route trace.error has scheduled 1/0, responses 0/0, trace completed responses 0/0, trace retry responses 0/0, trace store-conditional failed responses 0/0, trace read responses 0/0, trace write responses 0/0, trace prefetch responses 0/0, trace invalidate responses 0/0, trace clean responses 0/0, trace upgrade responses 0/0, trace llsc responses 0/0, trace locked-rmw responses 0/0, trace writable-intent responses 0/0, trace response data bytes 0/0, trace response fill data bytes 0/0, memory trace events 0/0, memory write completions 0/0, memory write completion bytes 0/0, trace data-cache responses 0/0, trace data-cache maintenance responses 0/0, trace data-cache clean maintenance responses 0/0, trace data-cache invalidate maintenance responses 0/0, trace data-cache errors 0/0, trace data-cache invalid-destination errors 0/0, trace data-cache bad-address errors 0/0, trace data-cache read errors 0/0, trace data-cache write errors 0/0, trace data-cache functional-read errors 0/0, trace data-cache functional-write errors 0/0, memory failures 1/1, memory failure invalid destinations 0/0, memory failure bad addresses 0/0, memory failure reads 0/0, memory failure writes 0/0, memory failure functional reads 0/0, memory failure functional writes 0/0, trace errors 0/1, trace invalid-destination errors 0/0, trace bad-address errors 0/0, trace read errors 0/0, trace write errors 0/0, trace functional-read errors 0/0, trace functional-write errors 0/0, trace htm accesses 0/0, trace htm begins 0/0, control acks 0/0, sync control acks 0/0, htm control acks 0/0, control failures 0/0, control failure invalid destinations 0/0, control failure bad addresses 0/0, control failure reads 0/0, control failure writes 0/0, control failure functional reads 0/0, control failure functional writes 0/0, sync control failures 0/0, tlb control failures 0/0, cache control failures 0/0, htm control failures 0/0, diagnostic control failures 0/0, sideband events 0/0, trace sideband failures 0/0, tlb sync events 0/0, trace tlb syncs 0/0, trace tlb sync flushed entries 0/0, cache flush events 0/0, trace cache flushes 0/0, trace cache flush data bytes 0/0, trace l1 invalidations 0/0, diagnostic print events 0/0, trace diagnostics 0/0, htm abort events 0/0, trace htm aborts 0/0",
    );
}
...
fn workload_replay_plan_rejects_underreported_trace_htm_access_summary() {
    ...
    assert_eq!(
        error.to_string(),
        "traffic trace replay summary for route trace.htm has scheduled 3/0, responses 2/2, trace completed responses 0/0, trace retry responses 0/0, trace store-conditional failed responses 0/0, trace read responses 0/0, trace write responses 0/0, trace prefetch responses 0/0, trace invalidate responses 0/0, trace clean responses 0/0, trace upgrade responses 0/0, trace llsc responses 0/0, trace locked-rmw responses 0/0, trace writable-intent responses 0/0, trace response data bytes 0/0, trace response fill data bytes 0/0, memory trace events 0/0, memory write completions 0/0, memory write completion bytes 0/0, trace data-cache responses 0/0, trace data-cache maintenance responses 0/0, trace data-cache clean maintenance responses 0/0, trace data-cache invalidate maintenance responses 0/0, trace data-cache errors 0/0, trace data-cache invalid-destination errors 0/0, trace data-cache bad-address errors 0/0, trace data-cache read errors 0/0, trace data-cache write errors 0/0, trace data-cache functional-read errors 0/0, trace data-cache functional-write errors 0/0, memory failures 0/0, memory failure invalid destinations 0/0, memory failure bad addresses 0/0, memory failure reads 0/0, memory failure writes 0/0, memory failure functional reads 0/0, memory failure functional writes 0/0, trace errors 0/0, trace invalid-destination errors 0/0, trace bad-address errors 0/0, trace read errors 0/0, trace write errors 0/0, trace functional-read errors 0/0, trace functional-write errors 0/0, trace htm accesses 0/2, trace htm begins 0/0, control acks 0/0, sync control acks 0/0, htm control acks 0/0, control failures 0/0, control failure invalid destinations 0/0, control failure bad addresses 0/0, control failure reads 0/0, control failure writes 0/0, control failure functional reads 0/0, control failure functional writes 0/0, sync control failures 0/0, tlb control failures 0/0, cache control failures 0/0, htm control failures 0/0, diagnostic control failures 0/0, sideband events 0/0, trace sideband failures 0/0, tlb sync events 0/0, trace tlb syncs 0/0, trace tlb sync flushed entries 0/0, cache flush events 0/0, trace cache flushes 0/0, trace cache flush data bytes 0/0, trace l1 invalidations 0/0, diagnostic print events 0/0, trace diagnostics 0/0, htm abort events 0/0, trace htm aborts 0/0",
    );
}
