...
#[test]
fn workload_replay_plan_rejects_same_route_inverted_actual_parallel_remote_flow_timing() {
    let plan = replay_plan()
        .add_expected_parallel_remote_flow(expected(
            WorkloadParallelRemoteFlowScope::Scheduler,
            0,
    ...
    let summary =
        WorkloadParallelExecutionSummary::default().with_parallel_scheduler_remote_flows([
            ParallelRemoteFlowRecord::new(PartitionId::new(0), PartitionId::new(1), 1, 3, 7),
            ParallelRemoteFlowRecord::new(PartitionId::new(0), PartitionId::new(1), 1, 17, 11),
        ]);
    let result =
        WorkloadResult::new(plan.manifest_identity(), 32).with_parallel_execution_summary(summary);
...
#[test]
fn workload_replay_plan_rejects_full_system_same_route_inverted_actual_parallel_remote_flow_timing()
{
    let plan = replay_plan()
        .add_expected_parallel_remote_flow(expected(
            WorkloadParallelRemoteFlowScope::FullSystem,
