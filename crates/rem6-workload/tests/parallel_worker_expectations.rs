...
#[test]
fn workload_replay_plan_derives_max_workers_from_batch_histograms() {
    let manifest =
        rem6_workload::WorkloadManifest::builder(id("parallel-workers-from-batches"), boot_image())
            .add_resource(kernel_resource())
            .unwrap()
            .add_required_resource(resource_id("kernel"))
            .build()
...
#[test]
fn workload_replay_plan_rejects_malformed_timeline_for_parallel_worker_use() {
    let manifest =
        rem6_workload::WorkloadManifest::builder(id("parallel-workers-malformed"), boot_image())
            .add_resource(kernel_resource())
            .unwrap()
            .add_required_resource(resource_id("kernel"))
            .build()
...
#[test]
fn workload_replay_plan_uses_stronger_max_worker_evidence_than_batch_histograms() {
    let manifest = rem6_workload::WorkloadManifest::builder(
        id("parallel-workers-prefers-stronger-partition-sets"),
        boot_image(),
    )
    .add_resource(kernel_resource())
    .unwrap()
