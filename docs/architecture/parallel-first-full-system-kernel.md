...
| MSHR resources can become implicit queue state inside a cache. | Cache MSHR queues are typed records with explicit entry allocation, target coalescing, prefetch reserve, ready state, service state, retry state, completion, and snapshot operations. | MSHR tests assert target merging, target limits, demand reserve behavior, ready ordering, service transitions, completion, and restore behavior. |
