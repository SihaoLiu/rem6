...
- The gem5 resources reproducibility work identifies complex disk image
  ...
  deterministic manifest data. Dispatch-declared execution expectations reject
  unreachable suite parallelism requirements before a run, and suite-level
  execution efficiency summaries record wall-clock span, serial work,
  capacity, idle worker time, speedup, and utilization. Dispatch-declared
  speedup and utilization thresholds are checked with integer ratios, and
  minimum simultaneous-worker checks turn multi-workload orchestration into a
  typed contract instead of ad hoc external scripts:
