...
| QoS scheduling can depend on memory-controller pointers and global requestor lookup. | QoS arbitration is explicit typed state with requestor IDs, checked priorities, queue policy state, snapshots, fabric batch reservation in grant order, transport first-hop batch scheduling through a shared arbiter, DRAM same-arrival timing batches ordered before bank, row, and bus timing are computed, and typed read/write direction preference among same-priority timing candidates. Fixed-priority assignment, FIFO/LIFO/LRG queue arbitration, NoC link reservation, transport event timing, DRAM timing order, and DRAM turnaround choice do not require a global object graph. | QoS tests assert fixed requestor priority assignment, default priority fallback, priority validation, highest-priority selection, FIFO/LIFO ordering, LRG requestor rotation, non-mutating empty polls, snapshot replay, QoS-driven shared-link reservation order, transport-level first-hop scheduling order, DRAM-level timing batch order, and DRAM read/write direction preference. |
