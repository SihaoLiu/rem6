...
| `rem6-boot` | Boot images, auto-detected typed ELF32 and ELF64 little-endian loadable-segment parsing, checked segment memory ranges, typed ELF class, machine, architecture, OS ABI, flags, and operating-system metadata, manifest-preserved boot metadata, segments, initial memory population, and workload input metadata. |
