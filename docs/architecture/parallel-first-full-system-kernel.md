...
| QoS scheduling can depend on memory-controller pointers and global requestor lookup. | QoS arbitration is explicit typed state with requestor IDs, checked priorities, queue policy state, snapshots, fabric batch reservation in grant order, and transport first-hop batch scheduling through a shared arbiter. Fixed-priority assignment, FIFO/LIFO/LRG queue arbitration, NoC link reservation, and transport event timing do not require a global object graph. | QoS tests assert fixed requestor priority assignment, default priority fallback, priority validation, highest-priority selection, FIFO/LIFO ordering, LRG requestor rotation, non-mutating empty polls, snapshot replay, QoS-driven shared-link reservation order, and transport-level first-hop scheduling order. |
