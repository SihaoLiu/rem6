...
| `rem6-stats` | Typed counters, structured scope/name paths, typed unit registration, structured units and rate units, registry ownership, statistics reset, typed dump history, schema-and-reset-scope-checked snapshot deltas, and run metadata. |
...
| Statistics | Tests for registry ownership, structured counter scope/name identity, path grammar, structured unit and rate grammar, monotonic reset behavior, typed dump records, schema-and-reset-scope-checked deltas, and aggregation into system summaries. |
