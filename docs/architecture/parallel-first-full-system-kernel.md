...
data-cache scheduler, and merged full-system progress counts plus compact
summaries by transition kind, partition, and subject. Workload result summaries
add deterministic dimension lists, per-dimension record slices, counts, tick
windows, compact summaries, and threshold-driven livelock diagnostic counts
