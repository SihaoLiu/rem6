...
counts, subject queries, and transition-kind summaries for CPU-scheduler,
data-cache scheduler, and merged full-system scopes before workload replay
translates them into manifest-verifiable result summaries; clean-diagnostic
violations include the dirty livelock subjects so a failing replay identifies
the stuck component or resource. Useful work resets the active window so retry-heavy but productive
...
Result summaries expose livelock diagnostic subject queries across the same
scopes so replay failures and post-run analysis can point to the stalled
component and dominant transition kind rather than only the aggregate counter.
...
| Livelock diagnostics | Tests that create repeated progress-free transitions, assert exact replay records, subject and kind queries, and bounded diagnostics. |
...
- run summaries can expose the diagnostic context by subject and transition kind.
