...
| QoS scheduling can depend on memory-controller pointers and global requestor lookup. | QoS arbitration is explicit typed state with requestor IDs, checked priorities, queue policy state, snapshots, and fabric batch reservation in grant order. Fixed-priority assignment, FIFO/LIFO/LRG queue arbitration, and NoC link reservation do not require a global object graph. | QoS tests assert fixed requestor priority assignment, default priority fallback, priority validation, highest-priority selection, FIFO/LIFO ordering, LRG requestor rotation, non-mutating empty polls, snapshot replay, and QoS-driven shared-link reservation order. |
