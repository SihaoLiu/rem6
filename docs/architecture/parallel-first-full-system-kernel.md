...
| Replacement policy state can be hidden behind cache tags. | Cache replacement policies are typed per-set records with explicit invalidate, reset, touch, victim, and snapshot operations. | Replacement tests assert LRU, FIFO, MRU, LFU, BRRIP, and TreePLRU decisions and restore behavior. |
