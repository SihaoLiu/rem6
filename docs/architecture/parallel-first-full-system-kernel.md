...
| `rem6-boot` | Boot images, focused parser/image/error module boundaries, auto-detected typed ELF32 and ELF64 little-endian loadable-segment parsing, checked segment memory ranges, typed ELF class, machine, architecture, OS ABI, flags, operating-system metadata, section-derived ABI fallbacks, manifest-preserved boot metadata, segments, initial memory population, and workload input metadata. |
