...
| `rem6-net` | Ethernet packet payloads, wire-length metadata, packet FIFO capacity, reservation, slack, fixed full-duplex link timing, deterministic delay variation, explicit transmission records, delivery drain, learning-switch forwarding, ordered output queue tail drops, output queue timing, ready-output records, and snapshot state. |
