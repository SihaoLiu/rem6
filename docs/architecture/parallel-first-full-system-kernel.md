...
| MSHR resources can become implicit queue state inside a cache. | Cache MSHR queues are typed records with explicit entry allocation, target coalescing, prefetch reserve, ready state, service state, retry state, completion, and snapshot operations. MSI cache banks can attach these queues and replay coalesced same-line read targets from a single fill. | MSHR and MSI bank tests assert target merging, target limits, demand reserve behavior, ready ordering, service transitions, completion, restore behavior, same-line read miss coalescing, and restored coalesced targets. |
