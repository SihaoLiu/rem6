...
| Front-end speculation can hide predictor state and unwind behavior. | Branch predictors and return stacks are per-model typed state with explicit prediction, update, speculative history, repair, and snapshot records. | Predictor tests assert counter training, target updates, return-stack operations, speculation commit, speculation repair, restore behavior, and incompatible snapshot rejection. |
