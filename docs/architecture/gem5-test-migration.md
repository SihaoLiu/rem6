...
| `tests/gem5/cpu_tests` | `rem6-cpu`, `rem6-system` | partial | RISC-V atomic execution, frontend, basic/GShare/Tournament retired branch-predictor training, non-deferred, completed memory-data, and completed MMIO-load retire-to-in-order commit-cycle records, O3 policy, and store-conditional progress tests exist. | Extend in-order timing across fetch/decode/execute stalls and memory backpressure, then add ROB/LSQ-backed O3 execution tests. |
...
| `tests/gem5/se_mode` | `rem6-system`, `rem6` CLI | partial | RISC-V SE startup, real ecall handling, newlib `printf`, time, file/stdin smoke tests, `ENOSYS` records, and guest write artifacts exist. | Add more static-libc syscalls, process/thread lifecycle behavior, host-backed file policy, and distro-like user binaries. |
| `tests/gem5/memory` | `rem6-memory`, `rem6-cache`, `rem6-dram`, `rem6-fabric` | partial | Stores, requests, responses, page maps, TLBs, translation queues, cache banks, MSI CacheBank-backed RISC-V and accelerator-DMA topology data paths, DRAM/NVM counters, and fabric activity tests exist. | Add full CPU-facing multi-level cache and NoC path tests, bank-backed scheduler/resource summary parity, DRAM refresh, and preset coverage. |
