...
| `tests/gem5/cpu_tests` | `rem6-cpu`, `rem6-system` | partial | RISC-V atomic execution, frontend, retired branch predictor training, in-order state, O3 policy, and store-conditional progress tests exist. | Add executable in-order timing tests that drive real RISC-V fetch/decode/execute through cycle records, then add ROB/LSQ-backed O3 execution tests. |
