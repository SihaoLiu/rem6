...
- [x] CLI `run --execute --stats-format json --debug-flags Exec,Fetch,Data,Cache,Dram,Fabric,Memory,Syscall,HostAction` emits
  deterministic instruction, fetch, data-access, cache hierarchy, DRAM hierarchy including low-power target/port/bank state counters, fabric lane/hop, memory-transport with response-latency ticks, and RISC-V SE syscall plus m5 host-action trace records from real RISC-V execution paths.
...
remaining broad CSR GDB register-cache coverage, broader debug-run stat parity beyond current trace, host-action, classification, and selected aggregate counters, runtime resource counters,
