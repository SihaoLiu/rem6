...
`sim.debug.branch_trace.mispredictions`, `sim.debug.branch_trace.flushed`, and CPU/kind/outcome scoped stats from executed in-order branch-prediction cycle records.
The branch debug test checks flushed counts against emitted flushed-sequence lists.
