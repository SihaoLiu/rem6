...
- [x] RVV unmasked floating-point `vfadd.vv` exact finite SEW=32 lane execution, reserved-`frm` trap, and CPU fetch-ahead integration have tests.
...
`vfadd.vv` exact finite SEW=32 floating-point lanes, reserved-`frm` trap, and
fetch-ahead integration, plus `vxrm`/`vxsat`/`vcsr` CSR visibility through the
