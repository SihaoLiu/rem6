...
shells, DMA routing, top-level `accelerator-run` NPU/GPU command dispatch and completion stats, and a
...
coalesced memory records, and snapshot restore of queued ISA programs, `rem6 accelerator-run` CLI smoke coverage for scheduled NPU inference and GPU kernel commands with command, completion, trace-event, and scheduler stats, and
