...
- [x] RVV unmasked floating-point `vfadd.vv`, `vfsub.vv`, and `vfmul.vv` exact finite SEW=32 lane execution have tests, with `vfadd.vv` fetch-ahead coverage and `vfsub.vv`/`vfmul.vv` CPU fetch-stream coverage.
...
`vfadd.vv`/`vfsub.vv`/`vfmul.vv` exact finite SEW=32 floating-point lanes,
reserved-`frm` trap, `vfadd.vv` fetch-ahead coverage, and `vfsub.vv`/`vfmul.vv`
CPU fetch-stream coverage, plus `vxrm`/`vxsat`/`vcsr` CSR visibility through
the CPU fetch stream or hart execution; mask logical `.mm` forms through decode,
