...
`getpriority`/`setpriority` raw priority reporting and nice-state updates, current-process `pidfd_open` and `pidfd_send_signal(..., 0)` fd-lifecycle checks,
