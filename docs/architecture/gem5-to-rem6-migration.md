...
- [x] CLI `multi-run` sequentially orchestrates multiple real `run --config`, `gups --config`, `gpu-run --config`, and `trace-replay --config` entries from TOML and command-qualified `--run` entries through the top-level binary and emits aggregate JSON and stats artifacts, including explicit failure summaries when configured to continue after child errors.
...
layout CLI test, `rem6 multi-run` mixed TOML and command-qualified run/GUPS/GPU/trace-replay aggregate artifact/stat plus continue-on-failure CLI tests, `rem6 gups` profile-summary CLI tests, `rem6 gpu-run --config`
