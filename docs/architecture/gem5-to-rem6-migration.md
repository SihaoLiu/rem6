...
- [x] RVV integer `vmerge` masked vv/vx/vi forms, `vmv.v` unmasked v/x/i forms, and `vcompress.vm` decode, hart execution, and CPU fetch-stream execution have tests.
...
UF, and directed-boundary overflow accrual, `fdiv.s` and `fdiv.d`
...
`vmv.v`, compress `vcompress.vm`, fixed-point narrow clip `vnclipu.wi`, and
unmasked floating-point `vfadd`, `vfsub`, `vfmin`, `vfmax`, `vfmul`, and
...
CPU fetch-stream tests; and traps, translation helpers, and SE ecall plumbing.
...
add/sub/divide/fused multiply-add, double-precision divide, and narrow
...
`vector_fixed_point`, `vector_integer`, `vector_merge_move`,
`vector_mask_compare`, `vector_mask_logical`, `riscv_frontend`, `sv39`, and
`riscv_vector_float_frontend`, and privileged RISC-V tests.
...
- [x] Basic, GShare, BiMode, Tournament, and TAGE-SC-L branch predictors are trained from retired control flow.
...
and data-translation drivers issuing a bounded fetch-ahead for completed straight-line 4-byte integer instructions, predictor-selected conditional
branches including top-level `rem6 run --riscv-branch-predictor gshare`, `bimode`, and `tage-sc-l` fetch-steering evidence, and direct RISC-V `JAL`/`JALR` targets before retire, with direct `JAL`
...
stats, top-level default RISC-V instruction/data traffic through L1/L2/L3 MSI
...
taken RISC-V branch; they also expose branch-speculation prediction, repair, younger-removal, and maximum-pending counters for a real nested conditional branch run where the corrected older branch prevents younger wrong-path register writes from committing. CLI `rem6 run --riscv-branch-predictor gshare`, `bimode`, and `tage-sc-l` drive the normal RISC-V fetch-ahead path with the selected predictor and change deterministic fetch-steering timing while preserving the final architectural register state.
RISC-V in-order timing tests cover retired taken/fall-through redirect,
completed-fetch overlap, serial/translated/parallel fetch-ahead, direct `JAL`
and register-target `JALR` fetch-ahead preservation, compressed
...
**Score calculation:** 9 of 14 items have executable evidence, or 64% raw. The bucket cap is
single-axis because Ruby-scale protocol networks, router/virtual-channel NoC
behavior beyond counted flits and credit-delay attribution, broader DRAM
refresh breadth, and hierarchy-wide prefetch translation consumers remain
incomplete.
...
credit depth, and fabric JSON/stat counters including flit and credit-delay
counts. Volatile CLI external-memory
profiles carry refresh/recovery timing; DDR4, DDR5, and
HBM presets validate `tREFI`/`tRFC`; DDR and LPDDR CLI RISC-V paths emit
refresh and precharge-powerdown stats from routed fetches. DRAM target activity
...
active-partition, worker-count, and profiled backing-DRAM access,
target/port/bank, row, command, and ready-latency stats in artifact JSON and
`StatsRegistry`; a combined CLI trace-replay run proves explicit MSI data-cache plus explicit fabric routing
...
run. Bare top-level `rem6 run --isa riscv --execute` now selects the cache/fabric/DRAM hierarchy by default; `--memory-system direct` remains an explicit direct-memory configuration for timing-focused runs.

**Not migrated:** Ruby-scale protocol networks, gem5-class router and virtual-channel NoC detail, broad JEDEC preset validation, broader hierarchy-level prefetch translation consumers, and broad DRAM refresh/preset breadth.
...
data-cache run, explicit MSI L1-to-L2-to-DRAM plus L1-to-L2-to-L3-to-DRAM
data/instruction-cache fill smoke coverage, and a combined top-level RISC-V
cache/fabric/DRAM smoke exposing `sim.memory.fabric.*` counters including
flits and credit-delay ticks, and `--memory-system cache-fabric-dram` preset
coverage. DRAM memory-profile tests cover bank-level resource counters,
...
**Next evidence:** Ruby-scale protocol-network execution, router and
virtual-channel NoC routing beyond counted fabric flits, broader low-power
state-transition coverage, and hierarchy-level prefetch translation consumers.
...
fd/link visibility, vector I/O, positional vector I/O, guest-file `sendfile`,
...
`clock_nanosleep` validation, `gettimeofday`, `clock_getres`, `clock_gettime64`, `CLOCK_TAI` `clock_gettime`,
...
futex wait mismatch, zero-timeout wait, wait-bitset timeout validation,
...
hard-link/`renameat`/`renameat2` flags=0/unlink/legacy `mkdir`/`mkdirat`/`AT_REMOVEDIR`, and
...
extension probes, top-level CLI DBCN shared-memory write, top-level CLI TIME
`set_timer` deadline artifact/stat reporting plus boot-hart and HSM-started secondary-hart supervisor-handler interrupt delivery, library/workload
...
top-level CLI RFENCE remote FENCE.I artifact/stat records, and
...
guest address against qemu; static raw `prctl`
...
endpoints, positional vector I/O unit coverage for guest files, pipe `ioctl(FIONREAD)` unread-byte reporting, terminal `ioctl(TIOCGWINSZ/TCGETS)` reporting, plus a static newlib program that roundtrips bytes through `pipe2`,
...
`riscv_se_stdio`, `riscv_se_pvec`, `riscv_se_sync`, `riscv_se_tee`, `riscv_se_truncate`, `riscv_se_mknod`, `riscv_se_vmsplice`, `riscv_se_xattr`, `riscv_syscall_mknod`, `riscv_syscall_pipe`, `riscv_syscall_eventfd`, `riscv_syscall_ioctl`, `riscv_syscall_readv`,
...
- [x] GDB RV64D floating-point, including XML-aligned `fflags`/`frm`/`fcsr` and placeholder numbering, advertised RV64 CSR target descriptions including supervisor `sscratch`, supervisor environment `senvcfg`, translation `satp`, interrupt aliases `sie`/`sip`, machine identity `mhartid`/`mvendorid`/`marchid`/`mimpid`, machine ISA `misa`, counter `cycle`/`time`/`instret`, machine-counter aliases `mcycle`/`minstret`, and first PMP entries `pmpcfg0`/`pmpaddr0`, and RV64 machine status, interrupt, trap, identity, ISA, environment-config, counter, and PMP0 CSR register-cache paths exist, with `cycle`/`mcycle`, `time`, and `instret`/`minstret` remaining GDB-visible across real GDB single-step execution and PMP0 writes updating the same `RiscvCore` PMP state consumed by CPU access checks.
...
`mhartid`/`mvendorid`/`marchid`/`mimpid`, machine ISA `misa`, counter `cycle`/`time`/`instret`, and first PMP registers `pmpcfg0`/`pmpaddr0`,
RV64 machine status, interrupt, trap, identity, ISA, environment-config, counter, and PMP0 CSR register-cache paths, including independent
GDB-visible `cycle` and `time` state across CSR writes and single-step execution and PMP0 writes reflected in the live `RiscvCore` PMP table, and
...
from executed cache, transport, and DRAM paths,
...
fabric-route activity and flit counters, top-level trace-replay aggregate resource
...
target/port/bank, row, command, and latency counters,
...
`rem6 run --power-output` plus `rem6 gpu-run --power-output` McPAT-shaped and
DSENT-shaped power-analysis exports.

**Not migrated:** Complete gem5 text-stat parity, full debug execution control,
remaining broad CSR GDB register-cache coverage, integrated debug-run stat roll-up,
...
PMP0 CSR target/register-cache coverage for `pmpcfg0`/`pmpaddr0` through both the core packet handler and top-level `rem6 run --gdb-listen`,
...
`numInsts`/`numCycles` aliases, and multicore CPU `numInsts`/`numCycles`
...
**Score calculation:** 14 of 19 items have executable evidence, or 74% raw.
The bucket cap is single-axis because GPU memory behavior now has narrow
...
- [x] CLI `run`, `gups`, `trace-replay`, and `gpu-run` plus TOML configuration have tests; repository `gups` and `gpu-run` example configs run through the top-level CLI without recompilation, TOML-driven `run` output layouts create nested artifact directories for run JSON, stats JSON, and power analysis output, `gups` emits traffic profile summaries from the executed controller, and TOML-driven `gpu-run` executes recorded GPU global memory traffic through cache/DRAM and writes activity-derived power-analysis and NoMali-compatible adapter output.
...
- [x] CLI `gpu-run` routes recorded coalesced scalar GPU global memory requests through direct memory, explicit fabric, or MSI data-cache and DRAM-backed runtime stats after GPU workgroup completion, with bank-level cache counters, tagged next-line data-cache prefetch counters, and fabric lane/hop activity plus flit counts exposed in the top-level artifact.
...
selection, credit-depth validation, lane/hop activity, flit counts, and fabric stats,
...
recorded memory accesses, checked-in GUPS and GPU
...
tests, suite tests, resource acquisition executor tests, repository GUPS and
GPU example-config CLI tests, `rem6 run --config` nested multi-artifact output
layout CLI test, `rem6 gups` profile-summary CLI tests, `rem6 gpu-run --config`
...
DRAM read, explicit fabric route lane/hop activity and fabric stats, and
...
| `tests/gem5/example_configs`, `tests/gem5/learning_gem5` | `rem6` CLI, `rem6-platform`, `rem6-workload` | 40% single-axis | CLI and TOML tests cover several execution, trace-replay, and GPU micro-run paths, including checked-in GUPS and GPU example configs that run without recompilation. | Add broader example suites spanning run, trace replay, resources, and full-system handoff. |
...
| `tests/gem5/m5_util`, `tests/test-progs/m5-exit` | `rem6-isa-riscv`, `rem6-system`, `rem6-workload` | 50% single-axis | RISC-V m5 exit, fail, stats, checkpoint, hypercall, and work markers reach typed host actions; direct `rem6 run` JSON records repeated work-marker payloads, stats reset/dump id and tick metadata, nonempty RISC-V core checkpoint metadata including branch-predictor state plus store-backed and DRAM-backed memory checkpoint component/chunk metadata, checkpoint chunk checksum changes after guest stores, hypercall selector/response metadata, and m5 `sum` return-value execution. | Add broader payload breadth, other ISA entries, and clock-domain behavior. |
...
DRAM power records in deterministic run JSON. Top-level
`rem6 gpu-run --power-output` emits activity-derived GPU compute-unit, GPU
...
and DRAM activity records, `--debug-flags Power` run-JSON records,
...
**Evidence:** CLI static RISC-V smoke tests, RISC-V DTB handoff tests, and RV64F/RV64D tests.
...
- Do not count unknown-syscall diagnostics as implemented syscall coverage or
  tool-detected static smokes as broad workload parity.
