...
ROB/LSQ-backed O3 run tests beyond retired-lifecycle, broader FU-latency opcode classes beyond current scalar/vector integer mul/div plus vector multiply-add/widening/saturating-multiply and scalar/vector float add/compare/misc/mul/FMA/div/sqrt event count/cycle/min/max/avg stats, LSQ address-matched forwarding, address-mismatch suppression, vector LSQ byte accounting, direct/cache-backed LSQ data-response/op-class total/min/max/avg latency, direct-conditional taken/not-taken/predicted-target-match/predicted-taken-not-taken-mismatch, direct-unconditional, direct-call, return, indirect-unconditional, indirect-unconditional predicted-wrong-target, and linked indirect-call branch event/taken/target/link/squash/misprediction/wrong-target classification, broader scalar/vector/float/atomic LSQ operation-shape/order matrices beyond current ordered atomic and float/vector runtime JSON/stat counters plus ordered-atomic dump/reset scoping, failed store-conditional outcome, and data-response/op-class total/min/max/avg latency plus branch-target/squash/outcome matrices, broader `m5_reset_stats` scoping beyond current ROB/LSQ byte, LSQ matrix, and O3 debug reset epoch/tick trace metadata, broader restored-counter checkpoint dump/replay traces beyond current restored O3 debug checkpoint-restore scope metadata and same-checkpoint/distinct-label two-restore trace rows, plus broader multicore checkpoint trace matrices beyond the current two-hart O3 restore trace row.
