...
secondary-hart release with supervisor entry state, `satp=0`,
`sstatus.SIE=0`, `a0=hartid`, and `a1=opaque`, current-hart `hart_stop`
as a no-return stop that does not write `sbiret`, and retentive current-hart
`hart_suspend` through the CPU execution gate, with the default non-retentive
suspend type resuming at `resume_addr` with the same supervisor entry-state
contract, and SBI IPI pending interrupts waking retentive suspended harts; typed
