...
unmasked RVV unit-stride load/store for the current single-register vector group
slice (`vle32.v`/`vse32.v` under e32/m1) moving bytes through the normal data
access path and adding fixed vector LSU execute-stage latency before data wait,
...
vector load/store forms beyond the current unmasked unit-stride single-register slice, broader TAGE-SC-L and multiperspective-perceptron mutable predictor-family speculation snapshots
