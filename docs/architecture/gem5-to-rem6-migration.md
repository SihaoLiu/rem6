...
layout CLI test, `rem6 multi-run` mixed TOML and command-qualified run/GUPS/GPU/trace-replay aggregate artifact/stat, run-child checkpoint action count roll-up, child-output and power/GPU extra-artifact path, and continue-on-failure CLI tests, `rem6 gups` profile-summary CLI tests, `rem6 gpu-run --config`
