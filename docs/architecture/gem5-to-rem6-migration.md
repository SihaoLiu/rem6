...
that does not write `sbiret`, and retentive current-hart `hart_suspend` through
the CPU execution gate, with the default non-retentive suspend type resuming at
...
semantics beyond the `hart_start` pending slice, RFENCE hypervisor-fence and
completion semantics, full Linux boot, and real benchmark workloads.
...
HSM wake and pending-state tests, RFENCE completion tests, then a real Linux
boot smoke.
