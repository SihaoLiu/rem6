...
ROB/LSQ-backed O3 run tests beyond retired-lifecycle, FU-latency-class, LSQ address-matched forwarding, address-mismatch suppression, vector LSQ byte accounting, direct-conditional taken/not-taken, direct-unconditional, direct-call, return, indirect-unconditional, and indirect-call branch event/taken/target/link/squash/misprediction classification, broader scalar/vector/atomic LSQ operation-shape/order and branch-target/squash/outcome matrices, `m5_reset_stats` scoping, and restored-counter checkpoint dump/replay traces.
