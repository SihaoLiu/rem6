...
- [x] Startup stack, argv/envp/auxv, `brk`, `mmap`, stdio, file create/truncate/read/write/append, vector I/O, `ppoll`, basic `rt_sigaction`/`rt_sigprocmask`, empty `rt_sigpending` mask reporting, no-pending zero-timeout `rt_sigtimedwait`, time, cwd, `chdir`/`fchdir`, random, resource, and wait slices have tests.
...
fd/link visibility, vector I/O, `ppoll`, time, cwd, random, resource, wait, unknown
