...
full 32-byte register-group load/store execution, unmasked and masked e32/m1 `vlseg2e32`/`vsseg2e32` segment load/store CPU data-path execution, and misaligned rejection; and traps, translation
...
Detailed O3 commit evidence includes raw `sim.cpu0.o3.commit.committed_inst_type.mem_read`/`sim.cpu0.o3.commit.committed_inst_type.int_mul`/`sim.cpu0.o3.commit.committed_inst_type.float_misc`, raw combined `sim.cpu0.o3.iew.branch_mispredicts`/`sim.cpu0.o3.commit.branch_mispredicts`, dump/reset samples `sim.host_actions.stats_dump.cpu0.o3.commit.committed_inst_type.mem_read`/`sim.host_actions.stats_dump.cpu0.o3.commit.committed_inst_type.int_mul`/`sim.host_actions.stats_dump.cpu0.o3.commit.committed_inst_type.float_misc`, reset-scoped combined `sim.host_actions.stats_dump.cpu0.o3.iew.branch_mispredicts`/`sim.host_actions.stats_dump.cpu0.o3.commit.branch_mispredicts`, and text aliases `system.cpu.commit.committedInstType_0::MemRead`, `system.cpu.commit.committedInstType_0::MemWrite`, `system.cpu.commit.committedInstType_0::IntMult`, `system.cpu.commit.committedInstType_0::IntDiv`, `system.cpu.commit.committedInstType_0::FloatMisc`, and `system.cpu.commit.committedInstType_0::SimdFloatMisc` from the same detailed-mode runtime counters.
**Next evidence:** Remaining per-cycle in-order squashes and stalls beyond current movement, resource/ordering blocked-cycle, fetch/data/execute wait, and widened trap/debug edges, remaining
non-contiguous masked fault-suppression edge cases beyond current unit-stride load/store all-inactive no-touch, active-span, and strided/indexed e32/m1 translated leading-gap slices, remaining leading translated segment variants, broader strided, indexed,
remaining segment forms beyond current e16/m1 masked, e32/m1 masked/unmasked, e32/m1 three-field, and e64/m1 two-line slices, fault-only faulting/suppression behavior, arbitrary multi-line, and broader register-group vector
load/store forms beyond current unit-stride m1, no-fault fault-only-first loads covering four unmasked e8/e16/e32/e64 m1 cases and three full e32 register-group cases (m2/m4/m8), three e8/m1, two e16/m1, two e32/m1, and one e64/m1 single-line plus one e64/m1 non-contiguous multi-line unmasked/masked strided slices, four e8/m1 including e16-index, e32-index, and e64-index and four e16/m1 including e8-index, e32-index, and e64-index unmasked/masked non-contiguous unordered indexed, four e32/m1 including e8-index, e16-index, and e64-index unmasked/masked contiguous plus six e32/m1 including e8-index, e16-index, e64-index, and three e32-index unmasked/masked non-contiguous variants and four e64/m1 including e8-index, e16-index, e32-index, and e64-index unmasked/masked contiguous plus four e64/m1 including e8-index, e16-index, e32-index, and e64-index unmasked/masked non-contiguous unordered indexed slices, plus remaining segment/full-register-group slices beyond current e16/e32/e64 segment and e16/m2/e32/m2/m4/m8 evidence, broader TAGE-SC-L and multiperspective-perceptron mutable predictor-family speculation snapshots
and rollback beyond the current top-level conditional and indirect selected fetch-ahead slices, broader checker integration for future timing CPU modes beyond the current switch quiescence stats, then broader
ROB/LSQ-backed O3 run tests beyond retired-lifecycle, broader FU-latency opcode classes beyond current scalar/vector integer mul/div plus vector multiply-add/widening/saturating-multiply and scalar/vector float add/compare/misc/mul/FMA/div/sqrt event count/cycle/min/max/avg stats, LSQ address-matched forwarding, address-mismatch suppression, vector LSQ byte accounting, direct/cache-backed LSQ data-response/op-class total/min/max/avg latency beyond current ordered-atomic aggregate/op-class sample-counted runtime latency, direct-conditional taken/not-taken/predicted-target-match/predicted-taken-not-taken-mismatch, direct-unconditional, direct-call, return, indirect-unconditional, indirect-unconditional predicted-wrong-target, and linked indirect-call branch event/taken/target/link/squash/misprediction/wrong-target classification, broader scalar/vector/float/atomic LSQ operation-shape/order matrices beyond current ordered atomic and float/vector runtime JSON/stat counters plus ordered-atomic aggregate/op-class latency runtime stats and LSQ matrix/latency dump/reset scoping, failed store-conditional outcome, and data-response/op-class total/min/max/avg latency plus branch-target/squash/outcome matrices, broader `m5_reset_stats` scoping beyond current ROB/LSQ byte, LSQ matrix/latency, IQ/IEW dispatch, and O3 debug reset epoch/tick trace metadata, broader restored-counter checkpoint dump/replay traces beyond current restored O3 debug checkpoint-restore scope metadata and same-checkpoint/distinct-label two-restore trace rows, plus broader multicore checkpoint trace matrices beyond the current two-hart O3 restore trace row.
...
| `tests/gem5/processor_switch_tests` | `rem6-system`, `rem6-cpu` | 20% unit-slice | Host-assisted switch admission, execution-mode metadata, top-level real RISC-V `m5_switch_cpu` detailed-mode metadata recording including repeated-switch previous-mode metadata and distinct generated transfer manifest labels, CLI-selectable `timing` and TOML-backed `functional` switch target modes, continued execution, checkpoint-backed core/memory/scheduler/cache-bank state-transfer artifact counts, component/chunk metadata, top-level transfer stats, repeated-switch `xregs` and O3-runtime transfer checksum accumulator stats, and quiescence-gate metadata, workload replay restore-by-label of generated transfer manifests, reserved-label protection, and generated-transfer rollback of pre-switch execution-mode authority exist. | Add broader quiescence-gated model handoff and rollback validation across CPU backends. |
...
including register-file checkpoint evidence, simple GPU command-table evidence for reset, no-effect, deterministic cycle-counter start/stop low/high register state, perf-sample, clean-cache, and clean-invalidate commands, MMU address-space no-effect and ignored-command evidence, ignored
...
no-effect commands, deterministic cycle-counter start/stop low/high register state, perf-sample, clean-cache, clean-invalidate, MMU address-space config registers and command table, job-slot next-register/start-next and command table, ignored-command, IRQ-clear, shader/tiler/L2 low/high and L3 zero-present power, register-fault, job/MMU interrupt-block, IRQ status-register reads, GPU/job/MMU interrupt-callback-transition, multi-artifact envelope, and output-path conflict evidence.
