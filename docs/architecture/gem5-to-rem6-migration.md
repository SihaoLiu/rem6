...
- [x] GDB RV32D floating-point plus RV32 CSR target descriptions and register-cache read/write paths exist for FP registers, `fflags`/`frm`/`fcsr`, supervisor, machine, interrupt, translation, vector fixed-point CSRs, RV32 `mstatush`, RV32 counter-high CSRs, and XLEN-mapped vector-configuration CSRs; RV32 CSR target descriptions also advertise packed PMP config CSRs `pmpcfg0` through `pmpcfg3`, with `pmpcfg1`/`pmpcfg3` core packet-handler writes updating live PMP entries 4 through 7 and 12 through 15.
...
- [x] CLI `run --gdb-listen` applies pre-execution RISC-V register writes,
  ...
  the mutated core and memory state, including vector data registers, vector-configuration CSRs, machine environment CSR `menvcfg`, and supervisor/machine trap CSRs; it also serves RV32 target descriptions from RV32 ELF metadata and accepts RV32-only packed PMP config CSR read/write packets.
...
RV64D FP, FP CSR, RV64 CSR register-cache including supervisor `sscratch`, `senvcfg`, and `menvcfg`,
...
RV32 CSR target/register-cache coverage for `mstatush`, counter-high CSRs, plus packed `pmpcfg0` through `pmpcfg3`
