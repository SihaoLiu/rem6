...
TLS metadata, symbol-table count metadata, extended section count/string-index ABI-note OS
detection, plus RV64F/RV64D scalar load/store, arithmetic, comparison, conversion, NaN-boxing, and accrued flag slices.
...
rejection, `.tbss` TLS and symbol-count JSON/stats, ELF32 extended
