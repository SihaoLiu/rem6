...
flags=0/unlink/`mkdirat`/`AT_REMOVEDIR`, and registered-directory `getdents64` slices; typed
...
| `tests/gem5/se_mode` | `rem6-system`, `rem6` CLI | 50% single-axis | RISC-V SE startup, ecalls, static newlib smokes, selected syscalls, cwd-aware registered paths, at-family file and directory mutation, registered-directory enumeration, `ENOSYS` records, and guest writes exist. | Split hello, multicore SE, RVV intrinsic, and other-ISA subrows; add broader libc and lifecycle behavior. |
