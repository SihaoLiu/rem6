...
unsupported-command, IRQ-clear state, shader power-state IRQ evidence, misaligned/out-of-range register fault records, job/MMU interrupt-block raw/mask/status evidence, callback identifiers, and observed workgroup
...
ignored-command, IRQ-clear, shader-power, deterministic fault-record, and job/MMU interrupt-block slices, broader register fault behavior, real interrupt delivery, external-tool ingestion beyond rem6-shaped adapter artifacts,
...
ignored-command, IRQ-clear, shader power, register-fault, job/MMU interrupt-block, multi-artifact envelope, and output-path conflict evidence.

**Next evidence:** External McPAT/DSENT tool ingestion, broader NoMali PIO command/register/fault/interrupt behavior, calibrated activity models, and
