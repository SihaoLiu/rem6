...
five-stage in-order pipeline width and exposing that width through executed-run
JSON/stat output, top-level default RISC-V instruction/data traffic through L1/L2/L3 MSI
...
the live `RiscvCore` in-order pipeline snapshot and emits per-stage width stats
...
cache/fabric/DRAM smoke exposing aggregate, per-virtual-network, and
per-link/per-VN lane `sim.memory.fabric.*` counters including flits and
credit-delay ticks, fabric roll-up inside aggregate memory-resource stats, and `--memory-system cache-fabric-dram` preset
