...
behavior beyond counted flits, broader DRAM refresh breadth, and hierarchy-wide
prefetch translation consumers remain incomplete.
...
credit depth, and fabric JSON/stat counters including flit counts. Volatile CLI external-memory
...
cache/fabric/DRAM smoke exposing `sim.memory.fabric.*` counters including flits, and `--memory-system cache-fabric-dram` preset coverage. DRAM memory-profile tests cover bank-level resource counters, resource-activity stats, and activity-window counter deltas.
...
contention, per-lane flit counts, and per-hop activity details are emitted
