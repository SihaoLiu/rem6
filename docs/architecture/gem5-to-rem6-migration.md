...
record-count stats, plus data-trace load/store/atomic classification stats and
...
breakpoint patch/restore tests, CLI data-access probe tests, CLI `Exec`, `Fetch`, `Data`, `Dram`, `Fabric`, `Memory`, and `Syscall` debug-flag tests, including `Data` load/store/atomic classification stats, `Syscall` return/exit/blocked outcome stats, and `Dram`, `Fabric`, and `Memory` debug trace record-count stats, and histogram registry/output tests.
