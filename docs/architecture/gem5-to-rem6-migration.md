...
Top-level `rem6 power-import` ingests McPAT-shaped XML, McPAT text reports, and DSENT-shaped CSV artifacts into deterministic typed JSON summaries and optional output artifacts; McPAT report import maps printed runtime/leakage power while deriving residency and temperature because the report omits them.
...
`rem6 gpu-run --power-output` compute-unit plus explicit-fabric activity records, `rem6 power-import` McPAT XML, McPAT report, and DSENT CSV ingestion summaries plus output artifacts, `--debug-flags Power` run-JSON records, envelope reporting,
