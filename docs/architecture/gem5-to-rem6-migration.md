...
Pipeline debug runs emit per-cycle `Pipeline` debug records with `sim.debug.pipeline_trace.records`, `sim.debug.pipeline_trace.branch_prediction_flushed`, and CPU/state scoped stats from the executed in-order cycle plans.
