...
and a static raw `fchownat`/`fchown` program that matches qemu for no-op owner
changes, missing paths, bad flags, fd-based ownership calls, `AT_EMPTY_PATH`,
and bad fds.
