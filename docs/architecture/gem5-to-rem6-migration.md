...
- [x] CLI `run --gdb-listen` applies pre-execution RISC-V register writes,
  ...
  the mutated core and memory state, including vector data registers, vector-configuration CSRs, machine environment CSR `menvcfg`, machine counter-inhibit CSR `mcountinhibit`, and supervisor/machine trap CSRs; it also serves RV32 target descriptions from RV32 ELF metadata and accepts RV32-only packed PMP config CSR read/write packets.
...
RV64D FP, FP CSR, RV64 CSR register-cache including supervisor `sscratch`, `senvcfg`, `menvcfg`, and `mcountinhibit`,
