...
| `tests/gem5/processor_switch_tests` | `rem6-system`, `rem6-cpu` | 20% unit-slice | Host-assisted switch admission, execution-mode metadata, top-level real RISC-V `m5_switch_cpu` detailed-mode metadata recording, continued execution, checkpoint-backed core/memory state-transfer artifact counts, and workload replay restore-by-label of generated transfer manifests exist. | Add broader quiescence-gated model handoff across CPU backends and rollback validation. |
