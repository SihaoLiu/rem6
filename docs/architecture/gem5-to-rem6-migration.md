...
ecall and compares the exit path with qemu; a static raw program that creates and enters `work`, reads `/proc/self/cwd` through `readlinkat`, and checks the `/work` suffix against qemu; a static raw program that opens a registered guest file, reads `/proc/self/fd/<fd>` through `readlinkat`, checks the guest-path suffix and missing-fd `ENOENT`, and compares the exit path with qemu; a static raw program
