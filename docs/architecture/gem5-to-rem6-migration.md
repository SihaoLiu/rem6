...
and retentive current-hart `hart_suspend` through the CPU execution gate, with
the default non-retentive suspend type resuming at `resume_addr` with the same
supervisor entry-state contract, and SBI IPI pending interrupts waking
