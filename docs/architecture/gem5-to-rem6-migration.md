...
including register-file checkpoint evidence, hard-reset, soft-reset, perf-sample and clean-cache completion IRQs, ignored
...
perf-sample, clean-cache, ignored-command, IRQ-clear, shader/tiler/L2 low-word power, deterministic fault-record, and job/MMU interrupt-block slices, broader command/power/register fault behavior, real interrupt delivery, external-tool ingestion beyond rem6-shaped adapter artifacts,
...
perf-sample, clean-cache, ignored-command, IRQ-clear, shader/tiler/L2 power, register-fault, job/MMU interrupt-block, multi-artifact envelope, and output-path conflict evidence.
