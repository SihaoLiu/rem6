...
through direct and cache-backed normal data access paths, one unmasked plus one masked e32/m1 `vlseg2e32`/`vsseg2e32` segment load/store slice moving field-interleaved bytes and preserving inactive fields through direct and cache-backed paths, plus no-fault fault-only-first load slices covering four unmasked e8/e16/e32/e64 m1 cases and three full e32 register-group cases (m2/m4/m8) through those same paths, with vector LSU execute-stage latency before data wait on covered slices,
...
segment forms beyond current e32/m1 unmasked/masked slices, fault-only faulting/suppression behavior, arbitrary multi-line, and broader register-group vector
