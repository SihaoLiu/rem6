...
ROB/LSQ-backed O3 run tests beyond retired-lifecycle, FU-latency-class, LSQ address-matched forwarding, address-mismatch suppression, vector LSQ byte accounting, scalar/vector/atomic LSQ operation-shape classification, `m5_reset_stats` scoping, and restored-counter checkpoint dump/replay traces.
...
families, and broader ROB/LSQ-backed O3 execution beyond retired-lifecycle/FU-latency/LSQ-forwarding debug, scheduled checkpoint restore/replay, and checkpoint state.
