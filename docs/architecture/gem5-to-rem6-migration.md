...
- [x] CLI `run --execute --stats-format json --debug-flags Exec,Fetch,Data,Fabric,Memory,Syscall` emits
  deterministic instruction, fetch, data-access, fabric lane/hop, memory-transport, and RISC-V SE syscall trace records from real RISC-V execution paths.
...
top-level `rem6 run --execute --stats-format json --debug-flags Exec,Fetch,Data,Fabric,Memory,Syscall`
...
JSON emitted from completed RISC-V data-access records, fabric-trace JSON
...
breakpoint patch/restore tests, CLI data-access probe tests, CLI `Exec`, `Fetch`, `Data`, `Fabric`, `Memory`, and `Syscall` debug-flag tests, including `Fabric` and `Memory` debug trace record-count stats, and histogram registry/output tests.
