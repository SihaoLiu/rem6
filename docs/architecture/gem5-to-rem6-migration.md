...
ROB/LSQ-backed O3 run tests beyond retired-lifecycle, FU-latency-class, LSQ address-matched forwarding, address-mismatch suppression, vector LSQ byte accounting, direct/cache-backed LSQ data-response/op-class latency, direct-conditional taken/not-taken/predicted-target-match/predicted-taken-not-taken-mismatch, direct-unconditional, direct-call, return, indirect-unconditional, indirect-unconditional predicted-wrong-target, and linked indirect-call branch event/taken/target/link/squash/misprediction/wrong-target classification, broader scalar/vector/float/atomic LSQ operation-shape/order matrices beyond failed store-conditional outcome and data-response/op-class latency plus branch-target/squash/outcome matrices, broader `m5_reset_stats` scoping beyond current ROB/LSQ counters and O3 debug reset epoch/tick trace metadata, broader restored-counter checkpoint dump/replay traces beyond current restored O3 debug checkpoint-restore scope metadata and same-checkpoint/distinct-label two-restore trace rows, plus broader multicore checkpoint trace matrices beyond the current two-hart O3 restore trace row.
...
families, and broader ROB/LSQ-backed O3 execution beyond retired-lifecycle/FU-latency/LSQ-forwarding/ordering/data-latency/op-class/branch-event/taken/link/squash/misprediction-kind debug, scheduled checkpoint restore/replay, and checkpoint state.
