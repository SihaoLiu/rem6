...
max-queue-delay, credit-delay, and max-credit-delay stats from emitted fabric trace records while `--debug-flags Dram` exposes aggregate bank access, row, refresh, command, and ready-latency stats from emitted DRAM trace records. Volatile CLI external-memory
...
access/read/write, port-command, low-power state/residency/exit-latency fields, and hierarchy-scoped bank access, byte, row, refresh, command, and ready-latency stats, fabric-trace JSON
