...
no-effect commands, perf-sample, clean-cache, clean-invalidate, MMU address-space config registers and command table, job-slot next-register/start-next and command table, ignored-command, IRQ-clear, shader/tiler/L2 power, register-fault, job/MMU interrupt-block, multi-artifact envelope, and output-path conflict evidence.
