...
full 32-byte register-group load/store execution, unmasked e32/m1 `vlseg2e32`/`vsseg2e32` segment load/store CPU data-path execution, and misaligned rejection; and traps, translation
...
through direct and cache-backed normal data access paths, one unmasked e32/m1 `vlseg2e32`/`vsseg2e32` segment load/store slice moving field-interleaved bytes through direct and cache-backed paths, plus no-fault fault-only-first load slices covering four unmasked e8/e16/e32/e64 m1 cases and three full e32 register-group cases (m2/m4/m8) through those same paths, with vector LSU execute-stage latency before data wait on covered slices,
...
segment, fault-only faulting/suppression behavior, arbitrary multi-line, and broader register-group vector
