...
credit-delay fields plus per-link/per-VN/per-hop trace-replay stat families for transfer, byte, flit, occupied-tick, queue-delay, max-queue-delay, and credit-delay counters; CLI
...
**Next evidence:** Ruby-scale protocol-network execution, router and virtual-channel NoC arbitration and occupancy beyond per-hop fabric stat aggregation, broader multi-bank low-power state-transition coverage, and hierarchy-level prefetch translation consumers.
