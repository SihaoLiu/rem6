...
local-artifact paths, and benchmark orchestration remains absent.
...
GPU/accelerator shells, DMA routing, and a minimal GPU scalar ISA program
execution path with completion, queued-workgroup snapshot evidence, visible
compute-unit assignment, coalesced memory access records, and top-level GUPS
traffic profile JSON/stats output.

**Not migrated:** Full gem5 stdlib ergonomics, host/network/archive resource
acquisition executors, runtime handoff of acquired suite resources into
executable workload replay, broad GPU ISA semantics, GPU cache/DRAM interaction,
and broad benchmark orchestration.
...
for executable workload replay, host-backed and archive-backed workload
acquisition, data-driven full-system workload declarations, and GPU memory
...
| `tests/gem5/gem5_resources` | `rem6-workload`, `rem6` CLI | 50% single-axis | Resource declarations, identity, provenance, disk-image construction records, library-level in-memory acquisition executor records, and manifest/suite-level `rem6 resource-acquire` execution exist. | Add host-backed, archive-backed, and broader artifact-kind acquisition coverage. |
...
| `tests/gem5/stdlib` | `rem6-workload`, `rem6-platform`, `rem6` CLI | 50% single-axis | Workload manifests, resource payloads, manifest/suite-level CLI resource acquisition, suite dispatch plans, Linux handoff intent, and TOML/CLI tests exist. | Add host/archive resource acquisition, broader stdlib object coverage, and ergonomic topology/workload definitions. |
