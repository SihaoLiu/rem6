...
behavior beyond top-level per-VN lane pressure and credit-delay attribution, broader DRAM
...
**Next evidence:** Ruby-scale protocol-network execution, router and virtual-channel NoC arbitration beyond top-level per-VN lane pressure, broader multi-bank low-power state-transition coverage, and hierarchy-level prefetch translation consumers.
...
`mhartid`/`mvendorid`/`marchid`/`mimpid`, machine ISA `misa`, counter `cycle`/`time`/`instret`, packed PMP config `pmpcfg0`, and PMP address registers `pmpaddr0` through `pmpaddr3`,
...
GDB-visible `cycle` and `time` state across CSR writes and single-step execution and PMP0/PMP1/PMP2/PMP3 writes reflected in the live `RiscvCore` PMP table, and
...
PMP CSR target/register-cache coverage for packed `pmpcfg0` and `pmpaddr0` through `pmpaddr3` in both the core packet handler and top-level `rem6 run --gdb-listen`,
