...
ROB/LSQ-backed O3 run tests beyond retired-lifecycle, FU-latency-class, LSQ address-matched forwarding, address-mismatch suppression, vector LSQ byte accounting, direct-conditional and direct-unconditional branch event/squash classification, broader scalar/vector/atomic LSQ operation-shape/order and branch-target/squash matrices, `m5_reset_stats` scoping, and restored-counter checkpoint dump/replay traces.
...
families, and broader ROB/LSQ-backed O3 execution beyond retired-lifecycle/FU-latency/LSQ-forwarding/ordering/branch-event/squash-kind debug, scheduled checkpoint restore/replay, and checkpoint state.
