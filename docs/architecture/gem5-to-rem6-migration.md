...
**Score calculation:** 10 of 15 items have executable evidence, or 67% raw. The bucket cap is single-axis because Ruby-scale protocol networks, router/virtual-channel NoC behavior beyond top-level shared-link/per-VN lane arbitration and credit-delay attribution, broader DRAM refresh modes and named full JEDEC timing-table breadth beyond custom timing rows, and broader hierarchy-wide prefetch translation consumers remain incomplete.
...
- [x] DRAM/NVM profiles expose bank, timing, QoS, low-power, and routed execution slices, including custom CLI DRAM timing rows in executed JSON and stats.
...
- [ ] Broader DRAM refresh modes and named full JEDEC timing tables beyond custom timing rows are complete.
...
profiles carry refresh/recovery and custom timing-table rows; DDR4, DDR5, and
...
exposes per-bank access/read/write, byte, row, command, refresh, HBM2 JEDEC terminal all-scheduler-bank refresh counters from a top-level CLI run that crosses `tREFI`, and LPDDR terminal low-power plus multi-bank exit counters.
...
**Not migrated:** Ruby-scale protocol networks, gem5-class router and virtual-channel NoC detail, named full JEDEC timing-table breadth beyond custom timing rows, broader instruction-cache and non-MSI hierarchy-level prefetch translation consumers, and additional DRAM refresh modes.
...
**Next evidence:** Ruby-scale protocol-network execution, router and virtual-channel NoC arbitration beyond top-level shared-link/per-VN lane pressure, additional DRAM refresh modes and named full JEDEC timing-table coverage beyond custom timing rows, broader low-power state matrices, and instruction-cache/non-MSI hierarchy-level prefetch translation consumers.
