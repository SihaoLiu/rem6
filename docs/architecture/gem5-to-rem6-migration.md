...
- [x] RV64F/RV64D scalar load/store, arithmetic, comparisons, conversions, legal FP arithmetic and integer-to-float rounding-mode decode, exact static non-RNE integer-to-float conversion execution, inexact integer-to-float accrued flag updates, rounding-insensitive static arithmetic execution, `fadd.s`/`fsub.s` exact-wide-sum directed rounding, `fadd.d` finite directed rounding with NX and RoundUp overflow-boundary flags, `fmul.s` exact-product and `fmul.d` normal finite exact-product plus directed-overflow and directed-underflow subnormal rounding, `fdiv.s` and `fdiv.d` finite-quotient directed rounding with NX, `fsqrt.s` and `fsqrt.d` directed rounding with NX, `fcvt.s.d` directed rounding with NX, `fcvt.d.s` signaling-NaN invalid accrual, `fmadd.s` exact-window directed rounding, narrow `fmadd.d` exact-window static/dynamic directed rounding slices, `fadd.s`/`fsub.s`/`fmul.s`/`fmadd.s` invalid, overflow, underflow, and inexact accrued flags, and NaN-boxing have tests.
...
execution, `fadd.s`/`fsub.s` exact-wide-sum static/dynamic directed rounding and `fadd.d` finite static directed rounding plus RoundUp overflow flags
...
plus RV64D `fadd.d` RoundUp and `fsqrt.d` directed rounding/NX ELF smokes and RV64F/RV64D scalar FP slices including `fcvt.s.d` directed rounding/NX and `fcvt.d.s` signaling-NaN invalid accrual.
...
ELF32 extended counts, ABI-note OS, DTB, RV64D `fadd.d` RoundUp and `fsqrt.d` ELF smokes, and RV64F/RV64D `fcvt.s.d`/`fcvt.d.s` tests.
