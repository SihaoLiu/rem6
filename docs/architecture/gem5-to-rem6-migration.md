...
single-axis because full CPU-facing L1/L2/L3 plus NoC and DRAM is not the default instruction/data path.
...
- [x] Top-level RISC-V instruction/data traffic can use explicit L1/L2/L3 cache hierarchy and fabric-routed DRAM, including a `rem6 run --memory-system cache-fabric-dram` preset that selects the existing MSI hierarchy, fabric, and DRAM path.
...
**Not migrated:** Default broad CPU-facing hierarchy, Ruby-scale protocol networks,
...
cache/fabric/DRAM smoke exposing `sim.memory.fabric.*` counters and `--memory-system cache-fabric-dram` preset coverage. DRAM
