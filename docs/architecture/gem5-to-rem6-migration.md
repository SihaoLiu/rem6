...
scheduler epoch, dispatch, batch, active-partition, and worker-count stats, and
`--data-cache-dram-memory-profile` attaches a profiled backing memory target whose
data-cache DRAM accesses appear in artifact JSON and `StatsRegistry`; a combined
CLI trace-replay run proves explicit MSI data-cache plus explicit fabric routing
...
protocol, scheduler, and profiled backing-DRAM access stats from the top-level
