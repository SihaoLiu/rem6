...
fabric JSON/stat counters including flit and credit-delay counts, optional router input-VC/output-port stage latency and queue-delay timing through executed replay summaries, and
...
**Not migrated:** Ruby-scale protocol networks, gem5-class router, flit, routing, and virtual-channel NoC detail beyond the current optional per-hop router-stage slice, named full JEDEC timing-table breadth beyond custom timing rows, broader instruction-cache and non-MSI hierarchy-level prefetch translation consumers, and additional DRAM refresh modes beyond per-bank/all-bank policy.
