...
behavior beyond top-level shared-link/per-VN lane arbitration and credit-delay attribution, broader DRAM
refresh modes and full timing-table breadth, and hierarchy-wide prefetch translation consumers remain
incomplete.
...
shared physical-link arbitration, credit depth, and aggregate, per-virtual-network, per-link, and per-link/per-VN lane
...
exposes per-bank access/read/write, byte, row, command, refresh, HBM2 JEDEC terminal all-scheduler-bank refresh counters from a top-level CLI run that crosses `tREFI`, and whole-profiled LPDDR scheduler-bank terminal low-power counters.
...
**Not migrated:** Ruby-scale protocol networks, gem5-class router and virtual-channel NoC detail, full JEDEC timing-table breadth, broader hierarchy-level prefetch translation consumers, and additional DRAM refresh modes.
...
CLI `run` also has DDR profile refresh smoke coverage that exposes default and custom refresh timing fields plus nonzero refresh stats from RISC-V DRAM execution, HBM2 JEDEC terminal refresh coverage that emits one `tRFC` window across all 16 scheduler banks after a top-level run crosses `tREFI`, plus LPDDR default and custom low-power timing, active-powerdown, precharge-powerdown, self-refresh, and target/port/bank hierarchy low-power stats from routed RISC-V fetch requests, including all profiled LPDDR scheduler banks after terminal idle residency; CLI `run` cache/DRAM resource smoke coverage exposes unified memory-resource cache CPU-response, directory-decision, backing-DRAM access, cache-bank, L1/L2/L3, instruction/data cache hierarchy, and hierarchy-scoped cache prefetch queue/translation-queue counters in artifact JSON and `StatsRegistry`.
...
**Next evidence:** Ruby-scale protocol-network execution, router and virtual-channel NoC arbitration beyond top-level shared-link/per-VN lane pressure, additional DRAM refresh modes and full JEDEC timing-table coverage, multi-bank low-power transition/exit coverage, and hierarchy-level prefetch translation consumers.
