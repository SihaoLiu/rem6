...
`vdivu`/`vdiv`/`vremu`/`vrem`, integer reductions `vredsum`/`vredand`/`vredor`/`vredxor`/`vredminu`/`vredmin`/`vredmaxu`/`vredmax` plus widening reductions `vwredsumu`/`vwredsum`, base widening add/subtract `vwaddu`/`vwadd`/`vwsubu`/`vwsub` vv/vx decode with representative execution, and mask compare
