...
in-order branch-prediction, branch-misprediction, general flush,
...
`cycle`/`instret`, RV64 machine status, interrupt, trap, and counter CSR
...
translation `satp`, interrupt aliases `sie`/`sip`, and counter
`cycle`/`instret`, RV64 machine CSR register-cache including `mscratch`
write/readback through guest CSR execution and live `cycle`/`instret` reads
