...
and rollback beyond the current direct selected-family slices, gem5-class checker coverage across future timing CPU modes, and
KVM equivalents.
...
and rollback for conditional/indirect selected fetch-ahead paths, full checker integration for future timing CPU modes, then a
