...
- [x] RVV integer unmasked `vslideup`/`vslidedown` vx/vi forms plus `vslide1up`/`vslide1down` vx and `vrgather` vv/vx/vi forms, mask reductions `vcpop.m`/`vfirst.m`, mask prefixes `vmsbf.m`/`vmsof.m`/`vmsif.m`, `vmerge` masked vv/vx/vi forms, `vmv.v` unmasked v/x/i forms, `vcompress.vm`, `vzext`/`vsext` masked/unmasked extension-family decode, `vf2` extension hart execution, and unmasked slide/gather, mask reductions, mask prefixes, plus `vzext.vf2` CPU fetch-stream execution have tests.
...
`vmv.v`, unmasked slide `vslideup`/`vslidedown`/`vslide1up`/`vslide1down`, gather `vrgather`, mask reductions `vcpop.m`/`vfirst.m`, mask prefixes `vmsbf.m`/`vmsof.m`/`vmsif.m`, compress `vcompress.vm`, zero/sign extension `vzext`/`vsext` masked/unmasked decode with `vf2` hart execution and unmasked slide/gather, mask reductions, mask prefixes, plus `vzext.vf2` CPU fetch-stream execution, fixed-point narrow clip `vnclipu.wi`, and unmasked floating-point `vfadd`, `vfsub`, `vfmin`, `vfmax`, `vfmul`, and
