...
- [x] GDB RV64D floating-point, including `fflags`/`frm`/`fcsr`, and advertised RV64 supervisor CSR register target descriptions including `sscratch` plus `p`/`P`/`g`/`G` register-cache paths exist.
...
write/readback through guest CSR execution, and control-state tests,
