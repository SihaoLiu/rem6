...
response and directory decision counters from those runs; DRAM-backed MSI
data-cache line fills emit nonzero backing DRAM access counters from a CLI run
using `--dram-memory` and `--data-cache-protocol msi`; optional single-core CLI
RISC-V instruction fetch traffic can drive an MSI instruction-cache run path
with separate instruction-cache counters; volatile
...
coverage with DRAM-backed MSI data-cache fill accounting plus instruction-cache
fetch smoke coverage. DRAM memory-profile tests cover bank-level resource
...
- [x] GDB RV64D floating-point, including `fflags`/`frm`/`fcsr`, and advertised RV64 supervisor CSR register target descriptions including `sscratch` and `satp` plus `p`/`P`/`g`/`G` register-cache paths exist.
...
and advertised RV64 supervisor CSR target descriptions including `sscratch`
and `satp`
plus `p`/`P`/`g`/`G` register-cache paths, RISC-V software breakpoint
...
and `satp` write/readback through guest CSR execution, and control-state tests,
...
| `tests/gem5/memory` | `rem6-memory`, `rem6-cache`, `rem6-dram`, `rem6-fabric` | 56% single-axis | Stores, page maps, cache banks, topology slices, optional single-core CLI RISC-V MSI-bank, MESI-line, MOESI-line, and CHI-line data-cache routing with DRAM-backed MSI fill accounting, DRAM/NVM counters, CLI-selectable JEDEC-style refresh presets, prefetch queue counters, top-level trace-replay fabric-route activity stats, and top-level trace-replay data-cache run/protocol stats exist. | Add CPU-facing multi-level cache, NoC, broader DRAM refresh breadth, and full preset coverage. |
