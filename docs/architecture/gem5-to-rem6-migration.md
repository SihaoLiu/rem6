...
- [x] Linux at-family hard-link, `symlinkat`, `renameat`, `renameat2` flags=0, unlink, legacy `mkdir`, `mkdirat`, `unlinkat` with `AT_REMOVEDIR`, legacy `time`, and registered-directory `getdents64` syscalls mutate or expose registered guest files/directories or tick-derived time and have raw smoke evidence. Current qemu-riscv64 reports `ENOSYS` for raw `renameat` and raw legacy `mkdir`/`open`/`time`, so those smoke tests record the qemu boundary and verify rem6 registered-file, registered-directory, and deterministic-time behavior directly.
...
`clock_nanosleep` validation, `gettimeofday`, `clock_getres`, `CLOCK_TAI` `clock_gettime`,
