...
plus translation queue stats from the executed run, including a configured
single-core gem5-style `system.cpu.dcache.prefetcher.pfIssued` alias. CLI
...
run, including a configured single-core gem5-style
`system.cpu.icache.prefetcher.pfIssued` alias. Bare top-level `rem6 run --isa riscv --execute` now selects the cache/fabric/DRAM hierarchy by default; `--memory-system direct` remains an explicit direct-memory configuration for timing-focused runs.
