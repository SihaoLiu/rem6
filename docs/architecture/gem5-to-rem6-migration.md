...
exposes per-bank access/read/write, byte, row, command, refresh, and whole-profiled LPDDR scheduler-bank terminal low-power counters.
...
**Next evidence:** Ruby-scale protocol-network execution, router and virtual-channel NoC arbitration beyond top-level shared-link/per-VN lane pressure, broader DRAM refresh/preset matrix and multi-bank low-power transition/exit coverage, and hierarchy-level prefetch translation consumers.
