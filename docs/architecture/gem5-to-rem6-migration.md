...
- [x] Linux at-family hard-link, `symlinkat`, `renameat`, `renameat2` flags=0, unlink, legacy `mkdir`, `mkdirat`, `unlinkat` with `AT_REMOVEDIR`, legacy `time`, raw `clock_gettime64`, and registered-directory `getdents64` syscalls mutate or expose registered guest files/directories or tick-derived time and have raw smoke evidence. Current qemu-riscv64 reports `ENOSYS` for raw `renameat`, raw `clock_gettime64`, and raw legacy `mkdir`/`open`/`time`, so those smoke tests record the qemu boundary and verify rem6 registered-file, registered-directory, and deterministic-time behavior directly.
...
mapped-range, nodemask, privilege, and unsupported-flag validation slice, stdio, guest-backed file create/path `truncate`/`ftruncate`/read/write/append/positioned read-write/readback, xattr lifecycle, and open
...
`prlimit64` program that sets and requeries `RLIMIT_STACK` through `rem6 run --riscv-se`; a static raw
`prctl` no-new-privs, dumpable, and parent-death-signal programs that match qemu; a static raw
