...
`sim.memory.fabric.*` counters including flits and credit-delay ticks, fabric
roll-up inside aggregate memory-resource stats, and `--memory-system cache-fabric-dram` preset
...
- [x] CLI `run --execute --stats-format json --debug-flags Exec,Fetch,Data,Memory,Syscall` emits
  deterministic instruction, fetch, data-access, memory-transport, and RISC-V SE syscall trace records from real RISC-V execution paths.
...
top-level `rem6 run --execute --stats-format json --debug-flags Exec,Fetch,Data,Memory,Syscall`
...
JSON emitted from completed RISC-V data-access records, memory-transport trace
JSON emitted from real fetch/data `MemoryTrace` events with top-level debug trace record-count stats, plus RISC-V SE syscall-trace JSON emitted from real syscall trap handling,
...
remaining broad CSR GDB register-cache coverage, broader debug-run stat parity beyond current trace record counts, runtime resource counters,
...
breakpoint patch/restore tests, CLI data-access probe tests, CLI `Exec`, `Fetch`, `Data`, `Memory`, and `Syscall` debug-flag tests, including `Memory` debug trace record-count stats, and histogram registry/output tests.
...
**Score calculation:** 15 of 20 items have executable evidence, or 75% raw,
...
mixed-command multi-run slices.
...
- [x] CLI `multi-run` sequentially orchestrates multiple real `run --config`, `gups --config`, and `trace-replay --config` entries from TOML and command-qualified `--run` entries through the top-level binary and emits aggregate JSON and stats artifacts.
...
top-level `rem6 run --config` creation of nested run, stats, and power-analysis artifact directories from TOML-relative paths, top-level `rem6 multi-run` sequential orchestration of real `run --config`, `gups --config`, and `trace-replay --config` TOML and command-qualified `--run` entries with aggregate JSON and stats artifacts, and top-level GUPS traffic profile JSON/stats output.
...
ISA semantics, representative GPU cache/DRAM interaction, complete multi-run artifact compatibility, checkpoint restore orchestration, and broad benchmark orchestration.
...
layout CLI test, `rem6 multi-run` mixed TOML and command-qualified run/GUPS/trace-replay aggregate artifact/stat CLI tests, `rem6 gups` profile-summary CLI tests, `rem6 gpu-run --config`
...
| `tests/gem5/multisim`, `tests/gem5/suite_tests` | `rem6-workload`, `rem6-kernel`, `rem6` CLI | 52% single-axis | Suite planning, dispatch, execution summaries, occupancy contracts, and top-level `rem6 multi-run` sequential orchestration of real `run --config`, `gups --config`, and `trace-replay --config` TOML and command-qualified `--run` entries with aggregate artifacts exist. | Add checkpoint restore orchestration and suite-compatible multi-artifact workload semantics. |
