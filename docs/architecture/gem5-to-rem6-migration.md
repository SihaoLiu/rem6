...
- [x] Linux at-family hard-link, `symlinkat`, `renameat`, `renameat2` flags=0, unlink, legacy `mkdir`, `mkdirat`, `unlinkat` with `AT_REMOVEDIR`, and registered-directory `getdents64` syscalls mutate or expose registered guest files and directories and have raw smoke evidence. Current qemu-riscv64 reports `ENOSYS` for raw `renameat` and raw legacy `mkdir`/`open`, so those smoke tests record the qemu boundary and verify rem6 registered-file and registered-directory behavior directly.
