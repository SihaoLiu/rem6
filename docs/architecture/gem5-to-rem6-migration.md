...
existence checks, installed `SIG_IGN` nonzero `kill`/`tkill`/`tgkill` and
`rt_sigqueueinfo` non-delivery success without unknown-syscall records,
...
including stack/data/NPROC limits and unknown-pid rejection, empty `rt_sigpending` mask reporting,
...
matches qemu query/set/disable state; a static raw ignored-signal program that
drives `rt_sigaction(SIG_IGN)` plus `kill`, `tkill`, and `tgkill` through
`rem6 run --riscv-se`; direct and vector I/O unit coverage for pipe
...
protocol counters, top-level trace-replay data-cache scheduler resource
counters, top-level `rem6 run` retired-instruction probe summaries,
