...
fabric JSON/stat counters including flit and credit-delay counts, and
`--debug-flags Fabric` exposes aggregate lane/hop occupied-tick, queue-delay,
max-queue-delay, and credit-delay stats from emitted fabric trace records. Volatile CLI external-memory
...
- [x] CLI `run --execute --stats-format json --debug-flags Exec,Fetch,Data,Cache,Dram,Fabric,Memory,Syscall` emits
  deterministic instruction, fetch, data-access, cache hierarchy, DRAM hierarchy including low-power target/port/bank state counters, fabric lane/hop, memory-transport, and RISC-V SE syscall trace records from real RISC-V execution paths.
...
access/read/write, port-command, low-power state/residency/exit-latency fields, and hierarchy-scoped bank byte stats, fabric-trace JSON
