...
- [x] CLI `run --execute --stats-format json --debug-flags Exec` emits
  deterministic instruction trace records from real RISC-V execution events.
...
top-level `rem6 run --execute --stats-format json --debug-flags Exec`
...
debug-flag tests, and histogram
