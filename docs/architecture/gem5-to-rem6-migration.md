...
- [x] RVV integer unmasked `vslideup`/`vslidedown` vx/vi forms plus `vslide1up`/`vslide1down` vx and `vrgather` vv/vx/vi forms, mask reductions `vcpop.m`/`vfirst.m`, mask prefixes `vmsbf.m`/`vmsof.m`/`vmsif.m`, mask indexes `viota.m`/`vid.v`, `vmerge` masked vv/vx/vi forms, `vmv.v` unmasked v/x/i, `vmv.x.s`/`vmv.s.x` scalar/lane-zero forms, and `vmv<nr>r.v` whole-register moves, `vcompress.vm`, `vzext`/`vsext` masked/unmasked extension-family decode, `vf2` extension hart execution, and unmasked slide/gather, mask reductions, mask prefixes, mask indexes, plus `vzext.vf2` CPU fetch-stream execution have tests.
...
`vmv.v`, `vmv.x.s`/`vmv.s.x`, and `vmv<nr>r.v`, unmasked slide `vslideup`/`vslidedown`/`vslide1up`/`vslide1down`, gather `vrgather`, mask reductions `vcpop.m`/`vfirst.m`, mask prefixes `vmsbf.m`/`vmsof.m`/`vmsif.m`, mask indexes `viota.m`/`vid.v`, compress `vcompress.vm`, zero/sign extension `vzext`/`vsext` masked/unmasked decode with `vf2` hart execution and unmasked slide/gather, mask reductions, mask prefixes, mask indexes, plus `vzext.vf2` CPU fetch-stream execution, fixed-point narrowing shifts `vnsrl.wv`/`vnsra.wv`/`vnsrl.wx`/`vnsra.wx`/`vnsrl.wi`/`vnsra.wi` and narrow clip `vnclipu.wi`/`vnclip.wi`, and unmasked floating-point `vfadd`, `vfsub`, `vfmin`, `vfmax`, `vfmul`, and
