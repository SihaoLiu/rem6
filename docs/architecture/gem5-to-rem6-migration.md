...
completed RISC-V data-access records with load/store/atomic byte stats, cache-trace JSON emitted from executed instruction/data L1/L2/L3 memory-resource cache summaries with activity, CPU-response, directory-decision, backing-DRAM, bank, and selected prefetch stats,
DRAM-trace JSON
