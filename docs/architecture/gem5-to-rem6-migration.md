...
CLI run aggregate memory-resource activity, active-resource, instruction/data cache-resource hierarchy, prefetch queue/translation-queue, DRAM refresh/low-power stats, custom LPDDR low-power timing, and target/port/bank DRAM low-power hierarchy stats derived from executed cache, transport, and DRAM paths,
...
record-count, request/route/request-agent, event-kind, and response-status stats, plus data-trace load/store/atomic classification stats and
RISC-V SE syscall-trace JSON/outcome plus syscall-number/call-site/CPU/argument stats emitted from real syscall trap handling,
...
breakpoint patch/restore tests, CLI data-access probe tests, CLI `Exec`, `Fetch`, `Data`, `Dram`, `Fabric`, `Memory`, and `Syscall` debug-flag tests, including `Exec` retired-record and byte stats, `Fetch` byte stats, `Data` load/store/atomic classification and byte stats, `Memory` event-kind, response-status, and request/route/request-agent stats, `Syscall` return/exit/blocked outcome plus syscall-number/call-site/CPU/argument stats, `Memory` debug trace record-count stats, `Dram` debug trace record-count plus target access/read/write, port-command, and bank read/write byte aggregate stats, `Fabric` debug trace record-count plus lane-transfer and lane/hop byte/flit aggregate stats, and histogram registry/output tests.
