...
with runtime stats, and O3 policy helpers.

**Not migrated:** Overlapped Minor-like in-order timing with stalls and
squashes, executable O3 timing, speculation, checker, and KVM equivalents.
...
from executed RISC-V instructions.
...
translated execution, remote SFENCE.VMA scheduled completion events, plus HSM
...
gate, default non-retentive current-hart `hart_suspend` through
`SUSPEND_PENDING` until the scheduled resume event re-enters at `resume_addr`
with the same supervisor entry-state contract, stale non-retentive resume events
ignored after intervening state changes, and SBI IPI pending interrupts waking
retentive suspended harts; typed
...
`readlinkat` ecall and compares the exit path with qemu; and a static newlib
`open(".", O_DIRECTORY | O_CLOEXEC)` directory traversal smoke plus `open`
with `O_NOCTTY`, `O_NOFOLLOW`, and `O_SYNC` through the legacy `open` syscall
with newlib/libgloss flags.
...
non-retentive `hart_suspend` slices, RFENCE hypervisor-fence semantics and
broader completion coverage, full Linux boot, and real benchmark workloads.
...
`riscv_se_open_flags`, `riscv_se_permissions`, `riscv_se_proc`,
...
hypervisor-fence tests, then a real Linux boot smoke.
