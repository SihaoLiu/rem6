...
**Score calculation:** 11 of 15 items have executable evidence, or 73% raw. The bucket cap is single-axis because Ruby-scale protocol networks, competing-flow NoC arbitration and gem5-class flit/routing/VC breadth beyond current top-level shared-link plus two-hop/two-router rows, and broader DRAM refresh modes beyond per-bank/all-bank/bank-group policy plus named full JEDEC timing-table breadth beyond custom timing rows remain incomplete.
...
- [ ] Broader DRAM refresh modes beyond per-bank/all-bank/bank-group policy and named full JEDEC timing tables beyond custom timing rows are complete.
...
profiles carry refresh/recovery, per-bank/all-bank/bank-group refresh policy, and custom timing-table rows; DDR4, DDR5, and
...
**Next evidence:** Ruby-scale protocol-network execution, competing-flow NoC arbitration beyond top-level shared-link/per-VN lane pressure and current two-hop/two-router rows, gem5-class flit/routing/VC breadth, additional DRAM refresh modes beyond per-bank/all-bank/bank-group policy, named full JEDEC timing-table coverage beyond custom timing rows, and broader low-power matrices beyond the current LPDDR and NVM rows.
