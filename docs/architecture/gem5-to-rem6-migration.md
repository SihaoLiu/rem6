...
- [x] RV64F/RV64D scalar load/store, arithmetic, comparisons, conversions, legal FP arithmetic and integer-to-float rounding-mode decode, exact static non-RNE integer-to-float conversion execution, inexact integer-to-float accrued flag updates, rounding-insensitive static arithmetic execution, `fadd.s`/`fsub.s` exact-wide-sum directed rounding, `fmul.s` exact-product directed rounding, `fdiv.s` finite-quotient directed rounding with NX, `fmadd.s` exact-window directed rounding, narrow `fmadd.d` exact-window static/dynamic directed rounding slices, `fadd.s`/`fsub.s`/`fmul.s`/`fmadd.s` invalid, overflow, underflow, and inexact accrued flags, and NaN-boxing have tests.
...
UF, and directed-boundary overflow accrual, `fdiv.s` finite-quotient static
directed rounding with NX accrual, `fmadd.s` exact-window
...
add/sub/divide/fused multiply-add, and narrow double-precision `fmadd.d` slices,
and complete Linux privileged behavior.
...
**Score calculation:** 13 of 18 items have executable evidence, or 72% raw.
