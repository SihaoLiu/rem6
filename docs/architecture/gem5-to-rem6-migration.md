...
selected GShare fetch-ahead using pending branch-speculation and direct-jump
history when steering younger conditional branches without mutating committed
predictor history,
...
squashes, executable O3 timing, remaining non-GShare predictor-family
speculation snapshots and rollback, gem5-class checker coverage across future
timing CPU modes, and KVM equivalents.
...
completed-fetch overlap, selected GShare speculative-history overlay for younger
fetch-ahead branch prediction, scalar integer multiply execute-stage latency through
...
non-GShare predictor-family speculation snapshots and rollback, full checker integration for future timing
