...
stats, per-stage in-order flush plus branch-prediction and trap-redirect flush attribution stats,
plus aggregate and per-stage branch/trap redirect flush-cycle stats
from real redirect cycle records,
...
branch-prediction flush, redirect, branch-prediction redirect, and trap redirect counters from the same executed RISC-V
...
**Next evidence:** Remaining per-cycle in-order squashes and stalls beyond current movement, resource/ordering blocked-cycle, fetch/data/execute wait, and widened trap-redirect flush stats/debug attribution, remaining
