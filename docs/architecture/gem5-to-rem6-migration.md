...
plus page-span and translation queue stats from the executed run, including
configured single-core gem5-style `system.cpu.dcache.prefetcher.pfIdentified`/`pfIssued`/`pfSpanPage` aliases. CLI
...
run, including configured single-core gem5-style `system.cpu.icache.prefetcher.pfIdentified`/`pfIssued`/`pfSpanPage` aliases. Bare top-level `rem6 run --isa riscv --execute` now selects the cache/fabric/DRAM hierarchy by default; `--memory-system direct` remains an explicit direct-memory configuration for timing-focused runs.
