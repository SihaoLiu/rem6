...
Stats ledger note: the BTB slice now exposes `sim.cpu*.branch_predictor.btb.{lookups,hits,misses,updates,evictions}` in top-level run JSON and stats. It also maps real BTB update and hit-ratio values into gem5-style text aliases: `system.cpu.branchPred.BTBUpdates`, `system.cpu.branchPred.BTBHitRatio`, multicore `system.cpu0.branchPred.BTBUpdates`, `system.cpu0.branchPred.BTBHitRatio`, `system.cpu1.branchPred.BTBUpdates`, and `system.cpu1.branchPred.BTBHitRatio`. This does not claim gem5 `BTBMispredicted` semantics yet; the remaining gap is exact target-wrong/no-target misprediction accounting.
