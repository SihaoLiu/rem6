...
`sched_getaffinity`, single CPU/node `getcpu`, zero-duration `nanosleep` and
`clock_nanosleep` validation, time, cwd, random, resource, wait, unknown syscall, `kill(..., 0)`
