...
and rollback beyond the current top-level conditional and indirect selected fetch-ahead slices, broader checker integration for future timing CPU modes beyond the current switch quiescence stats, then a
...
CLI `Exec,Fetch` debug-flag evidence covers top-level debug trace record, category, active-flag, and payload-byte roll-up stats; CLI `Sbi` and `HostAction` cover record/category/active-flag roll-up stats against the emitted debug JSON.
