...
`mhartid`/`mvendorid`/`marchid`/`mimpid`, machine ISA `misa`, counter `cycle`/`time`/`instret`, packed PMP config `pmpcfg0`, and PMP address registers `pmpaddr0` through `pmpaddr7`,
...
GDB-visible `cycle` and `time` state across CSR writes and single-step execution and PMP0 through PMP7 writes reflected in the live `RiscvCore` PMP table, and
...
PMP CSR target/register-cache coverage for packed `pmpcfg0` and `pmpaddr0` through `pmpaddr7` in both the core packet handler and top-level `rem6 run --gdb-listen`,
