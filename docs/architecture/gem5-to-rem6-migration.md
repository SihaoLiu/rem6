...
**Next evidence:** Ruby-scale protocol-network execution, router and virtual-channel NoC arbitration beyond top-level shared-link/per-VN lane pressure, additional DRAM refresh modes and full JEDEC timing-table coverage, broader low-power state matrices, and hierarchy-level prefetch translation consumers.
