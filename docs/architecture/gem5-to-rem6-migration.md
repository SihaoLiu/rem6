...
- [x] MMIO bus, UART, PL011, CLINT, PLIC, RTC, timers, interrupt routes, and readfile MMIO/checkpoint attachment have tests.
...
`readfile_device` platform-MMIO tests, and `riscv_topology_readfile`.
...
| `tests/gem5/readfile_tests` | `rem6-platform`, `rem6-system`, `rem6` CLI | 40% single-axis | DTB/initrd handoff, CLI input-file plumbing, `PlatformBuilder` read-only readfile MMIO-window tests, and topology host-checkpoint capture of attached readfile payloads exist. | Bind CLI host readfile or resource payloads into the platform device and validate guest/Linux consumption. |
