...
selected TAGE-SC-L fetch-ahead using pending branch-speculation and direct-jump history overlays, selected multiperspective-perceptron fetch-ahead using pending thread-history snapshots, and selected GShare, BiMode, and Tournament fetch-ahead using live selected-predictor speculative history,
...
selected GShare, BiMode, and Tournament fetch-ahead now record live selected-predictor speculative history at issue and squash/repair it at retire for wrong-path nested branch runs, with top-level per-family rollback counters covering aggregate branch-speculation repairs plus selected younger cleanup and CPU checkpoint tests proving committed selected-family history snapshots and restore-time selected-record invalidation,
...
vector load-store functional-unit latency tables, TAGE-SC-L and multiperspective-perceptron mutable predictor-family speculation snapshots
