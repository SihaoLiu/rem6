...
- [x] Startup stack, argv/envp/auxv, `brk`, `mmap`, `mprotect`, stdio, file create/truncate/`ftruncate`/read/write/append, vector I/O, `ppoll`, single-word `sched_setaffinity`/`sched_getaffinity`, single CPU/node `getcpu`, zero-duration `nanosleep` validation, basic `rt_sigaction`/`rt_sigprocmask`, empty `rt_sigpending` mask reporting, no-pending zero-timeout `rt_sigtimedwait`, time, cwd, `chdir`/`fchdir`, random, resource, and wait slices have tests.
...
`mmap`, `mprotect`, stdio, guest-backed file create/truncate/`ftruncate`/read/write/append/readback and open
fd/link visibility, vector I/O, `ppoll`, single-word `sched_setaffinity` and
`sched_getaffinity`, single CPU/node `getcpu`, zero-duration `nanosleep` validation, time, cwd, random, resource, wait, unknown
syscall, basic signal action/mask state for `rt_sigaction` and `rt_sigprocmask`,
empty `rt_sigpending` mask reporting, no-pending zero-timeout `rt_sigtimedwait`,
cwd-aware registered-path lookup, at-family
...
| `tests/gem5/se_mode` | `rem6-system`, `rem6` CLI | 50% single-axis | RISC-V SE startup, ecalls, static newlib smokes, selected syscalls including `ppoll`, `mprotect`, `ftruncate`, single-word `sched_setaffinity`/`sched_getaffinity`, single CPU/node `getcpu`, zero-duration `nanosleep` validation, basic `rt_sigaction`/`rt_sigprocmask`, empty `rt_sigpending` mask reporting, and no-pending zero-timeout `rt_sigtimedwait`, cwd-aware registered paths, guest-backed file output/readback and open visibility, at-family file and directory mutation, registered-directory enumeration, `ENOSYS` records, and guest writes exist. | Split hello, multicore SE, RVV intrinsic, and other-ISA subrows; add broader libc and lifecycle behavior. |
