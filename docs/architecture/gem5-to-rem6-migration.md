...
`getpriority`/`setpriority` raw priority reporting and nice-state updates, current-process `pidfd_open`, `pidfd_send_signal(..., 0)`, and `pidfd_getfd` fd-lifecycle checks,
