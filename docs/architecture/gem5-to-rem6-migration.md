...
- [x] CLI `multi-run` sequentially orchestrates multiple real `run --config`, `gups --config`, `gpu-run --config`, and `trace-replay --config` entries from TOML and command-qualified `--run` entries through the top-level binary, preserves child-configured output artifacts, and emits aggregate JSON/stats artifacts plus explicit failure summaries when configured to continue after child errors.
...
top-level `rem6 run --config` creation of nested run, stats, and power-analysis artifact directories from TOML-relative paths, top-level `rem6 multi-run` sequential orchestration of real `run --config`, `gups --config`, `gpu-run --config`, and `trace-replay --config` TOML and command-qualified `--run` entries with child-configured output artifacts, aggregate JSON/stats artifacts, and explicit child-failure summaries, and top-level GUPS traffic profile JSON/stats output.
...
layout CLI test, `rem6 multi-run` mixed TOML and command-qualified run/GUPS/GPU/trace-replay aggregate artifact/stat, child-output artifact, and continue-on-failure CLI tests, `rem6 gups` profile-summary CLI tests, `rem6 gpu-run --config`
