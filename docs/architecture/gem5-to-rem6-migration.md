...
JSON emitted from completed RISC-V data-access records, DRAM-trace JSON
emitted from real top-level DRAM target, port, and bank activity, fabric-trace JSON
emitted from real top-level fabric lane and hop activity, memory-transport trace
...
remaining broad CSR GDB register-cache coverage, broader debug-run stat parity beyond current trace record and classification counters, runtime resource counters,
...
breakpoint patch/restore tests, CLI data-access probe tests, CLI `Exec`, `Fetch`, `Data`, `Dram`, `Fabric`, `Memory`, and `Syscall` debug-flag tests, including `Data` load/store/atomic classification stats, `Memory` event-kind and response-status stats, `Syscall` return/exit/blocked outcome stats, and `Dram`, `Fabric`, and `Memory` debug trace record-count stats, and histogram registry/output tests.
