...
single-axis because Ruby-scale protocol networks, flit-level NoC behavior,
broader DRAM refresh breadth, and hierarchy-wide prefetch translation consumers
remain incomplete.
...
credit depth, and fabric JSON/stat counters. Volatile CLI external-memory
...
**Not migrated:** Ruby-scale protocol networks, flit-level NoC, broad JEDEC preset validation, broader hierarchy-level prefetch translation consumers, and broad DRAM refresh/preset breadth.
...
cache/fabric/DRAM smoke exposing `sim.memory.fabric.*` counters and `--memory-system cache-fabric-dram` preset coverage. DRAM memory-profile tests cover bank-level resource counters, resource-activity stats, and activity-window counter deltas.
...
**Next evidence:** Ruby-scale protocol-network execution, flit-level NoC routing, broader low-power state-transition coverage, and hierarchy-level prefetch translation consumers.
...
contention, per-lane, and per-hop activity details are emitted
...
selection, credit-depth validation, lane/hop activity, and fabric stats,
