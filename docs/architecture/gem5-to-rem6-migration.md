...
CLI `run` also has DDR profile refresh smoke coverage that exposes refresh timing fields and nonzero refresh stats from RISC-V DRAM execution, plus LPDDR low-power timing, precharge-powerdown residency, and target/port/bank hierarchy low-power stats from routed RISC-V fetch requests; CLI `run` cache/DRAM resource smoke coverage exposes unified memory-resource cache CPU-response, directory-decision, backing-DRAM access, cache-bank, L1/L2/L3, instruction/data cache hierarchy, and cache prefetch queue/translation-queue counters in artifact JSON and `StatsRegistry`.
...
CLI run aggregate memory-resource activity, active-resource, instruction/data cache-resource hierarchy, prefetch queue/translation-queue, DRAM refresh/low-power stats, and target/port/bank DRAM low-power hierarchy stats derived from executed cache, transport, and DRAM paths,
