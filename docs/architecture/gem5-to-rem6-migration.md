...
### RISC-V SE, Workloads, and Linux Boot - 40% single-axis

**Score calculation:** 4 of 10 items have executable evidence. Static newlib
...
**Migrated:** RISC-V SE ecall path, many syscall slices, typed unknown-syscall
records, startup stack, and static smoke coverage.
...
`riscv_syscall_getrusage`, `riscv_se_resource`, `RiscvLinuxBootHandoffConfig`,
and RISC-V DTB handoff tests.
...
| `tests/gem5/se_mode` | `rem6-system`, `rem6` CLI | 45% single-axis | RISC-V SE startup, ecalls, static newlib smokes, selected syscalls, `ENOSYS` records, and guest writes exist. | Split hello, multicore SE, RVV intrinsic, and other-ISA subrows; add broader libc and lifecycle behavior. |
...
| `tests/test-progs` | `rem6-system`, `rem6` CLI, ISA crates | 30% unit-slice | Static RISC-V no-libc and newlib smoke binaries are generated when tools exist. | Add durable generated fixtures for hello, threads, and m5 utility shapes across ISAs. |
