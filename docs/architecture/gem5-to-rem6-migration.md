...
executor validation flow, GPU/accelerator shells, DMA routing, and a minimal GPU
...
acquisition beyond the host-file slice, network/archive resource acquisition
executors, runtime handoff of acquired suite resources into executable workload
replay, broad GPU ISA semantics, GPU cache/DRAM interaction, and broad benchmark
orchestration.
...
for executable workload replay, network/archive-backed workload acquisition,
...
| `tests/gem5/gem5_resources` | `rem6-workload`, `rem6` CLI | 50% single-axis | Resource declarations, identity, provenance, disk-image construction records, library-level in-memory acquisition executor records, and manifest/suite-level `rem6 resource-acquire` execution with local-artifact and host-file inputs exist. | Add network-backed, archive-backed, and broader artifact-kind acquisition coverage. |
