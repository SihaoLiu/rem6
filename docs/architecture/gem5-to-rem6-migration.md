...
breakpoint patch/restore tests, CLI data-access probe tests, CLI `Exec`, `Fetch`, `Data`, `Dram`, `Fabric`, `Memory`, and `Syscall` debug-flag tests, including `Data` load/store/atomic classification stats, `Memory` event-kind and response-status stats, `Syscall` return/exit/blocked outcome stats, `Dram` and `Memory` debug trace record-count stats, and `Fabric` debug trace record-count plus lane-transfer and lane/hop byte/flit aggregate stats, and histogram registry/output tests.
