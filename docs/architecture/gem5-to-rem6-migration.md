...
ignored-command, IRQ-clear, shader/tiler/L2 low-word power, deterministic fault-record, and job/MMU interrupt-block slices, broader power/register fault behavior, real interrupt delivery, external-tool ingestion beyond rem6-shaped adapter artifacts,
...
ignored-command, IRQ-clear, shader/tiler/L2 power, register-fault, job/MMU interrupt-block, multi-artifact envelope, and output-path conflict evidence.
