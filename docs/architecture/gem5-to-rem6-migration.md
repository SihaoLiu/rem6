...
endpoints, positional vector I/O unit coverage for guest files, pipe `ioctl(FIONREAD)` unread-byte reporting, terminal `ioctl(TIOCGWINSZ/TCGETS)` reporting, deterministic guest-only `socketpair` stream queue coverage for bidirectional direct ecall I/O, name queries, shutdown, selected `SOL_SOCKET` options, null-address `sendto`/`recvfrom` with no-network flags, and poll readiness without pipe identity, plus a static newlib program that roundtrips bytes through `pipe2`,
