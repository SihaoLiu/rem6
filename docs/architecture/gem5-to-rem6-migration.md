...
- [x] Static no-libc and newlib smoke binaries can be generated and compared with qemu when tools exist; tool-detected newlib directory-open and `O_NOCTTY`/`O_NOFOLLOW` coverage runs through the legacy `open` syscall and registered guest files.
...
`riscv_se_open_flags`, `riscv_se_permissions`,
