...
`rv64d`, `rv64d_fma`, `vector_config_prediction`, `vector_compress`,
...
input from a manifest `Input` resource into backing-store guest memory, with
data-cache-resident read destinations still rejected until a coherent writer or
topology-routed firmware store exists, without topology-routed firmware memory
transactions, minimal TIME `set_timer` STIP
scheduling, IPI
...
identity, ISA, and counter CSR register-cache paths, and
...
and live `cycle`/`time`/`instret` reads after top-level GDB single-step execution,
