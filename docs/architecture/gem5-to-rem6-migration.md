...
| `tests/gem5/processor_switch_tests` | `rem6-system`, `rem6-cpu` | 20% unit-slice | Host-assisted switch admission, execution-mode metadata, top-level real RISC-V `m5_switch_cpu` detailed-mode metadata recording, continued execution, checkpoint-backed core/memory state-transfer artifact counts, workload replay restore-by-label of generated transfer manifests, reserved-label protection, and generated-transfer rollback of pre-switch execution-mode authority exist. | Add broader quiescence-gated model handoff and rollback validation across CPU backends. |
