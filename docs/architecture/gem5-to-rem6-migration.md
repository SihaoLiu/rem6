...
The bucket cap is single-axis because GPU memory behavior now has a narrow
top-level cache/DRAM micro-run but is not representative, manifest and suite
...
- [x] CLI `gpu-run` routes recorded coalesced scalar GPU global memory requests through direct memory or MSI data-cache and DRAM-backed runtime stats after GPU workgroup completion, with bank-level cache counters exposed in the top-level artifact.
...
DRAM read, and transport stats from recorded coalesced GPU global memory requests, plus per-CU
