...
- [x] Unknown syscalls and unsupported signal-frame restore (`rt_sigreturn`) return `ENOSYS` with typed diagnostics, while known no-implementation entries (`lookup_dcookie`/`nfsservctl`, `clone`/`clone3`, `io_uring_*`, `process_madvise`/`process_mrelease`, `landlock_*`, `futex_waitv`, `set_mempolicy_home_node`, and `cachestat`) return `ENOSYS` without unknown records.
