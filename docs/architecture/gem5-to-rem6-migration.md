...
`vmv.v`, `vmv.x.s`/`vmv.s.x`, and `vmv<nr>r.v`, unmasked slide `vslideup`/`vslidedown`/`vslide1up`/`vslide1down`, gather `vrgather`, mask reductions `vcpop.m`/`vfirst.m`, mask prefixes `vmsbf.m`/`vmsof.m`/`vmsif.m`, mask indexes `viota.m`/`vid.v`, compress `vcompress.vm`, zero/sign extension `vzext`/`vsext` masked/unmasked decode with `vf2` hart execution and unmasked slide/gather, mask reductions, mask prefixes, mask indexes, plus `vzext.vf2` CPU fetch-stream execution, fixed-point narrow clip `vnclipu.wi`/`vnclip.wi`, and unmasked floating-point `vfadd`, `vfsub`, `vfmin`, `vfmax`, `vfmul`, and
