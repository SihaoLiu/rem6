...
| memory ports, packets, requests in `src/mem` root | `rem6-transport`, `rem6-memory`, `rem6-stats` | partial | Shared request/response transport exists, and `MemoryRequest` now has a stable checkpoint payload preserving operation, identity, address, size, line layout, data, byte masks, atomic operation, independent uncacheable and strict-order flags, and explicit before/after ordering edges. `MemoryResponse` also has a stable checkpoint payload preserving request identity, status, and optional response data without pulling response state into an untyped packet container. Native atomic swaps now cover both return-data and no-return request forms through line-store execution, coherence backing-store mutation, cache-controller hit paths, MSI/MESI/MOESI/CHI directory ownership and snoop decisions, DRAM access classification, and trace response/error matching. A typed communication monitor covers gem5 `CommMonitor` request/response statistics with explicit pending packet ids, periodic windows, and snapshot validation rather than embedding timing metadata in packet sender-state chains. More gem5 packet semantics need mapping as features are added. |
