...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  counts, deterministic dimension lists, per-dimension record slices, counts, and tick windows by kind, partition, and subject, declared-threshold livelock diagnostic counts, merged resource and
