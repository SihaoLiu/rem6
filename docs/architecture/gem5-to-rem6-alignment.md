...
an executable run artifact instead of only as a raw delivery list. The
standalone `rem6 gups` CLI now builds a zero-initialized table, runs the GUPS
controller through the same transport and backing-store response path, emits
the response statistics, exposes transport counters, and can dump the mutated
table bytes, so the response-driven XOR write path is reachable from a user
artifact without a guest ELF. A typed traffic
...
workload-manifest GUPS binding, fabric-backed GUPS transport, and broader transport
execution, QoS, or probe coverage remain open traffic-generator targets.
