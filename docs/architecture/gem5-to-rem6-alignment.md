...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  as a generic environment-call trap. It also handles `getpid`, `gettid`,
  `getuid`, `geteuid`, `getgid`, and `getegid` from gem5-default typed
  single-process identity state, and handles `brk` as a returning syscall by
  updating typed program-break state, writing the return value to `a0`, clearing
  the pending trap, restoring user execution, and resuming at the instruction
  after the ecall. Anonymous RISC-V Linux `mmap` and `munmap` are also handled as
  returning syscalls for page-aligned private/shared mappings: the table
  validates Linux arguments, tracks typed mmap regions from the RISC-V64 mmap
  base, slices regions for fixed maps or unmaps, writes return values to `a0`,
  clears pending traps, and resumes guest execution. Fork, clone, exec, and
  dynamic credential changes, file-backed mmap handling, backing-memory or
  page-table installation for returned ranges, `openat`, `futex`, `wait4`, and
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  returns for `getpid`/`gettid` and UID/GID queries, plus anonymous
  `mmap`/`munmap` region state returned and updated through the same real ecall
  path. It still lacks fork/clone process modeling, file-backed mmap handling,
  backing-memory installation for returned ranges, the broader syscall table,
  and guest-memory argument paths needed for `openat`/`futex`/`wait4`, and Linux
  handoff still needs an SBI-class firmware/runtime path rather than only
