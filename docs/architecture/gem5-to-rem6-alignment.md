...
| `ext/libelf`, `ext/libfdt`, `ext/softfloat`, `ext/gdbremote` | `rem6-boot`, `rem6-platform`, future debug and ISA support crates | partial | rem6-boot has typed ELF32 and ELF64 little-endian loaders plus an auto-detecting ELF entry point that validates the ELF identity and program-header shape, records typed ELF class, machine, and architecture metadata comparable to gem5 `ElfObject::determineArch()`, imports physical-address `PT_LOAD` segments into `BootImage`, skips non-loadable segments, preserves `p_memsz > p_filesz` zero-fill, and rejects unsupported class, encoding, version, or overflowing segment memory ranges with typed errors without libelf or unsafe parser state. Workload boot images preserve that ELF metadata through manifest round trips and include it in manifest identity, so ISA selection cannot silently drift when two images have the same entry and loadable segments but different ELF machine headers. rem6-platform has an initial typed DTS source tree and deterministic binary FDT/DTB writer for RISC-V platform boot descriptions, including Linux `/chosen` bootargs and initrd start/end metadata, and rem6-system installs initrd bytes plus generated or resolved-resource DTBs into guest memory with the RISC-V A1 register handoff for both store-backed and DRAM-backed memory. rem6 still needs broader endian/ISA coverage, kernel image loader breadth, bootloader handoff coverage, soft-float, and debug capability without vendoring unneeded code into the core. |
