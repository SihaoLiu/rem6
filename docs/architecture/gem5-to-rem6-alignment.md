...
| `src/sim` | 176 | `rem6-kernel`, `rem6-system`, `rem6-checkpoint`, `rem6-stats`, `rem6-power` | partial | Event queues, ticks, objects, exit events, power hooks, probes, checkpoints, and statistics need typed partitioned equivalents. Core scheduling, per-partition parallel activity summaries, typed wait-for/deadlock diagnostics, typed progress-free transition livelock diagnostics, typed probe events, typed power domains, and checkpoints exist. |
...
- Partitioned scheduler tests cover deterministic event order, lookahead,
  ...
  and useful-work reset of active livelock windows.
