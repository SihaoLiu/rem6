...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  register execution. RISC-V ISA tests cover `MUL`/`MULH`/`MULHSU`/`MULHU`/`DIV`/`DIVU`/`REM`/`REMU`
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  needs compressed and floating-point decode/execute coverage, the current
