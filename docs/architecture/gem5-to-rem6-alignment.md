...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  no-ack session transition state, session-level frame dispatch, interrupt
  request state, trailing bytes, and skipped prefix noise before any ISA
  register cache or socket loop can mutate simulator state.
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  no-ack request parsing, no-ack session transitions, session frame dispatch,
  interrupt request recording, bounded payload enforcement, acknowledgement, negative
