...
- Power equations should not depend on late string lookup into global
  ...
  in the same RC solve as powered domains.
...
- RISC-V PMA anchor refreshed on 2026-05-30: gem5's local `PMAChecker` keeps
  ...
  sender-state identity with an auditable request-id map. NoC scheduling
  consumption of those flags and broader response-latency modeling remain
  alignment targets.
...
- Kernel checkpoint restore schedule tests cover the separation between
  ...
  the replay clock or after the restored tick, and warmup slack summaries.
...
- Power tests cover typed power state domains, leader/follower matching,
  ...
  invalid topology rejection, update history, and snapshot restore.
