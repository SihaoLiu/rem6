...
requests with no data, no byte mask, response required, no response data,
writable intent, and no read/write byte accounting. Trace packet flags now map
`INST_FETCH` on `ReadReq`, `ReadCleanReq`, and `ReadSharedReq` packets to
native instruction-fetch requests, accepts `PHYSICAL` as trace-address metadata,
and maps
`UNCACHEABLE`, `STRICT_ORDER` plus `UNCACHEABLE`, `ACQUIRE`, and `RELEASE`
