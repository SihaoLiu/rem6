...
- RISC-V privileged ISA anchor refreshed on 2026-06-03: Supervisor trap return
  ...
  official privileged return rules before adding broader delegation and
  interrupt delivery.
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  paths, `mtvec`/`mcause`/`mtval` machine-trap CSR paths,
  `stvec`/`sepc`/`scause`/`stval` supervisor-trap CSR paths, synchronous
  `ecall`/`ebreak` Machine trap entry, Machine-mode `MRET`, and
  Supervisor-mode `SRET` PC/privilege/status restoration. RISC-V frontend tests also cover
