...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  handoff summaries, and requires tests for invalid platform, memory, and
  workload state before broad compatibility claims.
...
| gem5 source anchor | rem6 owner | Coverage | Notes |
| --- | --- | --- | --- |
| event queue and tick logic in `src/sim` | `rem6-kernel` | covered | Partitioned scheduling, conservative epochs, deterministic order, lookahead, scheduler snapshots, and typed parallel-worker failure reporting that preserves remaining partition events, keeps executed-time visibility, and rolls back local events scheduled by the panicked callback exist. |
