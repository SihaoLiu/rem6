...
- `rem6-virtio` now keeps queue indexes, queue specs, notify specs, queue
  ...
  enforce the hard per-source-file size budget, so block, queue, rng, MMIO
  transport, PCI transport, shared-memory, and future network/console VirtIO
  work does not recreate gem5-style mixed device, queue, and transport
  monoliths. VirtIO RNG now exposes gem5's device id 4 and zero-length config
  ...
  helpers. VirtIO MMIO transport now exposes gem5-visible magic, version,
...
- VirtIO tests cover legacy RNG device id and zero-config behavior,
  ...
  readable/empty RNG descriptor rejection, legacy MMIO magic/version/device/vendor id registers,
