...
operations. Line-sized `CleanEvict` command-id 10 packets whose effective
address after the configured offset is cache-line aligned map to native
maintenance requests with no data, no byte mask, no response, and no read/write
byte accounting. Line-sized `UpgradeReq` command-id 17 packets whose effective
address after the configured offset is cache-line aligned map to native
maintenance upgrade requests with no data, no byte mask, response required, no
response data, writable intent, and no read/write byte accounting; absent or
zero flags are accepted, and all other commands or nonzero flags fail with
typed errors before request construction. Trace replay
