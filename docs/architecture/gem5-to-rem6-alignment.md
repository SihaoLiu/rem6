...
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V currently covers typed counters, vector state foundations, RV64I/RV64A/RV64C scalar coverage, initial RV64D double-precision load/store plus RNE arithmetic, RNE square root, RNE integer-to-double and double-to-integer conversions, sign-injection, comparisons, min/max, classification, raw moves, Sv39 translation helpers, CPU memory-walker integration, trap/CSR/privileged-return foundations, and typed GDB target descriptions. Full F/D coverage including fcsr/fflags/dynamic and static non-RNE rounding and remaining operations, broader CPU data-path integration for hardware Sv39 walking, and ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before parity. |
...
| --- | --- | --- | --- |
| `src/arch/riscv` | `rem6-isa-riscv`, `rem6-cpu` | partial | RV64I decode and execution cover immediate/register arithmetic, word operations, loads/stores, branches, fences, `WFI`, `SFENCE.VMA`, counters, PMP, vector state plus limited vector execution, Sv39 helpers, CPU translation frontend, data translation, and RV64A LR/SC/AMO paths with typed reservations, aq/rl ordering, atomic memory requests, and cache-snoop reservation invalidation. An initial RV64D scalar slice covers checkpointed double-precision register state, double-precision load/store, RNE arithmetic, RNE square root, RNE integer-to-double and double-to-integer conversions, sign-injection, comparisons, min/max, classification, raw integer/float moves, raw-bit floating-point writes, integer comparison/misc writes, and CPU float-load response writeback. Full F/D coverage including fcsr/fflags/dynamic and static non-RNE rounding, remaining conversions, single precision, remaining F/D operations, interrupt-aware `WFI` quiesce/resume, fuller CPU pipeline TLB wiring, timed hardware page-table walker memory accesses, cache/memory AQ/RL timing enforcement, fuller vector execution, and richer traps remain alignment targets. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  `FLD`/`FSD` decode and memory access records, RNE
  `FADD.D`/`FSUB.D`/`FMUL.D`/`FDIV.D` execution, double-precision sign
  injection, comparison, min/max, classification, raw integer/float move,
  RNE `FSQRT.D`, and RNE integer-to-double and double-to-integer conversion
  execution, raw-bit floating-point write records, integer comparison and misc
  write records, and
  CPU data-response writeback into
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  coverage beyond the current double-precision load/store, RNE arithmetic,
  RNE square root, RNE integer-to-double and double-to-integer conversion,
  sign-injection, comparison, min/max, classification, and raw-move slice.
  Floating-point CSR state, exception flags, dynamic and static non-RNE
  rounding modes, FP-to-FP conversions, single precision, and the remaining
  F/D operations are still open. The current host-assisted mode has guest fd
  and futex state but not a syscall handler table, and Linux handoff still
  needs an SBI-class firmware/runtime path rather than only
