...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  no-ack session transition state, trailing bytes, and skipped prefix noise
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  no-ack request parsing, no-ack session transitions, bounded payload
  enforcement, acknowledgement, negative
