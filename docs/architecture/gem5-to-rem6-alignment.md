...
| `configs/boot`, `configs/dist`, `configs/splash2`, `configs/learning_gem5`, `configs/deprecated` | 27 | `rem6-boot`, `rem6-workload`, tests | partial | Boot and benchmark examples should become manifest resources, not external scripts. Linux boot handoff manifests now make initrd resources explicit, require matching resource definitions, validate initrd resource kind, validate resolved payload digest and size, and include bootargs plus DTB/initrd placement in manifest identity. Deprecated examples are audit input only. |
