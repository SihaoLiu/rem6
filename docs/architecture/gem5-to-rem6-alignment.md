...
- `rem6-virtio` now keeps queue indexes, queue specs, notify specs, queue
  ...
  fid to the opened file, `Tlopen` marks file fids open and reports qid plus
  I/O-unit data, `Tread` returns counted byte ranges, `Twrite` mutates and
  extends byte ranges, and `Tclunk` drops fid state. Missing names and stale
  fids return `Rlerror` errno payloads instead of panicking or depending on an
  external proxy. VirtIO RNG now exposes gem5's device id 4 and zero-length config
...
- VirtIO tests cover VirtIO 9P device id, mount-tag feature bit,
  ...
  file creation and opened-fid retargeting, `Tlopen` file-qid and I/O-unit
  replies, counted `Tread` ranges, `Twrite` counted replies plus overwrite
  mutation, `Tclunk` fid removal, and stale create, write, and read `Rlerror`
  handling, modern PCI version-1 feature exposure for
