...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  responses, `p n` single-register read requests, target-order register-byte
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  alive-or-missing response emission, `p n` single-register request
  parsing, target-order register byte hex response
