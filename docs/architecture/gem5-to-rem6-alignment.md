...
| `src/cpu/pred` | `rem6-cpu` branch predictor module | partial | A local two-bit predictor and return-address stack have independent typed prediction, update, target, speculative history, commit, repair, and snapshot state. Richer predictor families, indirect prediction, and pipeline integration remain open. |
...
- CPU branch prediction exposes typed prediction, update, target, speculative
  history, return-stack operation, commit, repair, and snapshot records with
  restore validation.
