...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  responses, `m addr,length` memory-read requests, memory hex responses,
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  responses, `m addr,length` memory-read request parsing, memory hex response
  emission, missing-memory error responses, `M addr,length:XX...`
