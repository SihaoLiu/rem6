...
- Observability and statistics need stronger contracts. A gem5 issue about
  ...
  or counter values, and successful reset records are retained in typed
  registry-owned history. Counter paths are structured scope/name identities
  with a checked dot-separated spelling before registration, registry-owned
  stat groups preserve reusable hierarchy ids, and snapshots carry a group
  catalog so exported samples and deltas can resolve group ids without a live registry.
