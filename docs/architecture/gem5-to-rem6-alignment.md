...
and instantiates implemented IDLE, EXIT, LINEAR, RANDOM, STRIDED, and
DRAM-family text modes, a rem6 HYBRID text mode aligned with gem5
`createHybrid`/`PyTrafficGen` semantics, plus resolver-backed TRACE text modes
with explicit binding options. gem5's legacy text parser does not parse a
`HYBRID` state, so rem6's spelling is an explicit typed replacement surface
rather than a source-compatible text-syntax claim. The DRAM-family binding
...
Standalone GUPS coverage models gem5 `GUPSGen` RandomAccess traffic with fixed
8-byte table elements, the default four-updates-per-entry target, optional
update limits, random-index reads, response-driven XOR writes, deterministic
...
linear, random, strided, DRAM-family, hybrid, and trace generators, preserves gem5's
transition-before-packet priority at equal ticks, surfaces exit and trace-exit
events, and avoids live RNG mutation while comparing packet and transition
timing. Non-memory command coverage, broader trace flag mapping, full
