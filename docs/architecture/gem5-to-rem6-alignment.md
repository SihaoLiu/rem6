...
- `rem6-storage` now owns gem5 `DiskImage`, `RawDiskImage`, and `CowDiskImage`
  ...
  unsupported-DMA errors. PCI BAR dispatch, external interrupt delivery,
  timing delay, DMA transfer execution, and checkpoint chunks remain open.
...
| `src/dev/storage/ide_ctrl.*` | `rem6-storage`, future `rem6-pci` attachment | partial | rem6 now has a typed IDE controller core with primary and secondary channels, device0/device1 selection, command and control forwarding into selected disks, missing selected-device no-op writes and zero reads instead of panic, shared interrupt state, BMI status capability bits, BMI interrupt write-one-to-clear, PRD table low-bit masking, and typed unsupported-DMA start errors. PCI endpoint identity, BAR dispatch, bus-master enable gating, external interrupt delivery, DMA PRD execution, timing delay, config-space timing registers, and checkpoint chunks remain open. |
...
- VirtIO tests cover modern PCI common-config feature-page selection,
  ...
  and typed unsupported-DMA start rejection.
