...
| `src/dev` | 418 | `rem6-mmio`, `rem6-uart`, `rem6-timer`, `rem6-interrupt`, `rem6-gpu`, `rem6-accelerator`, `rem6-platform`, `rem6-pci` | partial | UART, timer, interrupt, an initial typed RISC-V CLINT MMIO model with crate-level snapshot/restore, typed reset policy, platform/topology attachment, typed RISC-V DTS source emission, binary FDT/DTB emission, RISC-V DTB memory/A1 handoff, typed Linux `/chosen` bootargs and initrd DTB metadata, typed DTB and initrd blob installation for store-backed and DRAM-backed memory, GPU, accelerator, and initial PCI config-space paths exist. Storage, network, virtio, PS/2, QEMU bridge, and broader platform-specific devices remain alignment targets. |
...
| `src/dev/pci`, `src/dev/virtio`, `src/dev/storage`, `src/dev/net` | `rem6-pci`, future device crates | partial | rem6-pci has an initial typed PCI endpoint config-space model: function addresses validate bus/device/function ownership, type-0 identity and class header fields are little-endian bytes, BAR sizing writes apply typed memory or I/O masks, command bits gate exposed BAR ranges, read-only writes return typed errors, and snapshots restore config and BAR state with function/identity/class checks. Block, network, and virtio devices remain required for full-system breadth. |
