...
- RISC-V privileged ISA anchor refreshed on 2026-06-03: Machine trap delegation
  ...
  Supervisor when the matching `medeleg` bit is set, while Machine-mode traps
  stay in Machine mode.
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  `ebreak` Supervisor trap entry, Machine-mode `MRET`, and Supervisor-mode `SRET`
  PC/privilege/status restoration. RISC-V frontend tests also cover
