...
requires the header tick frequency to match the caller-declared frequency, and
replays packet ticks relative to state entry with gem5-style current-tick
clamping. The supported command subset is intentionally narrow: `ReadReq`
maps to `ReadShared`, `WriteReq` maps to `Write`, absent or zero flags are
accepted, and all other commands or nonzero flags fail with typed errors before
request construction. Trace replay snapshots preserve cursor, sequence,
summary, tick offset, and active state, while exit resets the cursor and reports
whether the trace was exhausted. Traffic state-machine coverage now includes a
...
timing. Gzip trace streams, non-memory command coverage, flag-to-request
mapping, full memory-controller-coupled DRAM/NVM execution, and
