...
| `src/dev/storage/ide_disk.*` | `rem6-storage` | partial | rem6 now has a typed IDE disk PIO core with gem5 task-file offsets, status/error bits, identify payload geometry/capacity/model fields, read-native-max, software reset, LBA PIO read/write, sector-count-zero handling, staged write payloads, and typed errors for CHS access, unsupported commands, invalid offsets, wrong data direction, and storage failures. DMA, controller channel selection, BMI registers, PCI BAR dispatch, timing delay, and checkpoint chunks remain open. |
