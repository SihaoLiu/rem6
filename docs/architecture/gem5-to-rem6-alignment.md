...
- Simple models are not automatically cheap or transparent. Recent call-stack
  ...
  per-virtual-network queue-delay budgets, and activity-window coverage
  contracts so NoC hotspots and bounded congestion remain typed replay
  evidence.
...
| `configs/network` | 2 | `rem6-fabric`, `rem6-transport` | partial | Network configuration must map to NoC lanes, virtual networks, credits, wait-for diagnostics, per-link activity, per-lane activity, per-virtual-network activity, and queue-delay budgets across those scopes. |
