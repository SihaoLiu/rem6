...
- `rem6-system` workload replay now keeps data-cache line harness ownership,
  ...
  then prove the restored live MMIO reads and final snapshots return to the
  captured state.
...
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V exists with typed counters, vector state, atomic memory operations, `ADDI`/`ORI`/`SLLI` integer immediate execution, `mhartid` CSR reads backed by explicit hart identity, and typed RV32/RV64 GDB target-description documents for CPU registers; ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before claiming parity. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  register state and FIFO-owned RX descriptor state. Direct topology store and
  DRAM tests cover redirected fetches and data loads into addressed targets with
  different line layouts. RISC-V ISA and frontend tests cover `ADDI`/`ORI`/`SLLI` integer
