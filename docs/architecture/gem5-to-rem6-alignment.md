...
- Simple models are not automatically cheap or transparent. Recent call-stack
  ...
  work are spent. Fabric/NoC summaries therefore preserve per-lane and
  per-virtual-network transfer, byte, occupancy, queue-delay, contention, and
  tick-window evidence instead of collapsing all network activity into one
  aggregate counter.
...
| `configs/network` | 2 | `rem6-fabric`, `rem6-transport` | partial | Network configuration must map to NoC lanes, virtual networks, credits, wait-for diagnostics, per-lane activity, and per-virtual-network activity. |
