...
failures, control acks, control failures, and sideband events. Deeper
...
| guest-host events and pseudo instructions | `rem6-system`, `rem6-workload` | partial | ROI, stats, checkpoint, checkpoint restore, stop, execution mode actions, custom guest-host calls, manifest-declared guest-host response payloads, focused system/trap event port modules, guest trap kind preservation, and absolute parallel trap delivery-boundary preflight are typed. Broader guest ABI support remains open. |
...
- CLI tests cover the initial standalone `rem6 run` binary path: RISC-V ELF
  ...
  replay duration, and actual final-tick budget rejection. CLI source-policy
  tests now keep the binary crate root under the facade budget while
