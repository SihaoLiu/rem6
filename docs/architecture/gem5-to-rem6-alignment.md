...
- Parallel simulation is not a natural baseline in gem5. Historical Parallel M5
  ...
  worker-slot active and idle ticks, worker-lane partition ticks, and
  partition activity as a structured `parallel.scheduler` object, so scripted
  checks can parse typed parallel evidence without scraping stat-path strings
  or terminal output. CLI RISC-V data-access stats also carry total and per-core
...
- The gem5 output path convention writes run artifacts such as configuration
  ...
  at guest traps through the host event path, and records per-core architectural
  state plus hierarchical committed-instruction and parallel-scheduler stats.
...
- RISC-V PMA anchor refreshed on 2026-05-30: gem5's local `PMAChecker` keeps
  ...
  lines reject the bypass until eviction/writeback support exists. gem5 also
  routes uncacheable writes through `BaseCache::allocateWriteBuffer` instead
  of an MSHR; rem6 cache banks with a typed write queue now do the same for
  MSI/MESI/MOESI/CHI by enqueuing the original write request, preserving its
  uncacheable-plus-strict flags, exposing it as a ready write-queue issue,
  and avoiding MSHR or line installation. When that queue entry is issued,
  MSI/MESI/MOESI/CHI banks now retain a typed in-flight uncacheable-write
  record across snapshot/restore and match the later memory write response
  back into a CPU response without touching the fill path. This keeps gem5's
  useful `handleUncacheableWriteResp` timing shape while replacing Packet
...
- `rem6-timer` has an initial typed ARM Cortex-A9 CPU local timer/watchdog
  ...
  direct zero-delay event scheduling shape.
...
| `src/cpu/minor` | future in-order pipeline crate or `rem6-cpu` module | planned | Needs cycle-visible pipeline state, stalls, branch effects, and checkpoints. Its future RISC-V prediction path must use PC-only branch targets plus explicit vector-configuration updates so predicted streams cannot inherit stale `vl` or `vtype` from copied dynamic instruction state. |
...
| standalone simulator entry point and output artifacts | `rem6` | partial | The `rem6` binary crate now provides an initial tested `run` command for ELF-backed inputs. It validates requested ISA against ELF architecture, rejects mismatches before producing stats, emits a single JSON artifact with schema, input identity, ELF metadata, load status, structured `parallel.scheduler` evidence, and hierarchical typed stats, and can run RISC-V ELF inputs with `--execute --cores <n>` through real per-core fetch/data endpoints, per-core partitions, the memory transport, and the parallel system driver until a guest trap reaches the host. RISC-V CLI runs expose distinct `mhartid` values per core, so a shared ELF can branch by hart while still using the same parallel execution path. The main artifact and CLI stats now include the configured parallel worker limit plus recorded parallel scheduler dispatch, batch, total-worker, active-partition, remote-send, worker-tick, capacity, idle-worker, per-worker-slot active/idle ticks, actual worker-lane partition active ticks, and per-partition worker, dispatch, remote-send, remote-receive, and pending-event evidence rather than only the coarse epoch and max-worker counters. CLI stats also include per-core and aggregate data load/store/atomic bytes, aggregate and route/source fetch/data memory transport request and response counts, round-trip tick totals, and maximums. Requested `--dump-memory <addr>:<bytes>` ranges are read from the post-run store and emitted as hex in the same artifact, `--output <path>` writes the full JSON artifact to a file while stdout reports the artifact path, and `--stats-output <path>` writes the stats array as a separate file for direct stats consumers. Platform construction, checkpoint/runtime options, broader ISA execution, and additional output formats remain open. |
...
- Cache MSHR, MSI/MESI/MOESI/CHI cache-bank, and MSI bank directory harness
  ...
  bypass while preserving dirty snapshot evidence, and require direct
  uncacheable writes to enter the typed write queue without allocating MSHR
  entries, pending fills, or resident cache lines. The same protocols now
  cover queued uncacheable write response matching after snapshot restore:
...
- CLI tests cover the initial standalone `rem6 run` binary path: RISC-V ELF
  ...
  registers in the JSON artifact.
...
- RTC core tests cover MC146818-compatible binary and BCD calendar registers,
  ...
  interrupt routes. SP805 tests cover watchdog countdown, zero-load minimum-clock
