...
- Observability and statistics need stronger contracts. A gem5 issue about
  ...
  event sequences, and stale cursors before replacing the live registry, so
  removed callback/listener state cannot be reintroduced as reused ids after a
  checkpoint. Probe component names, point names, and listener names also use
...
- Stats tests cover registry-owned stat groups, self-describing group catalogs
  ...
  identifier-grammar, and cursor-preserving snapshot restore records with
