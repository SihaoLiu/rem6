...
| `src/proto` | 9 | `rem6-proto`, future adapters | partial | Protobuf-like exchange must produce typed rem6 data before entering simulation. rem6-proto has typed instruction, packet, and O3 dependency trace records, validation, canonical maps, window-checked dependencies, stable identity, and checked binary frame envelopes; concrete protobuf and gzip adapters remain open. |
...
- Proto-boundary tests cover typed instruction, packet, and O3 dependency trace
  ...
  truncation rejection, and checksum mismatch rejection.
