...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  the real user-mode ecall path. RISC-V `uname` number 160 now writes a
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  chunks into the guest buffer through simulated memory, and `uname` number 160
  by writing a gem5-compatible RV64 Linux `utsname` payload, and `getcwd`
  number 17 by writing the modeled target cwd. It also handles `wait4` number
