...
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V currently covers typed counters, vector state foundations, RV64I/RV64A/RV64C scalar coverage, initial RV64D double-precision load/store plus RNE arithmetic, sign-injection, comparisons, Sv39 translation helpers, CPU memory-walker integration, trap/CSR/privileged-return foundations, and typed GDB target descriptions. Full F/D coverage including fcsr/fflags/dynamic rounding and remaining operations, broader CPU data-path integration for hardware Sv39 walking, and ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before parity. |
...
| --- | --- | --- | --- |
| `src/arch/riscv` | `rem6-isa-riscv`, `rem6-cpu` | partial | RV64I decode and execution cover immediate/register arithmetic, word operations, loads/stores, branches, fences, `WFI`, `SFENCE.VMA`, counters, PMP, vector state plus limited vector execution, Sv39 helpers, CPU translation frontend, data translation, and RV64A LR/SC/AMO paths with typed reservations, aq/rl ordering, atomic memory requests, and cache-snoop reservation invalidation. An initial RV64D scalar slice covers checkpointed double-precision register state, double-precision load/store, RNE arithmetic, sign-injection, comparisons, raw-bit floating-point writes, integer comparison writes, and CPU float-load response writeback. Full F/D coverage including fcsr/fflags/dynamic rounding, conversions, single precision, remaining F/D operations, interrupt-aware `WFI` quiesce/resume, fuller CPU pipeline TLB wiring, timed hardware page-table walker memory accesses, cache/memory AQ/RL timing enforcement, fuller vector execution, and richer traps remain alignment targets. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  injection and comparison execution, raw-bit floating-point write records,
  integer comparison write records, and CPU data-response writeback into
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  sign-injection, and comparison slice. Floating-point CSR state, exception
  flags, dynamic rounding modes, conversions, single precision, and the
  remaining F/D operations are still open. The current host-assisted mode has
  guest fd and futex state but not a syscall handler table, and Linux handoff
  still needs an SBI-class firmware/runtime path rather than only
