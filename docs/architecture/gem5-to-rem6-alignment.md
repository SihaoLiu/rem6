...
- Parallel simulation is not a natural baseline in gem5. Historical Parallel M5
  ...
  state, and invalid completion writes return typed errors while preserving the
  outstanding claim.
  Wait-for
...
- PLIC/MMIO tests cover PLIC-compatible 32-bit priority, pending, enable,
  ...
  completion, typed wrong-completion errors that preserve claimed state, and
  platform interrupt-controller routing through the same PLIC register map
