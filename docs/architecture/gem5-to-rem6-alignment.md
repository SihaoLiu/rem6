...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  layouts. RISC-V ISA and frontend tests cover `LR.D` and `SC.D` decode,
  ...
  response plus new-value memory update, and `AMOADD.D` typed read-modify-write
  execution through the memory store, cache hit path, ISA, and CPU frontend.
  Memory and cache-controller tests cover atomic responses that capture old
  bytes before masked writes. RISC-V
...
- Cache MSHR, MSI/MESI/MOESI/CHI cache-bank, and MSI bank directory harness
  ...
  MSHR-to-transport QoS class export, per-cycle MSI bank run QoS counts by
  effective requestor and priority, parallel-cycle history counts by effective
  requestor and priority, and byte-snapshot restore of MSHR queue configuration
  plus target QoS state.
...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  arbitration, and direct target batch priority assignment from an explicit
  QoS requestor. DRAM tests cover QoS-driven same-arrival request ordering
