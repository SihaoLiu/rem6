...
| `src/arch/riscv` privileged return | `rem6-isa-riscv` | partial | Machine `mtvec`, `mepc`, `mcause`, and `mtval` CSR read/write/set/clear execution is present, with immediate CSR paths covered for the trap CSRs that exercise bit updates. Synchronous `ecall` and `ebreak` Machine trap entry updates `mepc`, `mcause`, `mtval`, `mstatus`, privilege, and handler PC, and Machine-mode `MRET` PC/status restoration is present. `SRET`, trap delegation, interrupts, other synchronous exceptions, lower-privilege return legality, and CSR legality checks remain alignment targets. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  paths, `mtvec`/`mcause`/`mtval` machine-trap CSR paths, synchronous
  `ecall`/`ebreak` Machine trap entry, and Machine-mode `MRET`
