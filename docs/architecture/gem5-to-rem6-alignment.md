...
event count, and final tick; workload manifests can require those minima and a
maximum final tick before replay verification accepts a result. The standalone
`rem6 gups` CLI now builds a zero-initialized table, runs the GUPS controller
through the same transport and backing-store response path, emits the response
statistics, exposes transport counters, and can dump the mutated table bytes,
so the response-driven XOR write path is reachable from a user artifact
without a guest ELF. A typed traffic
