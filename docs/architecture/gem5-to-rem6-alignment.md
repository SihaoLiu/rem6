...
- PCI tests cover typed legacy INTx line mapping, invalid interrupt-pin and
  ...
  subsystem IDs, Expansion ROM reads and writes, Expansion ROM size probing,
  and minimum-grant plus maximum-latency read-only byte behavior.
  Capability-list tests cover ordered MSI plus MSI-X chaining, next-pointer
  preservation across capability control writes, and overlap rejection without
  ...
  interrupt line/pin bytes, bridge-control writes, writable bus-number and
  memory-window registers, subordinate config routing, and downstream BAR
  host-range filtering through bridge windows.
