...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  as a generic environment-call trap. It also handles `brk` as a returning
  syscall by updating typed program-break state, writing the return value to
  `a0`, clearing the pending trap, restoring user execution, and resuming at
  the instruction after the ecall. Anonymous RISC-V Linux `mmap` is also
  handled as a returning syscall for page-aligned private/shared mappings: the
  table validates Linux arguments, tracks typed mmap regions from the RISC-V64
  mmap base, writes the mapped address to `a0`, clears the pending trap, and
  resumes guest execution. File-backed mmap handling, backing-memory or
  page-table installation for returned ranges, `openat`, `futex`, `wait4`, and
  guest-memory argument copying remain open before broad libc workloads can
  run. The table also exposes
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  resume for returning `brk` calls, plus anonymous `mmap` address allocation
  state returned through the same real ecall path. It still lacks file-backed
  mmap handling, backing-memory installation for returned ranges, the broader
  syscall table, and guest-memory argument paths needed for `openat`/`futex`/
  `wait4`, and Linux handoff still needs an SBI-class firmware/runtime path
