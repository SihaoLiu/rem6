...
| `src/dev/serial`, `src/dev/riscv`, `src/dev/lupio`, `src/dev/i2c` | `rem6-uart`, `rem6-mmio`, `rem6-interrupt`, `rem6-timer`, future device crates | partial | UART, timer, MMIO, interrupts, and an initial RISC-V CLINT model exist. The CLINT path keeps gem5's `msip`, `mtimecmp`, and read-only `mtime` MMIO layout while replacing direct `System::threads` interrupt mutation with typed interrupt ports and scheduler events, including parallel scheduling. Device-tree emission, CLINT checkpointing, reset semantics, RTC input modeling, and other platform devices remain open. |
...
- Timer/MMIO tests cover typed RISC-V CLINT `msip` software interrupts,
  ...
  read-only `mtime` from scheduler ticks, and the same `mtimecmp` path under
