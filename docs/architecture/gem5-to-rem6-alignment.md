...
| `src/cpu/testers`, `src/cpu/trace`, `src/cpu/probes` | `rem6-traffic`, tests, trace, stats crates | partial | `rem6-traffic` has an initial linear memory traffic generator matching gem5's cache-line block-size rejection, address wrap for accepted full-block ranges, independent random period and read/write samples, requestor-id write payload bytes, data-limit stop, retry-delay compensation for non-elastic request injection, typed overflow-checked summary counters, and full snapshot restore of grid-validated cursor, overflow-checked counters, deterministic request sequence, and RNG state. It also rejects non-full-block ranges before emitting typed memory requests that would cross the configured range. Random, strided, DRAM/NVM, hybrid, trace-backed generators, Markov config parsing, transport execution, traffic QoS, trace replay, probes, and the remaining gem5 traffic tests remain open and should feed typed events and summaries. |
