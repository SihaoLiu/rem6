...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  register completion, failed-condition completion without memory mutation, and
  typed load-reservation recording and clearing. RISC-V cluster tests cover
  peer reservation invalidation after a completed overlapping store so a later
