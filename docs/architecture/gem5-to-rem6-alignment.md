...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  parallel runs also expose remote-send counts at batch, epoch, run, and
  source-partition activity scope so cross-partition communication volume is
  observable without replaying callbacks.
