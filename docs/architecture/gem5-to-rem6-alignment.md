...
print tree that the packet trace does not encode. Broader TLBI execution
semantics, TLBI completion forms, CPU-visible HTM behavior, and full diagnostic
printing remain open because they need CPU/MMU/debug-visible event contracts,
and sync flags beyond `KERNEL` need additional typed metadata before import can
accept them.
