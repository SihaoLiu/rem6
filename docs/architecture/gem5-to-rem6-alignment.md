...
`ReadUnique`, `WriteReq` maps to `Write`, and `WriteLineReq` maps to `Write`
only when the address is cache-line aligned and the packet size equals the
configured cache line size; absent or zero flags are accepted, and all other
commands or nonzero flags fail with typed errors before request construction.
Trace replay snapshots preserve cursor, sequence, summary, tick
