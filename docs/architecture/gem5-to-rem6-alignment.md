...
- Observability and statistics need stronger contracts. A gem5 issue about
  ...
  or counter values. Stats deltas are also typed records derived only from
  snapshots in the same reset scope with nondecreasing tick and counter values.
...
| `src/sim` | 176 | `rem6-kernel`, `rem6-system`, `rem6-checkpoint`, `rem6-stats`, `rem6-power` | partial | Event queues, ticks, objects, exit events, power hooks, probes, checkpoints, and statistics need typed partitioned equivalents. Core scheduling, recorded initial and final parallel-epoch partition frontiers, per-partition parallel activity summaries, typed subsystem-local and merged full-system wait-for/deadlock diagnostics, scheduler-recorded progress-free transition aggregation, typed progress-free transition livelock diagnostics, typed probe events, typed power domains, monotonic stats reset handling through host actions, and checkpoints exist. |
...
- Stats tests cover counter reset epochs, reset-scope-checked snapshot deltas,
...
- Power tests cover typed power state domains, leader/follower matching,
  ...
  reset-scope-checked stat-delta binding through the core stats delta API,
