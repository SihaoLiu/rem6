...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  scheduler handoff, basic `vCont?` resume-action queries, `vCont;c/C/s/S`
  action packets, ordered typed resume-action records, `D[;pid]` detach
  requests, `k` terminate requests without response packets, `vKill;pid`
  terminate-process requests, typed disconnect records, `p n` single-register read requests, target-order register-byte
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  preservation, typed resume-request recording, resume stop-reply emission,
  `vCont?` resume-action query parsing, `vCont;c/C/s/S` action parsing,
  malformed `vCont` preservation, ordered resume-action recording,
