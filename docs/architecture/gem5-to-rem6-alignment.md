...
- RISC-V privileged ISA anchor refreshed on 2026-06-03: CSR address bits
  ...
  `cycle`/`instret` aliases, and machine `mcycle`/`minstret` aliases.
...
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V currently covers typed counters, vector state foundations, RV64I/RV64A/RV64C scalar coverage, initial RV64F/RV64D scalar load/store plus RNE arithmetic, RNE fused multiply-add/subtract, RNE square root, sign-injection, comparisons, min/max, classification, raw moves, single-precision and double-precision integer conversions, single-double FP conversions, `fflags`/`frm`/`fcsr` CSR access, `FDIV.S`/`FDIV.D` divide-by-zero and zero-divide-zero accrued flag updates, Sv39 translation helpers, CPU memory-walker integration, trap/CSR/privileged-return foundations, and typed GDB target descriptions. Full F/D coverage including dynamic and static non-RNE rounding, exception flags for remaining FP operations, and remaining operations, broader CPU data-path integration for hardware Sv39 walking, and ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before parity. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  single-precision NaN-boxing, RNE integer-to-double and
  double-to-integer conversion execution, RNE integer-to-single and
  single-to-integer conversion execution, RNE single-double conversion
  execution, `fflags`/`frm`/`fcsr` CSR read/write access with real
  ...
  paths, `medeleg`/`mideleg` and `mtvec`/`mcause`/`mtval` machine-trap CSR paths,
  ...
  writes, executable `cycle` and `instret` CSR reads from hart state, user read
  aliases, user write
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  single/double integer conversions, and single-double FP conversions.
  Complete exception flags beyond the current `FDIV.S`/`FDIV.D` cases,
  dynamic and static non-RNE rounding modes, broader FP-to-FP conversion
