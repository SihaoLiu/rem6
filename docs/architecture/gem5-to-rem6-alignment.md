...
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V currently covers typed counters, vector state foundations, RV64I/RV64A/RV64C scalar coverage, initial RV64D double-precision load/store plus RNE arithmetic, RNE square root, RNE integer-to-double conversions, sign-injection, comparisons, min/max, classification, raw moves, Sv39 translation helpers, CPU memory-walker integration, trap/CSR/privileged-return foundations, and typed GDB target descriptions. Full F/D coverage including fcsr/fflags/dynamic and static non-RNE rounding and remaining operations, broader CPU data-path integration for hardware Sv39 walking, and ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before parity. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  RNE `FSQRT.D`, and RNE integer-to-double conversion execution, raw-bit
  floating-point write records, integer comparison and misc write records, and
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  RNE square root, RNE integer-to-double conversion, sign-injection,
  comparison, min/max, classification, and raw-move slice. Floating-point CSR
  state, exception flags, dynamic and static non-RNE rounding modes,
  float-to-integer and FP-to-FP conversions, single precision, and the remaining
