...
| `src/cpu/testers`, `src/cpu/trace`, `src/cpu/probes` | `rem6-traffic`, tests, trace, stats crates | partial | `rem6-traffic` has initial linear and random memory traffic generators matching gem5's read/write selection, requestor-id write payload bytes, data-limit stop including zero-as-unlimited, retry-delay compensation for non-elastic request injection, typed overflow-checked summary counters, and deterministic snapshot restore. Linear traffic keeps full-block range validation and cursor restore; random traffic keeps gem5's byte-sampled absolute block alignment, including fixed-period RNG consumption and gem5-valid unaligned or partial ranges. Strided, DRAM/NVM, hybrid, trace-backed generators, Markov config parsing, transport execution, traffic QoS, trace replay, probes, and the remaining gem5 traffic tests remain open and should feed typed events and summaries. |
