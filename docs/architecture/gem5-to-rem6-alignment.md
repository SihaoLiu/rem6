...
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V currently covers typed counters, vector state foundations, RV64I/RV64A/RV64C scalar coverage, initial RV64F/RV64D scalar load/store plus RNE arithmetic, RNE square root, sign-injection, comparisons, min/max, classification, raw moves, single-precision and double-precision integer conversions, Sv39 translation helpers, CPU memory-walker integration, trap/CSR/privileged-return foundations, and typed GDB target descriptions. Full F/D coverage including fcsr/fflags/dynamic and static non-RNE rounding and remaining operations, broader CPU data-path integration for hardware Sv39 walking, and ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before parity. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  `FADD.D`/`FSUB.D`/`FMUL.D`/`FDIV.D` execution, RNE `FSQRT.S` and
  `FSQRT.D`, sign injection, comparison, min/max, classification, raw
  integer/float moves, single-precision NaN-boxing, RNE integer-to-double and
  double-to-integer conversion execution, RNE integer-to-single and
  single-to-integer conversion execution, raw-bit floating-point write records,
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  RNE arithmetic, RNE square root, sign-injection, comparison, min/max,
  classification, raw-move slice, and single/double integer conversions.
  Floating-point CSR state, exception flags, dynamic and static non-RNE
  rounding modes, FP-to-FP conversions, and the remaining F/D operations are
  still open. The current host-assisted mode has guest fd
