...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  the instruction after the ecall. Anonymous RISC-V Linux `mmap` and `munmap`
  are also handled as returning syscalls for page-aligned private/shared
  mappings: the table validates Linux arguments, tracks typed mmap regions
  from the RISC-V64 mmap base, slices regions for fixed maps or unmaps, writes
  return values to `a0`, clears pending traps, and resumes guest execution.
  File-backed mmap handling, backing-memory or page-table installation for
  returned ranges, `openat`, `futex`, `wait4`, and guest-memory argument
  copying remain open before broad libc workloads can run. The table also exposes
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  resume for returning `brk` calls, plus anonymous `mmap`/`munmap` region state
  returned and updated through the same real ecall path. It still lacks
  file-backed mmap handling, backing-memory installation for returned ranges,
  the broader syscall table, and guest-memory argument paths needed for
  `openat`/`futex`/`wait4`, and Linux handoff still needs an SBI-class
