...
| `configs/boot`, `configs/dist`, `configs/splash2`, `configs/learning_gem5`, `configs/deprecated` | 27 | `rem6-boot`, `rem6-workload`, tests | partial | Boot and benchmark examples should become manifest resources, not external scripts. Linux boot handoff manifests now make initrd resources explicit, require matching resource definitions, validate initrd resource kind, validate resolved payload digest and size, include bootargs plus DTB/initrd placement in manifest identity, and let replay install resolved initrd bytes without a script side effect. Deprecated examples are audit input only. |
...
- System topology tests cover typed RISC-V DTB handoff from the platform model
  ...
  memory. Workload replay tests cover resolved Linux initrd payload installation
  into guest memory snapshots from typed manifest handoff state.
...
- Workload manifests record boot images, resources, topology, host events,
  ...
  with initrd resource validation, explicit required-resource payload
  resolution, result metadata, execution mode switches, host action summaries,
