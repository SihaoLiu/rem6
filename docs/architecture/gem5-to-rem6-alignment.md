...
- `rem6-storage` now owns gem5 `DiskImage`, `RawDiskImage`, and `CowDiskImage`
  ...
  overlay reads. `rem6-virtio` can now execute block requests directly against
  ...
  private sector backend. Storage source-policy tests keep the crate under the
  facade and hard per-source-file budgets, avoiding gem5-style panic paths,
  process-exit save callbacks, and mutable SimObject disk state.
