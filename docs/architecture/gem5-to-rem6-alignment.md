...
- PCI tests cover typed legacy INTx line mapping, invalid interrupt-pin and
  ...
  interrupt line/pin bytes, bridge-control writes, bridge BAR0/BAR1 install
  and command-bit-gated host mapping, writable bus-number and memory-window
  registers, subordinate config routing, and downstream BAR host-range
  filtering through bridge windows.
