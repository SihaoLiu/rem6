...
- Simple models are not automatically cheap or transparent. Recent call-stack
  ...
  byte, occupancy, queue-delay, peak queue-delay, per-link and per-lane
  queue-delay budgets, and activity-window coverage contracts so NoC hotspots
  and bounded congestion remain typed replay evidence.
...
| `configs/network` | 2 | `rem6-fabric`, `rem6-transport` | partial | Network configuration must map to NoC lanes, virtual networks, credits, wait-for diagnostics, per-link activity, per-lane activity, and per-virtual-network activity. |
