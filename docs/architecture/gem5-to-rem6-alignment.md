...
| `src/cpu/pred` | `rem6-cpu` branch predictor module | partial | A local two-bit predictor, branch target buffer, and return-address stack have independent typed prediction, lookup, update, target, replacement, speculative history, commit, repair, and snapshot state. Richer predictor families, indirect prediction, and pipeline integration remain open. |
...
- CPU branch prediction exposes typed prediction, branch-target lookup,
