...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  values to `a0`, clears pending traps, and resumes guest execution. Fork,
  clone, exec, futex wake on child-clear-TID, dynamic credential changes,
  file-backed mmap handling, backing-memory or page-table installation for
  returned ranges, `openat`, `futex`, `wait4`, and guest-memory argument copying
  remain open before broad libc workloads can run. The table also exposes
