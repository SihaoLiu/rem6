...
- Compatibility bugs cluster around cross-subsystem seams. Recent public gem5
  ...
  per-CPU queries, so parallel full-system replay can keep LR/SC starvation
  evidence attached to the run result instead of hiding it in warnings and
  protocol-local latency side effects.
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  writes. RISC-V vector-config prediction tests cover branch-prediction targets
