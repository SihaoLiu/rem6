...
- CLI tests cover the initial standalone `rem6 run` binary path: RISC-V ELF
  ...
  worker-capacity ticks, and idle-worker ticks. CLI tests also cover ELF-backed
  RISC-V load/store execution through the data endpoint path and require
  per-core and aggregate data-access stats plus a post-store memory dump in the
  artifact. A two-core CLI test now reads `mhartid` from the same ELF,
