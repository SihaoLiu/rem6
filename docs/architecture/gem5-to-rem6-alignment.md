...
- The gem5 output path convention writes run artifacts such as configuration
  ...
  instruction execution, device boot, and full-system run plumbing remain
...
| standalone simulator entry point and output artifacts | `rem6` | partial | The `rem6` binary crate now provides an initial tested `run` command for ELF-backed inputs. It validates requested ISA against ELF architecture, rejects mismatches before producing stats, and emits a single JSON artifact with schema, input identity, ELF metadata, load status, and hierarchical typed stats. Full instruction execution, platform construction, checkpoint/runtime options, and richer output sinks remain open. |
...
- CLI tests cover the initial standalone `rem6 run` binary path: RISC-V ELF
  ...
  stats, while ISA/header mismatches fail before any stats are emitted.
