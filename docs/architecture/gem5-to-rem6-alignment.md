...
- Configuration and experiment reproducibility are too script-dependent in
  ...
  disk images, benchmark inputs, exit events, workload suites, and downloads
  remain workflow state outside the core simulator. rem6 therefore keeps typed
  builders, manifests, artifact identities, custom guest-host calls,
  manifest-declared responses, deterministic suite identities, and explicit
  boot handoff reports as the authority for platform and workload state.
