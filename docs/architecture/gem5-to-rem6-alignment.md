...
- Configuration and experiment reproducibility are too script-dependent in
  ...
  completion summaries, dispatch-declared suite execution expectations,
  suite-level execution efficiency summaries, declared suite speedup and
  utilization thresholds, and explicit boot handoff reports as the authority
  for platform and workload state.
