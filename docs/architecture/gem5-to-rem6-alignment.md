...
- Parallel simulation is not a natural baseline in gem5. Historical Parallel M5
  ...
  route identities from flat stats paths. When explicitly requested, CLI
  RISC-V execution can route guest fetch and data requests through the typed
  DRAM memory controller rather than a zero-timing line store, and the run
  artifact plus hierarchical stats expose the selected DDR, HBM, LPDDR, or NVM
  profile technology, topology labels, active DRAM targets, ports, banks,
  access counts, row-hit and row-miss counts, ready-latency totals and maxima,
  ...
  checkpoint path automatically. PLIC context output state is now derived from
  the same pending, enable, threshold, route, priority, and outstanding-claim
  rules used by the MMIO claim path, giving platform interrupt wiring a typed
  asserted-line view without adding a second delivery authority.
  Wait-for
...
- Replacement policy ownership is a fragile Ruby and CHI boundary in gem5. A
  ...
  in generated protocol code or callback side effects. MSI, MESI, MOESI, and
  CHI cache banks now use that boundary for clean-line capacity eviction and
  preserve replacement state in bank snapshots. MSI and MESI banks reject
  modified or transient capacity victims, while MOESI and CHI banks reject
  dirty or transient capacity victims, with typed state instead of silently
  dropping ownership before writeback integration exists. The same boundary
  carries cache partition ownership: way and max-capacity policies filter
  owner-tagged replacement candidates before victim selection, and an empty
  filtered candidate set stays an explicit miss-allocation failure instead of
  silently bypassing the partition rule. Partition-owned resident removal
  requires the same manager so release accounting cannot silently drift, and
  plain installs reject partition-owned victims unless a partition-aware path
  can update the manager. Restored directories can rebuild partition-manager
  capacity counters from resident owners before allocating again. Failed
  plain-install and partition-manager update paths are staged, so rejected
  allocations preserve replacement metadata and manager state.
- gem5 cache tags split set selection between SetAssociative and
  ...
  cross-set candidate aging under explicit skewed indexing. Partition-aware
  install paths turn those same locations into owner-tagged candidates before
  the replacement policy sees them, so partition filters compose with
  set-associative and skewed indexing geometry. Partition-aware removal releases
  the recorded owner through the typed manager before clearing residency, and
  restore-time manager rebuilding derives current capacity from resident owner
  metadata, while plain installs cannot silently replace owned residents.
  Failed partition-aware allocations commit neither candidate-aging nor
  victim-counter state. MSI, MESI, MOESI, and CHI cache banks can construct
  replacement directories with explicit cache indexing policies. rem6-cache also has
  protocol-neutral sector-tag metadata that uses gem5's sector-sized tag-entry
  indexing, per-sector replacement state, per-sub-block validity,
  last-sub-block invalidation, whole-sector
  ...
  dirty/transient victim rejection, and snapshot restore. rem6-cache also has
  protocol-neutral compressed-tag metadata for gem5-style superblocks,
  max-compression-ratio validation, per-sub-block compressed sizes,
  capacity-gated co-allocation, whole-superblock eviction, last-sub-block
  invalidation, replacement updates, and snapshot restore validation.
  MSI, MESI, MOESI, and CHI cache banks can now opt into those compressed tags
  for explicit compressed-size fills, hit-path replacement access, resident
  removal, whole-superblock clean eviction, dirty/transient victim rejection
  with tag rollback, snapshot restore, and tag-backend mode mismatch rejection.
...
- Configuration and experiment reproducibility are too script-dependent in
  ...
  platform and workload state. The standalone `rem6 run` entry point can now
  load existing run options from a checked `[run]` TOML table and let explicit
  CLI flags override those defaults. Relative paths in the file resolve from
  the config directory, which starts moving experiment setup out of recompiled
  Rust while leaving full topology and suite configuration as open work.
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  checksums, malformed checksums, run-length encoded response data,
  malformed run-length encodings, notification packets without protocol
  acknowledgements, corrupted-notification skipping, legacy sequence-id
  rejection, command-payload literal star bytes, `qAttached[:pid]`
  attach-state queries, attached-or-created replies, `qC` current-thread
  queries, `QC thread-id` replies, `qfThreadInfo` and `qsThreadInfo`
  active-thread list queries, payload-bounded thread-list chunk replies,
  ...
  checks, `QStartNoAckMode` requests,
  advertised-support gating for no-ack mode, unsupported-command empty
  responses, `?` stop-reason queries, typed stop replies, `g` register-read
  requests, `G XX...` all-register write requests, all-register write `OK`
  responses, basic `H op thread-id` thread-selection requests and `OK`
  responses, basic `T thread-id` thread-alive requests for `0`, `-1`, and
  positive single-process IDs, plus alive-or-missing
  responses, legacy `c [addr]`, `C sig[;addr]`, `s [addr]`, and
  `S sig[;addr]` resume requests, typed resume-request records for later
  scheduler handoff without immediate stop replies, `vContSupported`-gated
  basic `vCont?` resume-action queries, `vCont;c/C/s/S` action packets,
  ordered typed resume-action records, `D[;pid]` detach
  requests, `k` terminate requests without response packets, `vKill;pid`
  terminate-process requests, typed disconnect records with post-disconnect
  packet suppression, basic `Z/z type,addr,kind` breakpoint and watchpoint
  requests, idempotent typed trap records, `p n` single-register read requests, target-order register-byte
  hex responses, unavailable-register `x` responses, missing-register error
  responses, `P n...=r...` single-register write requests, register-write
  `OK` responses, `m addr,length` memory-read requests, memory hex responses,
  response-payload-capped oversized memory-read errors, missing-memory error
  responses, `M addr,length:XX...` memory-write requests, all-or-error
  memory-write responses, no-ack session transition
  state, last-response retransmission on negative acknowledgement,
  session-level frame dispatch, interrupt request state, trailing bytes, and
  skipped prefix noise before any ISA register cache or socket loop can mutate
  simulator state.
  The Xfer registry keeps architecture-specific XML documents outside the
  command parser, so RISC-V, Arm, x86, and future register descriptions can be
...
- Observability and statistics need stronger contracts. Public gem5 issue
  ...
  labels. rem6 also models gem5's `PcCountTracker` as typed PC-count pairs,
  target-PC filters, one-shot target-pair hit updates, cursor-free snapshot
  restore validation, and optional RISC-V retired-PC probe consumption instead
  of CPU pointer plus manager pointer coupling.
  gem5's global/local `InstTracker` split is also represented as typed global
  retired-instruction thresholds plus local listener state, with matching
  RetiredInsts probe filtering, start/stop control, one-shot threshold hit
  updates, counter overflow reporting, and snapshot restore validation for
  duplicate or already-passed thresholds. Memory footprint probes now keep
  gem5's request-only, in-memory packet filtering and cache-line/page footprint
  accounting while replacing the implicit `System*` dependency with checked
  address ranges, reset-scoped current footprints, total footprints, probe event
  payloads, real RISC-V memory-route and MMIO data-access request input, and
  snapshot validation.
  Memory trace probes now keep gem5's packet
  field capture, header requestor metadata, tick ordering, and optional
  nonzero-PC emission as typed records rather than writing directly to a global
  output stream from the probe callback. Communication monitoring now keeps
  gem5-style request/response transaction counts, burst lengths, address masks,
  inter-transaction times, outstanding request counts, response latencies,
  cumulative read/write byte totals, and periodic bandwidth windows as typed
  sparse buckets with checked pending packet ids instead of Packet
  sender-state pointers. Bandwidth samples are converted to bytes per second
  through an explicit tick-frequency setting, and snapshot restore rejects
  ...
  MemCheckerMonitor now also consumes real RISC-V memory-route retry responses
  as non-forwarded response probes that preserve the pending checker
  transaction instead of requiring a completed-response data payload. Atomic
  read-modify-write checker data, functional reset producers, and failed/error
  response producers remain open integration targets.
...
- Simple models are not automatically cheap or transparent. Recent call-stack
  ...
  assumption.
  Separately, `rem6-kernel` now keeps source clock domains as typed
  performance-point tables with explicit domain ids, current performance
  levels, sorted nonzero period validation, no-op same-level updates, derived
  divider construction, overflow checks, frequency queries, and explicit
  ...
  direction. rem6 also keeps gem5's ideal QoS turnaround source behavior as a
  typed highest-priority opposite-on-tie policy, using the implementation's
  read/write tie rule instead of the contradictory header wording.
  rem6-fabric also keeps gem5's proportional-fair QoS score formula as a typed
  requestor policy, maps the gem5 rank into rem6's low-number-served-first
  priority convention, validates finite weights in
  `[0,1]` and requestor counts against configured priority levels, and rejects
  unknown proportional-fair requestors as typed configuration errors instead of
  letting untracked requestors silently perturb policy state.
- Power equations should not depend on late string lookup into global
  ...
  seed after checkpointing. McPAT, DSENT, and DRAMPower remain external tools,
  but rem6 now emits typed external-analysis records with stable target names,
  state residency, temperature, per-target power, and aggregate power so those
  adapters consume validated simulator evidence instead of probing string stats
  or linking third-party models into the core runtime.
- Compatibility bugs cluster around cross-subsystem seams. Recent public gem5
  ...
  preserves both counters, and CSR execution now checks address-encoded minimum
  privilege for the implemented CSR families before applying target CSR effects,
  routing lower-privilege accesses through the existing synchronous
  illegal-instruction trap and delegation path.
  ...
  `vtype`. The in-order CPU pipeline records the same PC-only prediction
  ...
  snapshot/restore plus external checkpoint payload round trips preserve
  streaks and diagnostics. `RiscvClusterRun` now
  ...
  or moved as wake and requeue records are produced. The RISC-V SE syscall
  path now reads the guest futex word for `FUTEX_WAIT` and
  `FUTEX_WAIT_BITSET`, rejects empty wait/wake bitsets, and returns Linux
  `EAGAIN` through the real ecall-resume path when the observed value differs
  from the expected value; equal-value blocking still needs a suspend/resume
  outcome before broad threaded-libc claims. gem5 does not serialize
  ...
  fd-table scavenging. gem5's local `dup` path notes that file descriptions are
  ...
  fill-service targets, while writeback, clean-evict, clean-shared, and
  invalidation maintenance requests are represented as explicit post-fill
  downstream requests on MSHR completion records. MSI, MESI, MOESI, and CHI
  banks enqueue queue-backed writeback and clean-evict post-fill maintenance
  requests before returning the fill result, and reject unsupported operations
  or queue-unavailable writeback and clean-evict paths without mutating the
  pending fill. Clean-shared and invalidation maintenance are preserved as
  result-level post-fill downstream requests instead of write-queue entries,
  and the MSI bank directory harness exposes transport fill completions that
  return those requests for routing. That keeps maintenance traffic observable
  to NoC, directory, and memory scheduling instead of hiding it behind a local
  ...
  buffer, dropping completions, or silently bypassing bandwidth. A stateful
  transfer buffer now retains deferred completions across cycles and drains
  them before newly ready work, preserving program-order auditability while
  keeping every admitted completion inside the declared future window.
  ...
  directly into live TLB state. Translation page-map entries also carry typed
  global/non-global scope through snapshots and checkpoint payloads, so page
  resolver fills preserve the scope before ASID-scoped invalidation runs.
...
Research anchors refreshed through 2026-06-03:
...
- RISC-V privileged ISA anchor refreshed on 2026-05-30: `mcycle` and
  `minstret` are 64-bit machine counters, while `cycle`, `time`, and
  `instret` are 64-bit user-readable counters, with RV32 exposing high-half CSR
  aliases rather than independent counters. rem6 now exposes executable
  `cycle`, `time`, and `instret` CSR reads through hart state instead of
  limiting counter coverage to standalone bank tests; `time` is currently backed
  by the hart cycle counter until a fuller CLINT timebase is wired through CPU
  execution.
...
- RISC-V privileged ISA anchor refreshed on 2026-06-02: `mepc` is the Machine
  exception program counter consumed by `MRET`, which restores privilege and
  interrupt-enable state from `mstatus`. rem6 keeps `mepc`, MIE/MPIE, MPP, and
  MPRV as typed hart/status state for Machine-mode return instead of hiding the
  transition inside callback-owned trap return state.
...
- RISC-V privileged ISA anchor refreshed on 2026-06-03: Supervisor trap return
  ...
  CSRs include `stvec`, `sscratch`, `sepc`, `scause`, and `stval`. rem6 keeps
  those CSRs as typed hart-owned state and implements `SRET` status restoration
  against the official privileged return rules. Supervisor interrupt entry
  honors `stvec` direct versus vectored modes after interrupt delegation.
- RISC-V privileged ISA anchor refreshed on 2026-06-03: Machine trap delegation
  ...
  Machine trap CSRs. rem6 now keeps delegation registers as typed hart-owned
  state and delegates current lower-privilege `ecall` and `ebreak` traps to
  Supervisor when the matching `medeleg` bit is set, treats lower-privilege
  `xRET` execution as an illegal-instruction trap, and keeps Machine-mode traps
  in Machine mode.
- RISC-V privileged ISA anchor refreshed on 2026-06-03: CSR address bits
  `csr[9:8]` encode the minimum required privilege. rem6 enforces that rule at
  execution time for the implemented CSR families: `mhartid`,
  `mstatus`/`sstatus`, Machine and Supervisor trap CSRs, `satp`, user
  `cycle`/`time`/`instret` aliases, and machine `mcycle`/`minstret` aliases.
  Lower-privilege access raises illegal-instruction through the existing
  synchronous trap/delegation path.
- RISC-V privileged ISA anchor refreshed on 2026-06-03: `mie`/`mip` hold
  interrupt enable and pending bits, `sie`/`sip` expose delegated Supervisor
  interrupt views, and `mideleg` masks delegated interrupts at Machine mode.
  rem6 now keeps those interrupt CSRs as typed hart-owned state, enters typed
  interrupt traps with the high cause bit set before executing the next
  instruction, and preserves delegated interrupt codes through the guest-host
  trap path.
...
- RISC-V PMA anchor refreshed on 2026-05-30: gem5's local `PMAChecker` keeps
  ...
  response without installing the returned bytes. MSI, MESI, MOESI, and CHI
  banks also remove the bypassed resident way from the replacement directory,
  so replacement metadata cannot report a line that the bank no longer owns.
  Dirty resident
...
Implementation evidence through 2026-06-11:
...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  It also has a typed guest fd table used by the initial syscall emulation
  path.
  ...
  RISC-V SE syscall emulation now has an initial Linux user table that consumes
  real user-mode `ecall` traps produced by CPU fetch/decode/execute when `a7`
  names `exit` or `exit_group`, forwarding the guest `a0` code through the
  system event path to the host stop controller instead of treating that ecall
  as a generic environment-call trap. It also handles `getpid`, `getppid`,
  `gettid`, `getuid`, `geteuid`, `getgid`, and `getegid` from gem5-default typed
  single-process identity state, records the `set_tid_address` child-clear-TID
  ...
  guest-file bytes plus zero fill through the configured guest-memory writer
  before committing the region, preserves fd offsets for file-backed mappings,
  slices regions for fixed maps or unmaps, writes return values to `a0`, clears
  pending traps, and resumes guest execution. The table also records RISC-V
  robust-list setup and handles current-thread robust-list queries by writing
  the recorded head pointer and length through the configured guest-memory
  writer. It also consumes gem5-style ignore-return startup calls for
  `nanosleep`, `sched_yield`, the initial `rt_sig*` signal-action family, and
  advisory memory-management calls such as `mprotect`, `msync`, `mlock`,
  `munlock`, `mlockall`, `munlockall`, `mincore`, `madvise`, and `mbind`,
  allowing those user-mode `ecall` traps to return zero and continue instead of
  becoming generic scheduled traps. The gem5-style ignored `rseq` path returns
  `-ENOSYS` through the same ecall-resume path. RISC-V `futex` wake and
  wake-bitset operations now mask gem5-style private and realtime flags, reject
  empty wake bitsets, wake typed guest futex waiters by thread group, return the
  woken count, and resume guest execution. RISC-V `FUTEX_WAIT` and
  `FUTEX_WAIT_BITSET` now consume the mismatch path by reading the RV64 guest
  futex word through simulated memory, comparing it with the expected value,
  returning `EAGAIN` on mismatch, and resuming guest execution. RISC-V `dup`
  now allocates the lowest free guest descriptor,
  and `dup3` installs the requested destination descriptor with `O_CLOEXEC`
  validation and same-fd rejection while sharing source file-description state.
  RISC-V `close` now removes descriptor entries, releases unreferenced file
  descriptions, returns `-EBADF` for invalid descriptors, and makes later
  descriptor operations fail through the same real ecall-resume path. RISC-V
  ...
  shared by file description. RISC-V `ioctl` number 29 now consumes the common
  libc tty-probe path from real user-mode ecall traps: valid guest fds return
  `ENOTTY` for terminal-style requests and currently unsupported ioctl request
  codes, while invalid fds return `EBADF`. RISC-V `write` now consumes a real user-mode
  ...
  registered guest-file size for end-relative seeks, reports `ESPIPE` for
  seeded standard descriptors, and reports `EBADF` or `EINVAL` for invalid
  descriptors, whence values, or negative resulting offsets.
  RISC-V `readlinkat` number 78 now consumes a real user-mode ecall when guest
  memory reading and writing are configured, copies the guest pathname from
  simulated memory, resolves it against typed registered guest symlink targets
  such as `/proc/self/exe`, writes the target bytes back to the simulated guest
  buffer with Linux-style truncation and no added NUL byte, and returns the
  copied byte count through the ecall-resume path. Non-`AT_FDCWD` directories,
  empty or missing paths, zero buffer sizes, guest faults, and overlong
  pathnames return Linux error values for this registered-link subset.
  ...
  only a table-level responder. RISC-V `getrandom` number 278 now validates the
  Linux generic flag bits, fills a bounded guest-buffer chunk from a
  deterministic byte stream through simulated memory, preserves the stream
  position across guest faults, and returns the transferred byte count through
  the real user-mode ecall path. RISC-V `clock_gettime` number 113 now writes
  an RV64 Linux `timespec`, and `gettimeofday` number 169 writes an RV64 Linux
  `timeval`, both derived from the current simulated tick. Time-buffer write
  failures return `EFAULT`, and unsupported `clock_gettime` ids return
  `EINVAL` without touching guest memory. RISC-V `prlimit64` number 261 now
  handles current-process `RLIMIT_STACK` and `RLIMIT_DATA` queries, writes the RV64
  Linux `rlimit` payload into simulated guest memory, permits pid zero or the
  modeled current process id, and reports Linux error values for unsupported
  resources, non-current pids, and guest-buffer write failures. RISC-V `uname`
  number 160 now writes a
  gem5-compatible RV64 Linux `utsname` payload into simulated guest memory,
  including `Linux`, `sim.gem5.org`, the gem5-default `5.1.0` release, the
  fixed gem5 version string, and `riscv64`, then resumes the user-mode ecall.
  RISC-V `getcwd` number 17 writes the modeled target cwd into simulated guest
  memory with gem5-style zero padding and `ERANGE` handling, then resumes the
  user-mode ecall. RISC-V `wait4` number 260 now consumes typed pending child
  statuses from the syscall state, writes the POSIX wait-status integer plus a
  ...
  for returned ranges, host filesystem path resolution
  ...
  without merging per-descriptor close-on-exec bits.
...
- `rem6-isa-riscv` now keeps GDB target-description XML generation in a focused
  `gdb_target` module and error reporting in a focused `error` module. Its
  source-policy tests keep ISA growth under the hard per-source-file size
  budget, while the target-description API returns typed annex documents that
  callers can register with the generic debug Xfer registry without adding a
  debug-crate dependency to the ISA crate. RV32/RV64 CPU register documents are
  generated from a stable ABI register table, and RV64 target descriptions now
  include a supervisor CSR annex for the implemented `sstatus`, `stvec`,
  `sepc`, `scause`, and `stval` state. FPU debug documents and broader CSR
  register-cache wiring remain open.
...
- `rem6-memory` now keeps `MemoryRequest`, `MemoryResponse`, response status,
  atomic request payload validation, typed compare-swap operands, optional
  stream and substream IDs, and atomic read-modify-write byte materialization in
  a focused `request` module.
...
- `rem6-system` workload replay now keeps data-cache line harness ownership,
  ...
  and restore. Replay-level SINIC restore tests now mutate guest-visible
  register state and FIFO-owned RX descriptor state after checkpoint capture,
  clear pending soft-interrupt status after checkpoint capture, then prove the
  restored live MMIO reads and final snapshots return to the captured state.
...
Traffic generator coverage now includes a typed trace replay slice aligned with
gem5 `TraceGen` packet traces. The rem6 importer accepts the gem5 protobuf
stream magic followed by length-delimited `PacketHeader` and `Packet` records,
auto-decompresses gzip-wrapped packet traces using gem5's content-magic
convention before validating the inner magic, requires the header tick
frequency to match the caller-declared frequency, preserves header object-id,
version, and id-string metadata for packet owner lookup, and replays packet
ticks relative to state entry with gem5-style current-tick clamping. The supported
...
`Write`, `SwapReq` command-id 34 maps to a native atomic swap request with
deterministic synthetic write data, writable intent, and both read and write
byte accounting; matching gem5 `ATOMIC_RETURN_OP` keeps old-value response data,
while `ATOMIC_NO_RETURN_OP` writes the swap payload without returning old bytes.
`LoadLockedReq`
command-id 26 maps to a native load-locked read request with response data, read
byte accounting, and shared-read coherence
intent, `StoreCondReq` command-id 27 maps to a native store-conditional write
...
accounting, `StoreCondFailReq` command-id 28 maps to a forced-fail
store-conditional write that carries deterministic synthetic write data but
always returns failed status without backing-data mutation, `SCUpgradeReq`
command-id 18 maps to a native store-conditional upgrade request with writable
intent and no payload, `SCUpgradeFailReq`
...
synthetic payload bytes for write-like commands because gem5 packet traces do
not store packet data.
Line-sized `CleanEvict` command-id 10 packets whose effective address after the
configured offset is cache-line aligned map to native maintenance requests with
no data, no byte mask, no response, and no read/write byte accounting.
Line-sized `CleanSharedReq` command-id 42 packets whose effective address after
the configured offset is cache-line aligned map to native clean-shared
maintenance requests with no data, no byte mask, response required, no response
data, no writable intent, no read/write byte accounting, and no backing-data
mutation. Line-sized `CleanInvalidReq` command-id 44 packets whose effective
...
Line-sized `InvalidateReq` command-id 54 packets whose effective address after
...
`MemSyncResp`, `MemFenceResp`, `CleanSharedResp`, `CleanInvalidResp`,
`InvalidateResp`, and `HTMReqResp` command-ids 2, 3, 5, 6, 14, 15, 19, 21,
...
on RISC-V data routes to typed CPU cluster outcomes. Workload summaries and
manifest expectations can now require those HTM begin records directly instead
of only requiring a generic HTM control acknowledgement. Replayed HTMReq
control failures remain ordered failure records and do not start a CPU
transaction or capture a data-cache rollback snapshot; they also surface as
failed workload HTM begin records with trace metadata while staying out of HTM
acknowledgement counts. During an active traced transaction,
...
those line sets. `UpgradeResp`, `ReadExResp`, and `LockedRMWReadResp`
writable-intent response policy also reaches the same workload data-cache
consumer, so upgrades record writable line intent while exclusive or locked
reads record both the read set and writable line intent before later write
packets arrive. Abort sidebands also roll back data-cache lines in the write set
by restoring the route-scoped snapshot captured at trace HTM begin.
...
a controller-aware target helper for `MemoryTransport` responders: when a
delivered response-required memory request has no queued response or failure
...
control ack or failure record at the replay tick. Control acknowledgements and
failures emitted from trace matches retain the sync or HTM source identity in
the control runtime, so a later source's queued action cannot satisfy an
earlier source that has no matching response. If the controller replays the
same response-required sync or HTM source without a matching control action, or
...
expectations count those executed HTM accesses separately from raw memory
responses, so a replay artifact can require that the HTM access policy reached
...
traffic-trace summaries and manifest expectations also carry write-completion
count and byte-count checks, so replay artifacts can require the executable
`WriteCompleteResp` consumer instead of inferring it from response deliveries
or raw memory trace length.
...
sideband consumers. The standalone `rem6 trace-replay` CLI now builds that
manifest path from an explicit packet-trace file, route, memory range, tick
frequency, line layout, agent, and control partition; it resolves the trace
input as a workload resource, expects the trace-only run to idle after event
exhaustion, rejects runs whose actual final tick exceeds the configured
...
stat ids remain stable. RISC-V fetch-route workload replay can now bind trace
response and error actions to the real fetch responder. Profiled memory
controller timing and broader CPU/cache execution paths remain separate
contracts.
...
optional address, optional size, optional packet id, optional PC metadata, and
gem5 response/error classification plus read/write classification and
...
through target helpers, and workload-bound fetch-route replay now feeds
matching trace responses or errors into the real fetch responder. These paths
record failed CPU port events at the trace failure tick without fabricating
retry or successful responses. Deeper cache-controller error injection beyond
those executable replay failure records and run-level data-cache error
...
typed HTM trace event into rollback snapshot capture, and HTM abort sideband
events pass the abort event into rollback restore, so non-quiescent data-cache
lines report contextual controller errors instead of unattributed scheduler or
protocol errors. Data-cache accepted trace responses, trace
errors, and HTM access records also surface as replay-level executed evidence,
run-level data-cache error outcomes, and summary counts, separate from generic
...
traffic. Delivered RISC-V and GPU or accelerator DMA
...
preserves gem5's line-sized cache-line-aligned shape contract for
cache-originated `ReadCleanReq`, `ReadSharedReq`, and `ReadExReq` trace packets,
and maps `ARCH_BITS`, `UNCACHEABLE`, standalone `STRICT_ORDER`,
`ACQUIRE`, `ACQUIRE_PC`, `RELEASE`, `PRIVILEGED`, `SECURE`, `PT_WALK`,
`EVICT_NEXT`, and `KERNEL` into typed rem6 request metadata, accepts
matching-packet `LLSC`, `LOCKED_RMW`, and `MEM_SWAP` operation selector flags,
maps matching-packet `MEM_SWAP_COND` on `SwapReq` to native compare-swap
requests with an explicit zero compare operand because gem5 packet traces omit
`Request::extraData`, accepts matching-packet
`ATOMIC_RETURN_OP` or `ATOMIC_NO_RETURN_OP` on `SwapReq` as the atomic response
data selector, routes the no-return variant through native memory execution and
trace response matching, and rejects
unsupported flag bits and
non-fetch packet combinations before request construction. Trace replay also
preserves optional packet `pkt_id` and `pc` fields as typed request-event
metadata. Trace replay snapshots preserve cursor, sequence, summary, tick
offset, and active state, while exit resets the cursor and reports whether the
trace was exhausted. Traffic state-machine coverage now includes a
typed gem5-style transition graph with deterministic Markov row selection,
duration-based transition ticks, no-timed-transition states, checked snapshots,
duplicate transition rejection, and exact row-sum validation instead of gem5's
dense-id matrix assumptions and silent duplicate-edge overwrite. The typed text
config parser accepts gem5 `TrafficGen::parseConfig` declarations, preserves raw
state-mode parameters, turns malformed fields, sparse text state ids, duplicate
records, unknown references, and invalid probabilities into typed diagnostics,
and instantiates implemented IDLE, EXIT, LINEAR, RANDOM, STRIDED,
DRAM-family, GUPS, and resolver-backed TRACE text modes plus a rem6 HYBRID
text mode aligned with gem5 `createHybrid`/`PyTrafficGen` semantics, all
through explicit binding options that can attach controller-level stream and
substream IDs to generated memory requests. gem5's legacy text parser does not parse
`HYBRID` or `GUPS` states, so those rem6 spellings are explicit typed
replacement surfaces rather than source-compatible text-syntax claims. The
DRAM-family binding
constructs typed DRAM, DRAM_ROTATE, and NVM traffic generators with bank, rank,
page-or-buffer, address-mapping, sequential-packet, data-limit, non-elastic
timing, snapshot, and checked geometry state; DRAM_ROTATE keeps deterministic
bank/rank rotation and its
limited gem5 read-percentage modes while ordinary DRAM and NVM keep
randomized series selection. The HYBRID binding constructs distinct DRAM and
NVM side geometry, samples the selected side by `nvm_percent` once per series,
keeps side and read/write kind stable across sequential packets, shares the
global data limit and timing policy, and snapshots the active side cursor.
The GUPS binding models gem5 `GUPSGen` RandomAccess traffic with fixed 8-byte
table elements, the default four-updates-per-entry target, optional update
limits, random-index reads, response-driven XOR writes, deterministic
snapshots, and typed unknown-completion errors while replacing opaque request
pointer maps with sequence-keyed pending reads and writes. `rem6-system` can
now drive a GUPS traffic controller through a static-latency `MemoryTransport`
route: completed read responses are decoded as 8-byte RandomAccess values, fed
back into the GUPS state, and unlock the XOR write request on the same route;
retry or malformed read responses fail explicitly before a write is emitted.
The GUPS transport run now also carries typed response statistics over real
transport deliveries, including completed, retry, store-conditional-failed,
read, write, and response-data-byte counts, so response handling is visible as
an executable run artifact instead of only as a raw delivery list. The same
run can now produce a workload GUPS summary with scheduled requests, response
status counts, read/write response counts, response data bytes, memory-trace
event count, and final tick. Workload manifests can declare GUPS runs by route,
memory target, table range, update limit, RNG state, agent, and maximum final
tick; `rem6-system` executes those declarations through the topology route and
zero-initialized backing store, then verifies the resulting workload summary
against manifest minima before accepting the result. The standalone `rem6 gups`
CLI now builds a zero-initialized table, runs the GUPS controller through the
same transport and backing-store response path, emits the response statistics,
exposes transport counters, and can dump the mutated table bytes, so the
response-driven XOR write path is reachable from a user artifact without a
guest ELF. A typed traffic
...
fabric-backed GUPS transport, and broader transport execution, QoS, or probe
coverage remain open traffic-generator targets.

## Top-Level Module Map

| gem5 source anchor | Local files | rem6 owner | Coverage | Alignment target |
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V currently covers typed counters, vector state foundations, RV64I/RV64A/RV64C scalar coverage, initial RV64F/RV64D scalar load/store plus RNE arithmetic, RNE fused multiply-add/subtract, RNE square root, sign-injection, comparisons, min/max, classification, raw moves, single-precision and double-precision integer conversions including static and dynamic non-RNE float-to-integer rounding, single-double FP conversions, `fflags`/`frm`/`fcsr` CSR access, `FDIV.S`/`FDIV.D` divide-by-zero and zero-divide-zero accrued flag updates, `FSQRT.S`/`FSQRT.D` invalid flag accrual for negative nonzero non-NaN and signaling-NaN inputs, NaN/out-of-range invalid and valid rounded-result inexact flag updates for float-to-integer conversions, and NaN invalid-flag updates for `FMIN`/`FMAX`, FP comparisons, and fused multiply-add/subtract with single-precision NaN-boxing preserved. Sv39 translation helpers, CPU memory-walker integration, page-fault trap entry with precise `stval`/`mtval` values, trap/CSR/privileged-return foundations, and typed GDB target descriptions are also present. Full F/D coverage including non-RNE arithmetic and integer-to-float rounding, exception flags for remaining FP operations, and remaining operations, broader CPU data-path integration for hardware Sv39 walking, and ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before parity. |
...
| `src/cpu` | 363 | `rem6-cpu`, `rem6-kernel`, `rem6-system`, `rem6-traffic` | partial | RISC-V cluster execution exists, RISC-V data access records expose absent memory-route metadata for MMIO accesses as typed optional state instead of panic-only accessors, CPU cluster parallel epochs retain both initial and final partition frontiers through full-system run summaries, and CPU cluster scheduler epochs plus runs expose exact remote-send records, remote-send endpoint counts, progress-free transitions, ordered batch timelines, longest parallel-window evidence, batch worker-count, duration-weighted worker-count tick, worker-tick, partition-set, and same-partition-set streak evidence from the kernel without relying on caller-local batch scans for scheduler-scope counts. RISC-V store-conditional progress now records per-CPU failure streaks, typed threshold diagnostics, success resets, snapshot/restore state, external checkpoint payload round trips, cluster-run aggregation, and full-system run aggregation for LR/SC starvation evidence instead of depending on periodic warnings. rem6-cpu now has typed HTM transaction uid/depth/failure-cause state, active-transaction snapshot/restore, and a RISC-V core consumer that restores the saved architectural hart checkpoint on abort while keeping committed architectural updates after commit. Branch prediction has typed state for several gem5-style predictors, and GShare history updates reject stale prediction records before per-thread history or counters can drift. RISC-V branch-prediction control-flow targets now cannot carry vector configuration copied from older dynamic instruction state. RISC-V O3 full-system branch-target safety now rejects the gem5 #2211-shaped combination where RAS is disabled and indirect target path hashing is also disabled. rem6-cpu also has an initial typed in-order pipeline scheduler that validates per-stage widths, advances only the oldest ready prefix, records structural stalls, keeps younger ready work blocked behind the first older stall, validates branch redirect sequence and stage ownership before recording younger pipeline flush evidence, records PC-only branch prediction resolution evidence with per-prediction repair targets plus correct-prediction, misprediction, and flushed-younger summary counters, applies cycle plans into the next in-flight pipeline state with retirement and flush removal, emits before/plan/after cycle records, compact cycle summaries, and disjoint-mergeable run summaries with overlap rejection, round-trips typed in-flight snapshots with a cycle cursor after prediction-driven flushes, duplicate-sequence restore rejection, and cycle-overflow rejection, and encodes external checkpoint payloads with decode-first validation of magic, version, widths, stage codes, payload size, and duplicate sequences. It also has typed O3 inter-stage unblock, dependency-release, issue-queue, and writeback-transfer policies that compensate for backward signal delay, defer commit-visible misc-register publication until architectural commit, keep distributed issue capacity keyed by queue plus OpClass, and report overfull IEW-to-commit completion sets as deferred work before any TimeBuffer-like index can exceed its declared future window. The writeback transfer path also has a stateful deferred-completion buffer that drains older overflow before new ready work on later cycles. rem6-traffic now has typed idle and exit control states plus initial typed linear, random, strided, DRAM-family, hybrid, GUPS, and trace-replay traffic generators that emit memory requests with gem5-style write payload bytes, model random byte-sampled absolute block alignment, fixed-period random sampling, strided offset and superblock cursor jumps, DRAM/NVM bank/rank/page-or-buffer address geometry, DRAM_ROTATE bank/rank/type rotation, gem5 packet-trace framing plus gzip-wrapper decoding for `ReadReq`, `ReadCleanReq`, `ReadSharedReq`, `ReadExReq`, `WriteReq`, `SwapReq` atomic read-write packets, `LoadLockedReq` and `StoreCondReq` as typed LLSC trace packets with backing-store reservation checks, `StoreCondFailReq` as an explicit forced-fail store-conditional packet, `LockedRMWReadReq` and `LockedRMWWriteReq` as native locked-RMW packets, cache-line-aligned line-sized `WriteLineReq`, cache-line-aligned line-sized `WritebackDirty` or `WritebackClean`, cache-line-aligned line-sized `WriteClean` non-evicting clean writes, cache-line-aligned line-sized `CleanEvict`, `CleanSharedReq`, `CleanInvalidReq`, `InvalidateReq`, and `UpgradeReq` maintenance packets, `HTMReq` and `HTMAbort` typed trace events, `PrintReq` typed diagnostic trace events, `FlushReq` typed cache trace events, gem5 ordinary response and error response typed trace events plus controller-level response/error matching against pending memory, sync, and HTM replay sources, typed mapping for gem5 `ARCH_BITS`, `UNCACHEABLE`, standalone `STRICT_ORDER`, `ACQUIRE`, `ACQUIRE_PC`, `RELEASE`, `PRIVILEGED`, `SECURE`, `PT_WALK`, `EVICT_NEXT`, `KERNEL`, and matching operation request flags with unsupported flag-bit rejection, typed GUPS RandomAccess reads with response-driven XOR writes and transport response statistics, typed Markov transition graphs, typed gem5 traffic text declarations with implemented IDLE/EXIT/LINEAR/RANDOM/STRIDED/DRAM/DRAM_ROTATE/NVM/HYBRID/GUPS plus resolver-backed TRACE binding, implemented-leaf graph binding, zero data limit as unlimited, non-elastic retry-delay timing, and checked counter/snapshot state. gem5 simple, checker, fuller Minor, O3, KVM-style switching, plus full memory-controller-coupled traffic DRAM/NVM execution, broader response/error propagation through caches, memory controllers, and CPU models beyond the tested RISC-V fetch/data failure consumers, broader trace flag mapping, transport execution, QoS/probes, and remaining traffic tests need typed rem6 equivalents or explicit replacement models. |
| `src/dev` | 418 | `rem6-mmio`, `rem6-uart`, `rem6-timer`, `rem6-interrupt`, `rem6-gpu`, `rem6-accelerator`, `rem6-platform`, `rem6-pci`, `rem6-virtio`, `rem6-storage`, `rem6-net` | partial | UART, timer, interrupt, RISC-V CLINT/PLIC, RTC/timer/watchdog, PL011 UART, platform topology, DTB/initrd handoff, GPU/accelerator shells, PCI, VirtIO MMIO/PCI, block/console/RNG devices, storage image layers, SimpleDisk, IDE disk/controller/PCI paths, Ethernet links/switching, and SINIC MMIO/PCI/DMA/checkpoint paths are typed Rust device models with focused crates and decode-first checkpoint validation. Storage and SINIC can be registered through RISC-V topology/system paths, so they are no longer purely future board work. PS/2, QEMU bridge, non-RISC-V device trees, real TUN/TAP, and broader platform-specific devices remain alignment targets. |
...
| `configs/dram`, `configs/nvm` | 5 | `rem6-dram`, `rem6-memory` | partial | External DDR, HBM, LPDDR, and NVM profiles have typed topology, stable technology and topology-dimension labels, geometry, bank-group geometry, timing, burst spacing, same-bank-group burst spacing, command-window bandwidth limits, low-power idle-window timing, parallel port counts, topology-unit counts, scheduler bank counts, topology bank counts, bank-group capacity summaries, manifest identity, checkpoint encoding, restore-time profile validation, and activity metadata. Runtime activity profiles carry profiled-target counts, the selected technology label when the aggregate is not mixed, profile geometry constants, profile timing constants, profile command-window constants, profile low-power timing constants, optional NVM media timing constants, profile-derived port, topology-unit, bank, and bank-group capacity denominators separately from active counts, and NVM persistent-write plus pending-queue counters when the selected profile uses persistent media. Parallel resource summaries preserve the same labels without reopening the original topology. DRAM same-arrival QoS timing batches can consume proportional-fair assigned priorities, respect same-agent memory-ordering barriers before priority or turnaround selection, current-direction turnaround can enforce a bounded same-direction burst while refusing to switch to an empty or ordering-blocked direction, and highest-priority opposite-on-tie turnaround can select the opposite read/write direction only when the highest-priority batch has both directions available. Low-power activity summaries distinguish open-row active powerdown from closed-bank precharge powerdown and self-refresh, use per-state exit latency, and can project terminal idle windows for previously active banks to an explicit end cycle without inventing a following access or counting untouched banks. Topology CPU and DMA runs feed their final ticks into that projection so full-system summaries retain terminal low-power evidence. DRAM checkpoint payloads write a split-exit low-power timing tag while still decoding the older shared-exit tag; system and workload summaries preserve active, precharge, and self-refresh counters as typed resource activity evidence, and workload manifests can require minimum low-power evidence during replay verification. Workload resource summaries preserve the strongest explicit DRAM target, port, or bank active-resource lower bound. NVM media timing can model separate read-media, write-media, send latency, pending-read buffers, and pending-write queue depth. Profile breadth and fuller media behavior remain open. |
...
| `src/cpu/testers`, `src/cpu/trace`, `src/cpu/probes` | `rem6-traffic`, tests, trace, stats crates | partial | `rem6-traffic` has typed idle and exit control states plus initial linear, random, strided, DRAM-family, hybrid, GUPS, and trace-backed memory traffic generators matching gem5's no-packet idle, typed exit-on-entry, read/write selection, requestor-id write payload bytes, fixed-period RNG consumption before mixed read/write selection, data-limit stop including zero-as-unlimited, retry-delay compensation for non-elastic request injection, typed overflow-checked summary counters, and deterministic snapshot restore. Linear traffic keeps full-block range validation and cursor restore; random traffic keeps gem5's byte-sampled absolute block alignment with gem5-valid unaligned or partial ranges; strided traffic keeps gem5's offset start, superblock/stride cursor jump, post-jump wrap, shape validation, and gap-rejecting snapshot restore. DRAM-family traffic keeps DRAM, DRAM_ROTATE, and NVM text geometry as typed config, validates supported address mappings, bank/rank/page-or-buffer geometry, sequence length, and rotate read percentages, emits page-contained read/write series with snapshots and summary accounting, and models DRAM_ROTATE bank/rank/type rotation separately from randomized DRAM/NVM series selection. GUPS traffic keeps controller-bound text configuration, fixed 8-byte RandomAccess table elements, default four-updates-per-entry targets, optional update limits, random-index reads, response-driven XOR writes, deterministic snapshots, typed completion diagnostics, a static-latency system transport runner that feeds completed read responses back into controller state before emitting XOR writes on the same route, and a standalone CLI artifact that drives the runner against a backing store while exposing response stats, transport counters, and memory dumps. Trace-backed traffic keeps gem5 packet-trace magic, gzip-wrapper decoding, and varint32 record framing, header tick-frequency validation, header object-id, version, and id-string metadata preservation, optional packet `pkt_id` and `pc` metadata preservation, `ReadReq`, line-shaped `ReadCleanReq`, and `ReadSharedReq` to shared read, line-shaped `ReadExReq` to unique read, `WriteReq` to write, `SwapReq` to typed atomic read-write swap, `LoadLockedReq` to typed load-locked with backing-store reservation tracking, `StoreCondReq` to typed store-conditional with failed-response status when the reservation check fails, `StoreCondFailReq` to an explicit forced-fail store-conditional operation without backing-data mutation, `LockedRMWReadReq` and `LockedRMWWriteReq` to native locked-RMW operations, cache-line-aligned line-sized `WriteLineReq` to write, cache-line-aligned line-sized `WritebackDirty` or `WritebackClean` to native writeback mapping, cache-line-aligned line-sized `WriteClean` non-evicting clean writes, and cache-line-aligned line-sized `CleanEvict`, `CleanSharedReq`, `CleanInvalidReq`, `InvalidateReq`, and `UpgradeReq` maintenance mappings, `HTMReq` and `HTMAbort` typed trace events, `PrintReq` typed diagnostic trace events, `FlushReq` typed cache trace events, absent-or-zero flags acceptance, typed `ARCH_BITS`, `UNCACHEABLE`, standalone `STRICT_ORDER`, `ACQUIRE`, `ACQUIRE_PC`, `RELEASE`, `PRIVILEGED`, `SECURE`, `PT_WALK`, `EVICT_NEXT`, and `KERNEL` sync flag mapping, typed non-prefetch `INST_FETCH` mapping on `ReadReq`, `ReadCleanReq`, and `ReadSharedReq` packets to instruction-fetch requests, `PHYSICAL` trace-address metadata acceptance, typed `PREFETCH` and `PF_EXCLUSIVE` mapping to native read and writable prefetch requests with prefetch precedence on combined read-family flags, matching-packet `LLSC`, `LOCKED_RMW`, and `MEM_SWAP` flag acceptance, `MEM_SWAP_COND` mapping to native compare-swap with an explicit zero compare operand because gem5 packet traces omit `Request::extraData`, matching-packet `ATOMIC_RETURN_OP` and `ATOMIC_NO_RETURN_OP` acceptance on `SwapReq`, conflicting return/no-return flag rejection, `MemoryOperation::AtomicNoReturn` emission, no-return swap response/error matching, unsupported flag-bit rejection, unsupported non-fetch `INST_FETCH` rejection, unsupported write-only non-exclusive `PREFETCH` rejection, unsupported mismatched operation flag rejection, entry-relative packet ticks, default non-elastic current-tick clamping, optional elastic delay, cursor snapshots, and reset-on-exit behavior. The typed traffic state machine keeps gem5-style initial-state entry, duration-driven transition ticks, Markov row selection, deterministic snapshots, and exact row-sum validation while rejecting duplicate states, duplicate transitions, unknown references, and invalid probabilities before runtime. The text config parser accepts gem5 `STATE`, `INIT`, and `TRANSITION` records as typed declarations, validates sparse state ids, malformed fields, duplicate rows, and probability precision before graph construction, and instantiates implemented IDLE, EXIT, LINEAR, RANDOM, STRIDED, DRAM, DRAM_ROTATE, NVM, HYBRID, GUPS, and resolver-backed TRACE modes through explicit binding options, including fixed or random stream/substream tagging for generated memory requests. The traffic controller binds graph states to implemented leaf generators, emits typed request, transition, exit, trace-exit, trace response/error match events, and route-scoped HTM begin and abort outcome records for RISC-V data-route events, keeps same-tick transition priority, applies request stream metadata, and snapshots machine, leaf state, and pending trace response sources. Traffic shared event, kind, and summary types are split out so additional trace features can grow without turning existing generators into a monolith. Full memory-controller-coupled DRAM/NVM execution, fabric-backed GUPS transport, full non-memory trace command execution, broader trace flag mapping, transport execution, traffic QoS, probes, and the remaining gem5 traffic tests remain open and should feed typed events and summaries. |
...
Trace kernel-sync note: packet-trace support now maps gem5 `KERNEL` request
flags to native `kernel_sync` memory-request attributes. The metadata survives
...
Trace store-conditional-fail note: packet-trace support now distinguishes gem5
`StoreCondReq`, which can succeed when a matching reservation exists, from
`StoreCondFailReq`, which rem6 represents as an explicit forced-fail
store-conditional request that never mutates backing data.
...
transactional-memory execution exists yet. rem6-cpu now has the CPU-local HTM
checkpoint and abort-restore contract that workload trace HTMReq response
matches and HTM abort sidebands bind to through declared RISC-V data routes.
`PrintReq` packet-trace commands now
...
| `src/mem/qos` | `rem6-fabric`, `rem6-dram`, `rem6-transport`, `rem6-workload` | partial | rem6-fabric has typed QoS requestor IDs, checked priorities, fixed-priority assignment, proportional-fair score policy with weight and requestor-count validation plus snapshot restore, FIFO/LIFO/LRG queue arbitration, non-mutating empty polls, queue-arbiter snapshots, and QoS-ordered fabric batch transmission that reserves shared links in grant order. rem6-transport can attach a shared QoS arbiter to parallel batch submission so request priority and requestor identity affect first-hop NoC reservation before partition events are scheduled, can order single- and multi-hop direct same-tick target deliveries with the same typed arbiter before invoking target handlers, respects same-agent memory-ordering barriers and same-agent strict-order requests when direct QoS batches or shared-fabric first-hop reservations choose eligible requests, exposes a typed `TransportQosClass`, and lets cache-originated transactions override QoS requestor separately from the downstream request's cache-agent identity. rem6-coherence can now export MSI bank scheduled misses as typed downstream transport requests, preserving effective MSHR QoS through `TransportQosClass` so same-tick cache-originated memory requests can be batched and ordered by transport QoS without Packet sender-state inheritance. rem6-dram can order same-arrival timing batches through the same typed arbiter before bank, row, and bus timing are computed, filters same-agent acquire/release memory-ordering barriers before QoS priority or turnaround selection, prefers the current read/write bus direction among same-priority candidates, implements gem5 source-compatible highest-priority opposite-on-tie turnaround for read/write ties, explicitly escalates queued same-requestor candidates to their best assigned batch priority without embedding controller back pointers in the queue policy, accepts memory-controller QoS batches before storage responses are generated, pairs responses with scheduled DRAM grant order, and preserves assigned priority, effective priority, requestor, byte count, and escalation status as typed DRAM activity metadata. Parallel coherence, system, DMA, and workload-result summaries expose DRAM QoS access, byte, escalation, priority, and requestor diagnostics directly from typed activity profiles. Workload manifests declare fixed-priority or proportional-fair QoS policy, queue policy, turnaround policy, priority escalation, per-requestor fixed-priority intent, and per-requestor proportional-fair initial scores as typed replay-plan state; workload replay applies declared fixed-priority or proportional-fair priority state plus queue policy to shared fabric first-hop reservation, applies declared fixed-priority or proportional-fair priority state, queue, turnaround including highest-priority opposite-on-tie, and escalation policy to direct profiled DRAM accesses so replay summaries carry DRAM priority and requestor metadata, lets same-tick single- and multi-hop direct DRAM deliveries observe manifest QoS before target handling, coalesces same-tick direct QoS deliveries to the same profiled DRAM target into one memory-controller batch, and keeps that batch path active when a data-cache exists by operation-filtering cache-covered data deliveries before batching the remaining DRAM requests. This preserves gem5's fixed-priority, proportional-fair, queue-policy, turnaround, escalation, and bandwidth-accounting concepts while avoiding global requestor lookup, memory-controller back pointers, SimObject-name-only setup, and string-only stats. Broader cache/DRAM QoS policy integration remains open. |
| `src/mem/probes` | `rem6-stats`, runtime summaries | partial | Observability uses typed counters, typed probe points/listeners/events, and run summaries rather than string-only probes. `rem6-stats` now covers gem5-style PC-count and instruction-threshold trackers, memory footprint probes, memory trace packet records, stack-distance probes with explicit read/write access classification, line-aligned reuse distance, infinite-distance counts, exact typed sparse linear/log bucket snapshots, and probe-event filtering, plus communication-monitor request/response statistics, latency and inter-transaction buckets, address masks, outstanding request buckets, periodic bytes-per-second bandwidth samples, cumulative read/write byte totals, and restore-time pending/outstanding validation. Stats-enabled RISC-V system runs now emit real retired-instruction probe events from configured `RiscvInstructionStats` CPUs when `InstructionExecuted` turns are observed, real data-access request probe events from issued memory and MMIO `RiscvDataAccessEvent`s for stack-distance, optional memory-trace, optional memory-footprint, and communication-monitor request statistics, real data-access response probe events from completed memory-route and MMIO loads/stores for communication-monitor read bandwidth/latency and write latency while write bytes remain request-side, and real memory-route retry response probe events for `MemCheckerMonitor` pending-retention state. Optional request/response probe events also drive `MemCheckerMonitor` from real memory-route and MMIO load/store completion data, plus memory-route store-conditional completion data. Atomic read-modify-write checker data, functional reset producers, and error paths still need runtime producers before those tracker capabilities are system-driven. The run carries the probe registries plus tracker snapshots in `RiscvSystemRun`. It preserves the modeled behavior without importing gem5's string statistics backend or compressed histogram storage format. |
| memory ports, packets, requests in `src/mem` root | `rem6-transport`, `rem6-memory`, `rem6-stats` | partial | Shared request/response transport exists, and `MemoryRequest` now has a stable checkpoint payload preserving operation, identity, address, size, line layout, data, byte masks, atomic operation, independent uncacheable and strict-order flags, and explicit before/after ordering edges. `MemoryResponse` also has a stable checkpoint payload preserving request identity, status, and optional response data without pulling response state into an untyped packet container. Native atomic swaps now cover both return-data and no-return request forms through line-store execution, coherence backing-store mutation, cache-controller hit paths, MSI/MESI/MOESI/CHI directory ownership and snoop decisions, DRAM access classification, and trace response/error matching. Typed communication covers gem5 `CommMonitor` request/response statistics; typed `MemChecker` and `MemCheckerMonitor` models cover byte-level read/write lifetime validation, successful timing-request gating, response retry retention, failed store-conditional aborts, functional reset handling, packet-id/serial snapshots, reset-aware late completions, and typed invariant errors without packet sender-state ownership, warnings, or panic-only monitor side effects. RISC-V data-access stats can drive probe events, communication-monitor accounting, and `MemCheckerMonitor` from real memory-route and MMIO load/store requests and responses, memory-route store-conditional requests and responses, and memory-route retry responses; atomic read-modify-write checker data, functional reset producers, and error producers remain open. More gem5 packet semantics need mapping as features are added. |

Cache maintenance note: MSI, MESI, MOESI, and CHI cache banks forward direct
native `CleanShared` and `Invalidate` requests downstream before ordinary CPU
...
and page-span drops that do not have a translation path. The default full-queue
policy evicts the lowest-priority oldest pending entry before accepting a new
entry, matching gem5's queued insertion path. A configured
...
| gem5 source anchor | rem6 owner | Coverage | Notes |
| --- | --- | --- | --- |
| `configs/dram`, `ext/drampower`, `ext/dramsim2`, `ext/dramsim3`, `ext/dramsys` | `rem6-dram`, adapter crates | partial | rem6 has internal DRAM timing, burst spacing, same-bank-group burst spacing, command-window bandwidth limits, low-power idle-window timing, active-powerdown versus closed-bank low-power state accounting, split low-power exit latency, terminal idle-window projection, bank-group geometry, activity, and profiles. DRAM snapshot restore rejects profile target, line-layout, geometry, timing, parallel-port, or NVM media-timing drift before rebuilt controller state can expose stale profile evidence, while boxing the large mismatch payload so rich diagnostics do not bloat every `Result` error path. External DRAM simulators should be optional adapters. |
| `configs/nvm`, `src/mem/NVMInterface.py`, `src/mem/nvm_interface.*`, memory profile code | `rem6-memory`, `rem6-dram` | partial | NVM targets have typed controller/media-bank topology and can round-trip through manifests, checkpoints, and DRAM target activity metadata. DRAM activity profiles preserve typed read/write byte counts, and NVM target activity exposes persistent write access, byte counters, max pending NVM reads, max pending persistent writes, profile-level media timing, access-level persistent-ready cycles, checkpointed pending read/write completions, NVM read-buffer/write-queue wait-for diagnostics, manifest identity for NVM media timing, and restore-time rejection of profile media-timing drift without string stats. Standalone CLI NVM runs now expose persistent write counts, persistent write bytes, max pending NVM reads, and max pending persistent writes in artifact JSON and hierarchical stats. Richer NVM-specific bandwidth behavior remains open. |
| HBM, LPDDR, DDR class profiles | `rem6-dram` | partial | The profile shape exists for DDR, HBM, LPDDR, and NVM, each technology exposes stable lowercase labels for manifests, stats, and artifacts, topology dimensions name their parallel ports and topology units, and checkpoint restore validates that profile metadata still matches the target, memory layout, controller geometry, timing, and parallel-port shape. Standalone CLI RISC-V execution can select DDR, HBM, LPDDR, or NVM for the typed DRAM-backed memory path and reports the selected technology plus profile capacity in artifact JSON and hierarchical stats. A broader library of validated profiles is still needed. |
...
| `src/dev/storage/simple_disk.*` | `rem6-storage` | partial | rem6 keeps gem5's console-facing sector copy value while replacing raw `System::physProxy` writes, heap-buffer lifetime management, panic-only validation, and the unimplemented write path with `SimpleDisk`, `SimpleDiskGuestMemory`, typed transfer records, sector-shape validation, storage-range preflight, complete-payload staging, explicit flush, and typed guest-memory or storage errors. Board-level declarations that combine SimpleDisk with a concrete boot platform remain open. |
...
| standalone simulator entry point and output artifacts | `rem6` | partial | The `rem6` binary crate now provides an initial tested `run` command for ELF-backed inputs. It validates requested ISA against ELF architecture, rejects mismatches before producing stats, emits a single JSON artifact with schema, input identity, ELF metadata, load status, structured `parallel.scheduler` evidence, structured fetch/data transport evidence, optional DRAM memory-controller evidence, and hierarchical typed stats, and can run RISC-V ELF inputs with `--execute --cores <n>` through real per-core fetch/data endpoints, per-core partitions, the memory transport, and the parallel system driver until a guest trap reaches the host, the configured `--max-tick` budget stops the run, or an explicit `--max-instructions <count>` budget stops committed-instruction execution first. `rem6 run`, `rem6 gups`, and `rem6 trace-replay` can now load their existing runtime fields from checked TOML tables through `--config <path>`, with explicit CLI flags overriding file defaults and relative file paths resolving from the config directory for repeatable scripted experiments. RISC-V CLI runs expose distinct `mhartid` values per core, so a shared ELF can branch by hart while still using the same parallel execution path. Host-trap stop records carry the trap name and trap-origin PC separately from the current architectural core PC. `--start-address <addr>` can override the effective reset PC while the artifact still preserves the original ELF entry, and both loaded and executed artifacts plus CLI stats report the effective start address so bootloader, kernel handoff, and later platform flows do not have to conflate image metadata with runtime reset state. `--riscv-boot-a0 <value>` and `--riscv-boot-a1 <value>` seed the RISC-V boot argument registers, are applied before execution, and are recorded in both artifacts and stats so full-system handoff values are explicit runtime inputs rather than hidden platform-side register initialization. `--load-blob <addr>:<path>` reads an external binary blob, maps it into the CLI memory image after ELF load, preserves adjacent cache-line contents, and records blob address, byte count, and source path in artifacts plus numeric blob stats so DTB, initrd, and firmware handoff inputs are explicit rather than side effects. `--dram-memory` routes CLI fetch and data accesses through the typed DRAM memory controller using the loaded memory image, returning responses after DRAM ready cycles instead of treating external memory as a zero-timing line store; `--dram-memory-profile <ddr|hbm|lpddr|nvm>` selects the external profile used by that path, and the JSON artifact plus stats expose selected technology, topology labels, profile geometry constants, profile timing constants, profile command-window constants, profile low-power timing constants, NVM media timing constants, active DRAM target, port, bank, access, read/write, row-hit, row-miss, command, turnaround, ready-latency, profiled-capacity, technology one-hot, NVM persistent-write and pending-queue counters, and low-power counters. The CLI also accepts `--min-remote-delay <ticks>` as a scheduler lookahead runtime option and records the value in both the structured scheduler artifact and hierarchical stats. `--memory-route-delay <ticks>` separately controls the simple CLI memory-route request and response latency floor, must be at least the configured scheduler lookahead, and is emitted in the simulation artifact and hierarchical stats so transport timing is explicit instead of inferred from trace deltas alone. `--host-event-delay <ticks>` separately controls the guest-trap host-event delivery latency floor, defaults to the scheduler lookahead, must be at least the configured scheduler lookahead, and is emitted in the simulation artifact and hierarchical stats so trap delivery timing is explicit rather than tied implicitly to scheduler lookahead. The main artifact and CLI stats now include the configured parallel worker limit plus recorded parallel scheduler dispatch, batch, total-worker, active-partition, remote-send, worker-tick, capacity, idle-worker, per-worker-slot active/idle ticks, actual worker-lane partition active ticks, scheduler ready partitions, indexed initial and final partition frontiers, and per-partition worker, dispatch, remote-send, remote-receive, and pending-event evidence rather than only the coarse epoch and max-worker counters. The main artifact also exposes fetch/data transport totals and per-route source counters, while CLI stats include per-core and aggregate data load/store/atomic bytes, aggregate and route/source fetch/data memory transport request and response counts, round-trip tick totals, maximums, the configured instruction limit, and a typed stop-reason counter. Requested `--dump-memory <addr>:<bytes>` ranges are read from the post-run memory image and emitted as hex in the same artifact, `--output <path>` writes the selected run output format to a file while stdout reports the artifact path, and `--stats-output <path>` writes the selected stats payload as a separate file for direct stats consumers; when both output paths are requested, stdout reports both durable artifact paths and the selected format in one machine-readable envelope. The selected stats output can be the typed JSON array or a gem5-style text statistics block with begin/end markers plus row-level unit and reset-policy metadata. The `trace-replay` command reads a gem5 packet trace, constructs a typed workload manifest and resource payload, executes the trace replay controller through workload replay, and emits an artifact plus append-only stats counters for scheduled trace requests, matched responses, memory failures, typed control acknowledgements and failures, HTM request acknowledgements, and trace-only sideband events. Platform construction, checkpoint options, topology composition config, and broader ISA execution remain open. |
...
| `ext/nomali`, `ext/mcpat`, `ext/dsent` | `rem6-power`, future optional analysis adapters | external-adapter | Preserve modeling value behind typed records and stable APIs. rem6-power now emits validated external power-analysis records without linking the external tools into the simulator core. |
| `ext/libelf`, `ext/libfdt`, `ext/softfloat`, `ext/gdbremote` | `rem6-boot`, `rem6-platform`, `rem6-debug`, future ISA support crates | partial | rem6-boot has typed ELF32 and ELF64 little-endian and big-endian loaders plus an auto-detecting ELF entry point that validates the ELF identity and program-header shape, records typed ELF class, endian, machine, architecture, OS ABI, flags, and operating-system metadata comparable to gem5 `ElfObject::determineArch()` and `ElfObject::determineOpSys()`, including PPC64 ABI v1/v2 selection from ELF flags, endian-sensitive PPC64 default ABI selection when flags omit the ABI, Linux/Solaris/FreeBSD fallback from `.note.ABI-tag`, and Solaris fallback from `.SUNW_version` or `.stab.index` section names, imports physical-address `PT_LOAD` segments into `BootImage`, skips non-loadable segments, preserves `p_memsz > p_filesz` zero-fill, and rejects unsupported class, encoding, version, or overflowing segment memory ranges with typed errors without libelf or unsafe parser state. Its source boundary keeps ELF parsing, boot-image loading, and typed boot errors in focused modules so loader breadth does not grow into a gem5-style untyped utility layer. Workload boot images preserve that ELF metadata through manifest round trips and include it in manifest identity, so ISA or ABI selection cannot silently drift when two images have the same entry and loadable segments but different ELF endian, machine, OS ABI, or flags headers. rem6-platform has an initial typed DTS source tree and deterministic binary FDT/DTB writer for RISC-V platform boot descriptions, including root memory nodes plus Linux `/chosen` bootargs and initrd start/end metadata, and rem6-system installs initrd bytes plus generated or resolved-resource DTBs into guest memory with the RISC-V A0 hart-id and A1 register handoff for both store-backed and DRAM-backed memory. rem6-debug now owns GDB remote packet framing, checksum validation, acknowledgement and interrupt frame parsing, notification packets, command-safe packet parsing, run-length encoded response data, legacy sequence-id rejection, `qAttached[:pid]` attach-state queries, attached-or-created replies, `qC` current-thread queries, `QC thread-id` replies, `qfThreadInfo` and `qsThreadInfo` active-thread list queries, payload-bounded thread-list chunk replies, thread-list end replies, `qSupported` feature parsing, `qSymbol` no-symbol-needed replies, `qRcmd` monitor-command decoding with typed records, `.` page-table dump replies from externally supplied page-table payloads, `qXfer:features:read:annex:offset,length` target-description reads with advertised-support gating, annex-backed XML chunks, final-chunk markers, malformed-request preservation, missing-annex errors, and offset bounds checks, unsupported-command empty responses, `?` stop-reason queries, typed stop replies, `g` register-read requests, `G XX...` all-register write requests, all-register write `OK` responses, basic `H op thread-id` thread-selection requests, thread-selection `OK` responses, basic `T thread-id` thread-alive requests for `0`, `-1`, and positive single-process IDs, alive-or-missing thread responses, legacy `c [addr]`, `C sig[;addr]`, `s [addr]`, and `S sig[;addr]` resume requests, typed resume-request records without immediate stop replies, `vContSupported`-gated basic `vCont?` resume-action queries, `vCont;c/C/s/S` action packets, ordered typed resume-action records, `D[;pid]` detach requests, `k` terminate requests without response packets, `vKill;pid` terminate-process requests, typed disconnect records with post-disconnect packet suppression, basic `Z/z type,addr,kind` breakpoint and watchpoint requests, idempotent typed trap records, `p n` single-register read requests, target-order register-byte hex responses, unavailable-register `x` responses, missing-register error responses, `P n...=r...` single-register write requests, register-write `OK` responses, `m addr,length` memory-read requests, memory hex responses, response-payload-capped oversized memory-read errors, missing-memory error responses, `M addr,length:XX...` memory-write requests, all-or-error memory-write responses, no-ack request parsing with advertised-support gating, packet-level no-ack transition state, last-response retransmission on negative acknowledgement, session-level frame dispatch, interrupt request state, bounded payload configuration, and skipped-prefix reporting as typed data before architecture-specific register caches are added. rem6 still needs broader ISA loader coverage, kernel image loader breadth, bootloader handoff coverage, soft-float, and ISA register-cache/debug-session capability without vendoring unneeded code into the core. |
...
- Memory translation page-map tests cover offsets, permissions, cross-page
  ...
  stable checkpoint golden bytes, invalid page-map checkpoint magic, version,
  reserved-field, payload-size, invalid-permission, zero-page-count, and
...
- Memory translation TLB tests cover public gem5 issue #3010 by preserving
  global entries during non-global ASID and ASID-plus-page invalidations,
  removing matching non-global entries, preserving other ASIDs, snapshotting
  entry scope, and rejecting malformed restore/checkpoint payloads.
- Memory translation queue tests cover pending-request ready ordering, snapshot
  ...
  stable checkpoint golden bytes, invalid queue checkpoint magic, version,
  reserved-field, payload-size, access-code, access-size, duplicate-request,
  and forged-ready-tick rejection through snapshot decode validation, so a
  malformed checkpoint cannot reuse an existing stable ordering value,
  duplicate pending request identity, or forge completion timing.
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  writes, SINIC register/FIFO checkpoint chunks in workload replay checkpoint
  manifest summaries, and replay-level SINIC checkpoint restore of both
  register state, FIFO-owned RX descriptor state, and read-clear soft-interrupt
  ...
  register execution, and packed RV64C integer, load/store, and control-flow
  decompression with 2-byte PC advance through the current CPU fetch path.
  Initial RV64F/RV64D tests cover floating-point register state, `FLW`/`FSW`
  and `FLD`/`FSD` decode and memory access records, RNE
  `FADD.S`/`FSUB.S`/`FMUL.S`/`FDIV.S` and
  `FADD.D`/`FSUB.D`/`FMUL.D`/`FDIV.D` execution, RNE fused
  multiply-add/subtract execution with signaling-NaN invalid flag accrual,
  RNE `FSQRT.S` and `FSQRT.D` with invalid flag accrual, sign injection,
  comparison NaN invalid flag accrual, min/max, classification, raw integer/float moves,
  single-precision NaN-boxing, RNE integer-to-double and double-to-integer
  conversion execution, RNE integer-to-single and single-to-integer conversion
  execution, static and dynamic non-RNE float-to-integer conversion rounding,
  NaN/out-of-range invalid and valid rounded-result inexact flag accrual for
  float-to-integer conversions, RNE
  single-double conversion execution, `fflags`/`frm`/`fcsr` CSR read/write access with real
  `FDIV.S` divide-by-zero flag accumulation, raw-bit floating-point write records,
  integer comparison and misc write records, and CPU data-response writeback into
  floating-point registers. RISC-V core checkpoint tests cover floating-point
  register chunk capture and restore, including legacy checkpoint restore that
  clears live floating-point registers when the chunk is absent.
  RISC-V ISA
  tests cover `MUL`/`MULH`/`MULHSU`/`MULHU`/`DIV`/`DIVU`/`REM`/`REMU`
  ...
  execution, load-locked issue, matching-reservation store-conditional issue,
  ...
  address plus ASID system-event records, while RISC-V frontend tests cover
  all-scope `SFENCE.VMA` invalidation of the attached data translation TLB
  and ASID-separated data translation fills from the same virtual address,
  including ASID selection from the RISC-V `satp` CSR.
  ...
  derived data Sv39 access context, `mstatus`/`sstatus` reads, `sstatus`
  set/clear paths for the supervisor-visible bits, `mepc` read/write/set
  paths, `medeleg`/`mideleg` and `mtvec`/`mscratch`/`mcause`/`mtval`
  machine-trap CSR paths, `stvec`/`sscratch`/`sepc`/`scause`/`stval`
  supervisor-trap CSR paths, `mie`/`mip` and
  ...
  mapping. RISC-V ISA tests cover instruction, load, and store page-fault trap
  entry cause codes plus fault-address writes to `mtval` or delegated `stval`,
  and RISC-V frontend and cluster tests cover serial and parallel data-translation
  load page faults plus serial store page faults entering guest trap events.
  RISC-V frontend tests also cover
  hart-derived request-carried Sv39 access context for data translation,
  including MPRV/MPP-derived data privilege,
  ...
  default empty ordering and explicit before/after read-write ordering, and
  RISC-V frontend tests cover aq/rl ordering propagation into parallel transport
  atomic requests. RISC-V ISA tests cover typed `mcycle` and `minstret` machine
  writes, executable `cycle`, `time`, and `instret` CSR reads from hart state, user read
  aliases, user write
  rejection, read-only `CSRRS`/`CSRRC` zero-source and `CSRRSI`/`CSRRCI`
  zero-immediate aliases for `cycle`, `time`, `instret`, and `mhartid`, wrapping
  ...
  execute-time CSR address privilege checks for implemented Machine,
  Supervisor, and user CSR addresses, lower-privilege illegal-instruction trap
  routing through synchronous trap/delegation, and counter snapshot restore.
  Dedicated CSR counter tests cover RV32 low/high word address
  decoding, high-half user read aliases including `timeh`, machine low/high word
  writes that preserve the other half, and read-only errors for user high-half writes.
  RISC-V GDB target-description tests cover RV32 and RV64 target annex lists,
  `target.xml` architecture and CPU include records, stable ABI register order
  for 32 general registers plus `pc`, XLEN-scaled register bitsizes, unique
  annex names for the debug Xfer registry, explicit omission of the FPU annex,
  and the RV64 supervisor CSR annex for the listed status and trap CSR state.
  ...
  for atomic metadata, byte masks, independent uncacheable and strict-order
  flags, explicit ordering-edge presence, completed-response data, and retry
  responses, plus
  golden byte layouts for request and response payloads, invalid request
  operation-code rejection, invalid response status-code rejection, magic,
  version, reserved-field, and payload-size rejection, invalid response
  data-length rejection, and malformed mask-bit rejection.
...
- GPU and accelerator paths route command and DMA work through typed topology,
  ...
  metadata plus independent uncacheable and strict-order flags and typed
  request attributes, including `EVICT_NEXT` and `KERNEL`, plus native `CacheBlockZero` and `NoAccess` request operations
...
- Cache replacement-policy, indexing, and replacement-directory tests cover
  gem5-style LRU/FIFO/MRU/Random/LFU plus WeightedLRU occupancy and touch-order
  victim selection, Random draw-before-invalid priority and snapshot replay,
  ...
  SHiP signature-based insert/access, set-associative Random sector victim
  replay, and skewed-indexed sector residency with cross-set LRU, BIP, and
  second-chance replacement checks. Compressed-tag tests cover invalid gem5
  superblock shapes, capacity-gated co-allocation, uncompressed-superblock
  replacement, partial versus final invalidation, and snapshot restore of
  replacement state, duplicate-superblock avoidance, set-associative Random
  superblock victim replay, skewed-indexed cross-set LRU replacement, live
  rounded-up compression-factor snapshot restore, and corrupt compressed
  snapshot rejection for over-capacity superblocks, empty superblock metadata,
  and oversized compressed sub-blocks.
  MSI, MESI, MOESI, and CHI cache-bank compressed-tag tests cover explicit
  compressed-size co-allocation, default uncompressed fill behavior,
  whole-superblock clean eviction, dirty victim rollback, sibling-preserving
  upgrade fills, snapshot restore, and mode mismatch rejection.
  MSI, MESI, MOESI, and CHI cache-bank sector-tag tests cover same-sector
  sub-block fills and hits, whole-sector clean eviction, dirty and transient
  victim rejection with tag rollback, snapshot LRU-order restore, and
  tag-backend mode mismatch rejection.
- Cache MSHR, MSI/MESI/MOESI/CHI cache-bank, and MSI bank directory harness
  ...
  including original downstream request forwarding, response conversion without
  line installation, and replacement-directory resident-way cleanup. MSI, MESI,
  MOESI, and CHI replacement-directory tests also cover clean capacity
  eviction, hit recency updates, snapshot replay, modified or dirty
  capacity-victim rejection, and transient capacity-victim rejection before
  writeback replacement support exists. MSI, MESI, MOESI, and CHI tests also
  require dirty resident lines to reject this read bypass while preserving
  dirty snapshot evidence when no typed write queue is attached, and require
  direct uncacheable writes to enter the typed write queue without allocating
  MSHR entries, pending fills, or resident cache lines.
  Dirty-resident uncacheable-read tests require `accept_cpu_request` to
...
- Cache prefetch tests cover base access-observation filtering, tagged
  ...
  duplicate-attempt throttling, default full-queue eviction, and snapshot
  restore, cross-requestor queued duplicate filtering with post-translation
  duplicate-key preservation, snapshot restore, PIF
  ...
  replacement policy state, cache partitioning way and max-capacity filters,
  partition manager filter composition, partition-aware replacement-directory
  victim filtering, owner-preserving resident-line relocation, partition
  manager counter updates, partition-owned removal release, restored-manager
  counter rebuilding, plain-install owned-victim rejection, typed partitioning
  errors, staged failed-install rollback, empty filtered-candidate rejection,
  replacement directory sets and ways,
  cache-tag dueling sample assignment, hysteresis winner selection, TreePLRU one-way
  ...
  suppression, not-started translation-completion rejection, translation fault
  and redundant-drop completion, translation-completion ready latency, shared
  queued and stride prefetchers, PIF tables and buffers, IMP tables and address
  arrays, ISB training and mapping tables, and STeMS
  ...
  decrease with snapshot restore, Slim AMPM gem5 default parameters,
  DCPT-preferred candidate selection, AMPM fallback, source-tagged candidates,
  combined AMPM/DCPT snapshot restore, page-boundary candidate dropping, multi-source
  queued prefetch earliest-ready
  reporting, deterministic round-robin issue, no-ready polls that advance the
  gem5 scan-start chooser without issuing an entry, single-entry issue from the
  selected source, multi-source queued snapshot restore, and source-count or
  child-config mismatch rejection without partial mutation.
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  proportional-fair score ranking, weight and requestor-count constraints,
  typed configuration errors, snapshot replay, highest-priority queue
  selection, FIFO/LIFO same-priority ordering, LRG requestor rotation,
  non-mutating empty polls, and
  ...
  Workload manifest and replay tests cover proportional-fair QoS declarations,
  manifest identity hashing for proportional-fair weight and requestor scores,
  validation of proportional-fair weight, score, and requestor-count bounds,
  shared-fabric first-hop ordering from manifest proportional-fair state, and
  DRAM QoS activity summaries produced from manifest proportional-fair state.
  Workload data-cache replay also feeds manifest QoS policies into
  external-memory backed MSI, MESI, MOESI, and CHI data-cache backing DRAM
  runs, preserving per-priority and per-requestor activity evidence inside the
  recorded data-cache run summaries.
  ...
  opposite direction, highest-priority opposite-on-tie turnaround matching the
  gem5 source behavior when the highest-priority read and write queues are
  both nonempty, explicit same-requestor priority escalation inside timing
  batches, proportional-fair priority consumption, per-priority/per-requestor
  QoS access and byte accounting in DRAM activity profiles, gem5-like burst
  ...
  batching while a data-cache is present, data-cache backing DRAM QoS evidence
  for MSI, MESI, MOESI, and CHI cache protocols, and operation filtering so
...
- CLI tests cover the initial standalone `rem6 run` binary path: RISC-V ELF
  ...
  worker-limit stat and the resulting one-worker scheduler max. CLI tests cover
  loading `rem6 run` defaults from a `[run]` TOML config, resolving relative
  TOML paths from the config directory, and overriding those defaults with
  explicit CLI flags. They also cover ELF-backed RISC-V load/store execution
  through the data endpoint path
  ...
  a CLI error. TOML config tests cover `run`, `gups`, and `trace-replay`
  loading runtime fields through `--config`, CLI flag overrides, run blob-list
  replacement, and config-relative file paths for binary and trace inputs. CLI
  tests also cover `rem6 trace-replay` executing a gem5 packet
...
- Timer/MMIO tests cover typed RISC-V CLINT `msip` software interrupts,
  ...
  oversized hart counts, trailing bytes, and decode-first rejection that leaves
  live checkpoint state unchanged. CLINT reset tests cover `msip` clearing,
  ...
  callback can observe partial device state. They also cover `msip`, immediate
  `mtimecmp`, and RTC-driven timer assertion failures that reject remote
  delivery before changing the guest-visible CLINT register or asserted-line
  state.
...
- PLIC/MMIO tests cover PLIC-compatible 32-bit priority, pending, enable,
  ...
  context-specific MMIO claim routing for a second hart target. PLIC tests also
  cover context output-state visibility across enable, threshold, claim, and
  complete transitions. PLIC checkpoint tests cover context-local snapshot and
  restore of enable and threshold state, stable two-context checkpoint chunk
  bytes, truncated enabled-line records, trailing payload bytes, oversized
  context counts, and no-partial-restore rejection for malformed chunks, while
  system checkpoint tests cover PLIC bank decode-first restore and host
  checkpoint-action round trips through manifests. System
...
- PCI tests cover typed legacy INTx line mapping, invalid interrupt-pin and
  ...
  the hard module-size budget across CPU sources. RISC-V ISA source-policy
  tests keep target-description and error logic in focused modules while
  enforcing the hard module-size budget across ISA sources. Platform source-policy tests
...
- Platform tests cover typed RISC-V device-tree source emission for CPU
  ...
  UART compatible strings, UART interrupt-parent wiring, root memory nodes,
  DTS serialization, and deterministic binary FDT/DTB serialization with header
  offsets, reserve-map terminator, structure tokens, de-duplicated string table
  entries, and rejection of UART device-tree emission when no external
  interrupt controller is declared. Platform tests also cover Linux `/chosen`
  bootargs and initrd start/end metadata encoded as deterministic DTB
  properties.
- System topology tests cover typed RISC-V DTB handoff from the platform model
  into store-backed and DRAM-backed guest memory, per-core A0 hart-id setup,
  per-core A1 register setup for the DTB address, and preservation of Linux
  bootargs plus initrd metadata in the installed DTB. They also cover automatic
  host checkpoint-bank
...
- Power tests cover typed power state domains, leader/follower matching,
  ...
  and snapshot restore. External power-analysis export tests cover stable
  target sorting, duplicate-target rejection, state residency, temperature,
  per-target dynamic/static totals, aggregate power totals, and invalid
  temperature or power rejection.
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  malformed frame rejection, run-length response decoding, malformed
  run-length rejection, notification frame parsing, corrupted-notification
  skipping, legacy sequence-id rejection, command-payload literal star
  preservation, `qAttached[:pid]` attach-state query parsing,
  attached-or-created response emission, `qC` current-thread query parsing,
  `QC thread-id` response emission, `qfThreadInfo` and `qsThreadInfo`
  active-thread list parsing, payload-bounded thread-list response emission, `qSupported`
  feature parsing, stub feature response emission, `qSymbol` no-symbol-needed response emission,
  `qRcmd` monitor-command parsing and typed request recording,
  non-standard page-table dump request parsing and externally supplied dump response emission,
  `qXfer:features:read:annex:offset,length` target-description request
  parsing, malformed Xfer preservation, advertised-support gating, chunked
  Xfer response emission, missing-annex errors, and offset bounds errors,
  unsupported-command empty responses, `?` stop-reason query parsing, default
  stop reply emission, `g` register-read request parsing, `G XX...`
  all-register write parsing, all-register write `OK` responses, basic
  `H op thread-id` thread-selection parsing, thread-selection state
  recording, thread-selection `OK` responses, basic `T thread-id`
  thread-alive parsing for `0`, `-1`, and positive single-process IDs,
  alive-or-missing response emission, legacy `c [addr]`, `C sig[;addr]`,
  `s [addr]`, and `S sig[;addr]` resume parsing, malformed resume
  preservation, typed resume-request recording without immediate stop replies,
  `vContSupported`-gated `vCont?` resume-action query parsing,
  `vCont;c/C/s/S` action parsing, malformed `vCont` preservation,
  ordered resume-action recording,
  `D[;pid]`, `k`, and `vKill;pid` disconnect parsing, malformed disconnect
  preservation, typed disconnect recording, no-response terminate handling,
  post-disconnect packet suppression,
  `Z/z type,addr,kind` breakpoint and watchpoint parsing, malformed trap
  request preservation, idempotent typed trap recording,
  `p n` single-register request parsing, target-order register byte hex response
  emission, unavailable-register `x` responses, missing-register error
  responses, `P n...=r...` single-register write parsing, register-write
  `OK` responses, `m addr,length` memory-read request parsing, memory hex
  response emission, response-payload-capped oversized memory-read errors,
  missing-memory error responses, `M addr,length:XX...` memory-write request
  parsing, all-or-error memory-write responses, no-ack request parsing, no-ack
  session transitions, NACK retransmission, session frame dispatch, interrupt request
  recording, bounded payload enforcement, acknowledgement, negative
  acknowledgement, interrupt, and skipped-prefix stream parsing before socket
  or ISA register-cache integration.
- Workload manifests record boot images, resources, topology, host events,
  explicit expected stop reasons, checkpoint lineage, typed QoS policy intent,
  typed Linux boot handoff intent with device-tree and initrd resource
  validation, explicit required-resource payload resolution bound to manifest
  identity, resource acquisition provenance hashing, disk-image construction
  provenance hashing, RISC-V core
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  single/double integer conversions including static and dynamic non-RNE
  float-to-integer rounding, and single-double FP conversions.
  Complete exception flags beyond the current `FDIV.S`/`FDIV.D`,
  `FSQRT.S`/`FSQRT.D`, float-to-integer invalid/inexact conversions,
  `FMIN`/`FMAX`, comparison, and fused
  multiply-add/subtract NaN cases,
  non-RNE arithmetic and integer-to-float rounding, broader FP-to-FP conversion
  breadth, and the remaining F/D operations are still open. The current
  host-assisted mode has guest fd
  ...
  `set_tid_address` child-clear-TID state, robust-list set/get state with
  current-thread guest-buffer head and length writes, `mmap`/`munmap` region
  state with zeroed anonymous backing and opened registered guest-file contents
  installed through the configured guest-memory writer, gem5-style ignore-return
  advisory memory-management calls, ignored
  `rseq` returning `-ENOSYS`, `futex` wake/wake-bitset handling with empty
  bitset rejection, `FUTEX_WAIT`/`FUTEX_WAIT_BITSET` guest-word mismatch
  ...
  `ioctl` number 29 by reporting non-tty guest fds with `ENOTTY`,
  `lseek` number 62 by updating shared registered-file offsets for
  `SEEK_SET`, `SEEK_CUR`, and `SEEK_END`, `readv` number 65 by scattering
  registered-file or stdin bytes into RV64 `iovec` buffers, `writev` number 66
  by gathering RV64 `iovec` payloads into a typed guest write record,
  `prlimit64` number 261 by
  ...
  real data loads before exiting through the same syscall path. CLI and TOML
  ...
  before returning the selected child pid. It still lacks
  fork/clone process modeling, blocking futex wait, futex requeue, futex wake on
  child-clear-TID, shared mmap writeback, page-table installation and per-page
  protection for returned ranges, host filesystem path resolution,
  host-backed file contents
...
- Complete CHI-like coherence beyond the current line-state crate, single-line
  ...
  remaining cache/DRAM QoS policy integration.
- Broaden remaining device and platform coverage with PS/2, QEMU bridge,
  non-RISC-V device trees, real TUN/TAP integration, richer GPU runtime, fuller
  IDE board declarations, and broader platform-specific devices.
...
- Grow the manifest, workload resource model, and standalone configuration
  surface until gem5-style full-system experiments, topology sweeps, and
  workload suites are reproducible from data without editing and recompiling
  the simulator.
