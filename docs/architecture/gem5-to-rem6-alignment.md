...
- RISC-V PMA anchor refreshed on 2026-05-30: gem5's local `PMAChecker` keeps
  ...
  lines still reject uncacheable read bypass until read-side eviction/writeback
  support exists. gem5 also routes uncacheable writes through
  ...
  uncacheable bytes over the earlier full-line writeback bytes, while same-line
  writes and atomics are rejected as explicit write-queue conflicts instead of
  allocating a stale miss. When the uncacheable queue entry is issued, those
...
- Cache MSHR, MSI/MESI/MOESI/CHI cache-bank, and MSI bank directory harness
  ...
  read bypass while preserving dirty snapshot evidence, and require direct
  uncacheable writes to enter the typed write queue without allocating MSHR
  entries, pending fills, or resident cache lines. Dirty-resident
  ...
  clearing the resident line. They also require same-line cacheable reads to
  observe dirty writeback data overlaid with later uncacheable-write bytes, and
  same-line writes to stop at a typed write-queue conflict while those entries
  are pending. The same protocols now cover queued uncacheable write response
