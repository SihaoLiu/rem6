...
| HBM, LPDDR, DDR class profiles | `rem6-dram` | partial | The profile shape exists for DDR, HBM, LPDDR, and NVM, each technology and topology exposes a stable lowercase label for manifests, stats, and artifacts, and checkpoint restore validates that profile metadata still matches the target, memory layout, controller geometry, timing, and parallel-port shape. A broader library of validated profiles is still needed. |
