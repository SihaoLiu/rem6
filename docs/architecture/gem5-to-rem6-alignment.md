...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  `QStartNoAckMode` requests, trailing bytes, and skipped prefix noise before
  any ISA register cache or socket loop can mutate simulator state.
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  preservation, `qSupported` feature parsing, no-ack request parsing, bounded
  payload enforcement, acknowledgement, negative
