...
| `src/proto` | 9 | `rem6-proto`, future adapters | partial | Protobuf-like exchange must produce typed rem6 data before entering simulation. rem6-proto has typed instruction, packet, and O3 dependency trace records, validation, canonical maps, window-checked dependencies, stable identity, checked binary frame envelopes, and length-delimited frame streams with stream magic, version, varint32 record lengths, and embedded-frame validation; concrete protobuf and gzip adapters remain open. |
...
- Proto-boundary tests cover typed instruction, packet, and O3 dependency trace
  ...
  zero-length and overlong varint32 length validation, and corrupt
  embedded-frame rejection.
