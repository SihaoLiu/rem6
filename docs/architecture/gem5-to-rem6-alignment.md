...
| --- | --- | --- | --- |
| `src/arch/riscv` | `rem6-isa-riscv`, `rem6-cpu` | partial | RV64I decode and execution exist. rem6-cpu has an initial typed CPU translation frontend that queues virtual fetch/load/store translations, preserves CPU memory request identity separately from translation request identity, materializes mapped physical `MemoryRequest`s, records typed translation faults, and snapshots pending translation metadata without gem5-style `RequestPtr` mutation or delayed callback state hidden inside packets. Privileged ISA, concrete page tables, TLB replacement, walker memory accesses, atomics, vectors, and richer traps are alignment targets. |
