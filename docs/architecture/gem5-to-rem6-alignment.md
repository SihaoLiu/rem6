...
- Configuration and experiment reproducibility are too script-dependent in
  ...
  contracts, sustained-occupancy, full-occupancy, underoccupied-tick, and
  ...
  contracts, runtime full-occupancy and underoccupied-tick contracts, per-workload result
...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  verifiable without replaying callbacks. Workload result summaries also treat
...
- Observability and statistics need stronger contracts. A gem5 issue about
  ...
  or counter values. Counter paths are structured scope/name identities with a
...
| `src/dev` | 418 | `rem6-mmio`, `rem6-uart`, `rem6-timer`, `rem6-interrupt`, `rem6-gpu`, `rem6-accelerator`, `rem6-platform`, `rem6-pci` | partial | UART, timer, interrupt, an initial typed RISC-V CLINT MMIO model with crate-level snapshot/restore, typed reset policy, platform/topology attachment, typed RISC-V DTS source emission, binary FDT/DTB emission, RISC-V DTB memory/A1 handoff, typed Linux `/chosen` bootargs and initrd DTB metadata, typed DTB and initrd blob installation for store-backed and DRAM-backed memory, GPU, accelerator, and initial PCI endpoint, host config-space, 32-bit and 64-bit BAR, BAR MMIO, legacy INTx, MSI, and MSI-X paths exist. Storage, network, virtio, PS/2, QEMU bridge, and broader platform-specific devices remain alignment targets. |
...
| `src/dev/pci`, `src/dev/virtio`, `src/dev/storage`, `src/dev/net` | `rem6-pci`, future device crates | partial | rem6-pci has an initial typed PCI endpoint and host config-space model: function addresses validate bus/device/function ownership, type-0 identity and class header fields are little-endian bytes, BAR sizing writes apply typed memory or I/O masks, 64-bit memory BARs consume checked adjacent config slots and combine lower plus upper dwords into one logical range, fixed legacy I/O BARs preserve a declared address while ignoring config BAR writes, command bits gate exposed BAR ranges, an endpoint-owned PCI capability-list registry links multiple installed capabilities in order, rejects overlapping regions before mutation, and keeps next-pointer bytes stable while MSI/MSI-X control fields are written, MSI capabilities expose typed config-space headers, preserve clamped vector enable state, mask vectors, restore snapshots, derive typed message address/data pairs, and deliver enabled serial or parallel MSI assertions through rem6-interrupt without direct platform mutation, MSI-X capabilities expose typed config-space table and PBA registers, own BAR-local table/PBA state, mask vectors and functions explicitly, preserve table plus pending bits across snapshots, and deliver enabled serial or parallel MSI-X assertions while recording masked parallel sends as typed pending bits, Type-1 bridge configs expose typed PCI-to-PCI headers, validate primary/secondary/subordinate bus ranges, route subordinate config accesses only through declared bridge ranges, and filter downstream active BAR host mappings through memory, prefetchable-memory, and I/O windows, read-only writes return typed errors, snapshots restore config and BAR state with function/identity/class checks, host apertures implement CAM-sized and ECAM-sized per-function config slots, host reads route by decoded physical config addresses, missing functions return all-ones reads for guest enumeration, duplicate or out-of-aperture registration is rejected explicitly, active BARs map through typed IO, non-prefetchable memory, and prefetchable memory host bases, overlapping host BAR ranges are rejected before system topology consumes them, the config aperture can be attached to the typed MMIO bus with serial or parallel scheduler responses, active BAR host ranges can forward serial or parallel runtime MMIO requests into BAR-local device offsets with typed boundary errors, and typed PCI legacy INTx mapping plus endpoint-facing post/clear ports can deliver serial or parallel interrupt assertions through rem6-interrupt without direct platform mutation. Block, network, virtio devices, richer bridge MMIO forwarding, and richer platform swizzling remain required for full-system breadth. |
...
- PCI tests cover typed legacy INTx line mapping, invalid interrupt-pin and
  zero-line rejection, serial and parallel endpoint post/clear delivery through
  rem6-interrupt, and observable delivery errors when a parallel clear targets
  a mismatched source. PCI config and host tests cover 64-bit memory BAR lower
  ...
  of downstream host mappings. Capability-list tests cover ordered MSI plus
  MSI-X chaining, next-pointer preservation across capability control writes,
  and overlap rejection without mutating existing config bytes. MSI tests cover
  capability header exposure, clamped vector enable state, message-address
  masking, vector masks, snapshot restore, duplicate and invalid capability
  layouts, and serial plus parallel MSI delivery through typed rem6-interrupt
  routes. MSI-X tests cover typed
  ...
  delivery recording into the PBA. Type-1 bridge tests cover typed bridge
  header fields, writable bus-number and memory-window registers, subordinate
  config routing, and downstream BAR host-range filtering through bridge
  windows.
