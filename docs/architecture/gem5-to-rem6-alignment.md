...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  `-ENOSYS` through the same ecall-resume path. Fork, clone, exec, futex wake on
  child-clear-TID, dynamic credential changes, file-backed mmap handling,
  backing-memory or page-table installation for returned ranges, `openat`,
  `futex`, `wait4`, and guest-memory argument copying remain open before broad
  libc workloads can run. The table also exposes
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  state, gem5-style ignore-return advisory memory-management calls, and ignored
  `rseq` returning `-ENOSYS` through the same real ecall path. It still lacks
  fork/clone process modeling, futex wake on child-clear-TID, file-backed mmap
  handling, backing-memory installation for returned ranges, the broader syscall
  table, and guest-memory argument paths needed for `openat`/`futex`/`wait4`, and
