...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  the instruction after the ecall. `mmap`, `openat`, `futex`, `wait4`, and
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  resume for returning `brk` calls. It still lacks the broader syscall table
  and guest-memory argument paths needed for `mmap`/`openat`/`futex`/`wait4`,
  and Linux handoff still needs an SBI-class firmware/runtime path rather than only
