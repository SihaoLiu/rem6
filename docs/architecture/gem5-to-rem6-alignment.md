...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  those flow records, and workload-result replay derives route-level flow count,
  first/last delivery tick, and min/max delay evidence from exact remote-send
  records when aggregate flow records are absent. Workload manifests and replay
...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  remote-send records when aggregate flow records are absent, per-partition
