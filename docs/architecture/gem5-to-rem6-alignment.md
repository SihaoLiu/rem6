...
- Observability and statistics need stronger contracts. A gem5 issue about
  ...
  registration, and counter units are registered either as typed values or as
  parsed structured metadata that can preserve nested rate spelling while
  rejecting empty, whitespace-shaped, or malformed rate strings. This keeps
  gem5's useful stat name and unit discipline while returning typed errors
  instead of panicking. Stats dumps are
...
- Stats tests cover counter path grammar, structured unit and rate grammar,
  counter reset epochs, typed dump history, schema-and-reset-scope-checked
  snapshot deltas, and typed probe point, listener, event, payload, and snapshot
  records.
