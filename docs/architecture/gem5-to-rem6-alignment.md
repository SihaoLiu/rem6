...
- MSI, MESI, MOESI, and initial CHI-like line-state, cache-controller,
  ...
  run-resource summaries have test coverage; full-system workload replay paths
  already report MSI/MESI/MOESI/CHI data-cache protocol attribution, while
  direct topology data-cache attach currently reports MSI/MESI/MOESI.
...
- Complete CHI-like coherence beyond the current line-state crate, single-line
  ...
  DRAM-backed harnesses with recorded resource summaries, and workload-replay
  data-cache attribution, plus direct topology CHI data-cache attach and
