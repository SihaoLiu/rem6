...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  responses, basic `T thread-id` thread-alive requests and alive-or-missing
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  thread-alive parsing, alive-or-missing response emission, `p n` single-register request
