...
- PCI tests cover typed legacy INTx line mapping, invalid interrupt-pin and
  ...
  Capability-list tests cover ordered PM plus MSI chaining, MSI plus MSI-X
  chaining, next-pointer preservation across capability control writes, PMCSR
  snapshot restore, and overlap rejection without mutating existing config
  bytes. MSI tests cover capability header exposure,
  ...
  and command-bit-gated host mapping, snapshot restore of bridge config and BAR
  state, writable bus-number and memory-window registers, subordinate config
  routing, and downstream BAR host-range filtering through bridge windows.
