...
| `src/arch/riscv` privileged return | `rem6-isa-riscv` | partial | Machine `mepc` CSR register read/write/set/clear execution is present, with tests covering read/write/set; Machine-mode `MRET` PC/status restoration is present. `SRET`, trap delegation, lower-privilege return legality, and fuller trap entry remain alignment targets. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  paths, and Machine-mode `MRET` PC/privilege/status restoration. RISC-V
  frontend tests also cover hart-derived request-carried Sv39 access context
  for data translation, including MPRV/MPP-derived data privilege,
