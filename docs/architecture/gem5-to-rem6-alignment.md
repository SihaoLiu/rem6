...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  expected remote-flow counts, minimum max-worker use, and minimum active
  partition counts by scheduler scope, so cross-partition communication volume,
  real parallel occupancy, and real partition coverage are observable and
...
| gem5 source anchor | rem6 owner | Coverage | Notes |
| --- | --- | --- | --- |
| event queue and tick logic in `src/sim` | `rem6-kernel` | covered | Partitioned scheduling, conservative epochs, deterministic order, lookahead, scheduler snapshots, worker-local remote outboxes, ordered remote-send records, source-target remote-flow records carried through RISC-V cluster, full-system, and workload-result summaries, manifest-owned and result-verifiable expected remote-flow counts, minimum worker-use contracts, and active-partition contracts, source and target partition counts in recorded parallel summaries, and typed parallel-worker failure reporting that preserves remaining partition events, keeps executed-time visibility, commits successful callbacks' remote messages, and rolls back local and remote events scheduled by the panicked callback exist. |
...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  partition counts, and manifest identity changes for those expected
  communication contracts. Workload replay QoS tests cover same-tick DRAM
...
- Workload manifests record boot images, resources, topology, host events,
  ...
  validation, manifest-owned parallel remote-flow, worker-use, and
  active-partition verification contracts, result metadata, execution mode
