...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  queries, `QC thread-id` replies, `qSupported` feature tokens, `qSupported`
  response feature emission, `QStartNoAckMode` requests,
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  `QC thread-id` response emission, `qSupported` feature parsing, stub feature response emission,
