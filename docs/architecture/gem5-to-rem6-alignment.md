...
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V exists with typed counters, vector state, atomic memory operations, Sv39 virtual-address and PTE validation, `ADDI`/`SLTI`/`SLTIU`/`ANDI`/`XORI`/`ORI`/`SLLI`/`SRLI`/`SRAI` integer immediate execution, matching RV64I register-register ALU execution, RV64I word-width ALU execution, `mhartid` CSR reads backed by explicit hart identity, and typed RV32/RV64 GDB target-description documents for CPU registers; concrete Sv39 page-table walking remains open, and ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before claiming parity. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  decoding, PTE flag and PPN decoding, reserved permission rejection, non-leaf
  detection, leaf access permissions, and accessed/dirty fault classification.
