...
- `rem6-storage` now owns gem5 `DiskImage`, `RawDiskImage`, and `CowDiskImage`
  ...
  overlay reads. Storage source-policy tests keep the crate under the facade
  and hard per-source-file budgets, so IDE, simple-disk, VirtIO, and future
  storage controllers can share one typed image substrate instead of recreating
  gem5-style panic paths, process-exit save callbacks, and mutable SimObject
  disk state.
...
- VirtIO tests cover modern PCI common-config feature-page selection,
  ...
  Storage image tests cover raw sector reads and writes, flush accounting,
  snapshot restore, read-only rejection before mutation, typed range and image
  shape errors, nested copy-on-write overlays, deterministic dirty-sector
  snapshots, and explicit writeback to the child image.
