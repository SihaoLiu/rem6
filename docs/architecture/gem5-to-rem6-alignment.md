...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  `nanosleep`, `sched_yield`, and the initial `rt_sig*` signal-action family,
  allowing those user-mode `ecall` traps to return zero and continue instead of
  becoming generic scheduled traps. Fork, clone, exec, futex wake on
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  `set_tid_address` child-clear-TID state, plus anonymous `mmap`/`munmap` region
  state returned and updated through the same real ecall path. It still lacks
