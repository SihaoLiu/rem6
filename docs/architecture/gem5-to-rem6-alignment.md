...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  minimum max-worker use, minimum active partition counts, and clean parallel
  diagnostic contracts by scheduler or resource scope, so cross-partition
  communication volume, timing drift, real parallel occupancy, real partition
  coverage, and wait-for/deadlock cleanliness are observable and verifiable
...
| gem5 source anchor | rem6 owner | Coverage | Notes |
| --- | --- | --- | --- |
| event queue and tick logic in `src/sim` | `rem6-kernel` | covered | Partitioned scheduling, conservative epochs, deterministic order, lookahead, scheduler snapshots, worker-local remote outboxes, ordered remote-send records, source-target remote-flow records carried through RISC-V cluster, full-system, and workload-result summaries, manifest-owned and result-verifiable expected remote-flow counts and first/last tick windows, minimum worker-use contracts, active-partition contracts, clean diagnostic contracts, source and target partition counts in recorded parallel summaries, and typed parallel-worker failure reporting that preserves remaining partition events, keeps executed-time visibility, commits successful callbacks' remote messages, and rolls back local and remote events scheduled by the panicked callback exist. |
...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  of exact expected remote-flow counts and first/last tick windows, minimum
  max-worker use, minimum active partition counts, clean parallel diagnostic
...
- Workload manifests record boot images, resources, topology, host events,
  ...
  worker-use, active-partition, and clean diagnostic verification contracts,
  result metadata, execution mode switches, host action summaries, checkpoint
  restore labels, and statistics snapshots.
