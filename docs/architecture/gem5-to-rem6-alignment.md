...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  `stvec`/`sepc`/`scause`/`stval` supervisor-trap CSR paths, synchronous
  `ecall`/`ebreak` Machine trap entry, delegated lower-privilege `ecall` and
  `ebreak` Supervisor trap entry, Machine-mode `MRET`, Supervisor-or-higher
  `SRET`, and illegal lower-privilege `xRET` trap entry. RISC-V frontend tests also cover
