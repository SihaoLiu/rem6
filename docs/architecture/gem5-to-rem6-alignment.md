...
- CPU in-order pipeline tests cover typed per-stage width validation, oldest
  ...
  reporting. Run summary tests cover empty windows and aggregate cycle-window,
  redirect, state-change, and count totals across records, plus identity merges
  for partial summaries.
