...
- Simple models are not automatically cheap or transparent. Recent call-stack
  ...
  virtual-network contracts plus minimum per-virtual-network transfer,
  active-lane, queue-delay, and contended-lane contracts so NoC hotspots remain
  typed replay evidence.
