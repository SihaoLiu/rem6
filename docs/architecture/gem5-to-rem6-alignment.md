...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  missing-memory error responses, `M addr,length:XX...` memory-write
  requests, all-or-error memory-write responses, no-ack session transition
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  response emission, missing-memory error responses, `M addr,length:XX...`
  memory-write request parsing, all-or-error memory-write responses, no-ack request parsing, no-ack
