...
| `src/cpu/o3` | future out-of-order CPU crate plus `rem6-proto` trace contracts | planned | Needs rename, issue, reorder, load/store queue, speculation, squash, and executable typed traces. The trace contract already distinguishes prefetch records from response-blocking loads with retire-after-issue completion policy. |
