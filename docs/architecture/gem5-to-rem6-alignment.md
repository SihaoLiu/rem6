...
- Parallel simulation is not a natural baseline in gem5. Historical Parallel M5
  ...
  activity. CLI RISC-V data-access stats also carry total and per-core
  ...
  The CLI can also write the full JSON run artifact to a requested path while
  returning a small machine-readable output envelope on stdout, so scripted
  runs can keep durable stats artifacts without scraping terminal streams.
