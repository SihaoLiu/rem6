...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  clean parallel diagnostic contracts by scheduler or resource scope, so
...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  run-count expectations, clean parallel diagnostic expectations, and manifest
  identity changes for those expected communication contracts.
...
- Workload manifests record boot images, resources, topology, host events,
  ...
  data-cache protocol run-count verification contracts, and clean diagnostic
