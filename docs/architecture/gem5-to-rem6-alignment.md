...
- Compatibility bugs cluster around cross-subsystem seams. Recent public gem5
  ...
  path now reads the guest futex word for `FUTEX_WAIT` and returns Linux
...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  guest execution. RISC-V `FUTEX_WAIT` now consumes the mismatch path by
  reading the RV64 guest futex word through simulated memory, comparing it with
  the expected value, returning `EAGAIN` on mismatch, and resuming guest
  execution. RISC-V `dup` now allocates the lowest free guest descriptor,
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  `rseq` returning `-ENOSYS`, `futex` wake/wake-bitset handling, `FUTEX_WAIT`
  guest-word mismatch handling that returns Linux `EAGAIN`, and initial `dup`,
  `dup3`, `close`, `fcntl`, guest-backed `write`, and stdin-backed
