...
- `rem6-virtio` now keeps queue indexes, queue specs, notify specs, queue
  ...
  before completion state mutates. VirtIO RNG now exposes gem5's device id 4 and zero-length config
...
- VirtIO tests cover VirtIO 9P device id, mount-tag feature bit,
  ...
  rejection without completion mutation, modern PCI version-1 feature exposure
  for 9P, block, console, and RNG, legacy RNG device id and zero-config behavior,
