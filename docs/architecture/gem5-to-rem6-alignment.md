...
`WriteReq` maps to `Write`, absent or zero flags are accepted, and all other
commands or nonzero flags fail with typed errors before request construction.
Trace replay snapshots preserve cursor, sequence, summary, tick offset, and
active state, while exit resets the cursor and reports whether the trace was
exhausted. Traffic state-machine coverage now includes a
