...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  end replies, `qSupported` feature tokens, `qSupported` response feature emission, `QStartNoAckMode` requests,
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  active-thread list parsing, thread-list response emission, `qSupported` feature parsing, stub feature response emission,
