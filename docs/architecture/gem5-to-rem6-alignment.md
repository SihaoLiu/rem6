...
| memory ports, packets, requests in `src/mem` root | `rem6-transport`, `rem6-memory` | partial | Shared request/response transport exists, and `MemoryRequest` now has a stable checkpoint payload preserving operation, identity, address, size, line layout, data, byte masks, atomic operation, uncacheable-plus-strict flags, and explicit before/after ordering edges. More gem5 packet semantics need mapping as features are added. |
