...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  responses, no-ack session transition state, last-response retransmission
  on negative acknowledgement, session-level frame dispatch, interrupt request
  state, trailing bytes, and skipped prefix noise before any ISA register cache
  or socket loop can mutate simulator state.
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  unsupported-command empty responses, no-ack request parsing, no-ack session
