...
| guest-host events and pseudo instructions | `rem6-system`, `rem6-workload` | partial | ROI, stats, checkpoint, checkpoint restore, stop, execution mode actions, custom guest-host calls, manifest-declared guest-host response payloads, focused system/trap event port modules, guest trap kind preservation, absolute parallel trap delivery-boundary preflight, and manifest-declared gem5-style work-begin/work-end marker metadata with workload-summary ROI counts are typed. Broader guest ABI and ISA pseudo-instruction entry support remain open. |
