...
- Parallel simulation is not a natural baseline in gem5. Historical Parallel M5
  ...
  artifact plus hierarchical stats expose the selected DDR, HBM, or LPDDR
  ...
  low-power counters, technology one-hot stats, and profiled external-memory
  capacity denominators. CLI
...
| HBM, LPDDR, DDR class profiles | `rem6-dram` | partial | The profile shape exists for DDR, HBM, LPDDR, and NVM, each technology exposes stable lowercase labels for manifests, stats, and artifacts, topology dimensions name their parallel ports and topology units, and checkpoint restore validates that profile metadata still matches the target, memory layout, controller geometry, timing, and parallel-port shape. Standalone CLI RISC-V execution can select DDR, HBM, or LPDDR for the typed DRAM-backed memory path and reports the selected technology plus profile capacity in artifact JSON and hierarchical stats. A broader library of validated profiles is still needed. |
