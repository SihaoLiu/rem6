...
- Observability and statistics need stronger contracts. Public gem5 issue
  ...
  payloads, real RISC-V data-access request input, and snapshot validation.
...
Implementation evidence through 2026-06-03:
...
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V exists with typed counters, vector state, atomic memory operations, Sv39 address-decoding, typed page-table-walk foundations, a CPU translation result bridge, a CPU-layer Sv39 memory walker driver that starts ready frontend translations, batches initial and chained PTE reads through the parallel memory transport, records transport-delivered PTE responses, and completes the frontend, `ADDI`/`SLTI`/`SLTIU`/`ANDI`/`XORI`/`ORI`/`SLLI`/`SRLI`/`SRAI` integer immediate execution, matching RV64I register-register ALU execution, RV64I word-width ALU execution, RV64C integer, load/store, and control-flow decompression into existing scalar instruction records, initial checkpointed RV64D double-precision register state, `FLD`/`FSD` memory access, raw-bit load writeback, and RNE `FADD.D` execution, `mhartid` CSR reads backed by explicit hart identity, typed Machine and Supervisor trap CSR state plus synchronous Machine trap entry, current synchronous exception delegation, ISA-level interrupt CSR/trap entry, `xRET` privilege legality, execute-time CSR address privilege checks for implemented CSR families, and privileged return, typed RV32/RV64 GDB target-description documents for CPU registers, and an RV64 supervisor CSR annex for the listed status and trap CSRs; full F/D floating-point coverage, full CPU data-path integration for hardware Sv39 walking, and ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before claiming parity. |
...
| --- | --- | --- | --- |
| `src/arch/riscv` | `rem6-isa-riscv`, `rem6-cpu` | partial | RV64I decode and execution exist for a growing integer base, including signed/unsigned immediate comparisons, logical/arithmetic immediate shifts, register-register ALU operations, word-width ALU operations, and signed/unsigned conditional branches, with RV64A `LR.W`/`LR.D` and `SC.W`/`SC.D` paths that record a typed load-reservation address and size only after the read completes, submit load-reserved accesses as native load-locked memory requests, submit matching store-conditionals as native store-conditional memory requests, write architectural success or failure codes into `rd`, clear reservations on both outcomes, record local conditional failures without mutating memory, use cluster-level data-event tracking to invalidate peer reservations after completed overlapping writes, and let system data-cache response paths apply MSI, MESI, MOESI, and CHI invalidating snoops to matching core reservations before the requesting CPU response is delivered. All non-LR/SC RV64A word and doubleword AMOs now decode and execute as typed atomic memory accesses whose responses write the old memory value to `rd`, with word-width old values sign-extended to XLEN and word-width stores updating only four bytes. `MemoryRequest` carries an explicit atomic operation so the memory store and MSI, MESI, MOESI, and CHI cache-controller hit paths compute read-modify-write bytes only after capturing the old response bytes. `FENCE` and `FENCE.I` now decode as typed barrier instructions; execution records preserve fence predecessor/successor sets or instruction-cache barrier identity while advancing in-order without issuing a memory request. `WFI` now decodes as a canonical system instruction and records a typed wait-for-interrupt system event without memory, register, or trap side effects, giving future interrupt-aware schedulers explicit quiesce input instead of a hidden CPU callback. `SFENCE.VMA` now decodes canonical `rs1`/`rs2` operand forms, records typed virtual-address and ASID fence scopes, and lets the RISC-V core consume those system events to invalidate the attached data translation TLB with all-address, ASID, virtual-page, or ASID-plus-page scopes without reaching through CPU callback pointers. RV64A aq/rl flags now map to typed memory-ordering metadata on ISA memory accesses and CPU data-access records: release records a read/write fence before the access, acquire records a read/write fence after the access, and acquire-release records both. Machine `mcycle` and `minstret` counter writes are typed and observable through their user read aliases, user counter aliases reject writes, snapshot restore preserves both counters, and hart execution can read live `cycle` and `instret` values through the CSR instruction path. RISC-V PMP tables now keep raw `pmpaddr`, typed `pmpcfg`, decoded TOR/NA4/NAPOT ranges, active-rule counts, locked-entry behavior, lowest-entry match order, default S/U denial when implemented entries are inactive, and decode-first snapshots so checkpoint restore cannot silently drop protection state or partially mutate a live table. RISC-V vector configuration is typed hart state with `vl` and `vtype` records; branch-prediction targets carry only the target PC, while explicit vector-configuration updates are the only control-flow updates that can mutate that state. RISC-V vector element images and `vcompress.vm` tail policy are also typed: compress writes only the selected prefix and then applies undisturbed or deterministic all-ones agnostic tail handling by element width, so generated micro-op ordering cannot overwrite destination tail elements before the architectural tail rule is applied. RISC-V vector micro-op expansion merges macro instruction flags with micro-op-local flags before emitting records, so serialize-after, non-speculative, delayed-commit, and future scheduling constraints cannot disappear while lowering a vector macro-op. Generic `MemoryRequest`s now carry typed before/after read-write ordering metadata, and RISC-V data issue maps aq/rl metadata into the requests submitted through serial or parallel transport. rem6-isa-riscv now keeps Sv39 canonical virtual-address checks, VPN level extraction, page offsets, PTE-address calculation, PTE flag/PPN decode, leaf permission checks, level-specific physical-address composition, superpage alignment rejection, accessed/dirty fault classification, and a pure incremental `RiscvSv39WalkState`/`RiscvSv39WalkAdvance` walker API that tracks the pending PTE level and address plus recorded PTE addresses; each typed PTE `advance` call either requests the next PTE or completes as `RiscvSv39WalkResult`, and the synchronous `walk_sv39_page_table` helper drives that state machine. rem6-cpu has an initial typed CPU translation frontend that queues virtual fetch/load/store/atomic translation misses, derives ASID-scoped data translation requests from typed RISC-V `satp` state, can attach the typed rem6-memory TLB for immediate ASID-scoped hits, fills that TLB only when queued misses complete through the page resolver, preserves CPU memory request identity separately from translation request identity, materializes mapped physical `MemoryRequest`s, records typed translation faults, snapshots pending translation metadata plus optional TLB state, consumes the typed rem6-memory page resolver for mapped or faulted completions, resolves queued CPU translation requests through typed Sv39 page tables while preserving PTE addresses, leaf levels, and Sv39 page-fault metadata, exposes `RiscvSv39MemoryWalk`, `RiscvSv39MemoryWalkAdvance`, and `RiscvSv39MemoryWalkError` as a CPU-layer Sv39 memory-walk bridge that owns the `CpuTranslationRequest`, drives the ISA `RiscvSv39WalkState`, emits typed `MemoryRequest::read_shared` PTE requests with sequential `MemoryRequestId`s and explicit overflow errors, decodes matching little-endian PTE responses into `RiscvSv39Pte` with typed `RiscvSv39PteReadResponseError` identity, retry, missing-data, and size checks, and completes as `RiscvSv39TranslationResult`; `RiscvSv39MemoryWalker` starts ready frontend requests, tracks active PTE reads, batches initial and chained PTE reads through parallel memory transport, records transport-delivered PTE responses, advances pending responses, and completes the frontend without gem5-style `RequestPtr` mutation or delayed callback state hidden inside packets. rem6-memory also has a typed translation TLB with bounded capacity, ASID-keyed entries, deterministic LRU, permission rechecks on hits, hit/miss/fault/insert/eviction counters, explicit all-address-space and scoped invalidation, and snapshot restore. Translation page mappings now carry global/non-global scope through page-map snapshots and checkpoint payloads, and TLB fill preserves that scope so non-global ASID and ASID-plus-page invalidations retain global entries. The page resolver can emit explicit cross-page translation segments with first-failed-segment faults, and rem6-cpu can turn queued CPU translations into per-segment translated records with sliced store payloads and masks. Segmented completion can also fill ASID-scoped TLB page entries for each mapped segment so later single-page accesses hit without re-running the page resolver. A simple RISC-V data issue path can now own an optional data translation frontend, translate load, store, and RV64A atomic-family effective addresses through the page resolver and TLB before transport submission, keep the original ISA memory access in the data event, and record the physical request address explicitly. The translated RISC-V driver path polls typed translation readiness without issuing the next fetch early; the core, cluster, system run driver, and topology-built core paths can submit translated data requests through the parallel memory transport; and the MMIO-aware system path can route translated physical data addresses to either platform MMIO or memory without using the virtual address for device selection. The plain data issue path rejects a configured translation frontend when no page map is supplied. This replaces gem5's hidden split translation callback state with typed data that future RISC-V timing paths can schedule or replay deterministically. Interrupt-aware `WFI` quiesce and resume policy, fuller CPU pipeline TLB wiring, timed hardware page-table walker memory accesses, cache and memory enforcement of AQ/RL timing barriers, fuller vector instruction execution, and richer traps are alignment targets. |
...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  Initial RV64D tests cover double-precision floating-point register state,
  `FLD`/`FSD` decode and memory access records, RNE `FADD.D` execution, raw-bit
  floating-point write records, and CPU data-response writeback into
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  coverage beyond the current `FLD`/`FSD` and RNE `FADD.D` slice, the current
  host-assisted mode has guest fd and futex state but not a syscall handler
  table, and Linux handoff still needs an SBI-class firmware/runtime path
  rather than only DTB/initrd/register handoff.
