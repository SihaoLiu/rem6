...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  requests, target-order register-byte hex responses, no-ack session transition
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  stop reply emission, `g` register-read request parsing, target-order register
  byte hex response emission, no-ack request parsing, no-ack session
  transitions, NACK retransmission, session frame dispatch, interrupt request
