...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  attach-state queries, attached-or-created replies, `qSupported` feature tokens,
  `qSupported` response feature emission, `QStartNoAckMode` requests,
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  attached-or-created response emission, `qSupported` feature parsing, stub feature response emission,
