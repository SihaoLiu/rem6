...
- Observability and statistics need stronger contracts. A gem5 issue about
  ...
  registration, and counter units reject empty or whitespace-shaped strings
  while preserving simple rate-unit spelling. This keeps gem5's useful stat
  name and unit discipline while returning typed errors instead of panicking.
  Stats dumps are registry-owned typed records with stable dump ids and snapshot
  payloads rather than global output callbacks. Stats deltas are also typed
  records derived only from snapshots in the same reset scope with nondecreasing
  tick and counter values.
...
- Stats tests cover counter path and unit grammar, counter reset epochs, typed
  dump history, schema-and-reset-scope-checked snapshot deltas, and typed probe
