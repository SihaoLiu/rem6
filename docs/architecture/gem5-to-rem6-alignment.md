...
- PCI tests cover typed legacy INTx line mapping, invalid interrupt-pin and
  ...
  subsystem IDs, common cache-line-size and latency-timer writes, status
  ...
  maximum-latency read-only byte behavior.
  ...
  interrupt line/pin bytes, bridge-control writes, common cache-line-size and
  latency-timer writes, common status writes that do not create guest-owned
  bits, bridge BAR0/BAR1 install and command-bit-gated host mapping, snapshot
  restore of bridge config and BAR state, writable bus-number and memory-window
  registers, subordinate config routing, and downstream BAR host-range filtering
  through bridge windows.
