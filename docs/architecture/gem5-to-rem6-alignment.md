...
- gem5 cache tags split set selection between SetAssociative and
  ...
  MSI and MESI cache banks can now opt into those compressed tags for explicit
  compressed-size fills, hit-path replacement access, resident removal,
  whole-superblock clean eviction, dirty/transient victim rejection with tag
  rollback, snapshot restore, and tag-backend mode mismatch rejection. MOESI
  and CHI compressed-tag bank integration remain separate alignment targets.
...
- Cache indexing and replacement-directory tests cover gem5-style
  ...
  MSI and MESI cache-bank compressed-tag tests cover explicit compressed-size
  co-allocation, default uncompressed fill behavior, whole-superblock clean
  eviction, dirty victim rollback, sibling-preserving upgrade fills, snapshot
  restore, and mode mismatch rejection.
...
- Complete CHI-like coherence beyond the current line-state crate, single-line
  ...
  and MOESI/CHI compressed-tag bank integration.
