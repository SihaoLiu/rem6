...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  expected remote-flow counts, minimum max-worker use, minimum active partition
  counts, and clean parallel diagnostic contracts by scheduler or resource
  scope, so cross-partition communication volume, real parallel occupancy, real
  partition coverage, and wait-for/deadlock cleanliness are observable and
...
| gem5 source anchor | rem6 owner | Coverage | Notes |
| --- | --- | --- | --- |
| event queue and tick logic in `src/sim` | `rem6-kernel` | covered | Partitioned scheduling, conservative epochs, deterministic order, lookahead, scheduler snapshots, worker-local remote outboxes, ordered remote-send records, source-target remote-flow records carried through RISC-V cluster, full-system, and workload-result summaries, manifest-owned and result-verifiable expected remote-flow counts, minimum worker-use contracts, active-partition contracts, clean diagnostic contracts, source and target partition counts in recorded parallel summaries, and typed parallel-worker failure reporting that preserves remaining partition events, keeps executed-time visibility, commits successful callbacks' remote messages, and rolls back local and remote events scheduled by the panicked callback exist. |
...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  scheduler, merged full-system remote-flow records, replay-plan validation
  of exact expected remote-flow counts, minimum max-worker use, minimum active
  partition counts, clean parallel diagnostic expectations, and manifest
  identity changes for those expected communication contracts. Workload replay
  QoS tests cover same-tick DRAM
...
- Workload manifests record boot images, resources, topology, host events,
  ...
  validation, manifest-owned parallel remote-flow, worker-use, active-partition,
