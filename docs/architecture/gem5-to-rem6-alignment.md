...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  activity minima, data-cache protocol run-count expectations, and clean
  ...
  participation, and wait-for/deadlock cleanliness are observable and verifiable
...
- MSI, MESI, MOESI, and initial CHI-like line-state, cache-controller,
  ...
  attribution, and workload replay manifests can require a minimum recorded
...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  empty-epoch, and dispatch counts, total-worker counts, batch worker-count
  histograms, exact batch partition-set histograms, maximum consecutive
  partition-set streaks, per-partition activity summaries, replay-plan
  ...
  counts, per-partition activity minima, data-cache protocol run-count
  expectations, clean parallel diagnostic expectations, and manifest identity
  changes for those expected communication contracts.
...
- Workload manifests record boot images, resources, topology, host events,
  ...
  activity, active-partition, per-partition activity, data-cache protocol
  run-count verification contracts, and clean diagnostic verification
  contracts, result metadata, execution mode switches, host action summaries,
