...
- `rem6-system` has typed guest wait-status encoding for future syscall
  ...
  as a generic environment-call trap. `brk`, `mmap`, `openat`, `futex`, `wait4`,
  and guest-memory argument copying remain open before broad libc workloads can
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  and futex state plus an initial RISC-V Linux user `exit`/`exit_group`
  syscall handler wired to real user-mode ecall execution. It still lacks the
  broader syscall table and guest-memory argument paths needed for
  `brk`/`mmap`/`openat`/`futex`/`wait4`, and Linux handoff still needs an
  SBI-class firmware/runtime path rather than only
