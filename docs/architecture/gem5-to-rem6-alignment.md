...
- Observability and statistics need stronger contracts. A gem5 issue about
  ...
  look sequence-ordered while moving backward in simulated time. Probe
  component names, point names, and listener names also use the same checked
  identifier spelling as stat path segments, so probe identity cannot fall back
  to whitespace, separator, or punctuation-shaped log labels.
...
- Stats tests cover registry-owned stat groups, self-describing group catalogs
  ...
  snapshot deltas, and typed probe point, listener, event, payload,
  identifier-grammar, cursor-preserving, and time-monotonic snapshot restore
  records with malformed snapshot rejection.
