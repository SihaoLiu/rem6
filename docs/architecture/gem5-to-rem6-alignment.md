...
DRAM-family text modes, plus resolver-backed TRACE text modes with explicit
binding options. The DRAM-family binding constructs typed DRAM, DRAM_ROTATE,
and NVM traffic generators with bank, rank, page-or-buffer, address-mapping,
sequential-packet, data-limit, non-elastic timing, snapshot, and checked
geometry state; DRAM_ROTATE keeps deterministic bank/rank rotation and its
limited gem5 read-percentage modes while ordinary DRAM and NVM keep
randomized series selection. A typed
traffic controller now binds Markov graph states to the implemented idle, exit,
linear, random, strided, DRAM-family, and trace generators, preserves gem5's
...
mapping, hybrid traffic, full memory-controller-coupled DRAM/NVM execution, and
