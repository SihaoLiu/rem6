...
deterministic synthetic write data, writable intent, response data, and both
read and write byte accounting, while the matching gem5 `ATOMIC_RETURN_OP`
flag is accepted as an explicit return-data selector. `LoadLockedReq`
...
`ATOMIC_RETURN_OP` on `SwapReq` as a return-data selector, and rejects
