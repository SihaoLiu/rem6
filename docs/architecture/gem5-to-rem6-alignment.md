...
- The gem5 output path convention writes run artifacts such as configuration
  ...
  at guest traps through the host event path or at an explicit
  `--max-instructions <count>` committed-instruction budget, and records
...
- CLI tests cover the initial standalone `rem6 run` binary path: RISC-V ELF
  ...
  limit when execution is not enabled or the count is zero.
