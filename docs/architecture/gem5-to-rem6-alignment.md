...
| --- | ---: | --- | --- | --- |
| `src/arch` | 1187 | `rem6-isa-riscv`, future ISA crates | partial | Keep per-ISA decoding and architectural state as isolated crates. RISC-V exists with typed counters, vector state, atomic memory operations, `ADDI`/`SLTI`/`SLTIU`/`ANDI`/`XORI`/`ORI`/`SLLI`/`SRLI`/`SRAI` integer immediate execution, matching RV64I register-register ALU execution, RV64I word-width ALU execution, `mhartid` CSR reads backed by explicit hart identity, and typed RV32/RV64 GDB target-description documents for CPU registers; ARM, x86, Power, SPARC, MIPS, and AMDGPU ISA support need equivalent crate ownership before claiming parity. |
