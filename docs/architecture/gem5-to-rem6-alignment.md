...
- Observability and statistics need stronger contracts. A gem5 issue about
  ...
  syscall-emulation correctness gaps, RISC-V vector tracing failures, and a
  multicore CHI LR/SC race report, so rem6 keeps turning cross-subsystem
  activity into typed, identity-preserving evidence before broad parity claims.
...
| `ext/libelf`, `ext/libfdt`, `ext/softfloat`, `ext/gdbremote` | `rem6-boot`, `rem6-platform`, future debug and ISA support crates | partial | rem6-boot has an initial typed ELF64 little-endian loader that validates the ELF identity and program-header shape, imports physical-address `PT_LOAD` segments into `BootImage`, skips non-loadable segments, preserves `p_memsz > p_filesz` zero-fill, and rejects overflowing segment memory ranges with segment-indexed typed errors without libelf or unsafe parser state. rem6-platform has an initial typed DTS source tree and deterministic binary FDT/DTB writer for RISC-V platform boot descriptions, including Linux `/chosen` bootargs and initrd start/end metadata, and rem6-system installs initrd bytes plus generated or resolved-resource DTBs into guest memory with the RISC-V A1 register handoff for both store-backed and DRAM-backed memory. rem6 still needs broader ELF class/endian/ISA coverage, kernel image loader breadth, bootloader handoff coverage, soft-float, and debug capability without vendoring unneeded code into the core. |
