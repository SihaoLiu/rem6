...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  exact partition-set, or per-partition evidence, maximum scheduler empty epochs, minimum multi-worker
...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  evidence, per-partition activity minima, data-cache run attribution
