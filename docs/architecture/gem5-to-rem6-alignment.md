...
requests with no data, no byte mask, response required, no response data,
writable intent, and no read/write byte accounting; absent or zero flags are
accepted, and all other commands or nonzero flags fail with typed errors before
...
timing. Non-memory command coverage, flag-to-request mapping, full
