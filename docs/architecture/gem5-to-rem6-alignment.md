...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  multiply-add/subtract execution, RNE `FSQRT.S` and `FSQRT.D`, sign injection,
...
- Make the RISC-V execution path capable of mainstream software: RV64GC still
  ...
  Complete exception flags beyond the current `FDIV.S`/`FDIV.D` and
  `FMIN`/`FMAX` signaling-NaN cases,
