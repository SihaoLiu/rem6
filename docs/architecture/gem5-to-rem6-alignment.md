...
| `src/cpu/pred` | `rem6-cpu` branch predictor modules | partial | A local two-bit predictor, branch target buffer, indirect target predictor, and return-address stack have independent typed prediction, lookup, update, target, replacement, speculative history, commit, repair, and snapshot state. The indirect target predictor replaces opaque history pointers and random target replacement with typed records, per-CPU history, deterministic LRU, and restore validation. Richer predictor families and pipeline integration remain open. |
...
- CPU branch prediction exposes typed direction prediction, branch-target
  lookup, indirect-target lookup, deterministic replacement, update, target,
