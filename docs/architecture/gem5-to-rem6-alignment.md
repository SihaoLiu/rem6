...
- Replacement policy ownership is a fragile Ruby and CHI boundary in gem5. A
  ...
  in generated protocol code or callback side effects. MSI and CHI cache banks
  now use that boundary for clean-line capacity eviction and preserve
  replacement state in bank snapshots. MSI banks reject modified or transient
  capacity victims, and CHI banks reject dirty capacity victims, with typed
  state instead of silently dropping ownership before writeback integration
  exists.
...
- RISC-V PMA anchor refreshed on 2026-05-30: gem5's local `PMAChecker` keeps
  ...
  response without installing the returned bytes. MSI and CHI banks also
  remove the bypassed resident way from the replacement directory, so
  replacement metadata cannot report a line that the bank no longer owns.
  Dirty resident
...
- `rem6-cache` now keeps MSI and MESI controller state, snapshots, errors,
  ...
  lookup, and snapshot restore validation. MSI and CHI cache banks now attach
  that directory for clean-line capacity eviction, deterministic snapshot
  replay, and typed victim rejection before writeback integration: modified or
  transient victims for MSI, and dirty victims for CHI. This gives protocol
  banks one typed replacement boundary instead of Ruby/CHI-specific callback
  state.
...
- Cache MSHR, MSI/MESI/MOESI/CHI cache-bank, and MSI bank directory harness
  ...
  installation, and MSI/CHI replacement-directory resident-way cleanup. MSI
  and CHI replacement-directory tests also cover clean capacity eviction, hit
  recency updates, snapshot replay, MSI modified or transient capacity-victim
  rejection, and CHI dirty capacity-victim rejection before writeback
  replacement support exists. MSI, MESI, MOESI, and CHI tests also require
  dirty resident lines to reject this read bypass while preserving dirty
  snapshot evidence when no typed write queue is attached, and require direct
  uncacheable writes to enter the typed write queue without allocating MSHR
  entries, pending fills, or resident cache lines.
