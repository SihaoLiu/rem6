...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  plans can declare exact expected remote-send and remote-flow actual sets for
  scheduler, data-cache scheduler, or merged full-system scope, exact remote-flow first/last tick windows,
