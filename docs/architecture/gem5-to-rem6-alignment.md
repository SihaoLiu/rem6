...
and maps `ARCH_BITS`, `UNCACHEABLE`, `STRICT_ORDER` plus `UNCACHEABLE`,
...
- GPU and accelerator paths route command and DMA work through typed topology,
  ...
  including `EVICT_NEXT` and `KERNEL`, plus native `CacheBlockZero` and `NoAccess` request operations
