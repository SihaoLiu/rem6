...
- RTC checkpoint banks now round-trip the MC146818 selected address, CMOS byte
  ...
  script-side registration or gem5-style object-local serialization hooks.
...
- RTC core tests cover MC146818-compatible binary and BCD calendar registers,
  ...
  snapshot/restore, serial and parallel MMIO response paths, and typed
  rejection of bad widths, unmapped offsets, and read-only RTC writes. Platform
  tests cover RTC MMIO bus attachment and retained device lookup, while system
  checkpoint tests cover RTC bank decode-first restore, host checkpoint-action
  round trips through manifests, and automatic host checkpoint attachment for
