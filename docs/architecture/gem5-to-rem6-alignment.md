...
- PCI tests cover typed legacy INTx line mapping, invalid interrupt-pin and
  ...
  subsystem IDs, common cache-line-size and latency-timer writes, Expansion ROM
  reads and writes, Expansion ROM size probing, and minimum-grant plus
  ...
  latency-timer writes, bridge BAR0/BAR1 install and command-bit-gated host
  mapping, snapshot restore of bridge config and BAR state, writable bus-number
  and memory-window registers, subordinate config routing, and downstream BAR
