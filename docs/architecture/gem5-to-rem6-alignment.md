...
- Configuration and experiment reproducibility are too script-dependent in
  ...
  planned timeline wall-clock and worker-idle evidence, suite result-derived
  execution evidence, per-workload result start/final windows, worker-level
  suite completion summaries, round-robin or weighted suite dispatch plans,
  dispatch-declared suite execution expectations, suite-level execution
  efficiency summaries, declared suite speedup and utilization thresholds, and
