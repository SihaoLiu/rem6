...
`UNCACHEABLE`, `STRICT_ORDER` plus `UNCACHEABLE`, `ACQUIRE`, `RELEASE`,
`PRIVILEGED`, `SECURE`, and `PT_WALK` into typed rem6 request metadata, accepts
matching-packet `LLSC`, `LOCKED_RMW`, `MEM_SWAP`, and `MEM_SWAP_COND` as
redundant operation selector flags, and rejects unsupported flag bits and
