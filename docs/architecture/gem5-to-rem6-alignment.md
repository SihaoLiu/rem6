...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  activity minima, and clean parallel diagnostic contracts by scheduler or
  resource scope, so cross-partition communication volume, timing drift,
...
- Workload manifests record boot images, resources, topology, host events,
  ...
  activity, active-partition, per-partition activity, and clean diagnostic
  verification contracts, result metadata, execution mode switches, host action
