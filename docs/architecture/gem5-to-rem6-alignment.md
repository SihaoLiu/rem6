...
| `ext/libelf`, `ext/libfdt`, `ext/softfloat`, `ext/gdbremote` | `rem6-boot`, `rem6-platform`, future debug and ISA support crates | partial | rem6-platform has an initial typed DTS source tree for RISC-V platform boot descriptions. rem6 still needs equivalent ELF loading breadth, binary DTB/FDT writing, soft-float, and debug capability without vendoring unneeded code into the core. |
