...
- Cache prefetch tests cover tagged next-line candidate generation, DCPT masked
  ...
  snapshot restore, delay-queue snapshot restore, AMPM cross-hot-zone access-map
  ...
  queued prefetch earliest-ready reporting, deterministic round-robin issue,
  non-mutating no-op polls, and single-entry issue from the selected source.
