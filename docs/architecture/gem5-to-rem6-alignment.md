...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  responses, `?` stop-reason queries, typed stop replies, no-ack session
  transition state, last-response retransmission on negative acknowledgement,
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  stop reply emission, no-ack request parsing, no-ack session transitions,
  NACK retransmission, session frame dispatch, interrupt request recording,
  bounded payload enforcement, acknowledgement, negative
