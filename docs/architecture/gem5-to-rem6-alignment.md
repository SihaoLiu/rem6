...
- `rem6-virtio` now keeps queue indexes, queue specs, notify specs, queue
  ...
  enforce the hard per-source-file size budget, so block, queue, MMIO
  transport, PCI transport, shared-memory, and future network/console/rng
  VirtIO work does not recreate gem5-style mixed device, queue, and transport
  monoliths. VirtIO MMIO transport now exposes gem5-visible magic, version,
...
- VirtIO tests cover legacy MMIO magic/version/device/vendor id registers,
