...
- Parallel simulation is not a natural baseline in gem5. Historical Parallel M5
  ...
  activity. Planned batches now also expose stable host worker-lane records
  with lane, partition, start tick, horizon, and duration, so the simulator can
  audit which host worker would own each ready partition before callbacks run
  instead of proving multicore use only from aggregate occupancy. Workload
  replay summaries carry planned
