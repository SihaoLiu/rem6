...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  protocol run-count expectations, and clean parallel diagnostic contracts by
  scheduler or resource scope, so
...
- MSI, MESI, MOESI, and initial CHI-like line-state, cache-controller,
  ...
  data-cache runs, bounded unattributed data-cache runs, and a minimum recorded
...
- Fabric QoS tests cover explicit fixed-priority requestor mapping,
  ...
  expectations, data-cache protocol run-count expectations, clean parallel
  diagnostic expectations, and manifest identity changes for those expected
...
- Workload manifests record boot images, resources, topology, host events,
  ...
  attribution contracts, data-cache protocol run-count verification contracts,
  and clean diagnostic verification contracts, result metadata, execution mode
