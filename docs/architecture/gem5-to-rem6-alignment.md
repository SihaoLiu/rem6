...
yet. The next trace-policy integration target is trace HTMReq-driven transaction
begin plus cache HTM read/write-set mutation: trace response data now reaches
...
trace replay outcomes and binds aborts on RISC-V data routes to typed CPU
cluster outcomes, while trace HTMReq-driven transaction begin and cache
...
checkpoint and abort-restore contract that workload trace HTM abort sidebands
bind to through declared RISC-V data routes, while trace HTMReq-driven
transaction begin remains open. `PrintReq` packet-trace commands now
...
ordered workload-level sideband records plus typed data-route CPU cluster
outcomes. Trace HTMReq-to-begin binding and full CPU/cache HTM execution remain
open.
