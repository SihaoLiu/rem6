...
- Cache replacement-policy, indexing, and replacement-directory tests cover
  gem5-style LRU/FIFO/MRU/LFU plus WeightedLRU occupancy and touch-order victim
  selection, SetAssociative identity mapping, and SkewedAssociative per-way set
  hashing, including tag extraction, address regeneration, skewed candidate
  residency, MSI/MESI/MOESI/CHI bank-level skewed replacement indexing
  selection, cross-set relocation, BRRIP candidate aging, snapshot restore
  validation, and typed rejection of invalid indexing shapes before cache tag
  state can be allocated. Sector-tag tests cover invalid geometry rejection,
  sector-sized
  ...
  cross-set LRU, BIP, and second-chance replacement checks. Compressed-tag tests
  cover invalid gem5 superblock shapes, capacity-gated co-allocation,
  uncompressed-superblock replacement, partial versus final invalidation, and
  snapshot restore of replacement state, duplicate-superblock avoidance,
  skewed-indexed cross-set LRU replacement, live rounded-up compression-factor
  snapshot restore, and corrupt compressed snapshot rejection for over-capacity
  superblocks, empty superblock metadata, and oversized compressed sub-blocks.
