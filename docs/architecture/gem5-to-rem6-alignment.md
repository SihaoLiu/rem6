...
- Error surfaces are split across simulator fatal errors, simulator panics, and
  ...
  rejection, command-payload literal star bytes, trailing bytes, and skipped
  prefix noise before any ISA register cache or socket loop can mutate
  simulator state.
...
- Debug packet tests cover GDB remote packet checksum encoding and decoding,
  ...
  preservation, bounded payload enforcement, acknowledgement, negative
