...
- `rem6-storage` now owns gem5 `DiskImage`, `RawDiskImage`, and `CowDiskImage`
  ...
  and explicit flush accounting through `sync_all` instead of hidden exit
  callbacks. `rem6-virtio` can now execute block requests directly against
  ...
  private sector backend. Storage checkpoint banks now encode raw and COW image
  ...
  after decode-first validation has accepted every attached bank. Storage
  source-policy tests keep the crate under the facade and hard per-source-file
  budgets, avoiding gem5-style panic paths, process-exit save callbacks, and
  mutable SimObject disk state.
...
- VirtIO tests cover modern PCI common-config feature-page selection,
  ...
  explicit flush accounting, snapshot restore, read-only rejection before
  mutation, typed range, host-file, and image shape errors, nested
  ...
  plus COW snapshots and reject malformed chunks without partial restore.
