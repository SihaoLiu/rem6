...
- Full-system RISC-V tests drive multicore fetch, data access, traps, host stop,
  ...
  typed load-reservation recording and clearing, and `AMOSWAP.D` old-value
  response plus new-value memory update. Memory and cache-controller tests
  cover atomic responses that capture old bytes before masked writes. RISC-V
