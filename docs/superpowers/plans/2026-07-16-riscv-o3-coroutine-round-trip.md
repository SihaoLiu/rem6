...
Add these shared trace and transport snapshots to `coroutine/round_trip.rs`:
...
Create `coroutine/round_trip_lifecycle.rs` with the complete calibrated source:
...
This exact source keeps host source ticks separate from one-tick delivery ticks,
uses exact-one filtered switch and O3-event accessors, proves the live four-row
handoff with one copy of each transferred O3 row and no O3 store replay, locks
baseline Data trace counts to `[1, 1]`, and compares the live-switched tuple
exactly. Data-channel Memory trace counts are `[2, 2, 0, 2]` for request sent,
request arrived, response sent, and response arrived; the transferred load
request is `9` for direct and `11` for hierarchy with per-kind counts
`[1, 1, 0, 1]`, agent `0`, route `1`, and response latencies `32` or `34`.
The transport snapshot requires one route with id `1` and source `cpu0.dmem`,
and locks both aggregate and per-route counters to `[2, 2, 2, 2, 64, 32]` for
direct or `[2, 2, 2, 2, 68, 34]` for hierarchy. It also locks aggregate and
per-kind predictor lanes, validates exact checkpoint/restore arrays, ticks, and
labels, and requires timing-mode zero-schema aliases.

- [ ] **Step 2: Run lifecycle and ownership**
...
- [ ] **Step 3: Commit lifecycle evidence**
